...
| `MCP_TOOL_CATEGORIES` | `read,groups,steering,labeling,experiments,profiles,circuits,jlens,jobs,models,probe_monitors` | Which tool categories are exposed; add `admin` to enable destructive deletes |
| `MILLM_API_URL` | *(unset)* | Base URL of a miLLM runtime. Set it to enable the five cross-product `millm_*` categories (`millm_circuits`, `millm_clusters`, `millm_models`, `millm_runtime`, `millm_sensing`); they never appear unless this is set |
...
**Cross-product plane** — the five `millm_*` categories drive a live miLLM runtime and only appear when `MILLM_API_URL` is set:
