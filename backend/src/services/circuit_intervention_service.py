...
class CircuitInterventionService:
    ...
    def run(db, run_id: str, scope: Dict[str, Any], *,
        ...
        uuids) written into the manifest and ``report["validation"]``. A
        reproduction reads ``gpu["request"]`` back from the manifest it
        reproduces."""
        ...
        try:
            ...
            model, tokenizer, _c, _m = load_model_from_hf(
                ...
                **model_load_kwargs(device, placement, db=db,
                                    sae_ids_by_layer=sae_by_layer,
            ...
            for entry in manifest.get("layers", []):
                ...
                # On its LAYER's card: its suppression hook and its downstream
                # encode run where that layer's hidden state is (D5).
    ...
    def _downstream_acts(model, down_module, sae_d, feature_idx, input_ids, mask):
        ...
        hidden = to_module(captured["h"], sae_d, dtype=torch.float32)
    ...
    def _encoder_for(sae, feature_idx):
        ...
        def enc(hidden):
            ...
            # On the SAE's card; the suppression moves `a_u` back to the hidden
            # state's (see suppress_directional).
