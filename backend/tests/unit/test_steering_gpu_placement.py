...
def test_the_core_split_load_is_mapped_to_hold_each_layers_sae():
    ...
    model_rec = SimpleNamespace(repo_id="org/big", quantization="FP16", file_path=None)
    saes = {"sae_a": SimpleNamespace(d_model=5_120, n_features=65_536, hook_type=None),
            "sae_b": SimpleNamespace(d_model=5_120, n_features=16_384, hook_type=None)}
    seen = {}
...
def test_each_members_decoder_loads_on_its_layers_device(monkeypatch):
    ...
    # hook_type=None, as the nullable ExternalSAE column always has: the core refuses a
    # non-residual SAE before loading it (review R2-B).
    rows = iter([SimpleNamespace(layer=0, hook_type=None), SimpleNamespace(layer=1, hook_type=None)])
