...
def download_sae_task(
    ...
    try:
        ...
        # The hook the SAE was trained at, from its own cfg.json or its Gemma Scope set's
        # name, else NULL (review R2, B5). Consumers that read every SAE as the residual
        # stream refuse an MLP or attention SAE by this column, and NULL reads as residual.
        from ..services.sae_manager_service import resolve_sae_hook, resolve_sae_layer

        recorded_hook = resolve_sae_hook(sae_path, repo_id, filepath)
        # The layer the SAE reads (review R3-B): get_sae_info reads only the old SAELens
        # ``hook_point_layer``, so every SAELens 5 (``hook_layer``) and 6 (no layer key) download
        # recorded NULL -- which turns off every consumer's own-layer check, and feature
        # extraction hooks layer 0.
        recorded_layer = resolve_sae_layer(sae_path, recorded_hook.hook_type, repo_id, filepath)
        logger.info(
            f"SAE {sae_id} hook_type={recorded_hook.hook_type!r} (source: {recorded_hook.source}), "
            f"layer={recorded_layer!r}"
        )
        ...
        with get_sync_db() as db:
            ...
            if sae:
                ...
                sae.hook_type = recorded_hook.hook_type
                sae.sae_metadata = {**(sae.sae_metadata or {}), "hook_source": recorded_hook.source}
                sae.layer = recorded_layer if recorded_layer is not None else metadata.get("layer")
                if metadata:
