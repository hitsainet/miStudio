...
const FLAG_COPY: Record<string, string> = {
  ...
  // R3-D (R3D-12): the flag means a member with ZERO SIMILARITY to the cluster gets zero
  // strength (cluster_allocation_service), not that it fires rarely.
  inactive_member: 'A member has no similarity to the cluster — it gets no strength',
  // No copy on purpose: `solo` always arrives with `default_budget`, whose line already says
  // it; `over_budget` is raised only by the server's reference rebalance, and the store's
  // own rebalance shows an over-budget cluster by turning the numbers amber.
};
