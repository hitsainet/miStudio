...
async def cleanup_analysis_gpu():
    ...
    try:
        if torch.cuda.is_available():
            # EVERY CARD THIS PROCESS HOLDS MEMORY ON, NOT THE CURRENT DEVICE.
            #
            # This read and synchronized the current device, which in the API
            # process is index 0 — the 3080 Ti since 2026-09-13, whichever card
            # the analysis model was actually loaded onto. A card this process
            # never allocated on is skipped: synchronizing it would CREATE a
            # CUDA context there, hundreds of MB taken from a card another job
            # may be using.
            devices = [
            ...
            def _allocated_gb() -> float:
                return sum(torch.cuda.memory_allocated(index) for index in devices) / (1024**3)

            # Get memory before cleanup
            result["vram_before_gb"] = round(_allocated_gb(), 2)

            # Force garbage collection first
            gc.collect()

            # Clear the cache and wait for each card this process uses
            ...
            result["vram_after_gb"] = round(_allocated_gb(), 2)
