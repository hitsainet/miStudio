...
import { DEFAULT_DEAD_NEURON_THRESHOLD } from '../utils/trainingLoopFields';
...
export interface TrainingConfig {
  ...
  // Mixture and evaluation (SAE data-path arc).
  /** Weight per source, keyed by id: an extraction id on cached activations, a
   *  dataset id on the fly, and a dataset id for weights loaded from a template
   *  (which has no extraction ids). Serialised to a positional array in the order
   *  of the ids sent — the worker indexes `dataset_weights` against the sources it
   *  loads, in the order the ids were given, so a keyed map is the only shape that
   *  cannot drift. `utils/trainingHyperparameters.resolveSourceWeights` pairs a
   *  selected extraction with its dataset's weight. */
  dataset_weights_by_extraction?: Record<string, number>;
  holdout_fraction?: number;

  // Post-run evaluation and reproducibility (review R3-C, R2D-6). Unset = the
  // backend's default, which the request then omits.
  /** Measure what the SAE costs the model once training completes (backend default on). */
  evaluate_ce_delta?: boolean;
  /** Real tokens the post-run evaluation reads; 0 skips it (backend default 131,072). */
  evaluation_token_budget?: number;
  /** Held-out tokens evaluated per layer at every log step (backend default 100,000). */
  holdout_eval_tokens?: number;
  /** Held-out tokens per evaluation forward pass (backend default 2,048). */
  holdout_eval_chunk_tokens?: number;
  /** Random seed for init, calibration and sampling; unset derives one from the training id. */
  seed?: number;
  ...
  total_steps: number;
  warmup_steps?: number;
  /** Linear LR decay to 0 over the final N steps; 0 = off */
...
interface TrainingStoreActions {
  ...
  /** Re-run a completed training's post-run evaluation (a GPU job; the result lands on the training). */
  evaluateTraining: (
    trainingId: string,
    opts?: { tokenBudget?: number; force?: boolean }
  ) => Promise<void>;
...
const defaultConfig: TrainingConfig = {
  ...
  // Dead neuron handling
  // The backend schema's default. This read 10000 while the template form, the
  // payload builder and the schema said 1000 — harmless while the field was never
  // read, a 10x difference in when a latent counts as dead once resampling read it.
  dead_neuron_threshold: DEFAULT_DEAD_NEURON_THRESHOLD,
...
export const useTrainingsStore = create<TrainingStore>((set, get) => ({
  ...
  fetchTrainings: async (page = 1, limit = 50) => {
    ...
    try {
      ...
      set({
        error: apiErrorDetail(error, 'Failed to fetch trainings'),
        isLoading: false,
      });
      throw error;
    }
  ...
  fetchTraining: async (trainingId: string, silent = false) => {
    ...
    try {
      ...
      if (!silent) {
        set({
          error: apiErrorDetail(error, 'Failed to fetch training'),
          isLoading: false,
        });
      }
      throw error;
  ...
  createTraining: async (config: TrainingCreateRequest) => {
    ...
    try {
      ...
      set({
        error: apiErrorDetail(error, 'Failed to create training'),
        isLoading: false,
      });
      throw error;
    }
  ...
  deleteTraining: async (trainingId: string) => {
    ...
    try {
      ...
      set({
        error: apiErrorDetail(error, 'Failed to delete training'),
        isLoading: false,
      });
      throw error;
    }
  ...
  retryTraining: async (trainingId: string) => {
    ...
    try {
      ...
      const retryRequest: TrainingCreateRequest = {
        ...
        // The same GPU the run asked for — a retry is a new training, so
        // without this it would silently fall back to auto.
        gpu: failedTraining.gpu_request ?? 'auto',
      };
      ...
      set({
        error: apiErrorDetail(error, 'Failed to retry training'),
        isLoading: false,
      });
      throw error;
    }
  ...
  pauseTraining: async (trainingId: string) => {
    ...
    try {
      ...
      set({
        error: apiErrorDetail(error, 'Failed to pause training'),
        isLoading: false,
      });
      throw error;
    }
  ...
  resumeTraining: async (trainingId: string) => {
    ...
    try {
      ...
      set({
        error: apiErrorDetail(error, 'Failed to resume training'),
        isLoading: false,
      });
      throw error;
    }
  ...
  stopTraining: async (trainingId: string) => {
    ...
    try {
      ...
      set({
        error: apiErrorDetail(error, 'Failed to stop training'),
        isLoading: false,
      });
      throw error;
    }
  ...
  stopAndFinalizeTraining: async (trainingId: string) => {
    ...
    try {
      ...
      set({
        error: apiErrorDetail(error, 'Failed to stop and finalize training'),
        isLoading: false,
      });
      throw error;
    }
  ...
  finalizeTraining: async (
    ...
    try {
      ...
      set({
        error: apiErrorDetail(error, 'Failed to finalize training'),
        isLoading: false,
      });
      throw error;
    }
  ...
  /**
   * Queue the post-run evaluation of a completed training.
   *
   * @param trainingId - Training job ID
   * @param opts - tokenBudget overrides the training's evaluation_token_budget;
   *   force re-queues an evaluation recorded as pending or running
   */
  evaluateTraining: async (
    ...
    const params = new URLSearchParams();
    if (opts?.tokenBudget !== undefined) params.set('token_budget', String(opts.tokenBudget));
    if (opts?.force) params.set('force', 'true');
    const query = params.toString() ? `?${params.toString()}` : '';
    try {
      const response = await axios.post(`${API_BASE_URL}/${trainingId}/evaluate${query}`);
      const data = response.data?.data;
      get().updateTrainingStatus(trainingId, {
        evaluation: { status: 'pending', trigger: 'rerun', task_id: data?.task_id ?? null },
      });
    } catch (error: any) {
      set({
        error: apiErrorDetail(error, 'Failed to start the evaluation'),
      });
      throw error;
    }
  },
    ...
    set((state) => {
      ...
      if (updates.architecture_type && updates.architecture_type !== state.config.architecture_type) {
        ...
        if (fw.sparsityType === 'l1') {
          ...
          // Applied here or the visible control starts empty and, worse, keeps
          // whatever value leaked in from a previously-selected L1 framework.
          newConfig.target_l0 = fwDefaults.target_l0;
          newConfig.initial_threshold = fwDefaults.initial_threshold;
          newConfig.bandwidth = fwDefaults.bandwidth;
          newConfig.ste_bandwidth = fwDefaults.ste_bandwidth;
          // Clear non-L0 fields
  ...
  setConfigFromTraining: (training: Training) => {
    set({
      config: {
        ...
        // A stored training carries null for an optional field it never set. In the
        // form, unset is undefined, which the request omits (review R3-C).
        evaluation_token_budget: training.hyperparameters.evaluation_token_budget ?? undefined,
        holdout_eval_tokens: training.hyperparameters.holdout_eval_tokens ?? undefined,
        holdout_eval_chunk_tokens: training.hyperparameters.holdout_eval_chunk_tokens ?? undefined,
        seed: training.hyperparameters.seed ?? undefined,
      },
    });
  },
