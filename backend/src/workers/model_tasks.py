...
#: How often the statistics phase touches the extraction row. The activation
#: janitor treats a started row as abandoned once `updated_at` is 600 s old and
#: fails it once the row is an hour stale, so anything well under 600 s keeps a
#: live phase visibly alive.
...
def build_statistics_heartbeat(
    ...
    *,
    ...
    """Liveness and cancellation for the statistics phase.
    ...
    def on_statistics_progress(layer_index: int, n_layers: int, chunks_done: int, n_chunks: int):
        cancelled.raise_if_cancelled(
            f"stopped computing statistics at layer {layer_index + 1} of {n_layers}"
        )
        ...
        fraction = (layer_index + chunks_done / max(n_chunks, 1)) / max(n_layers, 1)
        record_progress(
        ...
        try:
            emit_extraction_progress(
                ...
                status="extracting",
                message=(
                    f"Computing statistics: layer {layer_index + 1} of {n_layers} "
                    f"({fraction * 100:.0f}% of the statistics phase)"
                ),
            )
        except Exception as emit_e:
            logger.warning(f"Failed to emit statistics progress: {emit_e}")

    return on_statistics_progress
...
def extract_activations(
    ...
    try:
        ...
        try:
            ...
            # The per-sample callback goes quiet when the GPU pass ends; this
            # carries liveness and Stop through the statistics phase after it.
