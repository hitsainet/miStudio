...
@gpu_job("jlens_fit", can_split=False)
...
def fit_jlens_artifact(
    ...
    #
    # `try/finally` around the LOAD and everything after it, because the release
    # must happen on the failure paths too: an OOM or a validation raise is
    # exactly when the card most needs to come back. The load used to sit
    # outside the guarded block, so a CUDA OOM part-way through `from_pretrained`
    # left its allocations on the card with nothing to release them.
    placement = None
    loaded = None
    try:
        with get_sync_db() as db:
            ...
            # device, index 0, which on this node is the 12 GB card whatever
            # else is free. The card is written to the task's row BEFORE the
            # load, so a fit that dies loading still says where it tried.
            #
            # ONE CARD, NEVER SPLIT (no `allow_shard`). The fit's backward runs
            # `autograd.grad(..., is_grads_batched=True)` with `retain_graph`
            # over the whole forward, in chunks sized to MAX_BACKWARD_BYTES
            # (2 GiB) of cotangents and gradients on top of the saved
            # activations. A split fills each card to its budget and leaves only
            # SHARD_RESERVE_MB (1 GiB) free there, so that workspace has nowhere
            # to go; and a batched backward across accelerate's cross-card
            # copies has not been run on two GPUs. A model no single card holds
            # is refused with the figures rather than started.
            # WITH ITS BACKWARD'S MEMORY, not a forward pass's 2 GiB (review
            # round 3): the fit keeps its forward graph for a batched backward,
            # about 3 GiB beyond the weights for a 12B model at the default
            # prompt length. Sized with the model's tokenizer, before the load.
            placement = jlens_progress.place_on_card(
                self.request.id, gpu_request, required_mb=estimate_weights_mb(record),
                headroom_mb=estimate_fit_working_mb(record, prompts, layers=layers, target_layer=target_layer),
            )
        ...
        # DROP OUR OWN REFERENCE FIRST. `clear_cache` nulls the CACHE's entry
        # and then runs gc + `torch.cuda.empty_cache()` — but this frame still
        # held `loaded`, so there was nothing for gc to collect and
        # `empty_cache` had no free blocks to return. The first version of this
        # release looked correct and recovered only the activation workspace:
        # 7706 MiB fell to 2608 MiB, which is LFM2's fp16 weights still sitting
        # on a card miLLM serves from.
        #
...
def revalidate_staged_artifact(
    ...
    from ..services.jlens_model_registry import estimate_weights_mb, load_for_readout
    from ..core.database import get_sync_db
    ...
    # on the failure paths too — including a load that dies part-way. See
    # `fit_jlens_artifact` for what a missing release cost once.
    placement = None
    loaded = None
    try:
        with get_sync_db() as db:
            ...
            # ON THE CARD THE CALLER CHOSE; see `fit_jlens_artifact`.
            #
            # MAY SPLIT, unlike the fit. Re-validation runs no backward: the
            # SEMANTIC check is a readout through `ReadoutService`, whose hooks
            # copy each residual off its card as it is produced.
            placement = jlens_progress.place_on_card(
                self.request.id,
                gpu_request,
...
def _fit_and_publish(
    ...
    try:
        ...
        if cancelled.reason == GPU_LEASE_LOST:
            # NOT THE OPERATOR'S STOP (multi-GPU Phase 3): the fit lost its GPU
            # lease and nothing made its row terminal. Recorded as its failure.
            logger.error("J-lens fit %s stopped: %s", self.request.id, cancelled.detail)
            jlens_progress.update_row(
                self.request.id, status="failed",
                error_message=f"Stopped: {cancelled.detail}. Run it again.",
            )
            return {"status": "failed", "model_id": model_id, "reason": GPU_LEASE_LOST,
