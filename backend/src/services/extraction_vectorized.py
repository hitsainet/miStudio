...
from src.utils.token_filters import decode_byte_level_token, is_junk_token

logger = logging.getLogger(__name__)


#: Comment leaders. The same prose reappears with and without one — a docstring
#: repeated as a `#` comment block — and for the purpose of "has this feature
#: already shown me this passage?" those are the same evidence.
_COMMENT_LEADERS = frozenset({"#", "//", "/*", "*/", "*", "--"})


def _key_form(tokens) -> tuple:
    """The token window reduced to what makes it DIFFERENT evidence.

    Exact token equality treated two copies of one passage as distinct whenever
    they differed in indentation or carried a comment leader, so a feature could
    fill most of its top-k with the same text. MEASURED on 600 random L11
    features (2026-09-25): this collapses examples for **13 features (2.2%)**,
    reclaiming 82 of 15,000 slots — 0.55% overall, but a **median of 6 slots and
    up to 15** on the features it touches, which are exactly the ones whose
    labels read as confident nonsense because the judge saw one passage six
    times.

    Deliberately conservative. It drops whitespace-only tokens and comment
    leaders and compares the decoded text, so `Ġfoo` and `foo` are one thing. It
    does NOT case-fold, stem, or drop punctuation: a Bloomberg byline feature
    whose examples differ only by name is 25 pieces of distinct evidence and
    every one must survive (BR-006).
    """
    form = []
    for token in tokens:
        text = decode_byte_level_token(str(token)).strip()
        if not text or text in _COMMENT_LEADERS:
            continue
        form.append(text)
    return tuple(form)


def _content_key(example: Dict) -> Optional[int]:
    """Identity of an example's CONTEXT, or None when it has none to key on.

    WHY THIS EXISTS. A corpus repeats itself: the Apache licence header appears
    in thousands of source files, so a feature that fires on it filled all 25 of
    its top-k slots with byte-identical text. Measured on the 32k L12 extraction
    (2026-09-19): 7,744 of 32,726 features -- 24% -- had all 25 examples firing
    on a single token, and a labeller reading those learns nothing it would not
    learn from one of them.

    KEYED ON CONTENT, NEVER ON `sample_index`. The duplicate licence headers sat
    at different sample indices; they were genuinely different rows holding
    identical text. Conversely a Bloomberg byline feature's 25 examples all read
    " story: <NAME> at <email>" and differ only by name -- those are distinct
    evidence and every one of them survives this key.

    The prime position is part of the identity: the same window with the peak on
    a different token is a different thing for the feature to have done.

    RETURNS None WHEN THERE IS NOTHING TO KEY ON, and the caller then treats the
    example as unique. That is failing OPEN, deliberately. This is a quality
    filter, not a safety gate: failing open keeps an example that might be a
    duplicate, while failing closed would silently discard evidence. Losing data
    is the worse error. It also matters in practice -- callers that build
    minimal example dicts (no tokens) must keep their existing behaviour rather
    than collapse every example into one.
    """
    tokens = example.get("tokens")
    if tokens:
        return hash((_key_form(tokens), example.get("prime_activation_index")))

    prime = example.get("prime_token")
    if prime:
        return hash((
            _key_form(example.get("prefix_tokens") or ()),
            _key_form([prime]),
            _key_form(example.get("suffix_tokens") or ()),
        ))
...
class IncrementalTopKHeap:
    ...
    def __init__(self, num_features: int, top_k: int = 100):
        ...
        #: Per feature: content key -> the counter of the heap entry holding it.
        #: Kept in step with `heaps` on every insert AND every eviction; a stale
        #: key would suppress later evidence that deserves a slot.
        self.seen: Dict[int, Dict[int, int]] = {}
        #: Observability: how many candidates were dropped as duplicate content.
    ...
    def add_batch(
        ...
        for i in range(len(feature_indices)):
            ...
            seen = self.seen.setdefault(feat_idx, {})

            # Use counter as tie-breaker to avoid comparing dicts
            # When activations are equal, counter ensures consistent ordering
            if len(heap) < self.top_k:
                # THE KEY IS COMPUTED HERE AND NOT EARLIER. Hashing a ~51-token
                # window for every candidate would be ruinous: at ~65% activation
                # frequency a 32k dictionary yields ~1.8M candidates per batch,
                # and this loop is already the extraction's CPU bottleneck. Only
                # examples that would actually be stored ever get keyed.
                key = _content_key(example)
                if key is not None and key in seen:
                    self._absorb_duplicate(heap, seen, key, activation, example)
                else:
                    heapq.heappush(heap, (activation, self.counter, example))
                    if key is not None:
                        seen[key] = self.counter
                    self.counter += 1
            else:
                ...
                if activation > min_activation:
                    ...
                    if key is not None and key in seen:
                        ...
                        evicted = heapq.heapreplace(
                            heap, (activation, self.counter, example)
                        )
                        evicted_key = _content_key(evicted[2])
                        if evicted_key is not None:
                            # Forget the evicted content, or its key would go on
                            # suppressing later examples that are not duplicates
                            # of anything still held.
                            seen.pop(evicted_key, None)
                        if key is not None:
                            seen[key] = self.counter
                        self.counter += 1
    ...
    def _absorb_duplicate(
        ...
        """Fold a repeat of content already held into the entry holding it.

        KEEPS THE STRONGEST INSTANCE, not the first seen: batches do not arrive
        in activation order, so first-seen would pin an arbitrary member of the
        duplicate set and discard better ones.

        The scan is over at most `top_k` entries (25 in production), so this is
        cheap, and it runs only for candidates that already passed the
        activation threshold.
        """
        held_counter = seen[key]
        for pos, (held_activation, counter, _held_example) in enumerate(heap):
            if counter == held_counter:
                if activation > held_activation:
                    heap[pos] = (activation, self.counter, example)
                    seen[key] = self.counter
                    self.counter += 1
                    heapq.heapify(heap)
        ...
        # The key outlived its entry, which means eviction bookkeeping missed a
        # case. Drop the key and store the example rather than silently losing
        # it: this path should never run, and losing evidence to a bookkeeping
        # slip is worse than keeping one extra.
        seen.pop(key, None)
        if len(heap) < self.top_k:
            heapq.heappush(heap, (activation, self.counter, example))
        else:
            evicted = heapq.heapreplace(heap, (activation, self.counter, example))
            evicted_key = _content_key(evicted[2])
            if evicted_key is not None:
                seen.pop(evicted_key, None)
        seen[key] = self.counter
    ...
    def get_heaps(self) -> Dict[int, List[Tuple[float, Dict]]]:
        ...
        logger.info(
            f"Duplicate contexts folded into an existing slot: "
...
def batch_process_features(
    ...
    Returns:
        Tuple of:
            ...
            - max_activations: Array of max activation values (shape: [N])
            - examples: List of example dictionaries (length: N)
            - fired_counts: Per-feature count of SAMPLES in which the feature
              fired at all (shape: [latent_dim]), counted BEFORE any filter.
              The first three are post-filter; this one is not, and the
              difference between them is what `filter_suppressed_neurons`
              measures.

        Note: Examples with all filtered tokens are skipped entirely — which is
        precisely why `fired_counts` exists and is returned separately.
    ...
    #: Per-feature count of samples in which the feature fired at all, summed
    #: on-device across sub-batches. See the comment at its accumulation site.
    fired_accum = None
    ...
    for vec_batch_start in range(0, batch_size, vectorization_batch_size):
        ...
        # 2. COUNT EVERY FIRING, BEFORE ANY FILTER SEES IT.
        #
        # `activation_frequency` is computed downstream from the examples this
        # function EMITS, and emission happens after the junk-token filter — so
        # the recorded frequency means "fraction of samples where the peak
        # landed on a non-junk token", not "fraction of samples where the
        # feature fired". Those diverge sharply for a feature whose strongest
        # activations sit on punctuation or on a word in the ~500-entry
        # WORD_FRAGMENTS blocklist.
        #
        # One `(latent_dim,)` accumulator, summed on-device and transferred once
        # per call — ~128 KB against the ~8 MB of dead top-k transfer this
        # replaces.
        #
        # Zero-padded positions are exactly 0.0 and the test is strict `>`, so
        # padding cannot inflate the count.
        fired_this_batch = (max_activations > 0).sum(dim=0)
        if fired_accum is None:
            fired_accum = fired_this_batch.clone()
        else:
            fired_accum += fired_this_batch
    ...
    # FOUR VALUES, NOT AN OPTIONAL FIELD. Changing the arity is a hard break
    # that every caller and every test sees immediately; an optional extra key
    # would be silently ignored by a caller that never learned to read it, which
    # is how `token_positions` and `activation_values` came to be computed on
    # every example and stored nowhere.
    if fired_accum is None:
        fired_counts = np.zeros(latent_dim, dtype=np.int64)
    else:
        fired_counts = fired_accum.cpu().numpy().astype(np.int64)
