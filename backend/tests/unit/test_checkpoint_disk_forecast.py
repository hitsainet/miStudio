...
class TestFreeSpaceIsMeasuredCorrectly:
    def test_it_reads_the_unprivileged_figure_not_the_raw_one(self, tmp_path):
        """`f_bavail`, not `f_bfree` — the gap is the root-reserved fraction.

        On this project's own volume the two differ by ~95 GiB. Reading
        `f_bfree` would promise the backend space it cannot allocate, turning a
        correct refusal into a run that fills the disk.
        """
        stat = os.statvfs(tmp_path)
        expected = stat.f_bavail * stat.f_frsize
        raw = stat.f_bfree * stat.f_frsize
        measured = CD.free_bytes_for(tmp_path)
        assert measured == expected
        if raw != expected:
            assert measured != raw, "free space was read as f_bfree (root-reserved included)"
...
class TestRoomForOneMoreStep:
    def test_it_asks_for_the_step_plus_the_reserve(self, tmp_path):
        ...
        assert free == CD.free_bytes_for(tmp_path)
