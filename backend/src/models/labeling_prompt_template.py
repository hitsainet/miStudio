...
class LabelingPromptTemplate(Base):
    ...
    # max_examples`, so a label is an inference from the extreme upper tail of a
    # feature's activation distribution. A feature that reads as "legal
    # language" in its top decile may be "formal register" across its range, and
    # nothing today can show that.
    #
    # SCALAR ON PURPOSE. `labeling_fingerprint.prompt_fingerprint_fields` is
    # DERIVED from `__table__.columns` minus an identity denylist and rejects
    # non-scalars outright, so a scalar column is covered by the fingerprint the
    # day it lands, while a JSONB blob would raise TypeError there.
    #
    # TWO COLUMNS, NOT ONE. Widening the evidence and restating its strength are
    # two variables. The 2026-09-01 triage experiment changed two at once
    # (SAE expansion AND context width) and nothing was attributable until one
    # was held fixed — so `activation_display` can be moved alone as an inert
    # control before `example_sampling` is trusted.
    example_sampling = Column(
        String(24), nullable=False, default='top_k', server_default='top_k',
    )  # 'top_k' | 'stratified'
    activation_display = Column(
        String(24), nullable=False, default='absolute', server_default='absolute',
    ...
    # Relationships
    #: `passive_deletes=True` is load-bearing. `labeling_jobs.prompt_template_id`
    #: is declared `ondelete="RESTRICT"`, but without this SQLAlchemy NULLs the
    #: child column before issuing the delete, so RESTRICT never fires: the
    #: delete succeeds and every job that used the template silently forgets
    #: which template produced its labels. `delete_template` documents that a
    #: template in use cannot be deleted; this is what makes that true.
    labeling_jobs = relationship(
        "LabelingJob", back_populates="prompt_template", passive_deletes=True
    )
