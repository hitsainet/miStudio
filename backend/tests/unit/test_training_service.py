...
class TestTrainingServiceStateManagement:
    ...
    async def _crashed(self, async_session, status=TrainingStatus.FAILED):
        """A training reaped into a terminal state, the way a dead worker leaves one."""
        training_data = TrainingCreate(
            model_id="m_test123",
            dataset_ids=["12345678-1234-5678-1234-567812345678"],
            hyperparameters=TrainingHyperparameters(
                hidden_dim=768, latent_dim=16384, l1_alpha=0.001,
                learning_rate=0.0003, batch_size=4096, total_steps=100000,
            ),
        )
        with patch('src.services.training_service._emit_training_event_sync'):
            training = await TrainingService.create_training(async_session, training_data)
        row = await TrainingService.get_training(async_session, training.id)
        row.status = status.value
        row.current_step = 1025
        row.error_message = "Training job stuck - no progress for 35 minutes."
        row.error_traceback = "Traceback (most recent call last): ..."
        row.completed_at = datetime.now(UTC)
        await async_session.commit()
        return training

    async def test_a_crashed_training_resumes_from_its_checkpoint(
        self, async_session, test_model, test_dataset
    ):
        """A FAILED run is resumable: the crash recovery, not just Finalize or Retry.

        A killed worker, an OOM or a rebooted node leaves the row FAILED with its
        checkpoints intact, and `resume_training_task` already continues from the
        newest complete one. Before this, a crash at step 120,000 of 150,000 could
        only be salvaged as an SAE at that checkpoint or restarted from step 0.
        """
        training = await self._crashed(async_session)

        with patch('src.services.training_service._emit_training_event_sync') as mock_emit:
            resumed = await TrainingService.resume_training(
                async_session, training.id, "task_after_the_crash"
            )

        assert resumed is not None, "a crashed run was refused"
        assert resumed.status == TrainingStatus.RUNNING.value
        assert resumed.celery_task_id == "task_after_the_crash"
        mock_emit.assert_called_once()

    async def test_resuming_a_crash_stops_the_row_claiming_it_failed(
        self, async_session, test_model, test_dataset
    ):
        """Otherwise a RUNNING row carries the old traceback and a completed_at."""
        training = await self._crashed(async_session)

        with patch('src.services.training_service._emit_training_event_sync'):
            resumed = await TrainingService.resume_training(async_session, training.id)

        assert resumed.error_message is None
        assert resumed.error_traceback is None
        assert resumed.completed_at is None
        # The progress it reached is NOT reset: that is what it resumes from.
        assert resumed.current_step == 1025

    async def test_a_cancelled_training_is_still_refused(
        self, async_session, test_model, test_dataset
    ):
        """CANCELLED was stopped on purpose; resuming it would undo that decision."""
        training = await self._crashed(async_session, status=TrainingStatus.CANCELLED)
