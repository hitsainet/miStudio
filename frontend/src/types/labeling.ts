...
export interface LabelingConfigRequest {
  ...
  /**
   * `"auto"` or a GPU UUID. Sent for the LOCAL method only: the other methods
   * call a remote endpoint and hold no card, so a GPU there would be a claim
   * about work this app does not do.
   */
  gpu?: string;
...
export interface LabelingJob {
  ...
  /** Local model only: what the job asked for ("auto", "all" or a GPU UUID); null on older jobs means auto */
  gpu_request?: string | null;

  /** Local model only: the GPU (UUID) the job ran on (the first card of a split) */
  gpu_uuid?: string | null;

  /** Local model only: every GPU (UUID), in fill order, when the judge was split across GPUs */
