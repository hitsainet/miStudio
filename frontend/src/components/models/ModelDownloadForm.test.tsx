 ...
 *
 * MUTATION CONTROL (multi-GPU Phase 2, 2026-09-14; restored byte-identically, sha256):
 *   F1 drop allowSplit from the GPU picker -> "offers "All GPUs" and sends gpu "all""
 *   Its first run also turned six Error Handling tests red: the new test queued
 *   mockResolvedValueOnce before an assertion that failed, and the queued value
 *   leaked into them. Queued just before the click now; the re-run fails only
 *   the one test.
 */
...
describe('ModelDownloadForm', () => {
  ...
  describe('Form Submission', () => {
    it('sends the chosen card UUID, not an index, as the GPU', async () => {
      useSystemMonitorStore.setState({ gpuList: TWO_CARDS } as never);
      mockOnDownload.mockResolvedValueOnce(undefined);
      render(<ModelDownloadForm onDownload={mockOnDownload} />);

      fireEvent.change(screen.getByLabelText('HuggingFace Model Repository'), {
        target: { value: 'LiquidAI/LFM2.5-1.2B-Instruct' },
      });
      fireEvent.change(screen.getByLabelText('GPU'), { target: { value: RTX_3090_UUID } });
      fireEvent.click(screen.getByRole('button', { name: /^download$/i }));

      await waitFor(() => expect(mockOnDownload).toHaveBeenCalledTimes(1));
      expect(mockOnDownload.mock.calls[0][4]).toBe(RTX_3090_UUID);
    });

    it('offers "All GPUs" and sends gpu "all" — the inspection load can run split', async () => {
      useSystemMonitorStore.setState({ gpuList: TWO_CARDS } as never);
      render(<ModelDownloadForm onDownload={mockOnDownload} />);

      fireEvent.change(screen.getByLabelText('HuggingFace Model Repository'), {
        target: { value: 'google/gemma-4-12b-it' },
      });
      ...
      // Queued only now. Queued before an assertion that fails, it outlives this
      // test (clearAllMocks keeps queued implementations) and resolves a download
      // the Error Handling tests expect to reject.
      mockOnDownload.mockResolvedValueOnce(undefined);
      fireEvent.click(screen.getByRole('button', { name: /^download$/i }));

      await waitFor(() => expect(mockOnDownload).toHaveBeenCalledTimes(1));
      expect(mockOnDownload.mock.calls[0][4]).toBe('all');
    });
