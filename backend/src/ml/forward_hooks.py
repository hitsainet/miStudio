...
class HookManager:
    ...
    def clear_activations(self) -> None:
        ...
        """
        # Drop every tensor reference in each layer's list, then the lists.
        #
        # This used to `del activation_list[i]` for i in range(len(list)): each
        # delete shifts the list left, so a list holding two or more tensors
        # raised IndexError half-way through and left the rest referenced (R1-B,
        # SAE training remediation review, 2026-09-15). `clear()` releases every
        # reference in one call, which is all the explicit deletes were for.
        for activation_list in self.activations.values():
