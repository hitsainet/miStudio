...
def _piece_names(tokenizer, ids) -> List[str]:
    """Decoded subwords, or their ids as strings when decode misbehaves.

    THESE GO INTO `interventions.json`. A tokenizer whose `decode` returns
    anything but a string makes `json.dumps` raise inside
    `record_intervention_result` — and the worker downgrades a filing failure to
    a warning, on the correct principle that the expensive half must survive the
    cheap half. The consequence is that a COMPLETED rung-2 GPU measurement is
    silently never filed, for a cosmetic reason. The endpoint already guards
    this in `_name_pieces`; the worker did not.
    """
    try:
        names = [tokenizer.decode([i]) for i in ids]
    except Exception:  # noqa: BLE001 - a label must not cost a measurement
...
def run_intervention_task(
    ...
    #: What the endpoint resolved: "auto", or the UUID of the card the caller
    #: named. A card that cannot be used fails the task with the reason.
    ...
    # EVERY NAME THE `finally` RELEASES EXISTS BEFORE ANYTHING CAN RAISE, and the
    # load is inside the guarded block: a CUDA OOM part-way through
    ...
    try:
        with get_sync_db() as db:
            ...
            # ON THE CARD THE CALLER CHOSE, or the one Auto picks against the
            # memory free now. This was `"cuda" if available` — the CURRENT
            # device, index 0. Released in the `finally` below — and the release
            # drops this frame's reference FIRST, because `clear_cache` runs gc
            # and `empty_cache` immediately, and a live reference here means
            # neither frees anything.
            #
            # MAY SPLIT: every perturbing hook moves its vector to the hidden
            # state's own device, and every forward's ids go to the embedding's
            # card (`service.capture_device`).
        ...
        # THE LENS MUST COVER THE LAYERS IT IS BEING CREDITED FOR.
        #
        # `artifact_id` is provenance: supplying it appends an `evidence_rung: 2`
        # record to that lens's `interventions.json`, the file that travels to
        # HuggingFace and into a serving runtime. The perturbation happens in the
        # residual stream and works at ANY layer, so nothing here used to stop a
        # swap at layers the artifact was never fitted for from being filed
        # against it — gemma-4-12b-it is fitted at L42-45 of 48, and a swap at
        # L18-63 would run, succeed, and be recorded as though the lens
        # justified it. That is false provenance, and this repo has already
        # shipped it once: an additive steer ran under the `coordinate_swap`
        # name and was written into `interventions.json` as such.
        #
        # REFUSED ONLY ON THE FILED PATH. Without `artifact_id` nothing is being
        # credited and an exploratory swap at any layer stays legitimate.
        if artifact_id:
            uncovered = sorted(set(layers) - set(transport.covers(layers)))
            if uncovered:
                raise ValueError(
                    f"layers {uncovered} are not fitted in artifact "
                    f"{artifact_id!r}, so it cannot be credited for an "
                    "intervention there. Drop artifact_id to run this as an "
                    "exploratory intervention, or choose layers the lens covers."
                )
        ...
        # THE DIRECTION'S OWN PIECES. The disclosure below used to be derived
        # from the TARGET alone while describing how the DIRECTION was built —
        # so a swap from a multi-piece token to a single-token partner reported
        # `is_multi_token: False` and filed a recipe claiming the older
        # single-token form, while the direction actually applied was a mean of
        # two rows. The mirror case wrote a caveat sentence that was simply
        # false. Both land in `interventions.json`, which travels.
        direction_pieces: List[str] = []
        if direction is None and direction_token:
            ...
            # A MULTI-PIECE WORD IS STILL ONE DIRECTION: THE MEAN OF ITS ROWS.
            #
            # This used to refuse anything that was not exactly one token, on
            # the reasoning that a multi-token string "has no single direction".
            # That excluded most of the interesting vocabulary: the common case
            # is not a phrase, it is an ORDINARY WORD the tokenizer splits into
            # subwords — ' Juniper' is [' Jun', 'iper'] for several tokenizers
            # this project runs — and there is nothing incoherent about the
            # concept those pieces jointly encode.
            #
            # The mean is then unit-normalised below like any other direction,
            # so the control stays norm-matched and `strength` keeps one meaning.
            # The pieces are recorded, because a direction that is not any single
            # token's row must not be reported as though it were.
            named = (
                service.W_U[ids].to(READOUT_DEVICE).to(torch.float32).mean(dim=0)
            )
            direction_pieces = _piece_names(service.tokenizer, ids)
            ...
            # A SWAP NEEDS BOTH COORDINATES, AND THIS BRANCH GAVE IT ONE.
            #
            # `target_token` alone satisfied the schema validator — it only
            # checks the two are different, and `None != " dog"` — and then fell
            # through to `named = None`. With no direction there is nothing to
            # hook, so NO HOOKS WERE REGISTERED: all three arms ran the plain
            # forward pass, the intervened arm came back bit-identical to the
            # baseline, and the run was filed as `evidence_rung: 2`. A swap that
            # perturbed nothing was reported as a measurement.
            raise ValueError(
                "coordinate_swap needs a direction to exchange: pass "
                "`direction_token` (the coordinate to move) alongside "
                "`target_token` (the one to exchange it with), or an explicit "
                "`direction` vector. With only one of them there is nothing to "
                "swap and the run would report the baseline as a finding."
            )
        else:
            named = None
        ...
        # THE TARGET IS SCORED BY ID, resolved once. A multi-piece target is
        # scored on its FIRST piece and the caveat says so — see the note below,
        # which this comment used to contradict outright by claiming the run was
        # refused instead.
        ...
        if not target_ids:
            raise ValueError(
                f"target {wanted!r} does not tokenise to anything; there is "
                "nothing to score"
            )
        # THE FIRST PIECE IS WHAT A NEXT-TOKEN DISTRIBUTION CAN ANSWER FOR.
        #
        # A rank is defined over ONE position, and the model emits one token
        # there. If ' Juniper' is [' Jun', 'iper'], the observable claim is that
        # ' Jun' arrives — the rest of the word is a later position this
        # measurement does not look at. Scoring the first piece is therefore the
        # honest reading of "did the replacement arrive", and it is WEAKER
        # evidence than a single-token target: ' Jun' also begins ' June' and
        # ' Junior', so a hit is consistent with a word the caller did not ask
        # for. That is stated in the caveat rather than left for the reader to
        # work out from the token count.
        target_id = int(target_ids[0])
        target_pieces = _piece_names(service.tokenizer, target_ids)
        ...
        if chosen is Primitive.COORDINATE_SWAP:
            ...
            if not partner_ids:
                raise ValueError(
                    f"swap partner {target_token!r} does not tokenise to "
                    "anything; there is no coordinate to exchange with"
                )
            # SAME RULE AS THE DIRECTION: the mean of the pieces. See above.
            swap_partner = (
                service.W_U[partner_ids]
                .to(READOUT_DEVICE)
                .to(torch.float32)
                .mean(dim=0)
            )
            partner_pieces = _piece_names(service.tokenizer, partner_ids)
        ...
        def final_rank(text, vector, top_k=50):
            ...
            """
            # THE EMBEDDING'S CARD (`capture_device`), not `model.device`: that
            # is the first parameter's device, which on a split model can be a
            # later card, and the lookup then fails on the embedding's card.
            ids = service.tokenizer(text, return_tensors="pt")["input_ids"].to(
                service.capture_device
            )
            n = int(ids.shape[-1])
            sites = _sites_for(n)
            handles = []
        ...
        # THE CAVEAT IS BUILT ONCE, HERE, AND SHARED.
        #
        # It used to be assembled only while composing the task's return value,
        # while the on-disk record stores `"evidence": summary` — so the
        # multi-piece disclosure never reached `interventions.json`, the file
        # the disclosure exists for, the one that travels to HuggingFace and
        # into a serving runtime. The reader who most needs to know that only
        # the first subword was scored is the one holding the published lens.
        multi_piece_notes = [
            # THE SCORED SIDE.
            (
                f"The target {wanted!r} is {len(target_pieces)} tokens "
                f"{target_pieces!r}, so the score is the rank of the FIRST piece "
                f"{target_pieces[0]!r} alone. That piece may begin other words, "
                "so a hit is weaker evidence than a single-token target."
                if len(target_pieces) > 1
                else None
            ),
            # THE PERTURBED SIDE, SEPARATELY. These are two different tokens on
            # a swap and either can be split; one sentence covering both said
            # something false whenever they disagreed.
            (
                f"The direction {direction_token!r} is {len(direction_pieces)} "
                f"tokens {direction_pieces!r}, so what was perturbed is the MEAN "
                "of their unembedding rows and is not any single token's "
                "direction."
                if len(direction_pieces) > 1
                else None
            ),
            # THE PARTNER, WHICH IS A THIRD TOKEN. On a swap the partner supplies
            # the coordinate exchanged IN, and it can split independently of the
            # other two — keying this note off `direction_pieces` alone left a
            # multi-piece partner exchanged in as a mean with nothing saying so.
            (
                f"The swap partner {target_token!r} is {len(partner_pieces)} "
                f"tokens {partner_pieces!r}, so the coordinate exchanged in is "
                "the MEAN of their rows."
                if len(partner_pieces) > 1
                else None
            ),
        ]
        full_caveat = " ".join(
        ...
        # The recorded evidence carries it too, not just the response.
        summary = {**summary, "caveat": full_caveat}
        ...
        if artifact_id:
            record = {
                ...
                "steering_recipe": {
                    ...
                    # WHAT THE DIRECTION ACTUALLY WAS. A subword-split word is
                    # steered along the MEAN of its pieces' rows, which is not
                    # any single token's direction — a consumer re-applying this
                    # recipe has to know that, and the token string alone does
                    # not say it. Absent-or-1 means the older single-token form.
                    "target_token_pieces": target_pieces,
                    "target_is_multi_token": len(target_pieces) > 1,
                    "direction_token_pieces": direction_pieces,
                    "direction_is_multi_token": len(direction_pieces) > 1,
                    "partner_token_pieces": partner_pieces,
                    "partner_is_multi_token": len(partner_pieces) > 1,
        ...
        return {
            ...
            "parameters": {
                ...
                "over_layer_budget": over_budget,
                # THE SAME NUMBER THE RECIPE RECORDS. It was written to disk and
                # omitted from the response, so a caller without an
                # `artifact_id` — every exploratory run — could not see what the
                # direction was normalised from. The recipe and the result must
                # not disagree about what was applied, and the way to guarantee
                # that is for both to report it.
                "direction_norm_before_scaling": direction_norm,
                # WHICH ID THE RANK IS ACTUALLY ABOUT.
                #
                # For a single token this is obvious; for a subword-split target
                # it is the FIRST piece, and that choice is the whole meaning of
                # the number being reported. It was described in prose and
                # nowhere in the data, so scoring a different piece would have
                # changed the finding while every test and the caveat still said
                # "first". A reader comparing two runs needs the id, not an
                # adjective.
                "scored_token_id": target_id,
            },
            "control": {
                "k": k,
                "seed": control_seed,
        ...
        "positions_skipped": skipped["n"],
            ...
            # was supplied.
            "caveat": full_caveat,
            # MACHINE-READABLE, not only prose. A consumer deciding whether to
            # trust a recipe should not have to parse a caveat sentence.
            "target_token_pieces": target_pieces,
            "target_is_multi_token": len(target_pieces) > 1,
            "direction_token_pieces": direction_pieces,
            "direction_is_multi_token": len(direction_pieces) > 1,
        ...
        # `final_rank` and the hook let go with it. `release_card` does
        # nothing for a CPU placement.
