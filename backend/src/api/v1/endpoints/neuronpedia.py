...
from ....services.sae_hook_support import UnsupportedSae
...
from ....schemas.neuronpedia import (
    ...
    LOGIT_LENS_GPU_DESCRIPTION,
)
from ....schemas.gpu import AUTO, GPU_REQUEST_PATTERN
from ....services.gpu_placement import GpuPlacementError
...
async def start_export(
    ...
    # Before the job row exists: an unknown card is a 400, not a failed export.
    # So is "all" when the export includes the logit lens, which runs on one
    # card: `logit_lens_device` would refuse it only after the job was created.
    gpu_request = resolve_gpu_request(request.gpu, can_split=not request.config.include_logit_lens)
    ...
    try:
        ...
        # An MLP or attention SAE: the export would label it resid_post (review R1-A, A5).
        # An SAE recording no layer: the export would publish it as layer 0 (R3-B, R3B-11).
...
async def compute_dashboard_data(
    ...
    # Before either path starts any work: an unknown card is a 400. So is "all"
    # when the logit lens is asked for: the lens runs on one card, and its
    # worker would refuse "all" only once it started.
    gpu_request = resolve_gpu_request(request.gpu, can_split=not request.include_logit_lens)
    ...
    from ....services.logit_lens_service import get_logit_lens_service, logit_lens_lease
    ...
    # THIS RUNS IN THE API PROCESS. The card is resolved and passed as a device;
    # it is never made current, because the current CUDA device is shared by
    # every request this process serves. Resolved outside the try so a card that
    # cannot be used is a 409 with its reason, not a generic 500.
    #
    # MULTI-GPU PHASE 3 (review round 1, R1-5): in per-card mode the card is
    # LEASED for the lens, like any job's, and a synchronous request waits for
    # none — a card another job holds is a 409 at once.
    lens_device = None
    lens_lease = None
    if request.include_logit_lens:
        lens_lease = logit_lens_lease(gpu_request, kind="dashboard_logit_lens_api", wait_timeout_s=0.0)
        try:
            lens_device = await lens_lease.__aenter__()
    ...
    try:
        if request.include_logit_lens:
            try:
                ...
                await lens_lease.__aexit__(None, None, None)
...
async def push_to_local_neuronpedia(
    ...
    logit_lens_k: int = 20,
    gpu: str = Query(
        AUTO, max_length=64, pattern=GPU_REQUEST_PATTERN,
        description=LOGIT_LENS_GPU_DESCRIPTION,
    ),
    db: AsyncSession = Depends(get_db),
):
    """
    ...
    # Before the tracking row exists: an unknown card is a 400. So is "all" when
    # the push computes dashboard data: its logit lens runs on one card.
    gpu_request = resolve_gpu_request(gpu.strip(), can_split=not compute_dashboard_data)
    ...
    # The push names its source by layer and writes neurons ON CONFLICT (model, layer,
    # index) DO UPDATE: an MLP SAE would overwrite the residual SAE of the same layer,
    # labelled residual (review R1-A, A5).
    from ....services.sae_hook_support import non_residual_hook_reason

    hook_reason = non_residual_hook_reason(sae.hook_type, "The local Neuronpedia push")
    if hook_reason:
        raise HTTPException(422, hook_reason)

    # The push NAMES its source by layer ("14-res-16k") and upserts neurons ON CONFLICT
    # (model, layer, index). An SAE recording no layer was published as layer 0, on top of
    # whatever really is layer 0 (review R3-B, R3B-11).
    from ....services.sae_hook_support import unrecorded_layer_reason

    layer_reason = unrecorded_layer_reason(sae.layer, "The local Neuronpedia push", sae_id)
    if layer_reason:
