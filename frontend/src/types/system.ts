...
/** One card's block inside `GET /system/all` when no `gpu_id` was given. */
export interface MonitoringGpuCard {
  gpu_id: number;
  metrics: GPUMetrics;
  info: GPUInfo;
  processes: GPUProcess[];
...
/** `GET /system/all?gpu_id=N`: the one card that was asked for. */
export interface SelectedGpuBlock {
  gpu_count: number;
...
/**
 * `GET /system/all` with no `gpu_id`: every card, none chosen for the caller.
 *
 * The server used to pick card 0 when no id was sent. With two cards that
 * showed the 12 GB card's numbers to anyone who had not chosen, so it now
 * returns them all and the client decides which to show.
 */
export interface AllCardsGpuBlock {
  gpu_count: number;
...
export interface AllMonitoringDataResponse {
  ...
  network_rates: NetworkRates;
  disk_rates: DiskRates;
  gpu?: SelectedGpuBlock | AllCardsGpuBlock;
}
