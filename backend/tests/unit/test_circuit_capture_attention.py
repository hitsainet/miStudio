...
class TestTheLoaderCarriesTheAttentionBackend:
    ...
    def test_an_out_of_memory_load_is_not_retried_so_the_backend_cannot_be_lost(self):
        """The loader used to retry an OOM at a larger format, re-invoking itself
        with an explicit kwarg list that had to remember `attn_implementation`.
        It no longer retries (review round 1, 2026-09-14): the one load that ran
        asked for eager, and the caller gets the out-of-memory error, not a model
        at a precision or backend it did not ask for.
        """
        calls = []
        ...
        with a, b, c, patch.object(model_loader.AutoModelForCausalLM,
            ...
            with pytest.raises(model_loader.OutOfMemoryError, match="at Q8"):
                model_loader.load_model_from_hf(
                    "vendor/m", attn_implementation="eager",
                    quant_format=model_loader.QuantizationFormat.Q8)

        assert len(calls) == 1, f"the out-of-memory load was retried: {calls}"
        assert calls[0].get("attn_implementation") == "eager"
        ...
        with patch("datasets.load_from_disk", return_value=ds), \
            ...
            with pytest.raises(Exception):
                cap.CircuitCaptureService.run_capture(
                    db, "cap_1", confirmed=True, device="cpu")
        return captured
