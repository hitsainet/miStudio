...
class TestTheColumnsDoNotLieAboutOldRows:
    ...
    @pytest.mark.parametrize("name", ["shuffled", "shuffle_seed"])
    def test_the_shuffle_columns_are_nullable_for_the_same_reason(self, name):
        """NULL means not recorded, and for these it is the ONLY way to say it.

        Every tokenization written before these columns is unshuffled in fact —
        extraction reads a PREFIX, so those corpora were consumed in corpus
        order — but the row must not claim a shuffle, or a seed, it never had.
        No sentinel is available either: 0 is a legitimate seed.
        """
        col = DatasetTokenization.__table__.columns[name]
        assert col.nullable, (
            f"{name} is NOT NULL, so every pre-existing row claims a value"
        )
        assert col.server_default is None and col.default is None, (
            "a default backfills a shuffle onto rows that were written in "
            "corpus order"
        )
    ...
    @pytest.mark.parametrize(
        ...
        ["text_column", "chat_format", "pack_sequences", "padding",
         "truncation", "add_special_tokens", "shuffled", "shuffle_seed"],
    )
...
class TestTheWorkerActuallyWritesThem:
    ...
    @pytest.mark.parametrize(
        ...
        ["text_column", "chat_format", "pack_sequences", "padding",
         ...
         # Same rule, and the failure is quieter: a row that records no seed
         # while its blocks were permuted makes the corpus unreproducible, and
         # one that records a seed it never applied is undetectable downstream.
...
class TestTheApiActuallyReturnsThem:
    """Recorded and unreadable is only half a fix.

    All eight provenance columns existed on the row and reached no caller: the
    response schema stopped at `custom_filter_chars`, so "is this corpus
    shuffled?" or "which column was it tokenized on?" could only be answered by
    opening the database.

    THE REGRESSION THIS GUARDS IS ONE REFACTOR AWAY. The endpoint hands ORM
    objects straight to the response and `from_attributes` reads the columns off
    them. `DatasetTokenization.to_dict()` omits all eight — so changing
    `data=tokenizations` to `data=[t.to_dict() for t in tokenizations]` would
    null every field while the schema went on advertising them. Declaring a
    field and never populating it is the read-side twin of a column added and
    never written.
    """
    ...
    def _row(**overrides):
        ...
        base = dict(
            id="tok_x", dataset_id=uuid4(), model_id="m_f0271325", max_length=2048,
            tokenized_path="/data/x", tokenizer_repo_id="LiquidAI/LFM2.5-1.2B-Instruct",
            vocab_size=65536, num_tokens=1123770368, avg_seq_length=2048.0,
            status="ready", progress=100.0, error_message=None, celery_task_id="c1",
            created_at=datetime.now(UTC), updated_at=datetime.now(UTC),
            completed_at=None, remove_all_punctuation=False, custom_filter_chars=None,
            text_column="Article", chat_format="auto", pack_sequences=True,
            # The STRING the database actually holds. The worker writes a bool
            # into a String(20) column for a packed run; SQLAlchemy coerces it,
            # and all five real rows read back 'false'.
            padding="do_not_pad", truncation="false", add_special_tokens=True,
    ...
    def _dump(self, row):
        from src.schemas.dataset import DatasetTokenizationResponse

        return DatasetTokenizationResponse.model_validate(row).model_dump()

    @pytest.mark.parametrize("name", PROVENANCE)
    def test_the_response_declares_it(self, name):
        from src.schemas.dataset import DatasetTokenizationResponse

        assert name in DatasetTokenizationResponse.model_fields, (
            f"{name} is recorded on the row and no caller can read it"
        )

    def test_the_response_carries_the_values_not_just_the_fields(self):
        out = self._dump(self._row())
        assert out["text_column"] == "Article"
        assert out["chat_format"] == "auto"
        assert out["pack_sequences"] is True
        assert out["padding"] == "do_not_pad"
        assert out["truncation"] == "false"
        assert out["add_special_tokens"] is True
        assert out["shuffled"] is True

    def test_seed_zero_reaches_the_caller_as_zero(self):
        """0 is a legitimate seed. A falsy coercion anywhere in the read path
        would turn a pinned, reproducible order into 'not recorded'."""
        out = self._dump(self._row(shuffle_seed=0))
        assert out["shuffle_seed"] == 0
        assert out["shuffle_seed"] is not None

    def test_a_historical_row_claims_nothing(self):
        """NEGATIVE CONTROL. A row predating these columns must read as None
        throughout — not as a default that asserts a provenance it never had."""
        from types import SimpleNamespace

        row = self._row()
        bare = SimpleNamespace(**{
            k: v for k, v in vars(row).items() if k not in self.PROVENANCE
        })
        out = self._dump(bare)
        assert all(out[name] is None for name in self.PROVENANCE), {
            name: out[name] for name in self.PROVENANCE if out[name] is not None
        }
