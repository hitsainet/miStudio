...
def mock_labeling_job():
    """A real LabelingJob row, not a Mock.

    IT WAS A `Mock()` WITH THIRTY-FIVE HAND-LISTED ATTRIBUTES. `Mock` invents an
    attribute for anything unset, so every field added to
    `LabelingStatusResponse` returned a `Mock` where Pydantic wanted a value —
    breaking four endpoint tests, twice in one session (`api_timeout`, then
    `skip_adjudicated`), each time with an error about types that said nothing
    about what was actually wrong.

    A hand-maintained list tracking a 35-field schema is the same drift class as
    everything else this arc removed. An ORM instance carries each column's real
    default, so a new column arrives correctly typed without anyone remembering
    this file. The endpoint's own response model still enforces completeness, and
    `test_orm_matches_migrated_schema` still holds the schema itself — this
    fixture was never the thing guarding either.

    Fields the tests assert on are set explicitly; the rest come from the model.
    """
    ...
    # A transient instance has not been through the DB, so column defaults are
    # not applied yet. Fill exactly the NOT NULL ones the response requires —
    # from the model's OWN declared defaults, never a literal repeated here.
    for column in LabelingJob.__table__.columns:
        if getattr(job, column.name, None) is None and column.default is not None:
            setattr(job, column.name, column.default.arg)

    # Extraction context, populated by `_enrich_labeling_responses` in production.
    job.model_name = None
