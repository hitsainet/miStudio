...
export interface ActivationExtractionConfig {
  ...
  /** `"auto"` (the card with the most free memory) or a GPU UUID */
  gpu?: string;
}
...
export interface Model {
  ...
  /**
   * Step of a running extraction while its status is 'extracting':
   * 'extracting' (GPU pass), 'merging' or 'statistics'. Stored server-side, so
   * it survives a reload; null when the server recorded none.
   */
