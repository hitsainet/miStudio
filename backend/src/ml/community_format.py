...
#: miStudio's TRAINING hook types and the TransformerLens hook each one captures
#: (review R1-A, A5): residual is the decoder layer's output (resid_post), mlp the
#: MLP module's output (mlp_out), attention the attention module's output after its
#: output projection (attn_out).
TRAINING_HOOK_POINTS = dict(
    residual="hook_resid_post",
    mlp="hook_mlp_out",
    attention="hook_attn_out",
)


def training_hook_point(layer: int, hook_type: str) -> str:
    """The TransformerLens hook name for an SAE miStudio trained at (layer, hook_type).

    Every exported cfg.json used to name the residual stream whatever the SAE's
    hook, so an MLP SAE from a multi-hook training was labelled a residual SAE.
    An unknown training hook type raises instead of defaulting.
    """
    if hook_type not in TRAINING_HOOK_POINTS:
        raise ValueError(
            "Unknown training hook type " + repr(hook_type) + "; expected one of "
            + ", ".join(sorted(TRAINING_HOOK_POINTS))
        )
    return "blocks." + str(int(layer)) + "." + TRAINING_HOOK_POINTS[hook_type]
...
class CommunityStandardConfig:
    ...
    def from_training_hyperparams(
        ...
        # Generate TransformerLens-compatible hook point name, for THIS SAE's hook
        # (review R1-A, A5). save_multilayer_community_checkpoint puts the hook type
        # into the hyperparameters it passes; this read "resid_post" for every SAE,
        # so an MLP or attention SAE was exported labelled as a residual one. A
        # config without one (single-hook legacy callers) is a residual SAE.
        hook_point = training_hook_point(layer, hyperparams.get("hook_type") or "residual")
        ...
        return cls(
            ...
            # Training resolves an unrecorded value to the FRAMEWORK default
            # (training_tasks: hp.get(..., fw["normalize_activations"])), so the
            # config must too. "none" here described an SAE that was never
            # trained that way, and every consumer reading cfg.json would then
            # encode in the wrong basis.
            normalize_activations=hyperparams.get("normalize_activations")
            or get_framework_defaults(arch_type)["normalize_activations"],
...
def convert_mistudio_to_community_weights(
    ...
    if is_jumprelu:
        ...
        # Decoder bias: decoder_bias (Standard/Skip), else TopK's b_pre, else decoder.bias.
        #
        # b_pre WAS DROPPED (found 2026-09-15 while building the post-run
        # evaluation's loader). TopKSAE keeps its centring bias as the parameter
        # `b_pre`; `decoder_bias` is only a property, so it is not in the state
        # dict. No key matched, every TopK export lost the bias, and a reloaded
        # TopK SAE centred its inputs on zero instead of the data mean.
        if "decoder_bias" in state_dict:
            community_weights["b_dec"] = state_dict["decoder_bias"]
        elif "b_pre" in state_dict:
            community_weights["b_dec"] = state_dict["b_pre"]
...
def load_gemma_scope_format(
    ...
    config = CommunityStandardConfig(
        ...
        extra_metadata={
            ...
            # B11: a params.npz records neither hook nor layer, so both values above are
            # ASSUMED when the path does not name a layer -- and an invented 0 is otherwise
            # indistinguishable from a recorded 0 for every reader of this config. Consumers
            # that decide where to read (the download task's row, the steering loader) must
            # not take an assumed layer as a record.
            "hook_point_assumed": layer is None,
        },
    )
...
def load_huggingface_sae_format(
    ...
    config = CommunityStandardConfig(
        ...
        extra_metadata={
            ...
            "l0_sparsity": l0_sparsity,
            # B11, as above: with no layer in the path the hook and layer written here are
            # this loader's assumption, not the file's record.
            "hook_point_assumed": layer is None,
        },
    )
