...
export interface ExtractionTemplateCreate {
  ...
  name: string;
  /**
   * Optional description. `null` CLEARS it and is not the same as omitting it:
   * the backend update uses `model_dump(exclude_unset=True)`, so an absent key
   * keeps the stored value while `null` writes the nullable column empty. The
   * form shares one payload object across create and update, so the create type
...
export interface ExtractionTemplateUpdate {
  /** Template name */
  name?: string;
  /** Description. `null` clears it; omitting the key keeps the stored value. */
  description?: string | null;
  /** Array of layer indices to extract from */
  layer_indices?: number[];
  /** Types of hooks to attach */
