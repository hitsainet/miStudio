...
### Reading the training card

The card is the live view of a run, and a few of its conventions are worth knowing:

- **L0 is shown as a count, not a fraction.** The card, its log lines and **Copy logs** all report
  L0 as the number of active features with the percentage beside it — `L0: 7 (0.02%)`, not
  `L0: 0.000`. The 10–100 target above is in those units. On a wide dictionary the fraction rounds
  to zero long before the count does.
- **The L0 chart carries a first → latest trend**, also in counts, so a slow drift is readable
  without reading off individual points.
- **The window is the last 1,000 logged steps**, which covers a full-length run rather than a
  recent sliver.
- **A missing point is drawn as a gap**, never as zero — a gap in reporting is not a run that
  reached zero sparsity.
- **A completion ETA** appears in your own timezone, computed from the recent step rate. It is
  deliberately absent for runs that are finished, paused, or have not reported in a while, because
  an estimate from a stalled clock recedes rather than converging.
...
:::warning Compare FVU within a layer, never across layers
FVU measures reconstruction error in each layer's own activation space, and the same error costs
the model different amounts depending on where in the stack it sits. So a lower FVU does **not**
mean a better SAE when the two layers differ.

Measured on one 16,384-feature run training three adjacent layers together:

| Layer | FVU (held-out) | Loss recovered vs mean-ablated |
|---|---|---|
| 11 | **0.2445** — best | 94.49% |
| 12 | 0.2967 | **95.37%** — best |
| 13 | 0.2967 | 94.26% |

FVU ranks layer 11 first by a clear margin. What the SAE actually costs the model ranks layer 12
first, with all three within about one point of each other. To compare SAEs **across** layers, use
**loss recovered vs mean-ablated** from the [model-cost evaluation](#model-cost-evaluation). Use FVU
to compare runs **at the same layer**.
:::
...
:::warning "Unseen" and "comparable" are not the same claim

Because both the training prefix and the held-out remainder are positional, what the evaluation
measures depends on the **block order of the tokenization** — check `shuffled` on it before
comparing numbers across runs.

- **Shuffled** (the default when a tokenization is written): the prefix is a uniform sample of the
  corpus and the held-out rows are drawn from the same distribution. The evaluation then measures
  generalisation, which is what these numbers are for.
- **Corpus order** (`shuffled` not set): the prefix is the corpus's opening slice and the held-out
  rows are its tail. Those can be materially different text — OpenHermes-2.5, for instance, is
  written one source at a time, so its first 10% of rows holds 2 of its 15 sources. The held-out
  rows are still genuinely unseen, but a score against them reflects a distribution shift as much
  as it reflects the SAE.

The same applies to the mixture: each corpus contributes its own tokenization's block order
independently, so a mixture can be uniform in some sources and positional in others.
:::
