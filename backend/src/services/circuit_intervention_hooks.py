...
def suppress_directional(hidden, decoder_weight, feature_idx, a_u, a_base=0.0,
    ...
    """
    # DEVICE AND DTYPE, NOT DTYPE ALONE. On a model split across GPUs the
    # decoder column and the encode that produced `a_u` can sit on a different
    # card from this layer's hidden state, and `hidden.sub_` then raises a
    # device mismatch at the first such layer. Only the dtype was ever
    # converted, because on one card the device always agreed. `delta` keeps
    # its own dtype so the arithmetic is bit-identical to the single-card path.
    d_i = decoder_weight[:, feature_idx].to(device=hidden.device, dtype=hidden.dtype)
    delta = (a_u - a_base)                          # [batch, seq]
    if hasattr(delta, "to"):
        delta = delta.to(hidden.device)
    if positions is not None:
        delta = delta * positions.to(device=delta.device, dtype=delta.dtype)
    # outer product delta ⊗ d_i → [batch, seq, d_model]
    hidden.sub_(delta.unsqueeze(-1) * d_i)
    return hidden
...
def suppress_feature_list(hidden, decoder_weight, feature_indices, a_us,
    ...
    # accumulate the combined delta·d_i then subtract once. `total` is on the
    # hidden state's card; each term is moved there first (see
    # suppress_directional — on a split model the decoder and the encode can be
    # on another card).
    total = torch.zeros_like(hidden)
    for idx, a_u, a_base in zip(feature_indices, a_us, a_bases):
        d_i = decoder_weight[:, idx].to(device=hidden.device, dtype=hidden.dtype)
        delta = (a_u - a_base)
        if hasattr(delta, "to"):
            delta = delta.to(hidden.device)
        if positions is not None:
            delta = delta * positions.to(device=delta.device, dtype=delta.dtype)
        total = total + delta.unsqueeze(-1) * d_i
    hidden.sub_(total)
    return hidden
