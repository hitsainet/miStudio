...
interface LayerRangePickerProps {
  ...
  busy?: boolean;
  /**
   * The footnote under the control.
   *
   * OVERRIDABLE because the same control now bounds two different things. The
   * default text describes a READOUT range ("re-read to capture only these"),
   * which is actively wrong when the range is choosing which layers an
   * intervention hooks — nothing is re-read and nothing is filtered.
   */
  hint?: string;
}
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 text-xs" data-testid="jlens-layer-range">
      ...
      <span className="basis-full text-[10px] text-slate-500 dark:text-slate-500">
        {hint ??
          'Narrowing filters the ranked lists only — the grid, rail and trajectory keep showing every layer that was read. Re-read to capture only these.'}
      </span>
    </div>
  );
}
