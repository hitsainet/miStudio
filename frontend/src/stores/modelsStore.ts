...
import { activeExtractionMessage } from '../utils/extractionPhase';
...
interface ModelsState {
  ...
  /** `gpu` is `"auto"` or a GPU UUID — required, so no caller can drop the user's choice. */
  downloadModel: (
    repoId: string,
  ...
  updateExtractionProgress: (modelId: string, extractionId: string, progress: number, status: string, message: string, phase?: string | null) => void;
...
export const useModelsStore = create<ModelsState>()(
  devtools(
    (set, _get) => ({
            ...
            models: state.models.map((model) =>
              model.id === modelId
                ? {
                    ...
                    // THE PERSISTED STEP. Status stays 'extracting' through the
                    // merge and the statistics, so without this a reload during
                    // either reads "Extracting Activations, 20000/20000".
                    extraction_phase: activeExtraction.phase ?? null,
                    extraction_message: activeExtractionMessage(activeExtraction),
                  }
                : model
            ),
          }));
      ...
      updateExtractionProgress: (modelId: string, extractionId: string, progress: number, status: string, message: string, phase?: string | null) => {
        console.log('[ModelsStore] updateExtractionProgress:', {modelId, extractionId, progress, status, message, phase});
        set((state) => ({
          models: state.models.map((model) =>
            model.id === modelId
              ? {
                  ...
                  // A message without a phase (starting, loading, saving)
                  // leaves the stored one alone; the label ignores it outside
                  // 'extracting' anyway.
                  extraction_phase: phase === undefined ? model.extraction_phase : phase,
                }
              : model
          ),
        }));
