...
def train_sae_task(
    ...
    try:
        ...
        if use_cached_activations:
            ...
            # RECORD WHAT THE RUN ACTUALLY DREW FROM EACH SOURCE.
            #
            # Only the REQUESTED `dataset_weights` were stored — a bare
            # positional array with nothing naming its positions — and the
            # realised split existed only in the log line above. So after the
            # fact nobody could tell what a run trained on, and a request that
            # could not be honoured left no trace outside stdout. Recorded in
            # BOTH branches: when nothing is allocated the whole pool is used,
            # which is itself a split (proportional to availability) worth
            # stating. Written to hyperparameters beside the request, following
            # `holdout_unmasked_extractions` — another runtime fact kept there.
            realised_mixture = dataset_mixture.realised_mixture_record(
                [e.id for e in extractions],
                plan_quotas if plan_quotas is not None else part_counts,
                requested_weights,
            )
            with self.get_db() as db:
                row = db.query(Training).filter_by(id=training_id).first()
                if row is not None:
                    # A NEW DICT: an in-place JSONB edit is invisible to the session.
                    row.hyperparameters = {
                        **(row.hyperparameters or {}),
                        'realised_dataset_mixture': realised_mixture,
                    }
                    db.commit()
            ...
            # The on-the-fly counterpart of the cached path's record: here the
            # realised split is the PER-REFILL quota each source contributes.
            with self.get_db() as db:
                row = db.query(Training).filter_by(id=training_id).first()
                if row is not None:
                    row.hyperparameters = {
                        **(row.hyperparameters or {}),
                        'realised_dataset_mixture': dataset_mixture.realised_mixture_record(
                            [src.label for src in token_sources], allocation, requested_weights,
                        ),
                    }
                    db.commit()
            activation_storage_plan = activation_plan.build_plan(
