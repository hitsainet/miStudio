...
celery_app.conf.update(
    ...
    task_routes={
        ...
        # Re-running a completed training's post-run evaluation loads its base
        # model: a GPU job, on the training queue in single mode (per-card mode
        # routes it through dispatch_gpu_task).
        "src.workers.training_evaluation_tasks.*": {
            "queue": "training",
        },
        "resume_training": {
            "queue": "training",
        },
        # CPU/DB cleanup, like delete_model_files and delete_dataset_files.
        # This was on the GPU `extraction` queue: on 2026-09-12 seven delete
        # requests for four SAE feature extractions sat behind ~3 hours of
        # activation extractions on the single solo worker, returned 202, and
        # changed nothing — so the operator retried, queueing duplicates.
        "delete_extraction": {
            "queue": "low_priority",
        },
        ...
        # PROBE MONITORS (032). Fully qualified, one entry per task, because
        # `task_routes` globs match the TASK NAME and a short name silently lands on
        # the default queue — which put two GPU trainings on the wrong queue here once.
        # The three GPU tasks share `extraction` with the other GPU profiles; the judge
        # is network-bound and belongs on `processing`, because taking a GPU lease for
        # an HTTP loop idles a card for its whole duration.
        ...
        # 033: building a definition runs real forward passes for the test vectors, so it
        # takes a card like any other GPU job.
        "src.workers.probe_monitor_tasks.build_probe_definition": {
            "queue": "extraction",
        },
        # Publishing is an HTTP upload. On the GPU queue it would hold a card's lease idle for
        # the whole network-bound transfer — the same reasoning that put the judge run here.
        "src.workers.probe_monitor_tasks.publish_probe_definition": {
            "queue": "processing",
        },
        ...
        # Same queue as the batches it starts. A sweep step is not itself heavy
        # — it does one batch inline and re-enqueues — but it must not outrun
        # the labeling work it schedules by sitting on a different queue.
        ...
        "cleanup_stuck_labeling_sweeps": {"queue": "low_priority"},
        "cleanup_stuck_enhanced_labeling": {"queue": "low_priority"},
        "cleanup_stuck_probe_monitor_runs": {"queue": "low_priority"},
    ...
    beat_schedule={
        ...
        # A probe run holds a GPU lease AND a multi-gigabyte token memmap, so a
        # phantom run keeps a card and the disk until something sweeps it.
        "cleanup-stuck-probe-monitor-runs": {
            "task": "cleanup_stuck_probe_monitor_runs",
            "schedule": 600.0,
            "options": {
                "queue": "low_priority",
                # SHORTER THAN THE PERIOD. A tick queued behind a long GPU task is
                # still delivered when the queue drains, so without an expiry an hour
                # of blocked ticks fires at once against state that has moved on.
                "expires": 540,
            },
        },
        ...
        # A sweep wedged between finishing a batch and queueing the next looks
        # `running` forever, and `create` refuses a second running sweep on the
        # extraction — so the wedge locks that extraction out of sweeping until
        # someone edits the row by hand. Hourly is ample: the threshold is six
        # hours, sized above a full 4.4 h batch.
        "cleanup-stuck-labeling-sweeps": {
            "task": "cleanup_stuck_labeling_sweeps",
            "schedule": 3600.0,
            "options": {
                "queue": "low_priority",
                "expires": 3540.0,
...
celery_app.autodiscover_tasks(
    [
        ...
        "src.workers.training_evaluation_tasks",  # Re-run a completed training's post-run evaluation
