...
export interface ExtractionStatusResponse {
  ...
  /** The FIRST corpus under a mixture; unchanged for single-corpus runs. */
  dataset_name: string | null;
  /**
   * Every corpus this extraction reads, in mixture order. Absent or null on
   * extractions created before multi-corpus extraction existed — "not
   * recorded", which is not the same as "one".
   */
...
export interface ExtractionStatistics {
  ...
  /**
   * Mean similarity of activation SHAPE across each feature's top examples.
   * Not an interpretability measure: it is highest for evidence that repeats
   * one token. Absent on extractions recorded before the rename.
   */
  avg_activation_consistency?: number;
}
...
export interface Feature {
  ...
  /** @deprecated Use `activation_consistency`; same value, honest name. */
  interpretability_score: number;
  /** Similarity of activation SHAPE across examples — not interpretability. */
  activation_consistency?: number;
  max_activation: number;
...
export interface BatchExtractionRequest {
  sae_ids: string[];
  /** Legacy single-corpus form, and the first entry of `dataset_ids`. */
  dataset_id: string;
  /**
   * Corpora every SAE in the batch reads. Without this the batch path would
   * narrow a multi-corpus selection to one corpus with no error at all.
   */
  dataset_ids?: string[];
  /**
   * Relative share of the evaluation samples per dataset, positional over
   * `dataset_ids`. Build only via `utils/extractionMixture`.
   */
...
export interface BatchExtractionResponse {
  ...
  dataset_id: string;
  /** The FIRST corpus of the batch mixture. */
  dataset_name: string | null;
  /** Every corpus every SAE in the batch reads, in mixture order. */
  dataset_names?: string[] | null;
}
