...
class TestTheCancelledDownloadCleansUpAfterItself:
    ...
    def test_it_targets_what_is_actually_on_disk(self):
        ...
        # ⚠ THE ARROW TREE IS NOW CONDITIONAL, AND THAT IS 032 FR-2.
        #
        # This assertion used to be `"___" in built` — the per-repo tree named
        # directly in the unconditional list. FR-2 made the download identity
        # (repo, config, split) instead of the repo, so several rows can share one
        # repo, and the tree is keyed on the REPO ALONE. Deleting it on a cancel
        # would therefore destroy a DIFFERENT config's completed download. The
        # justification the old list carried said exactly why that was safe before:
        # "one Dataset row per repo_id is enforced at `datasets.py`'s 409".
        #
        # So the tree is still cleaned — it must be, or the leak this test exists
        # to prevent returns — but only when nothing else reads it. Asserted
        # structurally: `hf_cache_dirname` must sit inside a branch guarded by
        # `_is_only_row_for_repo`. A substring check would pass on an `if False`.
        import ast as _ast

        tree = _ast.parse(inspect.cleandoc(src) if src.startswith(" ") else src)
        guarded = False
        for node in _ast.walk(tree):
            ...
            if "_is_only_row_for_repo" not in test_calls:
                continue
            body_calls = {
                n.func.id
                for n in _ast.walk(_ast.Module(body=node.body, type_ignores=[]))
                if isinstance(n, _ast.Call) and isinstance(n.func, _ast.Name)
            }
            if "hf_cache_dirname" in body_calls:
                guarded = True
                break
        assert guarded, (
            "the per-repo arrow tree is either not cleaned up at all (the cancelled "
            "download's real output leaks) or cleaned up unconditionally (cancelling "
            "one config deletes another config's completed data). It must be cleaned "
            "inside an `if _is_only_row_for_repo(...)` branch"
        )
