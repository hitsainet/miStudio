...
from ....services.gpu_dispatch import dispatch_gpu_task, gpu_delay
...
class FitRequest(BaseModel):
    ...
    #: WHICH GPU. A fit used to take `"cuda"`, the current device — index 0,
    #: which on this node is the 12 GB card whatever else was free.
...
async def fit(request: FitRequest, db: AsyncSession = Depends(get_db)) -> FitAccepted:
    ...
    # BEFORE THE CORPUS IS SAMPLED. An unknown card is a 400 naming the cards
    # that exist, and sampling a dataset first would make that answer wait on
    # a `load_from_disk` it did not need. "all" is a 400 too: a fit runs on ONE
    # card (see `fit_jlens_artifact`), and a 202 would queue a job that can only
    # be refused when the worker places it.
    ...
    task = gpu_delay(fit_jlens_artifact)(
    ...
    # same browser tab had started. The resolved GPU request rides in
    # `retry_params`, so a re-run lands on the card this one was asked for.
    jlens_progress.open_row(
        jlens_progress.FIT, request.model_id, task.id, retry_params={"gpu": gpu_request}
    )
...
class RevalidateRequest(BaseModel):
    ...
    allow_coverage_loss: bool = False
    allow_quality_regression: bool = False
    #: WHICH GPU. SEMANTIC runs a real forward pass, so this loads the model.
    gpu: GpuRequestStr = Field(AUTO, description=GPU_REQUEST_DESCRIPTION)
...
async def revalidate_staged(
    ...
    task = gpu_delay(revalidate_staged_artifact)(
        model_id=request.model_id,
        semantic_probe=request.semantic_probe,
        allow_coverage_loss=request.allow_coverage_loss,
        allow_quality_regression=request.allow_quality_regression,
    ...
    jlens_progress.open_row(
        jlens_progress.REVALIDATE,
        request.model_id,
        task.id,
        retry_params={"gpu": gpu_request},
    )
...
class BandReportRequest(BaseModel):
    ...
    #: WHICH GPU runs the profiling forward passes.
    gpu: GpuRequestStr = Field(AUTO, description=GPU_REQUEST_DESCRIPTION)
...
async def compute_band_report(
    ...
    gpu_request = resolve_gpu_request(request.gpu, can_split=True)
    task = gpu_delay(compute_band_report_task)(
        model_id=request.model_id,
        prompts=request.prompts,
...
async def readout(
    ...
    gpu_request = resolve_gpu_request(request.gpu, can_split=True)
    task = gpu_delay(compute_readout)(
        model_id=request.model_id,
        ...
        gpu_request=gpu_request,
        unload_after=request.unload_after,
    )
    # A ROW, SO THE CARD HAS SOMEWHERE TO BE RECORDED. The readout used to
    # capture on "any resident copy, else the CPU" and opened no row; it now
    # loads onto a GPU, and a job holding a card belongs in Active Operations.
    # `jlens_progress.READOUT` and the panel's label for it already existed.
    # `unload_after` rides beside `gpu`: a re-run that silently flipped it would
    # either strand a model on a card or pay a load the caller chose to avoid.
    jlens_progress.open_row(
        jlens_progress.READOUT,
        request.model_id,
        task.id,
        retry_params={"gpu": gpu_request, "unload_after": request.unload_after},
    )
...
async def probe(
    ...
    # A ROW for the card, for the readout's reason.
    jlens_progress.open_row(
        jlens_progress.PROBE,
        request.model_id,
        task.id,
        retry_params={"gpu": gpu_request},
    )
...
class InterventionRequest(BaseModel):
    ...
    #: WHICH GPU runs the three forward passes per trial.
    gpu: GpuRequestStr = Field(AUTO, description=GPU_REQUEST_DESCRIPTION)
...
def _name_pieces(tok, ids: List[int]) -> str:
    """The subwords a multi-piece string split into, for a human to read.

    NAMES, WITH THE IDS AS THE FLOOR. Seeing [' Jun', 'iper'] is what makes a
    multi-piece verdict actionable — it shows WHERE the split fell, which is
    usually one leading space away from being fixed.

    But this text goes straight to a user, and `decode` is the only call this
    endpoint makes that is not `encode`: a tokenizer that cannot decode renders
    its own repr into the sentence. Not hypothetical — it reached a test
    assertion as `2 tokens [<MagicMock name='mock.decode()' id=...>]`. Anything
    that is not a real string falls back to the ids, which are already on the
    response and cannot be wrong.
    """
    try:
        names = [tok.decode([i]) for i in ids]
    except Exception:  # noqa: BLE001 - a verdict must not fail on cosmetics
        return repr(ids)
...
async def token_check(
    ...
    a leading space usually decides it. A split string is not refused — the
    worker steers along the MEAN of its pieces' rows and scores the FIRST piece
    — but that is weaker evidence than a single token, and the spaced form is
    usually single. Saying which case you are in is knowable here, from the
    tokenizer alone, without loading any weights, instead of after a 202 and a
    slot on a single-GPU queue that may be behind a 45-minute fit.

    `usable` therefore still means EXACTLY ONE TOKEN. It is not "will the run
    succeed" — a multi-piece string runs — it is "is this the strong form".
    """
    from ....models.model import Model
    ...
    for raw in request.tokens:
        ...
        if n == 1:
            ...
            # MULTI-PIECE IS USABLE NOW, AND WEAKER. It used to be refused
            # outright, which excluded most of the interesting vocabulary: the
            # common case is an ORDINARY WORD the tokenizer splits into
            # subwords, not a phrase. The direction becomes the MEAN of the
            # pieces' rows and the score is the rank of the FIRST piece, which
            # is the only thing a next-token distribution can answer for.
            #
            # THE LEADING-SPACE HINT STILL COMES FIRST, because it is the cause
            # almost every time, it is invisible in a text box, and the spaced
            # form is usually a single token — strictly better evidence.
            alt = tok.encode(f" {raw.strip()}", add_special_tokens=False)
            detail = (
                f"{n} tokens {_name_pieces(tok, ids)} — usable: the direction is "
                "their MEAN and the score is the rank of the first piece alone, "
                "which is weaker than a single-token target."
                + (
                    f" Try ' {raw.strip()}' with a leading space — that is one"
                    " token, and a stronger measurement."
                    if len(alt) == 1
                    else ""
                )
            )
        out.append(
            TokenCheck(
                ...
                ids=ids,
                n_tokens=n,
                # `usable` STILL MEANS "one token", which is what every existing
                # caller reads it as. A multi-piece string is now accepted by the
                # worker but is NOT equivalent evidence, so widening this flag
                # would have quietly told every caller the two are the same.
                usable=(n == 1),
                detail=detail,
            )
        )
...
def _queue_without_leaking_token(task: Any, **kwargs: Any) -> Any:
    ...
    # Through the GPU routing helper: `acquire` is a GPU task (its card queue in
    # per-card mode); `publish` is not, and keeps its static route.
    return dispatch_gpu_task(
        task, gpu_request=kwargs.get("gpu_request"), kwargs=kwargs, kwargsrepr=repr(redacted)
    )
...
class AcquireRequest(BaseModel):
    ...
    #: WHICH GPU loads the model for the SEMANTIC readout. Chosen after the
    #: download, so Auto judges the memory free when the load happens.
    gpu: GpuRequestStr = Field(AUTO, description=GPU_REQUEST_DESCRIPTION)
...
async def acquire_artifact(
    ...
    # 4. THE CARD MUST EXIST. Which one is free is judged by the worker when it
    #    loads, but a card this node does not have is knowable now.
    gpu_request = resolve_gpu_request(request.gpu, can_split=True)
    ...
    # than in the worker so a job that never gets picked up still appears. The
    # GPU request rides in `retry_params`; the token never does.
    jlens_progress.open_row(
        jlens_progress.ACQUIRE,
        request.model_id,
        task.id,
        retry_params={"gpu": gpu_request},
    )
...
async def run_intervention(
    ...
    # THE COVERAGE REFUSAL, WHERE IT IS FREE.
    #
    # Naming `artifact_id` credits that lens: the run appends an
    # `evidence_rung: 2` record to its `interventions.json`, the file that
    # travels to HuggingFace and into a serving runtime. The worker refuses
    # layers the artifact was never fitted at — but only after this route has
    # returned 202 and the job has taken a slot on a single-GPU queue that may
    # sit behind a 45-minute fit. Which layers a lens covers is on disk and
    # needs no model, so the honest answer is a 400 before anything is queued.
    #
    # THE WORKER KEEPS ITS CHECK. This one reads the artifact listing; that one
    # reads the transport actually built for the run. A caller reaching the
    # worker by another route must still be refused.
    if request.artifact_id:
        # RESOLVED THE WAY THE WORKER RESOLVES IT — by the MODEL's repo id, via
        # `service.find(loaded.name)`, not by treating the slug as a key into
        # every artifact on disk. A slug belonging to a DIFFERENT model would
        # otherwise be found here and its coverage used to compute a 400 about
        # layers, pre-empting `_validated_report`'s accurate "that artifact is
        # not fitted for this model".
        service = JLensArtifactService(settings.jlens_artifacts_dir)
        repo_id = getattr(model_row, "repo_id", None)
        ref = service.find(repo_id) if repo_id else None
        # Only this model's own artifact is checked here. A mismatch is the
        # worker's to refuse, with the reason that actually applies.
        if ref is not None and ref.slug == request.artifact_id:
            # FROM config.yaml, NOT THE TENSORS. `_load_payload` does a full
            # `torch.load` of the Jacobian — ~276 MB for the artifacts on this
            # box and over a gigabyte for a full large-model fit — synchronously
            # inside an `async def`, which blocks the API event loop on every
            # Steer and Swap click. `fitted_layers` answers the same question
            # from the config file with the same empty-means-unknown semantics,
            # and is what `/jlens/artifacts` already uses.
            fitted = sorted(service.fitted_layers(ref))
            # EMPTY MEANS UNKNOWN, not "covers nothing". An artifact whose
            # payload will not deserialize is a different failure, and the
            # worker reports it with the right words.
            if fitted:
                uncovered = sorted(set(request.layers) - set(fitted))
                if uncovered:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail=(
                            f"layers {uncovered} are not fitted in artifact "
                            f"{request.artifact_id!r} (it covers {fitted}), so it "
                            "cannot be credited for an intervention there. Drop "
                            "artifact_id to run this as an exploratory "
                            "intervention, or choose layers the lens covers."
                        ),
                    )
    ...
    gpu_request = resolve_gpu_request(request.gpu, can_split=True)
    task = gpu_delay(run_intervention_task)(
        model_id=request.model_id,
        prompt=request.prompt,
...
async def cost_estimate(
    ...
    PASS `weights_bytes` FOR A FIT. The resident model dominates a fit's peak —
    12.79 of 16.6 GiB on the one run that has been measured — and this endpoint
    cannot infer it from `d_model` alone. Omitted, the peak is everything EXCEPT
    the weights, and the returned `basis` says so.
    """
