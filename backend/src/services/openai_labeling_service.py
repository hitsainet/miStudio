...
# the template asked the model to refuse when under 60% of its examples fit,
...
# 0.6, not 0.5: every template here tells the judge to refuse below 60% fit.
# At 0.5 a self-reported 5/10 was told to refuse and then kept its name anyway
# — the exact advisory-only gap this constant exists to close.
MIN_FIT_RATIO = 0.6
...
def _enforce_refusal(label: Dict[str, str]) -> Dict[str, str]:
    ...
    1. If the model reports that under MIN_FIT_RATIO of its examples fit, the label is
    ...
    The model's own assessment stays in `fit_count`/`confidence`, which the
    labeling write path persists as `features.label_fit_count` /
    `label_confidence` (until 2026-09-22 they were dropped at that step, and
    this sentence claimed otherwise). Only the authoritative fields change.
    """
...
"""
    ...
    async def generate_label(
        ...
        if not token_stats:
            ...
            # `error` marks a FAILURE. `category` is the judge's semantic answer
            # and cannot also carry that — overloading it is why 16,824 failed
            # features across the estate look finished.
            return {
                "category": "empty_features",
                "specific": fallback_label,
                "error": "no activating tokens were retrieved for this feature",
            }
        ...
        try:
            ...
            return {
                ...
                "specific": fallback_label,
                "error": f"rate limited by the judge endpoint: {e}",
            }
            ...
            # The exception text used to stop at these log lines. It is the only
            # account of why the feature has no label, so it travels with the
...
"""
    ...
    def _format_examples_block(
        ...
        if template_type == 'anthropic_logit':
            ...
            # REFUSE, rather than call a function whose signature this is not.
            #
            # This called `format_eleutherai_detection(examples=...,
            # template_config=...)` against a signature of
            # `(feature_explanation, test_examples)` — a guaranteed TypeError,
            # which the caller's broad `except Exception` at
            # generate_label_from_examples then swallowed into an error label.
            # So selecting the seeded `lpt_eleutherai_detection` template for a
            # labeling run produced a run where every single feature "failed"
            # for a reason nobody could see.
            #
            # A detection template SCORES an explanation that already exists; it
            # is not a labeling template and has no examples_block to render.
            # `LabelingTrialService.start_trial` already refuses these
            # (labeling_trial_service.py:168-172) — this is the same refusal on
            # the bulk path, made loud instead of silent.
            raise NotImplementedError(
                "eleutherai_detection is a scoring template, not a labeling "
                "template: it formats test passages for a detection run and "
                "cannot render an examples block. Choose a labeling template "
                "(template_type 'mistudio_context' or 'anthropic_logit')."
            )
    ...
    async def _generate_chunk_batched(
        ...
        for req in chunk:
            user_prompt = self._resolve_user_prompt(
                ...
                negative_examples=req.get("negative_examples"),
            )
    ...
    async def generate_label_from_examples(
        ...
        try:
            ...
            if self.save_requests_for_testing:
                import random
                if random.random() <= self.save_requests_sample_rate:
                    # `request_payload` is assigned AFTER the prompt is built,
                    # so any exception raised while building it — the detection
                    # template refusal included — reaches here with the name
                    # unbound. The handler would then raise UnboundLocalError
                    # out of its own except block, replacing a diagnosable
                    # failure with an opaque one, and only when debug saving is
                    # switched on. `locals()` is already the idiom used two
                    # lines below for `response`.
        ...
        for i in range(0, total, batch_size):
            ...
            for j, label in enumerate(batch_labels):
                if isinstance(label, Exception):
                    ...
                    label = {
                        "category": "error_feature",
                        "specific": fallback_name,
                        "error": f"{type(label).__name__}: {label}",
                    }
