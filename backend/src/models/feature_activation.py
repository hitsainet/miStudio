...
class FeatureActivation(Base):
    ...
    #: Which corpus this row's `sample_index` refers to.
    #:
    #: NULLABLE, no default, no server_default and no backfill — NULL means
    #: "not recorded", which is the truth for every row written before
    #: multi-corpus extraction existed. Under a mixture `sample_index` alone is
    #: NO LONGER UNIQUE per feature (each corpus contributes its own global
    #: range), so any identity, dedupe or exclusion logic must carry this
    #: column too or it silently weakens.
