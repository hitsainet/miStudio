...
OPERATOR_ROUTES = {
    ...
    "labeling_sweep": "/api/v1/labeling/resume-sweeps/{sweep_id}/cancel",
    "circuit_faithfulness": "/api/v1/circuits/{circuit_id}/faithfulness/cancel",
    # 032. The judge's route existed only after this guard demanded it: the scope was
    # registered with nothing able to use it, so a judge baseline could be started and
    # never stopped — thousands of paid calls for a result nobody was waiting for.
    "probe_monitor_run": "/api/v1/probe-monitors/runs/{run_id}/cancel",
    "probe_monitor_judge": "/api/v1/probe-monitors/judge-runs/{judge_run_id}/cancel",
