...
describe('Resume on a finished labeling job', () => {
  ...
  it('resumes a local-model job on the GPU it asked for', async () => {
    const uuid = 'GPU-247aa582-0d1b-e161-8156-983ed1fefc57';
    const { resumeLabeling } = stubStore();
    renderCard({ labeling_method: 'local', local_model: 'lfm2.5', gpu_request: uuid, gpu_uuid: uuid });
    fireEvent.click(await screen.findByText('Resume 2 of 52,857'));

    await waitFor(() => expect(resumeLabeling).toHaveBeenCalledTimes(1));
    // A resume is a new job: without the request it silently runs on Auto.
    expect(resumeLabeling.mock.calls[0][1]).toMatchObject({ labeling_method: 'local', gpu: uuid });
  });

  it('resumes an older local-model job, which recorded no request, on Auto', async () => {
    const { resumeLabeling } = stubStore();
    renderCard({ labeling_method: 'local', local_model: 'lfm2.5', gpu_request: null });
    fireEvent.click(await screen.findByText('Resume 2 of 52,857'));

    await waitFor(() => expect(resumeLabeling).toHaveBeenCalledTimes(1));
    expect(resumeLabeling.mock.calls[0][1]).toMatchObject({ gpu: 'auto' });
  });

  it('sends no GPU for a remote judge', async () => {
    const { resumeLabeling } = stubStore();
    renderCard();
    fireEvent.click(await screen.findByText('Resume 2 of 52,857'));

    await waitFor(() => expect(resumeLabeling).toHaveBeenCalledTimes(1));
    expect(resumeLabeling.mock.calls[0][1]).not.toHaveProperty('gpu');
  });
});
