...
def normalize_token_text(token: str) -> str:
    """Strip tokenizer word-boundary markers so a token compares as a word.

    TWO MARKER CONVENTIONS EXIST AND ONLY SENTENCEPIECE'S WAS HANDLED. Byte-level
    BPE — GPT-2, Llama 3, LFM2, and every model trained here — encodes a leading
    space as 'Ġ' (U+0120) and a newline as 'Ċ' (U+010A). So the comparison text
    for 'Ġthe' stayed 'Ġthe', which is in neither STOP_WORDS nor WORD_FRAGMENTS,
    and every space-prefixed token silently bypassed both filters. That is most
    tokens in running text: on the 16k L11 extraction the stored prime tokens are
    'Ġthe' (11,266), 'Ġand' (11,182), 'Ġto' (9,324) — roughly 18% of all stored
    evidence in the top fifteen alone, none of it reachable by
    `filter_stop_words=True`.

    ONE DEFINITION, THREE CALLERS. This logic was copy-pasted at three sites
    (`is_junk_token`, `clean_token_display`, `filter_token_stats`) and all three
    carried the same omission. Worse, the stats helper MUST agree with the filter
    or the reported counts describe filtering that did not happen. Extracting the
    decision is the fix; the missing marker was only the symptom.

    Returns the bare word, stripped and unchanged in case. Callers that compare
    against the lowercase sets apply `.lower()` themselves.

    >>> normalize_token_text('Ġthe')
    'the'
    >>> normalize_token_text('▁the')
    'the'
    >>> normalize_token_text('the')
    'the'
    >>> normalize_token_text('Ġ')
    ''
    """
    return (
        token.replace('▁', '')   # SentencePiece word boundary
        .replace('Ġ', '')        # byte-level BPE: leading space
        .replace('Ċ', '')        # byte-level BPE: newline
        .strip()
    )
...
def _byte_level_decoder() -> Dict[str, int]:
    """The inverse of byte-level BPE's byte->printable-character table.

    The table is fixed by the algorithm: bytes that are already printable map to
    themselves, everything else is lifted into U+0100 upward in order. Built once.
    """
    bs = (
...
def decode_byte_level_token(token: str) -> str:
    """Turn a byte-level BPE token back into the text it represents.

    STRIPPING THE MARKER IS NOT DECODING. `normalize_token_text` removes 'Ġ' and
    gives you the word, which is all a STOP_WORDS comparison needs. It does
    nothing for a token whose bytes are non-ASCII, and those stay mojibake:
    'âĢĻs' is the byte-level spelling of "’s" (U+2019 + 's') and was the SECOND
    most common prime token on the 16k L12 extraction at 3,015 rows. Anything
    that shows a token to a human — or to a judge — must decode, not strip.

    FAILS OPEN. A token holding any character outside the table is not
    byte-level (SentencePiece's '▁the', a special token like '<|endoftext|>')
    and comes back UNCHANGED for a marker-stripper to handle. Returning
    something mangled would be worse than returning the input.

    >>> decode_byte_level_token('âĢĻs')
    '’s'
    >>> decode_byte_level_token('Ġthe')
    ' the'
    >>> decode_byte_level_token('▁the')
    '▁the'
    """
    decoder = _byte_level_decoder()
    try:
        raw = bytes(decoder[ch] for ch in token)
    except KeyError:
        return token
...
def is_junk_token(
    ...
    Examples:
        ...
        False
    """
    # Clean token for comparison. See `normalize_token_text` for why this is not
    # inlined — three copies of it disagreed with the tokenizers in use here.
    #
    # Only `cleaned` changes. The single-char and punctuation tests deliberately
    # keep reading the RAW token, so their behaviour is untouched.
    cleaned = normalize_token_text(token).lower()
...
def clean_token_display(token: str) -> str:
    ...
    Examples:
        ...
        'world'
    """
    # DECODE first, then strip markers. Decoding is what turns 'âĢĻs' into "’s";
    # stripping alone leaves every non-ASCII token as mojibake on screen.
    cleaned = normalize_token_text(decode_byte_level_token(token))
    ...
    if apply_filters:
        ...
        for token, count in token_counter.items():
            ...
            if filter_special and token in ['<s>', '</s>', '<pad>', '<unk>', '\ufeff', '▁']:
                ...
                # MUST match `is_junk_token`'s normalisation exactly, or these
                # counts describe filtering that did not happen.
                cleaned = normalize_token_text(token).lower()
