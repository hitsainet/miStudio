...
from ..ml.model_devices import detach_dispatch_hooks, input_device
...
from .gpu_placement import Placement
...
def _on_cuda(device: Any) -> bool:
    """True when ``device`` is a CUDA device this process can use.

    The device is always handed in — ``gpu_placement.place_job`` chooses it in
    the worker. Nothing in this service picks a card, so a job placed on the
    3090 (index 1 since 2026-09-13) is measured and cleaned there, not on index 0.
    """
    return device is not None and torch.device(device).type == "cuda" and torch.cuda.is_available()


def _devices(devices: Any) -> tuple:
    """One device, or every device of a placement, as a tuple of torch devices.

    Memory is measured and released on EVERY card a job uses. Written for one
    card, these helpers took a single device, so a model split across two cards
    was measured on its first card and the other card's weights were neither
    released nor reported.
    """
...
def load_format_for(quantization: QuantizationFormat) -> QuantizationFormat:
    """The format an extraction actually loads a model row at.

    The single-card load always passes ``torch_dtype=float16``, so an FP32 row
    has always been extracted at fp16. A split loads through
    ``load_model_from_hf``, which would load FP32 at float32 — and the same row
    would then give different activations depending on how many cards it
    landed on. Mapped here so both paths, and the size the job is placed
    against, agree.
    """
    return QuantizationFormat.FP16 if quantization == QuantizationFormat.FP32 else quantization


def resolve_model_snapshot(model_path: str) -> str:
    """The directory holding a downloaded model's files.

    Downloads land in a HuggingFace cache (``models--org--name/snapshots/<hash>/``);
    older models were stored flat. Shared by the load and by the worker that
    sizes the job before placing it, so the two read the same config.
    """
...
class ModelLoadOutOfMemory(ActivationExtractionError):
    """The extraction's MODEL LOAD ran out of GPU memory, before any batch ran.

    Not a batch that ran out of memory: a smaller batch changes nothing about a
    model that does not fit, so ``classify_extraction_error`` suggests no retry
    parameters for it (review round 3; it used to suggest halving the batch).
    """
...
class ActivationService:
    ...
    def _log_gpu_memory(self, stage: str, devices: Any) -> None:
        """
        Log current GPU memory usage on each card.

        Args:
            stage: Description of the current stage (e.g., "before_load", "after_extraction")
            devices: The device, or every device, the job was placed on
        ...
        for device in _devices(devices):
            if _on_cuda(device):
                ...
                logger.info(f"[{device} Memory - {stage}] Allocated: {allocated:.2f} GB, Reserved: {reserved:.2f} GB")

    #: Reserved GB left on ONE card that still counts as a successful release.
    #: A CUDA context plus allocator bookkeeping is a few hundred MB; anything
    #: approaching a model's footprint is a pool that never came back. Checked
    #: per card: summed over a split, two healthy contexts would read as a leak,
    #: and a leak on one card could hide under a clean total.
    RESERVED_FLOOR_GB = 1.0

    @staticmethod
    def _gpu_memory(device: torch.device) -> dict:
        """Allocated and reserved GB on ``device``, or zeros when it is not a CUDA device."""
        if not _on_cuda(device):
    ...
    def _gpu_memory_on(self, devices: Any) -> Dict[str, dict]:
        """:meth:`_gpu_memory` for each device, keyed by its name."""
        return {str(device): self._gpu_memory(device) for device in _devices(devices)}

    def _release_gpu_memory(self, devices: Any) -> dict:
        """Hand the allocator's pool back on every card when there is no model object to drop.
        ...
        next extraction starting on a clear card or on a full one. A split load
        that died part-way left weights on each of its cards, so each is emptied.
        """
        import gc

        devices = _devices(devices)
        before = self._gpu_memory_on(devices)
        ...
        for device in devices:
            if _on_cuda(device):
                try:
                    ...
                    logger.warning(f"Could not empty the CUDA cache on {device}: {e}")

        return self._report_cleanup(devices, before)

    def _report_cleanup(self, devices: Any, before: Dict[str, dict]) -> dict:
        """Compare each card before and after, and say which one happened.

        ``before`` is :meth:`_gpu_memory_on` over the same devices. The result
        carries each card under ``"devices"`` and the totals across them under
        ``"before"``, ``"after"`` and ``"released"``.
        """
        per_device: Dict[str, dict] = {}
        for device in _devices(devices):
            key = str(device)
            was = before[key]
            now = self._gpu_memory(device)
            released = was["reserved"] - now["reserved"]

            if now["reserved"] > self.RESERVED_FLOOR_GB:
                logger.warning(
                    f"{device} did not give the memory back: reserved "
                    f"{was['reserved']:.2f} -> {now['reserved']:.2f} GB "
                    f"(released {released:.2f} GB) with {now['allocated']:.2f} GB "
                    "still allocated. That pool is unavailable to anything else on "
                    "the card until this process exits."
                )
            else:
                logger.info(
                    f"Model unloaded from {device}: released "
                    f"{released:.2f} GB, {now['reserved']:.2f} GB reserved"
                )
            per_device[key] = {"before": was, "after": now, "released": released}

        def _total(which: str) -> dict:
            return {
                field: sum(card[which][field] for card in per_device.values())
                for field in ("allocated", "reserved")
            }

        return {
            "before": _total("before"),
            "after": _total("after"),
            "released": sum(card["released"] for card in per_device.values()),
            "devices": per_device,
        }

    def _cleanup_model(self, model: torch.nn.Module, devices: Any) -> dict:
        """
        ...
        Args:
            ...
            devices: The device, or every device, the model was loaded on
                (``Placement.all_devices``)
        """
        import gc

        devices = _devices(devices)
        cuda = [device for device in devices if _on_cuda(device)]
        before = self._gpu_memory_on(devices)

        try:
            # Log memory before cleanup
            self._log_gpu_memory("before_cleanup", devices)

            # Synchronize CUDA to ensure all operations are complete
            for device in cuda:
                torch.cuda.synchronize(device)

            # A SPLIT MODEL CARRIES ACCELERATE'S DISPATCH HOOKS. They move
            # tensors between cards inside forward, `model.cpu()` below fights
            # them, and they hold references to the modules this cleanup is
            # dropping. A model loaded onto one card has none, so this is a
            # no-op there.
            try:
                detach_dispatch_hooks(model)
            except Exception as e:
                logger.warning(f"Could not remove dispatch hooks before cleanup: {e}")
            ...
            # Empty the CUDA cache on EVERY card the model used. The caching
            # allocator is per device: emptying the first card of a split
            # leaves the other card's share of the weights reserved for the
            # life of the worker.
            for device in cuda:
                with torch.cuda.device(device):
                    torch.cuda.empty_cache()
            ...
            self._log_gpu_memory("after_cleanup", devices)
    ...
    def extract_activations(
        ...
        *,
        placement: Placement,
        merge_progress_callback: Optional[Callable[[int, int, int, int], None]] = None,
        ...
        Args:
            ...
            placement: Where the job was placed, from
                ``gpu_placement.place_job``. Required, with no default: a
                default here would be a hard-coded card. A single-card
                placement loads the whole model onto its device; a split
                (``placement.is_shard``) loads across its cards within
                ``placement.max_memory``, and memory is measured and released
                on every one of them.
            merge_progress_callback: Optional callback
                function(layer_index, n_layers, batches_done, n_batches), called
                on this thread while per-batch files are merged into one file
                per layer: once before each layer's first copy, then after
                every copy. The merge phase's heartbeat and cancellation
                checkpoint.
        ...
        # Every card the job may hold memory on: one, or each card of a split.
        devices = placement.all_devices
        where = " + ".join(str(device) for device in devices)

        model = None  # Initialize to None for cleanup in finally block
        # Whether the model load returned. `model` is None again once the
        # extraction releases it, so it cannot say which phase ran out of memory.
        model_loaded = False
        try:
            # Log GPU memory before loading model
            self._log_gpu_memory("before_load", devices)

            # Load model
            logger.info(f"Loading model from {model_path} to {where}")
            model, tokenizer = self._load_model(model_path, quantization, placement=placement)
            model_loaded = True
            model.eval()  # Set to evaluation mode

            # Log GPU memory after loading model
            self._log_gpu_memory("after_load", devices)
            ...
            activations = self._run_extraction(
                ...
                on_merge_progress=merge_progress_callback,
            )

            # Log GPU memory after extraction
            self._log_gpu_memory("after_extraction", devices)
            ...
            if model is not None:
                self._cleanup_model(model, devices)
            ...
            if "out of memory" in str(e).lower():
                ...
                # A LOAD THAT RAN OUT OF MEMORY IS NOT A BATCH THAT DID: its
                # classification must not suggest halving the batch.
                failure = ActivationExtractionError if model_loaded else ModelLoadOutOfMemory
                raise failure(
            ...
            if model is not None:
                logger.info(f"Cleaning up model for extraction {extraction_id} on {where}")
                self._cleanup_model(model, devices)
            else:
                ...
                logger.info(
                    ...
                    f"whatever a failed load left on {where}"
                )
                self._release_gpu_memory(devices)
    ...
    def _load_model(
        ...
        Args:
            ...
            quantization: Quantization format
            placement: Where the job was placed. One card takes the whole
                model; a split (``placement.is_shard``) spreads it over its
                cards within ``placement.max_memory``. Nothing is offloaded to
                the CPU or to disk either way.
        ...
        actual_model_path = resolve_model_snapshot(model_path)

        # A GPU is required, and it is the one the job was placed on — every one,
        # for a split. This used to build `cuda:{gpu_id}` from an index that
        # defaulted to 0; the card's existence is now checked by the placement,
        # by UUID.
        for placed in placement.all_devices:
            if not _on_cuda(placed):
                raise ActivationExtractionError(
                    f"CUDA is not available on {placed}. GPU is required for activation extraction."
                )

        if placement.is_shard:
            return self._load_split_model(actual_model_path, quantization, placement)

        device = torch.device(placement.device)
        logger.info(f"Loading model to device: {device} (CUDA device: {torch.cuda.get_device_name(device)})")
    ...
    def _load_split_model(
        ...
        """Load a model that no single card can hold across the placement's cards.

        The single-card load forces ``device_map={"": device}`` because an
        automatic map without a budget lets accelerate offload to the CPU. A
        split needs accelerate to map the layers, so it goes through
        ``load_model_from_hf`` with the placement's own ``device_map`` and
        GPU-only ``max_memory``, passed through unchanged: that loader
        preflights the split's cards and REFUSES a load accelerate still maps
        to cpu or disk — one refusal, shared with every other split load,
        rather than a second copy of it here.

        Loaded at the same dtype and quantization as the single-card path
        (:func:`load_format_for`), with no out-of-memory fallback: an extraction
        read at a quantization nobody asked for is a silently different dataset.
        """
        load_format = load_format_for(quantization)
        model, tokenizer, _config, _metadata = load_model_from_hf(
            repo_id=actual_model_path,
            quant_format=load_format,
            device_map=placement.device_map,
            max_memory=placement.max_memory,
            local_files_only=True,
        )
        layout = getattr(model, "hf_device_map", None) or {}
        logger.info(
            "Model loaded split across %s (max_memory %s): %d modules over devices %s; "
            "dtype: %s, quantization: %s (loaded as %s)",
            " + ".join(str(device) for device in placement.all_devices),
            placement.max_memory, len(layout), sorted({str(target) for target in layout.values()}),
            model.dtype, quantization.value, load_format.value,
        )
        return model, tokenizer
    ...
    def _run_extraction(
        ...
        on_merge_progress: Optional[Callable[[int, int, int, int], None]] = None,
    ) -> Dict[str, np.ndarray]:
        """
        ...
        with HookManager(model) as hook_manager:
            ...
            with torch.no_grad():
                for batch_start in range(0, len(dataset), batch_size):
                    ...
                    for micro_batch_start in range(0, len(padded_input_ids), effective_micro_batch):
                        ...
                        # To the INPUT EMBEDDING's card, not `model.device`.
                        # transformers defines `model.device` as the first
                        # parameter's device; on a model split across cards
                        # that need not be where the embedding landed, and the
                        # lookup then fails with a device mismatch inside the
                        # first layer. On one card the two are the same device.
                        inputs_on = input_device(model)
                        input_ids_tensor = torch.tensor(micro_input_ids, dtype=torch.long).to(inputs_on)
                        attention_mask_tensor = torch.tensor(micro_attention_masks, dtype=torch.long).to(inputs_on)
            ...
            activations = self._merge_batch_files(
                accumulated_files, output_dir, on_merge_progress=on_merge_progress
            )
    ...
    def _merge_batch_files(
        self,
        accumulated_files: Dict[str, List[str]],
        output_dir: Path,
        on_merge_progress: Optional[Callable[[int, int, int, int], None]] = None,
    ) -> Dict[str, np.ndarray]:
        """Merge each layer's per-batch .npy files into one memory-mapped file.

        This is the merge phase. It runs after the GPU pass has reported N/N and
        before statistics, it is disk-bound, and on a 10,000 x 2,048-token
        extraction it runs long enough for the row to go stale without a
        heartbeat.

        `on_merge_progress(layer_index, n_layers, batches_done, n_batches)` runs
        on this thread once before each layer's first copy and again after every
        copy. It is the caller's heartbeat and cancellation checkpoint, so it is
        not wrapped in `except Exception`: OperatorCancelled derives from
        BaseException and must reach the task. The heartbeat swallows its own
        write and emit failures.

        Each shard is deleted once it has been copied. A layer's merged array is
        left at ``{layer}_temp_concat.npy``, which `_save_activations` renames.
        """
        activations: Dict[str, np.ndarray] = {}
        layers_to_merge = [
            (layer_name, temp_files)
            for layer_name, temp_files in accumulated_files.items()
            if temp_files
        ]
        n_merge_layers = len(layers_to_merge)
        for merge_index, (layer_name, temp_files) in enumerate(layers_to_merge):
            n_batches = len(temp_files)
            # Before the layer's first copy. The first of these calls is the
            # transition out of the GPU pass, so the caller records "merging"
            # before the long copy starts, not after it.
            if on_merge_progress is not None:
                on_merge_progress(merge_index, n_merge_layers, 0, n_batches)

            logger.info(f"Processing {n_batches} batch files for {layer_name}")
            ...
            current_idx = 0
            for i, (temp_file_path, rows) in enumerate(zip(temp_files, batch_sizes)):
                ...
                final_array[current_idx:current_idx + rows] = batch_array
                current_idx += rows
                ...
                if on_merge_progress is not None:
                    on_merge_progress(merge_index, n_merge_layers, i + 1, n_batches)
            ...
            logger.info(f"Concatenated {n_batches} batches for {layer_name}: final shape={final_array.shape}")
    ...
    def _calculate_statistics(
        ...
        for layer_index, (layer_name, activation_array) in enumerate(activations.items()):
            ...
            # Before the layer is reduced. The first of these calls is the
            # transition into the statistics phase, so the row reads
            # "statistics" while layer 0 is being reduced, not after it.
            if on_progress is not None:
                on_progress(layer_index, n_layers, 0, 1)
