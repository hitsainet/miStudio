...
#: What an artifact that cannot be sized raises: a bad config or a missing row.
#: A database error is not among them, so it fails the run rather than hide.
_UNSIZABLE = (RecordRunError, SteeringCoreError, KeyError, TypeError, ValueError)
...
class SteeringRecorderService:
    ...
    @classmethod
    def _resolve(cls, artifact: Dict[str, Any], db, device, structure=None):
        """Dispatch to the per-type resolver → (model_id, resolved_members).

        ``structure`` is passed only for a model split across cards, so each
        member's W_dec lands on its own layer's card."""
        extra = {} if structure is None else {"structure": structure}
        kind = artifact["kind"]
        if kind == "circuit":
            ...
            return resolve_circuit_members(circ, db, device, **extra)
        if kind == "cluster":
            return resolve_cluster_members(artifact["cluster_profile_id"], db, device,
                                           **extra)
        if kind == "features":
            return resolve_feature_members(
                artifact["features"], artifact["model_id"], db, device, **extra)
    ...
    def record_samples(cls, db, config: Dict[str, Any], *,
        ...
        ``device`` is the task's placed device; ``gpu`` is the card record
        (request, uuid, name) written into the manifest. ``placement`` is given
        when the model is SPLIT across cards: the loader then spreads it with
        the placement's budget, and each member's W_dec goes to its layer's card.
        """
        cfg = cls.create_config(config)
        artifact = cfg["artifact"]
        split = placement is not None and placement.is_shard
        ...
        try:
            ...
            if split:
                # Mapped to hold each layer's SAE beside it (the budget keeps
                # only SHARD_RESERVE_MB free per card). Unsizable: no allowance,
                # and the resolver below fails with its own message.
                try:
                    sae_ids = cls._sae_ids_by_layer(artifact, db)
                except _UNSIZABLE as e:
                    logger.warning("No per-layer SAE allowance for the split: %s", e)
                    sae_ids = None
                model, tokenizer, structure, disable_cache, device = \
                    load_model_and_structure(model_id, db, device, placement=placement,
                                             sae_ids_by_layer=sae_ids)
                # Per-layer devices come from the LOADED model: which card a
                # layer is on is accelerate's decision, made during the load.
                resolved_mid, resolved = cls._resolve(artifact, db, device,
                                                      structure=structure)
            else:
                model, tokenizer, structure, disable_cache, device = \
                    load_model_and_structure(model_id, db, device)
                resolved_mid, resolved = cls._resolve(artifact, db, device)
        ...
        payload = {
            ...
            # Which card generated these transcripts (the run row has it too;
            # cluster/feature manifests are read without the row).
            "gpu": gpu,
        }
    ...
    @classmethod
    def sizing_refs(cls, config, db) -> Optional[Tuple[str, List[Optional[str]]]]:
        """(model id, one SAE id per steered layer): what this recording loads.

        Placement sizes a split from these. None when the request cannot be
        sized — a bad config or a missing artifact — so the run places without
        a size and then fails where it always did, with the resolver's message.
        A database error is not caught: it fails the run rather than hide as an
        unsized placement.
        """
        try:
            artifact = cls.create_config(config)["artifact"]
            model_id = cls._artifact_model_id(artifact, db)
            by_layer = cls._sae_ids_by_layer(artifact, db)
        except _UNSIZABLE as e:
            logger.warning("Cannot size the recording for GPU placement: %s", e)
            return None
        return model_id, list(by_layer.values())

    @staticmethod
    def _sae_ids_by_layer(artifact, db) -> Dict[int, Optional[str]]:
        """{layer: SAE id}: the SAE the artifact's resolver loads onto each layer's card.

        One definition for the placement's size (:meth:`sizing_refs`) and the
        split's per-layer allowance (:meth:`record_samples`).
        """
        from .steering_core import _cluster_sae_ids_by_layer

        kind = artifact["kind"]
        if kind == "features":
            return {int(f["layer"]): f["sae_id"] for f in artifact["features"]}
        if kind == "circuit":
            from ..models.circuit import Circuit
