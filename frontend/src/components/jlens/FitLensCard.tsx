    ...
    <section className="rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-800">
      ...
      {open && (
        <div className="mt-3 space-y-3">
          ...
          <GpuSelect
            id="jlens-fit-gpu"
            label="GPU — the fit holds it for the whole run"
            value={gpu}
            onChange={setGpu}
            disabled={running}
            labelClassName="mb-1 block text-xs text-slate-600 dark:text-slate-400"
            selectClassName="w-full rounded border border-slate-300 bg-white px-2 py-1.5 text-xs text-slate-900 disabled:opacity-50 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
          />
