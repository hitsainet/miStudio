...
class TestAllIsRefusedWhenTheLensIsSubmitted:
    ...
    def test_the_push_is_a_400_and_no_row_is_written(self, monkeypatch):
        ...
        db = MagicMock()
        db.get = AsyncMock(return_value=SimpleNamespace(n_features=4, hook_type=None, layer=3))
        db.add = MagicMock()
        task = _celery_task()
...
class TestTheExport:
    ...
    def test_the_card_is_stored_on_the_job_row_and_read_back(self):
        ...
        db = MagicMock()
        # A REAL layer and id. This stand-in carried neither, because nothing read them --
        # and the export then published `sae.layer or 0` for any SAE recording no layer
        # (review R3-B, R3B-11). It is refused now, so a stand-in without one would test GPU
        # placement against a request that never reaches the placement.
        db.get = AsyncMock(return_value=SimpleNamespace(
            id="sae_1", layer=3, status=SAEStatus.READY.value, training_id=None, hook_type=None,
        ))
