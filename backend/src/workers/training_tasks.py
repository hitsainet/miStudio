    ...
    *,
    ...
    if base_model is None:
        ...
        with task.get_db() as db:
            ...
            model_path = str(settings.resolve_data_path(model_row.file_path)) if model_row else None
            quantization = model_row.quantization if model_row else None
        if not model_path:
            logger.info("Spliced-CE evaluation skipped: no model path on record")
            return
        base_model, tokenizer, _cfg, _meta = load_model_from_hf(
            model_path, quantization=quantization
        )
        base_model.eval()
        loaded_here = True
    ...
    try:
        ...
        ids = torch.tensor(
            [list(r) for r in batch["input_ids"]], dtype=torch.long, device=base_model.device
        )
                    ...
                    with HookManager(base_model) as hook_manager:
                        ...
                        for layer_idx, hook_type in layer_hook_combinations:
                            ...
                            if layer_key in hook_manager.activations and hook_manager.activations[layer_key]:
                                ...
                                # PADDING IS NOT TRAINING DATA HERE EITHER.
                                #
                                # This is the same defect as the cached path, in
                                # the other branch of the same function, with the
                                # mask already in local scope — it was built at
                                # :1389, handed to the forward, and then dropped.
                                # On the Bloomberg tokenization (3.3% real) ~97%
                                # of every batch drawn here was PAD, silently.
                                real = attention_mask_tensor.reshape(-1).bool()
                                if real.numel() == acts_flat.shape[0]:
                                    kept = acts_flat[real]
                                    if kept.shape[0] == 0:
                                        logger.warning(
                                            "Step %d: every position in this batch is "
                                            "padding; training on the raw batch instead",
                                            step,
                                        )
                                    else:
                                        acts_flat = kept
                                else:
                                    # Never silent — the alternative is training
                                    # on pads without knowing.
                                    logger.warning(
                                        "Attention mask (%d) does not match the "
                                        "flattened activations (%d); PAD positions "
                                        "will be sampled as real tokens.",
