...
export interface ReadoutRequest {
  ...
  gpu?: string;
  /**
   * False (the server's default) keeps the model loaded on its card, so the
   * next readout of it skips the load. True frees the card when the readout
   * finishes. A failed readout frees the card either way.
   */
