 ...
 *   * drop `isCancelled` from the delete gate   -> "offers delete" test fails
 *   * drop the 'cancelled' badge branch         -> "says Cancelled" test fails
 */
...
const completed = {
  ...
  statistics: { total_features: 32766 },
};
...
describe('ExtractionJobCard — a cancelled job is terminal', () => {
  afterEach(() => {
    vi.clearAllMocks();
    vi.unstubAllGlobals();
  });

  // The card knew four states — active, completed, failed, deleting — and a
  // cancelled job was none of them. So it rendered as "Unknown", and because
  // the delete affordance is gated on `isCompleted || isFailed`, it could never
  // be removed from the list. The backend was never the obstacle:
  // `delete_extraction` refuses only QUEUED and EXTRACTING.
  const cancelled = { status: 'cancelled', progress: 0.0184, nlp_status: null };

  it('says Cancelled, not Unknown', () => {
  ...
  it('offers delete, so the job can actually be cleared from the list', () => {
    renderCard(cancelled);

    expect(
      screen.getByTitle('Delete extraction'),
    ).toBeInTheDocument();
  });

  it('deletes on confirm and does nothing when the confirm is declined', () => {
    const onDelete = vi.fn();
    vi.stubGlobal('confirm', vi.fn().mockReturnValue(false));
    render(
      <ExtractionJobCard
        extraction={{ ...completed, ...cancelled } as any}
        onDelete={onDelete}
        onCancel={vi.fn()}
      />,
    );

    fireEvent.click(screen.getByTitle('Delete extraction'));
    // Declining must not delete — the confirm is the guard, not decoration.
...
describe('ExtractionJobCard — no interpretability claim', () => {
  afterEach(() => vi.clearAllMocks());

  // The card reported "X% interpretable" from the share of features whose
  // activation-consistency score exceeded 0.5. That score measures activation
  // SHAPE and never which tokens fire, and it is highest for evidence repeating
  // one token — 98.8% of such features passed. It was not interpretability.
  const withLegacyStat = {
    // An extraction recorded before the rename still carries the old field.
    statistics: { total_features: 32766, interpretable_count: 22478, avg_activation_frequency: 0.39 },
  };

  it('does not claim a share is interpretable in the summary line', () => {
    renderCard(withLegacyStat);

    expect(screen.getByText('32,766 features')).toBeInTheDocument();
    expect(screen.queryByText(/interpretable/i)).not.toBeInTheDocument();
  });

  it('does not show an Interpretable tile when expanded', () => {
    renderCard(withLegacyStat);

    fireEvent.click(screen.getByText('Browse 32,766 features'));

    expect(screen.getByText('Features Found')).toBeInTheDocument();
    expect(screen.queryByText('Interpretable')).not.toBeInTheDocument();
  });
});
