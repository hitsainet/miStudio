...
class TestTheReturnedIdIsTheRowsId:
    ...
    def test_the_endpoint_hands_its_id_to_the_task(self):
        ...
        # Multi-GPU Phase 3: the task is queued through the GPU routing helper,
        # `gpu_delay(extract_activations)(...)`; the keywords are on that call.
        calls = [
            node for node in ast.walk(A._tree(models_endpoint.extract_model_activations))
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Call)
            and getattr(node.func.func, "id", None) == "gpu_delay"
        ]
