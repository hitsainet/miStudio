...
ON THE CARD THE CALLER CHOSE, released when the report is written — see
...
def compute_band_report_task(
    ...
    # BOTH NAMES EXIST BEFORE ANYTHING CAN RAISE, so the `finally` can always
    ...
    try:
        with get_sync_db() as db:
            ...
            # MAY SPLIT: the band report captures through `ReadoutService`,
            # which moves the ids to the embedding's card, copies each residual
            # off its card, and normalises on the norm's own card.
...
def _profile_and_save(
    ...
    """The measurement itself, split out so the task can guarantee the release.
