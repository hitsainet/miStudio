...
class TestTheIdsGoToTheEmbeddingsCard:
    """Every measured forward pass sends its ids to `service.capture_device`.

    Multi-GPU Phase 2: `final_rank` sent them to `loaded.model.device`, which
    transformers defines as the FIRST parameter's device. On a model split across
    cards that can be a later card than the embedding's, and the lookup fails.
    Here the model claims `meta` while the embedding it indexes is on the CPU,
    which is where `capture_device` says the ids belong.

    ASSERTED ON THE DEVICE THE MODEL RECEIVED, not on the run failing. The first
    version relied on `meta` ids breaking the embedding lookup, and they do not:
    indexing a CPU tensor with `meta` indices returns real CPU values, so the
    mutation below ran every trial to a normal result and survived (M13,
    2026-09-14). A fixture whose wrong device still computes agrees with the
    defect by construction.

    MUTATION CONTROL: `.to(service.capture_device)` back to
    `.to(loaded.model.device)` -> this fails (the model receives `meta` ids).
    """

    def test_the_ids_follow_the_capture_device_not_model_device(self):
        received = []
        original_call = _Model.__call__

        def recording_call(self, input_ids=None):
            received.append(input_ids.device.type)
            return original_call(self, input_ids=input_ids)

        with patch.object(_Model, "device", "meta"), patch.object(_Model, "__call__", recording_call):
            out, _model = _run(prompts=["abcd", "efgh"])

        assert "hits" in out["baseline_top1"]
        # Three forward passes per trial: baseline, intervened, control.
        assert len(received) == 6, received
        assert set(received) == {"cpu"}, (
            f"the model received ids on {sorted(set(received))}; they belong on the "
            "embedding's card (service.capture_device), not on model.device"
        )
...
class TestTheCardComesBack:
    ...
    def _run_and_weigh(self, blow_up=False):
        ...
        # PLACED ON A CARD, which is what makes this a GPU run now — the task
        # no longer asks `torch.cuda.is_available()`.
        on_a_card = Placement(
            card=GpuCard(
                1, "GPU-247aa582-0d1b-e161-8156-983ed1fefc57",
                "NVIDIA GeForce RTX 3090", 24_576, 23_000,
            ),
        ...
        with patch("src.core.database.get_sync_db", fake_db), patch(
            ...
            "src.services.gpu_placement.place_job", return_value=on_a_card
        ), patch(
