...
from .sae_metrics import fvu_pair


def reconstruction_quality(x: torch.Tensor, x_hat: torch.Tensor) -> Dict[str, torch.Tensor]:
    """Both FVUs of a reconstruction, for an architecture's ``losses`` dict.

    EVERY ARCHITECTURE REPORTS BOTH, from one helper (remediation item 5). Only
    JumpReLU used to compute FVU at all, with its own inline copy of the legacy
    formula; the others reported nothing, so a Standard or TopK run showed "not
    reported" for the one metric that says whether training converged.

    ``fvu`` keeps the legacy global-mean meaning every stored row has; the
    per-dimension-centred value is ``fvu_centred``. Raw activation space, not the
    normalised training space. Metrics only: no gradient flows through either.
    See ``sae_metrics`` for why the two differ.
    """
    legacy, centred = fvu_pair(x, x_hat)
    return {'fvu': legacy, 'fvu_centred': centred}
...
#: The modes ``normalize_activations`` implements. Anything else raises there.
SUPPORTED_NORMALIZATIONS = ('none', 'constant_norm_rescale', 'anthropic_rescale')


def resolve_loaded_sae_normalization(sae_config: Any) -> str:
    """The normalisation to construct a LOADED SAE with — the one it trained with.

    ``create_sae`` defaults to ``constant_norm_rescale``; a loader that does not
    pass the recorded value silently normalises an SAE trained with ``none``
    (the community format's own default, and what an imported third-party SAE
    most likely carries). The features still fire and the numbers look
    plausible; the basis is wrong.

    * No config (miStudio-native weights carry none) -> ``constant_norm_rescale``,
      which is what miStudio trains with and what this path always used.
    * A recorded supported mode -> that mode.
    * Anything else is REFUSED here, naming the value, rather than surfacing
      as an opaque error on the first encode — or not at all.
    """
    if sae_config is None:
        return 'constant_norm_rescale'
    mode = getattr(sae_config, 'normalize_activations', None) or 'none'
    if mode not in SUPPORTED_NORMALIZATIONS:
        raise ValueError(
            f"SAE records normalize_activations={mode!r}, which miStudio does not "
            f"implement (supported: {', '.join(SUPPORTED_NORMALIZATIONS)}). "
            f"Encoding it any other way would read features in the wrong basis."
        )
    return mode
...
class SparseAutoencoder(nn.Module):
    ...
    def forward(
        ...
        if return_loss:
            ...
            # two quantities in one space. The reported FVU is unaffected — every
            # architecture computes it separately, in raw space, through
            # `reconstruction_quality`, which is the right space to report
            ...
            losses = {
                ...
                **reconstruction_quality(x, x_reconstructed),
            }
    ...
    def forward(
        ...
        if return_loss:
            ...
            losses = {
                ...
                'l1_penalty': l1_penalty,
                'l0_sparsity': l0_sparsity,
                # Against the TARGET layer: a transcoder reconstructs layer j
                # from layer i, so the variance to explain is x_target's.
                **reconstruction_quality(x_target, x_transcoded),
            }
...
class JumpReLUSAE(nn.Module):
    ...
    def __init__(
        ...
        self.activation = JumpReLU(
            ...
            bandwidth=bandwidth,
            # CONFIGURABLE, not hardcoded. The sigmoid STE on the z gradient is
            # the only path by which a dead JumpReLU latent can recover in a
            # default run — resampling is off for this framework and AuxK is
            # gated to TopK — so it is the last knob that should be unreachable.
    ...
    def _init_weights(self):
        ...
        # ENCODER AS THE DECODER TRANSPOSE.
        #
        # The standard SAE has always done this; JumpReLU took two INDEPENDENT
        # Kaiming draws, so encoder and decoder started describing unrelated
        # directions and early training was spent undoing that. Tying them at
        # init is the SAELens/Gemma Scope default.
        #
        # Precisely: `normalize_decoder()` runs immediately after this, so the
        # tie survives up to a per-column SCALE, not exactly. The DIRECTIONS are
        # what matters and they are shared; saying the weights are equal after
        # construction would overstate it.
        if not self.tied_weights:
    ...
    def forward(
        ...
        losses = {}
        if return_loss:
            # Reconstruction loss in NORMALIZED space, the space `sparsity_coeff`
            # is defined against (MIS-E2E-086 moved the other architectures;
            # JumpReLU was missed).
            #
            # It was `F.mse_loss(x_hat, x)` on RAW activations while the L0
            # penalty is a fixed λ per active latent. The ratio between the two
            # then depended on the hook point's scale: the norm-module captures
            # the earlier runs used had a per-element scale near 1, so λ=1e-3
            # worked; true resid_post on LFM2.5-1.2B has a per-element std of
            # ~0.03, so the entire reconstruction term was ~0.001 and each
            # active latent cost as much as all of it. train_24e5e7d3 collapsed
            # to under one active latent per token by step 2,800.
            loss_reconstruction = F.mse_loss(x_hat_norm, x_normalized, reduction='mean')
            ...
            # Zero ablation loss (decoder bias only), in the same normalized
            # space as the reconstruction loss so the two stay comparable.
            # FVU stays in raw space, where quality is reported.
            x_zero = self.b_dec.expand_as(x_normalized)
            loss_zero = F.mse_loss(x_zero, x_normalized, reduction='mean')
            ...
            losses = {
                ...
                # Legacy global-mean `fvu` and the headline `fvu_centred`, raw space.
                **reconstruction_quality(x, x_hat),
...
def pre_activations_with_training_normalization(sae, x: torch.Tensor) -> torch.Tensor:
    """Pre-activations `z`, with the SAE's OWN training-time normalization first.

    The pre-activation counterpart of `encode_with_training_normalization`,
    for callers that compare `z` against thresholds rather than consuming
    features — JumpReLU threshold calibration above all.

    It exists because that calibration used to hand-write `W_enc @ x + b_enc`,
    omitting the `- b_dec` centring that `encode()` applies. `W_enc @ b_dec` is
    negative on ~99% of a trained SAE's features, so the check read a healthy
    SAE as 2-3x sparser than it is, and — because `b_dec` is the mean
    activation, which grows with depth — the error grew with depth. Routing
    through `encode()` means the check measures the same basis the encoder
    applies, and routing through this module means it normalizes exactly once,
    the way training did.

    Takes RAW activations. Do not normalize before calling.
    """
    if getattr(sae, "normalize_activations", "none") != "none" and hasattr(sae, "normalize"):
        x, _coeff = sae.normalize(x)
    _, z = sae.encode(x, return_pre_activations=True)
    return z
...
def create_sae(
    ...
    **kwargs
    ...
    # `ste_bandwidth` belongs here: the training task passes it for EVERY
    # architecture, and without it in this set a Standard, Skip or Transcoder
    # SAE could not be constructed at all (TypeError from __init__).
    jumprelu_params = {
        'initial_threshold', 'bandwidth', 'ste_bandwidth', 'sparsity_coeff',
        'normalize_decoder', 'tied_weights',
    }
    ...
    if architecture_type in ('standard_saelens', 'standard_anthropic'):
        ...
        # `normalize_activations` too: a Transcoder has no normalisation, and the
        # training task passes the keyword for every architecture, so leaving it
        # in made the training task unable to construct a Transcoder (TypeError).
        transcoder_kwargs = {k: v for k, v in kwargs.items()
                             if k not in ('ghost_gradient_penalty', 'normalize_activations')
                             and k not in non_standard_params}
        return Transcoder(
        ...
        if bandwidth is None:
            bandwidth = 0.01
        ste_bandwidth = kwargs.pop('ste_bandwidth', None)
        if ste_bandwidth is None:
            ste_bandwidth = 0.5
