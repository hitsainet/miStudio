...
class TestConfig:
    def test_default_categories_exclude_admin(self):
        ...
        # An EXACT set, deliberately: widening the default surface should be a
        # decision someone makes, not something a new module does by existing.
        # `models` is here because those tools talk only to miStudio's own API,
        # and an opt-in category is unreachable for any agent that does not
        # already know to ask for it.
        # `probe_monitors` (033) is on by default: four of its five tools are reads, and the two
        # writes go through the evidence gate — a build below rung 2 is refused without an
        # acknowledgement, and publishing resolves its credential server-side. An opt-in category
        # is unreachable for any agent that does not already know to ask for it, which for an
        # export surface means the probes nobody can serve.
        assert {"read", "groups", "steering", "labeling", "experiments",
                "profiles", "circuits", "jlens", "jobs", "models",
                "probe_monitors"} == cats
        # Unified MCP categories are opt-in only
        assert not any(c.startswith("millm_") for c in cats)

    def test_an_unknown_category_is_dropped_not_honoured(self):
        """Fail-closed is preserved: an unrecognised name enables NOTHING.

        This is the half that must never change. Whether the server crashes or
        continues, a category it does not recognise must not turn tools on.
        """
        cats = make_settings(tool_categories="read,bogus").enabled_categories()
        assert cats == {"read"}
        assert "bogus" not in cats

    def test_an_unknown_category_no_longer_crashes_the_server(self):
        """It used to raise, which made every category addition a guaranteed
        crashloop window.

        The manifest and the image ship on DIFFERENT SCHEDULES — ArgoCD syncs
        k8s/base/mcp.yaml in minutes, the backend image takes about nine — so
        in that gap the new manifest runs against an image whose
        VALID_CATEGORIES predates it. Adding `models` and `millm_models` did
        exactly that: six restarts, and only the rolling update kept a healthy
        pod serving. A config that becomes valid on its own in minutes must not
        be able to take the server down.

        Nothing is lost: a typo in the DEPLOYED manifest is caught in CI by
        test_every_deployed_category_is_VALID, so the crash was a redundant
        second line of defence with an outage attached.
        """
        make_settings(tool_categories="read,bogus").enabled_categories()

    def test_the_unknown_name_is_reported_rather_than_silent(self):
        """The alternative to crashing must not be saying nothing — that is how
        a whole category goes missing unnoticed, which this repo has done."""
        settings = make_settings(tool_categories="read,bogus")
        assert settings.unknown_categories() == {"bogus"}
        assert settings.requested_categories() == {"read", "bogus"}

    def test_a_wholly_unknown_list_yields_no_categories(self):
        """Not an exception, but not a full tool surface either."""
        assert make_settings(tool_categories="bogus,nonsense").enabled_categories() == set()
