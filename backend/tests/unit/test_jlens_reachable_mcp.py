...
class TestCaller:
    ...
    def test_readout_issues_a_POST_with_the_prompt_and_model(self):
        ...
        # Sent explicitly, False included, so the request says what it asked for.
        assert body["unload_after"] is False

    def test_readout_sends_the_choice_to_unload_the_model(self):
        """MUTATION CONTROL U6: the tool stops sending `unload_after` -> this fails."""
        mcp, client, _names = self._tools()

        asyncio.run(
            mcp.call_tool(
                "jlens_readout",
                {"model_id": "m_abc", "prompt": "hello", "unload_after": True},
            )
        )

        assert client.post.await_count == 1
        assert client.post.await_args.kwargs["json_body"]["unload_after"] is True
...
class TestTheToolSAYSWhatAnAgentCannotOtherwiseKnow:
    ...
    def test_direction_token_no_longer_claims_multi_token_is_REFUSED(
        self, monkeypatch
    ):
        """It said "Multi-token strings are REFUSED rather than truncated".

        That is now the inverse of the behaviour: a split string is steered
        along the MEAN of its pieces' rows. An agent reading the old text would
        not attempt a perfectly legal intervention — and worse, would not know
        the direction it does get is not any single token's.

        MUTATION CONTROL: restore the old sentence and this fails.
        """
        text = self._schema(monkeypatch)["direction_token"]["description"]
        assert "REFUSED" not in text, text
        assert "MEAN" in text, text

    def test_target_token_states_that_only_the_FIRST_PIECE_is_scored(
        self, monkeypatch
    ):
        """The measurement changes shape for a split target, silently.

        A rank is defined over one position, so a multi-piece target is scored
        on its first piece alone. An agent that does not know this reads the
        top-1 rate as a claim about the whole word.
        """
        text = self._schema(monkeypatch)["target_token"]["description"]
        assert "FIRST piece" in text, text

    def test_artifact_id_states_the_COVERAGE_REFUSAL(self, monkeypatch):
        """Naming it now constrains which layers are legal.

        Supplying `artifact_id` credits that lens, so layers it was not fitted
        at are refused — a 400 an agent cannot predict from the old text, which
        described the field only as a provenance marker.

        MUTATION CONTROL: drop the coverage sentence and this fails.
        """
        text = self._schema(monkeypatch)["artifact_id"]["description"]
        assert "REFUSED" in text or "refused" in text, text
        assert "exploratorily" in text or "Omit it" in text, text
...
class TestTheGpuTravelsWithEveryGpuTool:
    """An agent names the card, or takes Auto (0xcc/plans/Multi-GPU-Plan.md, Phase 1).

    `gpu` is sent on EVERY call, "auto" included. A tool that dropped it would
    still queue the job — on whatever card Auto picked — while the agent
    believed it had named one, which is the silent substitution the plan
    forbids.

    MUTATION CONTROL: stop sending `gpu` from one tool -> its two cases fail.
    """

    RTX = "GPU-247aa582-0d1b-e161-8156-983ed1fefc57"

    CALLS = {
        "fit_jlens_artifact": ("/jlens/fit", {"model_id": "m_1", "prompts": ["a", "b"]}),
        "acquire_jlens_artifact": (
            "/jlens/acquire",
            {"model_id": "m_1", "repo_id": "org/lenses", "path_in_repo": "a/lens.pt"},
        ),
        "run_jlens_intervention": (
            "/jlens/interventions",
            {
        ...
        "compute_jlens_band_report": (
            "/jlens/band-report",
            {"model_id": "m_1", "prompts": ["a"], "control_seed": 4242},
        ),
        "jlens_readout": ("/jlens/readout", {"model_id": "m_1", "prompt": "hello"}),
    }

    @staticmethod
    def _tools():
        return TestCaller()._tools()

    @pytest.mark.parametrize("tool", sorted(CALLS))
    def test_a_named_card_is_sent(self, tool):
        mcp, client, _names = self._tools()
        path, arguments = self.CALLS[tool]

        asyncio.run(mcp.call_tool(tool, {**arguments, "gpu": self.RTX}))

        assert client.post.await_count == 1, "the tool made no call, or made several"
        assert client.post.await_args.args[0] == path
        assert client.post.await_args.kwargs["json_body"]["gpu"] == self.RTX

    @pytest.mark.parametrize("tool", sorted(CALLS))
    def test_auto_is_sent_when_no_card_is_named(self, tool):
        mcp, client, _names = self._tools()
        path, arguments = self.CALLS[tool]

        asyncio.run(mcp.call_tool(tool, dict(arguments)))

        assert client.post.await_count == 1
        assert client.post.await_args.args[0] == path
        assert client.post.await_args.kwargs["json_body"]["gpu"] == "auto"

    def test_the_parameter_says_a_named_card_is_never_swapped(self):
        """The one rule an agent cannot infer from a string-typed field."""
        mcp, _client, _names = self._tools()
        specs = {t.name: t for t in asyncio.run(mcp.list_tools())}
        for tool in self.CALLS:
            prop = specs[tool].inputSchema["properties"]["gpu"]
            assert prop.get("default") == "auto", tool
            assert "never swapped" in prop["description"], tool

    #: Which tools' jobs may split a model across GPUs (multi-GPU Phase 2). The
    #: fit may not: its batched backward needs workspace a split cannot hold.
    SPLITS = {
        "jlens_readout": True,
    ...
    def test_the_parameter_says_which_tools_may_split(self):
        """An agent told nothing asks a fit for "all" and learns from a failed task.

        MUTATION CONTROL: give the fit the split-capable wording -> this fails.
        """
        assert set(self.SPLITS) == set(self.CALLS), "a GPU tool is missing from SPLITS"
        mcp, _client, _names = self._tools()
        specs = {t.name: t for t in asyncio.run(mcp.list_tools())}
        for tool, splits in self.SPLITS.items():
            description = specs[tool].inputSchema["properties"]["gpu"]["description"]
            if splits:
                assert '"all" splits' in description, tool
                assert "gpu_uuids" in description, tool
            else:
                assert "ONE card" in description, tool
                assert '"all" splits' not in description, tool
