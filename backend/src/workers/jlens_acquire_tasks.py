...
# Waits in place: its placement comes after the artifact is fetched. Prechecked,
# because it arrives on the GPU queues like every other J-space task.
@gpu_job("jlens_acquire", handoff=False, precheck=True)
...
def acquire_jlens_artifact(
    ...
    the caller named. The card is chosen after the download, right before the
    model load, and a card that cannot be used fails the task with the reason.
    ...
    #
    # THE CARD IS CHOSEN HERE, AFTER THE DOWNLOAD. Auto judges live free
    # memory, and a multi-gigabyte download takes minutes — choosing at the top
    # of the task would judge the cards as they were before it. This was
    # `"cuda" if available`, the CURRENT device: index 0, the 12 GB card.
    #
    # MAY SPLIT: the acquisition reads the loaded model's dimensions and runs
    # the SEMANTIC check as a readout through `ReadoutService`; nothing here
    # runs a backward or picks a device of its own.
