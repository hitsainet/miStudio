...
class TestAMalformedSwapIsRefusedSYNCHRONOUSLY:
    ...
    @staticmethod
    def _run_route(layers, fitted, artifact_id="lens"):
        """Drive the ROUTE HANDLER, not the schema, with a stub artifact."""
        import asyncio
        from unittest.mock import MagicMock, patch
        ...
        db = MagicMock()
        res = MagicMock()
        # THE ROW CARRIES A repo_id NOW. The route resolves the artifact the way
        # the WORKER does — by the model's repo id — so a double returning a
        # bare object() models a model row that cannot exist.
        row = MagicMock()
        ...
        ref.slug = "lens"
        svc = MagicMock()
        svc.find.return_value = ref
        # FROM config.yaml, NOT THE TENSORS. The route reads `fitted_layers`
        # precisely so it does not `torch.load` a ~276 MB artifact inside an
        # async handler; a double that stubs `_load_payload` would model the
        # expensive path the route deliberately does not take.
        svc.fitted_layers.return_value = list(fitted)
        ...
        with patch(
            "src.api.v1.endpoints.jlens.JLensArtifactService", return_value=svc
        ), patch(
            "src.workers.jlens_intervention_tasks.run_intervention_task"
        ) as task, patch("src.workers.jlens_progress.open_row"):
            task.delay.return_value = MagicMock(id="t-1")
            out = asyncio.run(run_intervention(req, db=db))
        return out, task

    def test_layers_the_ARTIFACT_DOES_NOT_COVER_are_refused_at_the_door(self):
        """Knowable from disk, so it must not cost a slot on the GPU queue.

        Naming `artifact_id` credits that lens — the run appends an
        `evidence_rung: 2` record to its `interventions.json`, the file that
        travels to HuggingFace and into a serving runtime. The worker already
        refuses this, but only after a 202 and a queue slot that may sit behind
        a 45-minute fit.

        MUTATION CONTROL: delete the pre-dispatch check and this fails — the
        route returns a task id.
        """
        from fastapi import HTTPException

        with pytest.raises(HTTPException) as excinfo:
            self._run_route(layers=[18, 43], fitted=[42, 43, 44, 45])
        assert excinfo.value.status_code == 400
        detail = str(excinfo.value.detail)
        # NAMES THE OFFENDING LAYERS, WHAT IS COVERED, AND THE WAY OUT.
        assert "[18]" in detail, detail
        assert "42" in detail, detail
        assert "artifact_id" in detail, detail

    def test_covered_layers_are_queued_normally(self):
        """Negative control: the guard must not refuse a legitimate run."""
        out, task = self._run_route(layers=[43, 44], fitted=[42, 43, 44, 45])
        assert out.task_id == "t-1"
        assert task.delay.called

    def test_a_slug_belonging_to_ANOTHER_MODEL_is_left_to_the_worker(self):
        """The worker's refusal is the accurate one; do not pre-empt it.

        The route resolves this model's own artifact by repo id. A slug naming
        someone else's lens must NOT be looked up and used to compute a 400
        about layers — `_validated_report` refuses it in the worker with the
        reason that actually applies ("not fitted for this model").

        MUTATION CONTROL: resolve by scanning `list_artifacts()` for the slug
        and this fails — the run is refused here, with the wrong reason.
        """
        out, task = self._run_route(
            layers=[1, 2], fitted=[42, 43], artifact_id="someone-elses-lens"
        )
        assert out.task_id == "t-1"
        assert task.delay.called

    def test_no_artifact_id_means_no_coverage_check_at_all(self):
        """Nothing is being credited, so an exploratory run stays unrestricted.

        MUTATION CONTROL: drop the `if request.artifact_id:` condition and this
        fails — the run is refused for layers no artifact was asked to cover.
        """
        out, task = self._run_route(
            layers=[1, 2], fitted=[42, 43], artifact_id=None
        )
        assert out.task_id == "t-1"
        assert task.delay.called
...
class TestATypedTokenCanBeCheckedWithoutLoadingWeights:
    ...
    def _check(tokens, encoder):
        ...
        # A REAL `decode`. The verdict now names the pieces a multi-token
        # string split into, and a bare MagicMock renders its own repr straight
        # into text a user reads:
        #   "2 tokens [<MagicMock name='mock.decode()' id=...>]"
        # The endpoint falls back to the ids when decode misbehaves, and this
        # double models the tokenizer that actually exists.
        tok.decode = lambda ids: "".join(f"<{int(i)}>" for i in ids)
    ...
    def test_TWO_tokens_are_USABLE_BUT_WEAKER_with_the_pieces_shown(self):
        """A multi-piece string is accepted now, and is not equivalent evidence.

        It used to be refused outright, which excluded most of the interesting
        vocabulary: the common case is an ORDINARY WORD the tokenizer splits
        into subwords, not a phrase. The worker takes the MEAN of the pieces'
        rows and scores the FIRST piece, because a next-token distribution
        answers for exactly one position.

        `usable` STILL MEANS "exactly one token" and stays False. Widening it
        would have told every existing caller — the composer, the intervention
        card, any MCP client — that the two are the same measurement.

        MUTATION CONTROLS:
          * flip `usable` to `n >= 1`        -> the False assertion fails
          * drop the piece names             -> the "<4874>" assertion fails
          * drop the weaker-evidence sentence -> the "weaker" assertion fails
        """
        out, _l, _w = self._check(["Rome"], lambda s: [4874, 883] if s == "Rome" else [1])
        assert out[0].usable is False, "usable must keep meaning exactly one token"
        assert out[0].ids == [4874, 883]
        # THE IDS MADE CONCRETE AS PIECES. Seeing where the split fell is what
        # makes the verdict actionable.
        assert "<4874>" in out[0].detail, out[0].detail
        assert "weaker" in out[0].detail, out[0].detail

    def test_the_verdict_NEVER_CONTAINS_A_REPR_when_decode_misbehaves(self):
        """This text goes straight to a user.

        `decode` is the only call this endpoint makes that is not `encode`, and
        a tokenizer that returns something other than a string put
        `<MagicMock name='mock.decode()' id=...>` into the sentence. The ids are
        already on the response and cannot be wrong, so they are the floor.

        MUTATION CONTROL: interpolate `tok.decode(...)` directly and this fails.
        """
        import asyncio
        from unittest.mock import MagicMock, patch
        ...
        tok = MagicMock()
        tok.encode = lambda s, add_special_tokens=False: [4874, 883]
        # decode left as a bare MagicMock: returns a Mock, not a string.
        ...
        with patch(
            "src.services.jlens_model_registry.tokenizer_for", return_value=tok
        ), patch("src.services.jlens_model_registry.load_for_readout"):
            out = asyncio.run(
                token_check(TokenCheckRequest(model_id="m_1", tokens=["Rome"]), db=db)
            )

        assert "MagicMock" not in out[0].detail, out[0].detail
        assert "[4874, 883]" in out[0].detail, out[0].detail
...
class TestAcquisitionIsWiredEndToEnd:
    ...
    def test_the_module_is_in_the_AUTODISCOVER_list(self):
        ...
        does.
        """
        # ⚠ THIS WAS A SUBSTRING SEARCH OVER `inspect.getsource(celery_app)` until
        # 2026-09-26, and a substring is satisfied by ANY occurrence — a comment, a
        # docstring, or a `task_routes` key that happens to contain the module path.
        # It is now read from the `autodiscover_tasks(...)` CALL's arguments; see
        # `tests/support/celery_introspection.py` for the probe-monitor case that
        # proved the weaker form silent.
        from tests.support.celery_introspection import assert_autodiscovered

        assert_autodiscovered("src.workers.jlens_acquire_tasks")
