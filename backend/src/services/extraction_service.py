...
from src.services.gpu_dispatch import dispatch_gpu_task
...
from src.services.gpu_placement import AUTO, place_job
from src.services.base_model_budget import base_model_mb, budget_beside_sae, inference_sae_card_mb
from src.ml.model_devices import cuda_devices, detach_dispatch_hooks, input_device
...
from src.ml.model_loader import OutOfMemoryError, load_model_from_hf
...
def _mint_extraction_id(*, sae_suffix: str, position: Optional[int] = None) -> str:
    """A collision-free extraction id. THE ONLY PLACE ONE IS BUILT.

    The timestamp is SECOND-granular, so two extractions started inside the same
    second produced the same id — a primary-key collision, and worse, the same
    on-disk directory name, since the directory is derived from the id. Four
    random hex characters make a same-second pair distinct while keeping the id
    sortable and greppable: `extr_<ts>_<rand>_sae_<id8>[_<pos>]`.

    EIGHT hex characters, not four. Four (65,536 values) is ample for the two or
    three extractions a human starts in one second, but a burst of 200 has a ~26%
    chance of a collision by the birthday bound — which is exactly how the first
    version of this failed: it passed alone and failed in a parallel full-suite
    run, a flake in the fix for a collision bug. 2^32 keeps 200 same-second mints
    at ~5e-6.

    `position` is the multi-position batch suffix. It made a same-second pair at
    DIFFERENT positions distinct, which is why that site looked safe, but two
    runs of the SAME position inside one second still collided.

    There were two mint sites and the first fix changed one of them. A mutation
    that reverted the call site left every test green, because the tests exercised
    this function and nothing asserted production used it. Hence the AST guard in
    `tests/unit/test_extraction_id_is_collision_free.py`, which fails if an id is
    built anywhere but here.
    """
    stamp = datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')
    minted = f"extr_{stamp}_{secrets.token_hex(4)}_sae_{sae_suffix}"
    return minted if position is None else f"{minted}_{position:03d}"
...
def _cuda_devices_to_clean(device: Optional[Any] = None) -> List[Any]:
    """The CUDA devices a cleanup measures, synchronizes and resets.

    The cards the job was placed on, when they are named: one device, or a
    sequence of them for a model split across cards — its layers sit on each, and
    cleaning only the first left the rest of the model's memory unmeasured and
    unsynchronized. Otherwise every card this process holds memory on: a
    process-wide sweep for callers not scoped to one job. Never "the current
    device" — this used to read `memory_allocated(0)` and synchronize the current
    device, so a job placed on the 3090 (index 1 since 2026-09-13) measured and
    waited on the 3080 Ti instead.

    A card this process never allocated on is skipped, because synchronizing it
    CREATES a CUDA context there — hundreds of MB taken from a card another job
    may be using.
    """
    if not torch.cuda.is_available():
        return []
    if device is not None:
        named = device if isinstance(device, (list, tuple)) else [device]
        return [torch.device(d) for d in named if torch.device(d).type == "cuda"]
...
def _allocated_gb(devices: List[Any]) -> float:
    return sum(torch.cuda.memory_allocated(d) for d in devices) / (1024**3)
...
def cleanup_gpu_memory(
    ...
    Args:
        ...
        device: The device the job ran on, or every device of a split. When
            omitted, every card this process holds memory on is cleaned.
    """
    if not torch.cuda.is_available():
        return

    try:
        devices = _cuda_devices_to_clean(device)
        initial_memory = _allocated_gb(devices)
        logger.info(
            f"[GPU Cleanup - {context}] Starting cleanup on {devices}. "
            f"Memory before: {initial_memory:.2f}GB"
        )
        ...
        if models_to_cleanup:
            for i, model in enumerate(models_to_cleanup):
                if model is not None:
                    try:
                        if isinstance(model, torch.nn.Module) and len(cuda_devices(model)) > 1:
                            # A model split across cards is freed where it is:
                            # moving it to the CPU would copy a model too large
                            # for one GPU into host memory only to discard it.
        ...
        # Step 4: Synchronize each card to ensure all operations complete
        for d in devices:
            torch.cuda.synchronize(d)

        # Step 5: Empty CUDA cache (the caching allocator releases on every card)
        ...
        final_memory = _allocated_gb(devices)
...
class ExtractionService:
    ...
    async def start_extraction_for_sae(
        ...
        Args:
            ...
            gpu_request: ``"auto"`` or a GPU UUID from ``resolve_gpu_request``.
                Stored on the job row, which is what the worker places from.
        ...
        # An MLP or attention SAE (review R1-A, A5); the endpoints answer it with 422.
        from .sae_hook_support import refuse_non_residual, refuse_unrecorded_layer

        refuse_non_residual(external_sae.hook_type, "Feature extraction")
        # An SAE recording no layer: extraction hooked layer 0 and said nothing
        # (review R3-B, R3B-11). Refused here, before a job row exists.
        refuse_unrecorded_layer(external_sae.layer, "Feature extraction", external_sae.id)
        ...
        extraction_job = ExtractionJob(
            id=_mint_extraction_id(sae_suffix=sae_id[:8]),
        ...
        task_result = dispatch_gpu_task(
            extract_features_from_sae_task,
            gpu_request=getattr(extraction_job, "gpu_request", None),
            args=(sae_id, config),
    ...
    async def start_batch_extraction_for_saes(
        ...
        for position, sae_id in enumerate(sae_ids, start=1):
            try:
                ...
                # An MLP or attention SAE is skipped, the rest of the batch runs
                # (review R1-A, A5).
                from .sae_hook_support import non_residual_hook_reason, unrecorded_layer_reason

                hook_reason = non_residual_hook_reason(external_sae.hook_type, "Feature extraction")
                if hook_reason:
                    skipped_saes.append(dict(sae_id=sae_id, reason=hook_reason))
                    continue

                # An SAE recording no layer would be extracted at layer 0 (review R3-B,
                # R3B-11). Skipped with its reason, like an MLP one; the batch runs on.
                layer_reason = unrecorded_layer_reason(
                    external_sae.layer, "Feature extraction", sae_id
                )
                if layer_reason:
                    skipped_saes.append(dict(sae_id=sae_id, reason=layer_reason))
                    continue
                ...
                extraction_job = ExtractionJob(
                    id=_mint_extraction_id(sae_suffix=sae_id[:8], position=position),
                    external_sae_id=sae_id,
                    training_id=None,
                    status=ExtractionStatus.QUEUED,
                    config=config,
                    ...
                    context_suffix_tokens=config.get('context_suffix_tokens', 25),
                    progress=0.0,
                    # Each job carries the request itself: a chained job is
                    # started later by `_start_next_batch_job`, which passes
                    # only (sae_id, config), and is placed from this column.
                    gpu_request=gpu_request,
                    # Batch metadata
                    batch_id=batch_id,
    ...
    async def list_extractions(
        ...
        # Collect dataset ids from SAE-based extraction configs.
        #
        # `dataset_ids` is canonical under a mixture; `dataset_id` is the legacy
        # single-corpus key, still written alongside it. Reading only the legacy
        # key would load a name for the FIRST corpus and leave every other one
        # unresolvable, so a multi-corpus run would render as single-corpus.
        for ej in extraction_jobs:
            if ej.external_sae_id and ej.config:
                for ds_id in (ej.config.get("dataset_ids") or []):
                    if ds_id:
        ...
        for extraction_job in extraction_jobs:
            ...
            model_name = None
            dataset_name = None
            #: None means "not recorded" — a training-based extraction, or one
            #: created before multi-corpus extraction existed. Not "one corpus".
            dataset_names = None
            sae_name = None
            ...
            if extraction_job.training_id:
                ...
                if external_sae:
                    ...
                    cfg = extraction_job.config or {}
                    row_dataset_ids = [d for d in (cfg.get("dataset_ids") or []) if d]
                    if not row_dataset_ids and cfg.get("dataset_id"):
                        row_dataset_ids = [cfg["dataset_id"]]
                    if row_dataset_ids:
                        # `dataset_name` stays the FIRST corpus so every existing
                        # caller renders exactly what it rendered before;
                        # `dataset_names` carries the whole mixture.
                        dataset_name = datasets_map.get(row_dataset_ids[0])
                        # Fall back to the id when a corpus row has since been
                        # deleted: showing the id is honest, dropping the entry
                        # would silently shorten the mixture.
                        dataset_names = [datasets_map.get(d) or d for d in row_dataset_ids]
    ...
    def _sae_extraction_required_mb(self, sae_id: str) -> Optional[float]:
        """The memory an SAE feature extraction needs: its base model plus the SAE's card.

        None when the model's size is unknown (no linked model row, or no
        parameter count), and the job is then placed without a size, as before.
        """
        external_sae = self.db.query(ExternalSAE).filter(ExternalSAE.id == sae_id).first()
        model_id = getattr(external_sae, "model_id", None)
        if not model_id:
            return None
        model_record = self.db.query(ModelRecord).filter(ModelRecord.id == model_id).first()
        model_mb = base_model_mb(
            getattr(model_record, "params_count", None),
            getattr(model_record, "quantization", None),
            # A Q4 row's count is the packed count; its description is not.
            getattr(model_record, "architecture_config", None),
        )
        if model_mb is None:
            return None
        return model_mb + inference_sae_card_mb(
            getattr(external_sae, "d_model", None), getattr(external_sae, "n_features", None)
        )
    ...
    def extract_features_for_sae(
        ...
        # Bound before the try so the finally cleans the card this job was
        # placed on; a placement that failed placed nothing.
        placement = None

        try:
            # PLACE THE JOB FROM ITS OWN ROW.
            #
            # This was `device = "cuda"`: the current device, index 0, the
            # 3080 Ti since 2026-09-13. The request is read from THIS job's
            # `gpu_request` column, never from `config` — a batch's later jobs
            # are started by `_start_next_batch_job` with only (sae_id, config),
            # and each is judged against the memory free when IT starts. A card
            # that cannot be used raises GpuPlacementError, which the handler
            # below records as the job's failure. NULL on an old row means auto.
            #
            # SIZED, so a model no single card holds beside the SAE can be split
            # across cards: Auto cannot choose a split without a size. The SAE
            # stays on the first card; the base model may span several.
            placement = place_job(
                extraction_job.gpu_request or AUTO,
                required_mb=self._sae_extraction_required_mb(sae_id),
                allow_shard=True,
            )
            device = placement.device
            for column, value in placement.gpu_columns().items():
                setattr(extraction_job, column, value)
            # Committed BEFORE the SAE and the model load, so a job that dies
            # loading still says which card(s) it died on.
            self.db.commit()
            logger.info(
                f"Extraction {extraction_job.id} placed on {placement.describe()} as "
                f"{device} (requested {extraction_job.gpu_request or AUTO!r})"
            )
            placed_cards = [d for d in placement.all_devices if d.type == "cuda"]
            if placed_cards:
                torch.cuda.empty_cache()
                for card in placed_cards:
                    torch.cuda.synchronize(card)
                initial_memory = sum(torch.cuda.memory_allocated(card) for card in placed_cards) / (1024**3)
                logger.info(f"GPU memory on {placed_cards} before extraction: {initial_memory:.2f}GB allocated")
            ...
            # ONE CORPUS OR SEVERAL. `dataset_ids` is the multi-corpus field;
            # `dataset_id` is kept as the first of them, so every legacy reader
            # (list_extractions, the batch chain) keeps working untouched. A
            # config written before the mixture existed carries only the
            # singular key, which becomes a one-element list — and one element
            # reproduces the old arithmetic exactly (see extraction_mixture's
            # TestOneCorpusIsExactlyTheOldBehaviour).
            dataset_ids = list(config.get("dataset_ids") or [])
            if not dataset_ids:
                single = config.get("dataset_id")
                dataset_ids = [single] if single else []
            ...
            sae_state_dict, sae_config, format_type = load_sae_auto_detect(
                resolved_sae_path,
                # A string: safetensors' load_file takes "cuda:1", not a
                # torch.device.
                device=str(device)
            )
            ...
            sae = create_sae(
                ...
                # The basis it TRAINED in. Omitted, create_sae's default applied
                # to every SAE regardless of what its cfg.json recorded.
                normalize_activations=resolve_loaded_sae_normalization(sae_config),
            )
            ...
            # Load dataset(s) and tokenization(s), one per selected corpus.
            #
            # THE THIRD SELECTION SITE. Unordered `.first()` on
            # (dataset, model) while the uniqueness constraint is
            # (dataset, model, max_length) — several rows per pair are EXPECTED,
            # and this is the path that produces every feature's example
            # contexts. The arc made it MORE dangerous by giving the tokenized
            # directory a max_length: the arbitrary pick used to return the same
            # bytes, and now returns genuinely different data.
            #
            # The import stays HERE, lazily, not at module scope:
            # test_split_sae_feature_extraction and
            # test_extraction_runs_on_the_chosen_gpu monkeypatch this name on
            # `model_tasks`, which only takes effect because it is resolved at
            # call time.
            from ..workers.model_tasks import select_tokenization_for_model
            from .extraction_mixture import (
                batch_spans,
                calibration_indices,
                describe_spans,
                plan_spans,
            ...
            for ds_id in dataset_ids:
                record = self.db.query(Dataset).filter(Dataset.id == ds_id).first()
                if not record:
                    raise ValueError(f"Dataset {ds_id} not found")

                candidates = self.db.query(DatasetTokenization).filter(
                    DatasetTokenization.dataset_id == ds_id,
                    DatasetTokenization.model_id == model_record.id,
                ).all()
                tokenization = (
                    select_tokenization_for_model(candidates, model_record.id, ds_id)
                    if candidates else None
                )
                if not tokenization or tokenization.status != TokenizationStatus.READY:
                    raise ValueError(
                        f"No ready tokenization found for dataset {ds_id} with model {model_record.id}. "
                        f"Please tokenize the dataset first."
                    )
                dataset_records.append(record)
                tokenizations.append(tokenization)

            # EVERY CORPUS MUST SHARE A CONTEXT LENGTH, and this has to be
            # checked rather than trusted. The tokenized Arrow type is
            # `list<item: int32>` — variable-length, NOT a fixed-size list — so
            # concatenating a 512 and a 2048 tokenization SUCCEEDS SILENTLY and
            # one feature's examples would mix two context lengths with nothing
            # to warn on. Refuse, in the spirit of select_tokenization_for_model:
            # there, the fallback WAS the bug.
            lengths = {t.max_length for t in tokenizations}
            if len(lengths) > 1:
                detail = ", ".join(
                    f"{r.name}={t.max_length}" for r, t in zip(dataset_records, tokenizations, strict=True)
                )
                raise ValueError(
                    f"Selected corpora have different tokenization max_lengths ({detail}). "
                    f"Concatenating them would silently mix context lengths in one feature's "
                    f"examples. Re-tokenize them to a common max_length, or extract separately."
                )

            # Per-corpus slices, then one concatenated dataset. Concatenation is
            # what keeps `len(dataset)` meaning "rows this run reads" at every
            # later site — the progress denominator and both frequency
            # denominators are untouched — and it makes `sample_index` unique
            # across corpora for free, because the indices are already global.
            dataset_rows = []
            slices = []
            # Only the tokenization is read here; `dataset_ids` and
            # `dataset_records` are consumed by plan_spans below, and zipping
            # them in just to discard them hid what this loop depends on.
            for tokenization in tokenizations:
                ...
                loaded = load_from_disk(str(resolved_dataset_path))
                dataset_rows.append(len(loaded))
                slices.append(loaded)

            spans = plan_spans(
                [r.name for r in dataset_records],
                dataset_ids,
                dataset_rows,
                evaluation_samples,
                dataset_weights,
            ...
            selected = []
            for loaded, span in zip(slices, spans, strict=True):
                lo = min(start_sample, len(loaded))
                hi = min(lo + span.rows, len(loaded))
                selected.append(loaded.select(range(lo, hi)) if (lo or hi < len(loaded)) else loaded)

            dataset = selected[0] if len(selected) == 1 else concatenate_datasets(selected)
            logger.info(f"Mixture: {describe_spans(spans)}")
            ...
            base_model, tokenizer, model_config, metadata = load_model_from_hf(
                ...
                # The placed card as a string ("cuda:1"): the loader's preflight
                # then measures that card's free memory, and from_pretrained puts
                # the whole model on it. A split takes the placement's own
                # device_map over its cards, with the SAE's share kept free on
                # the SAE's card.
                device_map=placement.device_map,
                max_memory=budget_beside_sae(
                    placement, inference_sae_card_mb(hidden_dim, latent_dim)
                ),
                local_files_only=model_is_downloaded,
            )
            base_model.eval()
            # Where every batch's input ids go: the embedding's card, which on a
            # split need not be `device`, the SAE's card.
            ...
            # WHERE THE HOOK GOES. ``external_sae.layer or 0`` read an SAE recording no
            # layer at layer 0, silently, and every feature it then described came from
            # activations the SAE was never trained on (review R3-B, R3B-11). Both entry
            # points refuse such a row before a job exists; this is the worker's own guard,
            # for a job row created before they did.
            from .sae_hook_support import refuse_unrecorded_layer

            refuse_unrecorded_layer(external_sae.layer, "Feature extraction", external_sae.id)
            layer_index = external_sae.layer
            hook_types = ["residual"]
            ...
            #: Samples in which each feature fired at all — counted before the
            #: junk-token filter can discard the pair. int64: one count per
            #: sample, so it cannot exceed `len(dataset)`, but float rounding
            #: has no business in a count.
            feature_fired_counts = np.zeros(latent_dim, dtype=np.int64)
            # PER CORPUS, beside the combined counters rather than instead of
            # them. `activation_frequency` keeps its meaning — the fraction of
            # the rows THIS RUN read — which is what the dead-neuron gate and
            # the steering auto-baseline were fitted against. These answer the
            # question the combined figure cannot: does this feature fire on
            # code, or on chat? 5 x 32,768 x 8B = 1.3 MB.
            per_corpus_activation_counts = np.zeros((len(spans), latent_dim))
            per_corpus_fired_counts = np.zeros((len(spans), latent_dim), dtype=np.int64)

            # Process dataset with hooks
            #: What the threshold calibration saw and did. Recorded in the
            #: extraction statistics so a rescale is never silent: it replaces the
            #: thresholds the SAE spent its whole training learning.
            #:
            #: Defined HERE, at function scope, and not beside the calibration
            #: inside the `with` below. The statistics that read it are built after
            #: that block exits; assigning it inside would work only while nothing
            #: reorders the block — the shape of an earlier NameError on this path.
            threshold_calibration: Dict[str, Any] = {"ran": False}
            ...
            with HookManager(base_model) as hook_manager:
                ...
                # Assigned and never read (verified 2026-09-18: this is its only
                # occurrence in the file). Kept on the first corpus rather than
                # made per-corpus, because the sampling loop decodes from
                # `input_ids`, not from a text column. A candidate for deletion.
                text_column = (dataset_records[0].extra_metadata or {}).get("text_column", "text")
                ...
                if architecture_type == "jumprelu" and hasattr(sae, 'activation'):
                    ...
                    # THE HEAD OF EVERY CORPUS, not the first batch.
                    #
                    # This sets `sae.activation.log_threshold` from the z values
                    # it observes. Calibrating on `dataset[:batch_size]` samples
                    # whichever corpus sorts first, so under a mixture a code
                    # feature would have its firing threshold fixed from chat
                    # activations and would then read as dead on code — a bug
                    # the mixture creates, not one it inherits. With a single
                    # corpus this returns `range(min(batch_size, n))`, i.e. the
                    # identical slice (pinned by test_extraction_mixture).
                    calibration_batch = dataset[calibration_indices(spans, batch_size)]
                    cal_input_ids = []
                    ...
                    if cal_input_ids:
                        ...
                        cal_input_tensor = torch.tensor(cal_padded, device=model_input_device, dtype=torch.long)
                        cal_mask_tensor = torch.tensor(cal_masks, device=model_input_device, dtype=torch.long)
                        ...
                        with torch.no_grad():
                            ...
                            if hook_manager.activations:
                                ...
                                # Pre-activations through the SAE's OWN encoder.
                                #
                                # This was hand-computed as `W_enc @ x + b_enc`, omitting
                                # the `- b_dec` centring that `encode()` and train-time
                                # `calibrate_thresholds` both apply. `W_enc @ b_dec` is
                                # negative on ~99% of features, so the check read a
                                # healthy SAE as 2-3x sparser than it is — and because
                                # `b_dec` is the mean activation, which grows with depth,
                                # the error grew with depth: measured on a 16k SAE, the
                                # true L13 pass rate of 0.279% read as 0.1015%, clearing
                                # the 0.1% trigger by 1.5%. Three of its five corpora
                                # individually fell below it. Calling encode() means the
                                # check can never again drift from the encoder it guards.
                                # RAW activations: the helper normalizes exactly once, as training did.
                                z_values = pre_activations_with_training_normalization(sae, cal_activations_flat)
                                ...
                                if pass_rate < 0.001:  # Less than 0.1% pass
                                    # A safety net for a genuinely mis-scaled SAE (an imported
                                    # third-party one, say): set each threshold to HALF of that
                                    # feature's observed max z, so every feature can fire.
                                    ...
                                    threshold_calibration.update({
                                        "rescaled": True,
                                        "threshold_mean_before": float(threshold_mean),
                                        "threshold_mean_after": float(new_threshold_mean),
                                        "pass_rate_after": float(new_pass_rate),
                                    })
                ...
                with torch.no_grad():
                    ...
                    # BATCHES NEVER STRADDLE A CORPUS. `batch_process_features`
                    # returns `fired_counts` summed over the whole call, so a
                    # batch spanning two corpora could not be attributed to
                    # either and the per-corpus frequencies would be
                    # unrecoverable. With one corpus this is exactly the old
                    # `range(0, len(dataset), batch_size)` (pinned by
                    # test_extraction_mixture's single-span identity tests), and
                    # the old `(len + bs - 1) // bs` under-counts once batches
                    # are clipped at a boundary.
                    planned_batches = batch_spans(spans, batch_size)
                    total_batches = len(planned_batches)

                    for batch_start, batch_end in planned_batches:
                        ...
                        # `batch_end` comes from the plan, already clipped to the
                        # corpus boundary. Recomputing it here would read past
                        # that boundary and break the attribution below.
                        batch = dataset[batch_start:batch_end]
                        batch_corpus = span_of(spans, batch_start)
                        ...
                        input_ids_tensor = torch.tensor(padded_input_ids, device=model_input_device, dtype=torch.long)
                        attention_mask_tensor = torch.tensor(attention_masks, device=model_input_device, dtype=torch.long)
                        ...
                        if batch_sae_features:
                            ...
                            vectorization_batch_size = get_vectorization_config(
                                config=config,
                                available_vram_gb=torch.cuda.mem_get_info(device)[0] / (1024**3) if device.type == "cuda" else None,
                                latent_dim=latent_dim,
                                seq_len=max_seq_len
                            )
                            ...
                            for feat_idx in feature_indices:
                                ...
                                # Attributed by the batch's first index, which
                                # is sound only because `batch_spans` never
                                # straddles a corpus boundary.
                                if batch_corpus >= 0:
                                    per_corpus_activation_counts[batch_corpus][feat_idx] += 1
                            # PRE-FILTER, and deliberately kept separate.
                            #
                            # `feature_activation_counts` above counts only
                            # samples that SURVIVED the junk-token filter, which
                            # is what `activation_frequency` has always meant.
                            # That column feeds the dead-neuron gate and the
                            # frontend's steering auto-baseline, whose constants
                            # were fitted against those very numbers — so it is
                            # not touched. This is the honest figure beside it,
                            # and the ratio of the two is the first measurement
                            # of what the filter costs.
                            feature_fired_counts += fired_counts
                            if batch_corpus >= 0:
                                per_corpus_fired_counts[batch_corpus] += fired_counts
            ...
            # Cleanup base model. A split model is freed in place: moving it to
            # the CPU would copy a model too large for one GPU into host memory
            # only to drop it.
            if placement.is_shard:
                detach_dispatch_hooks(base_model)
            else:
                base_model.cpu()
            ...
            #: Same denominator, honest numerator.
            true_activation_frequencies = feature_fired_counts / len(dataset)

            # Process features
            #: Mean ACTIVATION CONSISTENCY — not interpretability; see
            #: `calculate_interpretability_score`, which keeps its historical name.
            total_activation_consistency = 0.0
            total_activation_freq = 0.0
            features_processed = 0
            dead_neurons_filtered = 0
            #: Cut by the dead-neuron gate despite firing often enough on the
            #: TRUE frequency — i.e. suppressed by the junk-token filter rather
            #: than by being dead. See the gate below.
            filter_suppressed_neurons = 0
            ...
            for neuron_idx in range(latent_dim):
                ...
                if not heap_items:
                    ...
                    # AN EMPTY HEAP IS THE TOTAL-SUPPRESSION CASE, and it was
                    # invisible to the counter that exists to measure exactly
                    # this. Every one of the neuron's peaks landed on a filtered
                    # token, so it emitted nothing at all — the most
                    # filter-suppressed a neuron can be. Counting it only in the
                    # frequency branch below biased the number toward "the
                    # filter costs little", which is the conclusion the
                    # instrument exists to test.
                    if true_activation_frequencies[neuron_idx] >= min_activation_frequency:
                        filter_suppressed_neurons += 1
                    continue

                neuron_activation_freq = activation_frequencies[neuron_idx]
                if neuron_activation_freq < min_activation_frequency:
                    dead_neurons_filtered += 1
                    # THE GATE IS UNCHANGED. It still reads the post-filter
                    # frequency, deliberately: pointing it at the true one would
                    # change which features exist in every future extraction, in
                    # the same commit that adds a column, and nothing downstream
                    # would report the difference.
                    #
                    # Instrumented instead. A neuron that fired often enough on
                    # its own terms and was cut anyway was cut BY THE FILTER, not
                    # by being dead — and the count of those is the first
                    # measurement of what `filter_fragments` and its siblings
                    # actually cost. That number is what any future decision to
                    # move the gate would need.
                    if true_activation_frequencies[neuron_idx] >= min_activation_frequency:
                        filter_suppressed_neurons += 1
                    continue
                ...
                feature = Feature(
                    ...
                    activation_frequency_true=float(
                        true_activation_frequencies[neuron_idx]
                    ),
                    activation_count_true=int(feature_fired_counts[neuron_idx]),
                ...
                for example in top_examples:
                    # WHICH CORPUS THIS ROW CAME FROM. `sample_index` is a global
                    # offset into the concatenated mixture, so on its own it no
                    # longer identifies a row: two corpora occupy different
                    # ranges of the same index space. `span_of` returns -1 when
                    # an index falls outside every span, and that records NULL —
                    # "not recorded" — rather than guessing a corpus.
                    example_span = span_of(spans, example["sample_index"])
                    example_dataset_id = spans[example_span].dataset_id if example_span >= 0 else None
                ...
                total_activation_consistency += interpretability_score
            ...
            statistics = {
                ...
                # WHAT THE JUNK FILTER COST, reported rather than inferred.
                # Neurons cut as dead that fired often enough on the true,
                # pre-filter frequency. A large number here means the filter,
                # not the model, decided which features exist.
                "filter_suppressed_neurons": int(filter_suppressed_neurons),
                # WHAT DEDUPLICATION COST, reported for the same reason as the
                # line above: how many top-k candidates were folded into a slot
                # already holding byte-identical context. A corpus repeats
                # itself -- on the 32k L12 run the Apache licence header filled
                # all 25 slots of some features, and 7,744 of 32,726 features
                # had every example firing on one token. Nobody could see that
                # without hand-querying the database, which is why it went
                # unnoticed. Now the run says it.
                "duplicates_suppressed": int(incremental_heap.duplicates_suppressed),
                "threshold_calibration": threshold_calibration,
                "avg_activation_consistency": float(total_activation_consistency / features_processed) if features_processed > 0 else 0.0,
                "min_activation_frequency_threshold": min_activation_frequency,
                # WHICH CORPORA THIS RUN ACTUALLY READ, and what each
                # contributed. Free-form JSONB, so no migration. `requested` is
                # what the operator asked for; `realised_fraction` is what the
                # allocator could honour once each corpus was capped at what it
                # holds — the two diverge when a corpus is smaller than its
                # share, and that divergence should be visible rather than
                # argued about.
                "dataset_mixture": [
                    {
                        ...
                        "stop": span.stop,
                        "requested_weight": (
                            float(dataset_weights[i]) if dataset_weights is not None else None
                        ),
                        "realised_fraction": (span.rows / len(dataset)) if len(dataset) else 0.0,
                        "live_features": int((per_corpus_activation_counts[i] > 0).sum()),
                    }
            ...
            # A LOAD that ran out of memory is not a batch that did. The loader's
            # message names the card, the figures and the remedy; the batch-size and
            # sequence-length advice below would replace it and cut it to 200 characters.
            is_oom_error = not isinstance(e, OutOfMemoryError) and (
                "CUDA out of memory" in error_str or "OutOfMemoryError" in error_str
            )
            ...
            cleanup_gpu_memory(
                ...
                # Every placed card: a split model's layers sit on each. A
                # placement that failed loaded nothing, so the process-wide
                # sweep (every card this process holds memory on) is all that
                # is left to do.
                device=placement.all_devices if placement is not None else None,
            )
