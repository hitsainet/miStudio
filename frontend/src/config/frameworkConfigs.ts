...
export const FRAMEWORK_CONFIGS: Record<string, FrameworkConfig> = {
  ...
  [SAEArchitectureType.JUMPRELU]: {
    ...
    // `target_l0` is SHOWN and `dead_neuron_threshold` is HIDDEN — the reverse
    // of what this list used to say, and the reverse was exactly wrong.
    //
    ...
    // highest-leverage sparsity knob JumpReLU has — and it was unreachable.
    //
    // `dead_neuron_threshold` is read only inside the resampling block, which
    // JumpReLU disables by default, and detection uses an EMA instead. It was
    // a control that did nothing, displayed next to one that could not be set.
    visibleFields: [
