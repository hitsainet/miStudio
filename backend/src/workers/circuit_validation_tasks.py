...
from .circuit_gpu import (circuit_faithfulness_manifest, circuit_required_mb,
                          discovery_capture_manifest, place_circuit_job)
...
@celery_app.task(bind=True, base=DatabaseTask,
                 ...
                 max_retries=0)
@gpu_job("circuit_validation")
def validate_circuit_edges(self, run_id: str, scope: Dict[str, Any],
                           gpu_request: Optional[str] = None) -> Dict[str, Any]:
    """Validate a discovery run's top edges (rung 2), or reproduce a manifest.

    The request travels with the task; the card is recorded in the manifest
    (`payload["gpu"]`) and `report["validation"]["gpu"]`.
    """
    ...
    with self.get_db() as db:
        try:
            placement, gpu = place_circuit_job(
                gpu_request, kind="validation", run_id=run_id, allow_shard=True,
            ...
            result = CircuitInterventionService.run(
                db, run_id, scope, device=placement.device, placement=placement,
                gpu=gpu,
...
def run_circuit_faithfulness(self, circuit_id: str,
    ...
    The GPU request travels with the task; the card is recorded in the manifest
    (`payload["gpu"]`) and the result.
    """
    from ..services.circuit_capture_service import MAX_SEQ_LENGTH
    ...
    with self.get_db() as db:
        try:
            placement, gpu = place_circuit_job(
                gpu_request, kind="faithfulness", run_id=circuit_id,
                allow_shard=True,
                # The SAEs the pass loads: its circuit's layers, not every layer
                # the capture recorded.
                required_mb=circuit_required_mb(
                    db, circuit_faithfulness_manifest(db, circuit_id),
                    encode_tokens=MAX_SEQ_LENGTH))
            result = CircuitFaithfulnessService.run(
                db, circuit_id, config, device=placement.device,
