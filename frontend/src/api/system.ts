 ...
 *
 * @param gpuId - GPU index. Required: the endpoint no longer assumes card 0.
 ...
 * ```
 */
export async function getGPUMetrics(gpuId: number): Promise<GPUMetricsResponse> {
 ...
 * ```
 */
export async function getGPUInfo(gpuId: number): Promise<GPUInfoResponse> {
  const query = buildQueryString({ gpu_id: gpuId });
 ...
 * ```
 */
export async function getGPUProcesses(gpuId: number): Promise<GPUProcessesResponse> {
  const query = buildQueryString({ gpu_id: gpuId });
 ...
 *
 * @param gpuId - GPU index. Omit it to get EVERY card in `gpu.cards`; there is
 *   no default card. With an id the response carries that one card, and an id
 *   that names no card is a 400 rather than a silent fallback to card 0.
 * @returns Promise resolving to all monitoring data
 * @throws {APIError} If request fails (400 unknown card, 503 if GPU monitoring unavailable, 500 for other errors)
 *
 * @example
 * ```typescript
 * const data = await getAllMonitoringData();
 * if (data.gpu && 'cards' in data.gpu) {
 *   data.gpu.cards.forEach((card) => console.log(card.gpu_id, card.metrics.utilization.gpu));
 * }
 * ```
 */
export async function getAllMonitoringData(
  ...
  if (gpuId === undefined) {
    return fetchAPI<AllMonitoringDataResponse>('/system/all');
  }
  const query = buildQueryString({ gpu_id: gpuId });
