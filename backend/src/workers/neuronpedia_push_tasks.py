...
@gpu_job("neuronpedia_push", handoff=False)
def push_to_neuronpedia_local_task(
    ...
    async def _do_push():
        ...
        try:
            ...
            # Only the logit lens (part of the dashboard data) uses a GPU. Placed
            # before any work; a card that cannot take it raises
            # GpuPlacementError, which the handler below records as this push's
            # failure. The dashboard step itself swallows its errors, so placing
            # there would turn a refused card into a push without a logit lens.
            lens_device = None
            if compute_dashboard_data:
                from ..services.gpu_placement import AUTO, place_job

                placement = place_job(gpu_request)
                lens_device = placement.device
                logger.info(
                    "Push %s: logit lens on %s (requested %s)",
                    push_job_id, placement.describe(), gpu_request or AUTO,
                )
