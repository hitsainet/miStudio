...
def _is_abandoned(run, status: str) -> bool:
    ...
    if not task_id:
        ...
        return True
    # WAITING FOR A GPU IS NOT ABANDONED (multi-GPU Phase 3). Calibration and the
    # recorder write "running" before they place, and a job handed off at
    # placement re-runs that write unchanged, so its clock freezes while it
    # waits. Inert in single mode.
    from src.workers.gpu_job import waiting_for_gpu

    if waiting_for_gpu(task_id):
        return False
    try:
...
def cleanup_stuck_circuit_runs_task(self):
    ...
    cleaned = 0
    # Multi-GPU Phase 3: each reaped GPU lifecycle frees the card(s) its dead
    # execution leased — only a lease nothing renews any more.
    ...
    with self.get_db() as db:
        ...
        for run in db.query(CircuitCaptureRun).filter(
            ...
            if not _is_abandoned(run, run.status):
                continue
            release_reaped_leases(run.celery_task_id)
            run.status = "failed"
        ...
        for run in db.query(CircuitDiscoveryRun).filter(
            ...
            if run.attribution_status in ("pending", "running") and _is_abandoned(
                ...
                release_reaped_leases(run.attribution_task_id)
            ...
            if run.validation_status in ("pending", "running") and _is_abandoned(
                ...
                release_reaped_leases(run.validation_task_id)
        ...
        for circuit in db.query(Circuit).filter(
            ...
            if _is_abandoned(
                ...
                release_reaped_leases(circuit.faithfulness_task_id)
        ...
        for circuit in db.query(Circuit).filter(
            ...
            if _is_abandoned(
                ...
                release_reaped_leases(circuit.calibration_task_id)
        ...
        for rec in db.query(SteeringRecordRun).filter(
            ...
            if _is_abandoned(_SubLifecycleView(rec.task_id, rec.updated_at),
                             rec.status):
                release_reaped_leases(rec.task_id)
                rec.status = "failed"
                cleaned += 1
        if cleaned:
            db.commit()
