...
class ExternalSAE(Base):
    ...
    # passive_deletes: both FKs are ON DELETE CASCADE (migration 79dd2387ebeb),
    # so the database removes the children. Without it, deleting an SAE made the
    # ORM SELECT every extraction job, feature and activation example into the
    # API process first — 113 GB on 2026-09-12, which stalled the GPU node.
    extraction_jobs = relationship(
        "ExtractionJob", back_populates="external_sae", cascade="all, delete-orphan", passive_deletes=True
    )
    features = relationship(
        "Feature", back_populates="external_sae", cascade="all, delete-orphan", passive_deletes=True
    )
