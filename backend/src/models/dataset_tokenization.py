...
class DatasetTokenization(Base):
    ...
    # ── How the text was selected and rendered ──────────────────────────────
    #
    # These were REQUEST-ONLY before, so an existing tokenization carried no
    # record of how it was built. That is how Bloomberg came to be tokenized on
    # its `Headline` column — auto-detection fell through to `text_columns[0]`,
    # nothing recorded the choice, and 28x more text sat unused in `Article`.
    text_column = Column(
        String(255),
        nullable=True,
        comment="Column the text was read from. NULL means it was auto-detected.",
    )
    chat_format = Column(
        String(20),
        nullable=True,
        comment=(
            "auto | chat_template | plain | none — how conversation columns were "
            "rendered. NULL means NOT RECORDED, which is the truth for every row "
            "written before this column existed: those used the `<|role|>` "
            "pseudo-markers. Defaulting them to 'auto' would have them claim the "
            "tokenizer's real template was used."
        ),
    )
    pack_sequences = Column(
        Boolean,
        nullable=True,
        comment=(
            "Whether documents were concatenated into full max_length blocks. "
            "NULL means not recorded (pre-existing rows were padded per document)."
        ),
    )
    padding = Column(
        String(20),
        nullable=True,
        comment="Padding strategy actually used (request-only before this column existed)",
    )
    truncation = Column(
        String(20),
        nullable=True,
        comment="Truncation strategy actually used (request-only before this column existed)",
    )
    add_special_tokens = Column(
        Boolean,
        nullable=True,
        comment="Whether BOS/EOS were added (request-only before this column existed)",
    )

    # ── Whether the blocks were shuffled ────────────────────────────────────
    #
    # Extraction reads `dataset.select(range(max_samples))` — a PREFIX from block
    # 0 — so an unshuffled corpus means every extraction ever run saw the same
    # opening slice. The bias is undetectable downstream, because the held-out
    # evaluation draws from rows just past the prefix: adjacent data, same slice.
    #
    # MEASURED 2026-09-17, after this column shipped: the two corpora originally
    # named here are NOT ordered. Bloomberg carries Spearman -0.06 between row
    # order and article date (4,025 rows sampled of 446,762), and codeparrot's
    # first quarter covers 30.3% of its 33,090 repositories — proportional, not
    # clustered. The real case is OpenHermes-2.5, written one source at a time:
    # 14 of its 15 labeled sources occupy a single contiguous row range, so the
    # first 10% of rows holds only airoboros2.2 and CamelAI (11.3% of the corpus)
    # and never reaches glaive-code-assist (18.2%) or the unlabeled 49.6%
    # remainder.
    shuffled = Column(
        Boolean,
        nullable=True,
