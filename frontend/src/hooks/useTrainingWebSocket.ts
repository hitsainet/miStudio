...
export const useTrainingWebSocket = (trainingIds: string[]) => {
  ...
  const handleProgress = useCallback((data: TrainingProgressEvent) => {
    ...
    updateTrainingStatus(data.training_id, {
      ...
      current_fvu_centred: data.fvu_centred,
  ...
  // The post-run evaluation's result (SAE training remediation, item 6). A
  // completed training is often not subscribed, so the evaluation panel also
  // polls; this is the fast path when it is.
  const handleEvaluation = useCallback((data: any) => {
    if (!data?.training_id || !data?.evaluation) return;
    updateTrainingStatus(data.training_id, { evaluation: data.evaluation });
  }, [updateTrainingStatus]);
  ...
  useEffect(() => {
    ...
    on('training:evaluation', handleEvaluation);
    ...
    return () => {
      ...
      if (handlers.evaluation) off('training:evaluation', handlers.evaluation);
  ...
  }, [on, off, handleProgress, handleStatusChanged, handleCompleted, handleFailed, handleCheckpointCreated, handleEvaluation]);
