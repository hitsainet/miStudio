...
export interface TrainingConfig {
  ...
  /** Weight per SELECTED EXTRACTION, keyed by extraction id. Serialised to a
   *  positional array in `extraction_ids` order at request time — the worker
   *  indexes `dataset_weights` against the extractions it loads, in the order
   *  the ids were given, so a keyed map is the only shape that cannot drift. */
