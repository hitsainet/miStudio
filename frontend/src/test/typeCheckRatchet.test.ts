 ...
 *
 * Was 432 — not the 427 a shell `grep -c` reported, because `grep -c` counts
 * LINES and some tsc lines carry more than one `error TSxxxx`. The number here
 * is produced by the same regex the check below uses, so the two cannot
 * disagree.
 *
 * 419 since 2026-09-07. The swap-composer work added a fifteenth `render` in
 * `RankedReadouts.test.tsx` and the gate went 432 → 433, which is exactly what
 * it is for. The cause was one untyped helper — `Object.entries` widens the
 * lens-type key to `string` — so every call site in that file had always been
 * a type error. Typing the helper's return removed all 14 at once; raising the
 * baseline by one would have kept them and taxed the next test too.
 */
const BASELINE = 419;
