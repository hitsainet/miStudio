...
def _placement():
    """Where these GPU tests run, chosen the way a worker chooses it.

    `extract_activations` takes the placement with no default, so a test that
    wants a GPU asks the placement for one rather than naming index 0.
    """
    from src.services.gpu_placement import place_job

    return place_job("auto")
