...
import { LabelingMethod, LabelingStatus } from '../../types/labeling';
...
export const LabelingJobCard: React.FC<LabelingJobCardProps> = ({
  ...
  const buildConfig = () => ({
    ...
    // A local judge resumes on the card its job asked for. A resume is a new
    // job, so without this it would silently run on Auto. Remote judges use
    // no GPU here and send none.
    ...(job.labeling_method === LabelingMethod.LOCAL ? { gpu: job.gpu_request ?? 'auto' } : {}),
  });
