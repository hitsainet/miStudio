...
from ..services import activation_buffer, activation_mask, dataset_mixture, feature_density
from .model_tasks import select_tokenization_for_model
from ..services.checkpoint_service import CheckpointService
from ..core.cancellation import GPU_LEASE_LOST, record_progress
...
def stop_signal_for(row, step: int, lease_lost: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if row.status == TrainingStatus.PAUSED.value:
        return {"status": "paused", "step": step}
    if row.status == TrainingStatus.CANCELLED.value:
...
def real_token_activations(acts_flat: torch.Tensor, attention_masks, step: int) -> torch.Tensor:
    """The rows of ``acts_flat`` that are real tokens, by the batch's attention mask.

    PADDING IS NOT TRAINING DATA. On the Bloomberg tokenization (3.3% real) ~97%
    of every on-the-fly batch was PAD until the mask was applied here.

    THE MASK IS BUILT ON THE CPU, from the batch's own lists — never the tensor
    handed to the model. The captured activations are on the CPU (HookManager
    copies each layer output there), while that tensor is on the model's input
    card, and a CPU tensor indexed by a mask on a GPU raises ("indices should be
    either on cpu or on the same device as the indexed tensor"). A CPU mask
    indexes a tensor on any device.
    """
    real = torch.as_tensor(attention_masks, dtype=torch.bool).reshape(-1)
    if real.numel() != acts_flat.shape[0]:
        # Never silent — the alternative is training on pads without knowing.
        logger.warning(
            "Attention mask (%d) does not match the flattened activations (%d); PAD "
            "positions will be sampled as real tokens.",
            real.numel(), acts_flat.shape[0],
        )
        return acts_flat
    kept = acts_flat[real]
    if kept.shape[0] == 0:
        logger.warning(
            "Step %d: every position in this batch is padding; training on the raw batch instead",
            step,
        )
        return acts_flat
    return kept
...
#: Sequences used for the spliced-CE measurement. Small on purpose — the
#: quantity is a mean over tokens, and the point is a usable signal, not a
#: tight confidence interval on a number nothing gates.
CE_EVAL_SEQUENCES = 32


def _evaluate_spliced_ce(
    task,
    *,
    training_id,
    hp,
    ...
    """Measure what each SAE's reconstruction costs the base model.

    Reports CE with the model untouched, with the reconstruction spliced in, and
    with the activation replaced by the SAE's bias alone — and the scale-free
    summary `loss_recovered = (ablated - spliced) / (ablated - baseline)`.

    WHY IT LIVES HERE RATHER THAN IN THE TRAINING LOOP. The cached-activation
    path reads pre-extracted tensors and never loads a base model, which is why
    `spliced_ce_delta` shipped with no caller at all. Running once at the end,
    after the checkpoint is written, is what makes it affordable AND safe: the
    model load cannot cost a training that already succeeded.

    Skips rather than raises whenever the pieces are not available; a missing
    evaluation is a gap in reporting, not a reason to fail a finished run.
    """
    from ..ml.forward_hooks import HookType
    from ..ml.layer_discovery import discover_transformer_structure
    from ..services.sae_evaluation import spliced_ce_delta

    if not hp.get('evaluate_ce_delta', True):
        logger.info("Spliced-CE evaluation disabled by hyperparameters")
        return

    if dataset is None or tokenizer is None:
        logger.info(
            "Spliced-CE evaluation skipped: this run has no tokenized dataset "
            "in hand (cached-activation path). Set a dataset_id to enable it."
        )
        return

    loaded_here = False
    if base_model is None:
        logger.info("Loading the base model for spliced-CE evaluation…")
        model_id = None
        with task.get_db() as db:
            row = db.query(Training).filter_by(id=training_id).first()
            if row is not None:
                model_id = row.model_id
            model_row = db.query(Model).filter_by(id=model_id).first() if model_id else None
            repo_id = model_row.repo_id if model_row else None
            quantization = model_row.quantization if model_row else None
            model_dir = (
                settings.resolve_data_path(model_row.file_path)
                if model_row and model_row.file_path else None
            )
        if not repo_id:
            logger.info("Spliced-CE evaluation skipped: no model on record")
            return
        # The same call the training's own base-model load makes. This once
        # passed `quantization=`, which `load_model_from_hf` does not accept, so
        # whenever this branch ran it raised TypeError and the caller logged
        # "evaluation failed" — it could never load a model. It also took the
        # default device_map "auto", which spreads the model over every card
        # instead of the one this training was placed on.
        base_model, _tokenizer, _cfg, _meta = load_model_from_hf(
            repo_id=repo_id,
            quant_format=QuantizationFormat(quantization),
            cache_dir=model_dir,
            device_map=str(device),
            local_files_only=bool(model_dir and model_dir.exists()),
        )
        base_model.eval()
        loaded_here = True

    try:
        structure = discover_transformer_structure(base_model)
        rows = min(CE_EVAL_SEQUENCES, len(dataset))
        if rows == 0:
            return
        batch = dataset.select(range(rows))
        # The embedding's card. `base_model.device` is the first parameter's,
        # which on a split model can be another card entirely.
        ids_device = input_device(base_model)
        ids = torch.tensor(
            [list(r) for r in batch["input_ids"]], dtype=torch.long, device=ids_device
        )
        if "attention_mask" in batch.column_names:
            mask = torch.tensor(
                [list(m) for m in batch["attention_mask"]],
                dtype=torch.long, device=ids_device,
            )
        else:
        ...
        for layer_idx, hook_type in layer_hook_combinations:
            if hook_type != HookType.RESIDUAL.value:
                # Only the residual stream is a whole-layer output we can
                # substitute; splicing an attention sub-output would measure
                # something else entirely.
                continue
            sae = models.get((layer_idx, hook_type))
            if sae is None:
                continue
            try:
                layer_module = structure.layers_module[layer_idx]
            except Exception:  # noqa: BLE001
                logger.warning("Could not resolve layer %s for CE eval", layer_idx)
                continue

            result = spliced_ce_delta(base_model, sae, layer_module, ids, mask)
            logger.info(
                "L%s/%s SPLICED CE: baseline=%.4f spliced=%.4f ablated=%.4f "
                "delta=%+.4f loss_recovered=%.1f%%",
                layer_idx, hook_type, result["ce_baseline"], result["ce_spliced"],
                result["ce_ablated"], result["ce_delta"],
                result["loss_recovered"] * 100,
            )
            task.log_metric(
                training_id=training_id,
                step=step,
                loss=result["ce_spliced"],
                loss_reconstructed=result["ce_baseline"],
                loss_zero=result["ce_ablated"],
                l0_mean=result["loss_recovered"],
                l1_sparsity=result["ce_delta"],
                # Negative-and-offset marks this as a CE row, distinct from both
                # in-sample (>=0) and held-out (-1-layer) rows, so nothing
                # averages three different quantities together.
                layer_idx=-1000 - int(layer_idx),
            )
    finally:
        if loaded_here:
            del base_model
...
def train_sae_task(
    ...
    try:
        ...
        if use_cached_activations:
            ...
            storage_mode, tokens_to_load = activation_buffer.plan_activation_storage(
            ...
            if requested_weights is not None or tokens_to_load < total_available_tokens:
                allocation = dataset_mixture.allocate_tokens(
                    part_counts, tokens_to_load, requested_weights
                )
                logger.info(
                    "Mixture: %s",
            ...
            #: Held-out activations per (layer, hook). Populated only when
            #: `holdout_fraction` is set, and capped so reserving a large
            #: fraction cannot blow memory — an evaluation set does not need to
            #: be big, it needs to be DISJOINT.
            holdout_activations = {}
            HOLDOUT_EVAL_CAP = 100_000

            def _gather_holdout(mmap, part_info, held_parts):
                ...
                held_flat = part_info.get('holdout_flat')
                if held_flat is not None and held_flat.size:
                    taken = sum(h.shape[0] for h in held_parts)
                    room = HOLDOUT_EVAL_CAP - taken
                    if room > 0:
                        held_parts.append(
                            activation_mask.gather_tokens(mmap, held_flat[:room])
                        )
            ...
            for key in layer_hook_combinations:
                ...
                for part_info in parts_info:
                    ...
                    if activation_stream_mode:
                        ...
                        _gather_holdout(mmap, part_info, held_parts)
                        continue
                ...
                if held_parts:
                    ...
                    holdout_activations[key] = torch.from_numpy(
                        np.ascontiguousarray(held)
                    ).to(device)
                    logger.info(
                        "  %s: %s held-out tokens reserved for out-of-sample eval",
            ...
            if hp.get('dataset_weights') is not None:
                # The on-the-fly path concatenates tokenized datasets and samples
                # documents uniformly; there is no per-source budget to weight.
                # Silently ignoring the request is the defect — an operator who
                # sets weights here would have no way to discover they did
                # nothing.
                logger.warning(
                    "dataset_weights is set but this run extracts activations "
                    "on the fly, where sources are concatenated and sampled "
                    "uniformly. The weights will be IGNORED. Use cached "
                    "extractions (extraction_ids) for a weighted mixture."
                )
        ...
        for step in range(start_step, total_steps):
            ...
            if step % status_check_interval == 0:
                with self.get_db() as db:
                    training = db.query(Training).filter_by(id=training_id).first()
                    signal = stop_signal_for(training, step, lease_lost=lease_lost_reason())
                if signal is not None:
                    logger.info(
                        f"Training {signal['status']} at step {step}"
                    ...
                    if signal.get("reason") == GPU_LEASE_LOST:
                        ...
                        record_progress(
                            ...
                            error_message=(
                                f"Stopped at step {step}: {signal['detail']}. Resume it from its "
                                "last checkpoint."
                            ),
                        )
                    return signal
            ...
            try:
                ...
                if use_cached_activations:
                    # Rolling buffer when the pool exceeds memory (every token is
                    # used, none repeated within a pass); fixed pool otherwise.
                    ...
                    # THE TOKENIZATION'S OWN MASK. Round 3 found the previous fix
                    # here was a NO-OP: it masked with a mask built from
                    # `[1] * len(input_ids)`, and these rows arrive from Arrow
                    # ALREADY PADDED to max_length — which is the exact diagnosis
                    # written for the cached path in the same commit. The mask was
                    # all-ones, so nothing was removed and ~81% of every Bloomberg
                    # batch was still padding. The column was in
                    # `batch.column_names` the whole time and never read.
                    batch_source_masks = []
                    if "attention_mask" in batch.column_names:
                        for m in batch["attention_mask"]:
                            batch_source_masks.append(
                                m if isinstance(m, list)
                                else (m.tolist() if hasattr(m, 'tolist') else list(m))
                            )
                    else:
                        logger.warning(
                            "Tokenization has no attention_mask column; every "
                            "position will be treated as a real token. "
                            "Re-tokenize with return_attention_mask=True."
                        )

                    # Pad sequences to same length.
                    # No hidden cap: the tokenization already decided the window.
                    # Truncating here silently discarded everything past 512 while
                    # the operator believed they had configured 1024 or 2048.
                    max_length = max(len(ids) for ids in batch_input_ids)
                    pad_token_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else tokenizer.eos_token_id

                    # THE SAME builder the extraction path uses. Two inline
                    # copies is how the second one came to reproduce the exact
                    # defect the first had just fixed.
                    (
                        padded_input_ids,
                        attention_masks,
                        _real_lengths,
                        ...
                        batch_input_ids, batch_source_masks, max_length, pad_token_id
                    )
                    if missing_masks and step % 100 == 0:
                        logger.warning(
                            "%d rows in this batch have no attention_mask; their "
                            "padding is being trained on", missing_masks,
                        )

                    # Convert to tensors
                    input_ids_tensor = torch.tensor(padded_input_ids, device=model_input_device)
                    attention_mask_tensor = torch.tensor(attention_masks, device=model_input_device)
                    ...
                    with HookManager(base_model) as hook_manager:
                        ...
                        for layer_idx, hook_type in layer_hook_combinations:
                            ...
                            if layer_key in hook_manager.activations and hook_manager.activations[layer_key]:
                                ...
                                # PADDING IS NOT TRAINING DATA HERE EITHER. The
                                # mask comes from the batch's lists, on the CPU
                                # where the captured activations are; see
                                # `real_token_activations`.
                                acts_flat = real_token_activations(acts_flat, attention_masks, step)
            ...
            if step % log_interval == 0:
                ...
                if holdout_activations:
                    for sae_key, held in holdout_activations.items():
                        ...
                        try:
                            with torch.no_grad():
                                x_h = held.to(device, non_blocking=True)
                                out = model_ref(x_h)
                                losses_h = out[2] if len(out) > 2 else {}
                                fvu_h = losses_h.get('fvu')
                                l0_h = losses_h.get('l0_mean')
                                logger.info(
                                    "  L%s/%s HELD-OUT (%s tokens): fvu=%s l0=%s",
                                    sae_key[0], sae_key[1], f"{held.shape[0]:,}",
                                    f"{float(fvu_h):.4f}" if fvu_h is not None else "n/a",
                                    f"{float(l0_h):.1f}" if l0_h is not None else "n/a",
                                )
                                if fvu_h is not None:
                                    def _f(v):
                                    ...
                                    self.log_metric(
                                        training_id=training_id,
                                        step=step,
                                        loss=_f(losses_h.get('loss_total')) or 0.0,
                                        loss_reconstructed=_f(losses_h.get('loss_reconstruction')),
                                        loss_zero=_f(losses_h.get('loss_zero')),
                                        l1_sparsity=_f(losses_h.get('loss_l0')),
                                        l0_sparsity=_f(losses_h.get('l0_sparsity')),
                                        fvu=float(fvu_h),
                                        l0_mean=_f(l0_h),
                            ...
                            logger.warning("Held-out evaluation failed: %s", exc)
                        finally:
                            if was_training:
                                model_ref.train()
        ...
        # WHAT THE RECONSTRUCTION COSTS THE MODEL.
        #
        # Every other number here — FVU, L0, dead count — lives in the SAE's own
        # space. None says what the reconstruction costs the MODEL, and
        # reconstruction error is not uniformly important: the directions that
        # carry the output are a small share of the variance, so an SAE can hit
        # FVU 0.02 and still wreck the next-token distribution.
        #
        # Run AFTER the checkpoint is saved, deliberately. It needs the base
        # model, which the cached-activation path never loads, so it may have to
        # load one — and a failure there must not be able to cost a completed
        # training.
        try:
            _evaluate_spliced_ce(
                self,
                training_id=training_id,
                hp=hp,
                models=models,
        ...
        except Exception as exc:  # noqa: BLE001 - the training is already saved
            logger.warning("Spliced-CE evaluation failed (training is safe): %s", exc)
...
def resume_training_task(training_id: str, checkpoint_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    gpu_delay(train_sae_task, gpu_request)(
        training_id=training_id,
