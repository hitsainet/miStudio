...
# ── the PROBE ladder (IDL-52) ─────────────────────────────────────────────────
#
# A probe makes NO causal claim. It is a detector, and its evidence is whether it
# detects its concept on data unlike its training data, and how it compares with
# an LLM monitor. So it gets its own ladder rather than borrowing the circuit
# one — a detector cannot borrow a circuit's words.
#
# IT LIVES HERE, beside `EvidenceRung`, for two reasons that are both load-
# bearing: the no-parallel-enums guard (IDL-35) forbids a rung enum anywhere else
# in `src/`, and co-locating them means the two ladders are read together, so a
# change to one is reviewed against the other.


class ProbeRung(IntEnum):
    """A probe's evidence, from the evaluations ACTUALLY RUN (032 FR-13).

    Each rung is a claim about generalization, never about mechanism:

      0 TRAINED       the probe exists
      1 HELD_OUT      the AUROC CI lower bound is above 0.5 on at least one
                      IN-distribution evaluation set
      2 UNSEEN_TASKS  the CI lower bound is above 0.5 on EVERY out-of-distribution
                      set run, and at least one was run
      3 JUDGE_COMPARED  rung 2, plus a completed judge run on those same
                      out-of-distribution sets

    The bound is STRICT: a lower bound of exactly 0.5 does not pass. An interval
    touching chance is consistent with a probe that detects nothing.
    """
...
#: Words a probe's language may never use (IDL-52). "causal" because a probe makes
#: no causal claim; "validated" and "guarantee" because an AUROC interval is
#: evidence of detection on some data, not a warrant; "safe" because a detector
#: says what it detected, never that a system is safe. A grep test enforces this
#: over the two dictionaries below, so the rule cannot be broken by a rewording.
PROBE_FORBIDDEN_WORDS: tuple[str, ...] = ("causal", "safe", "guarantee", "validated")

#: The ONLY source of user-facing probe rung language. miLLM mirrors these
#: verbatim wherever a probe is armed, so a change here is a cross-plane change.
