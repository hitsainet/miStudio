...
class TestR1Fixes:
    ...
    def test_list_returns_slim_summaries(self, client):
        """`count` is patched too: the endpoint pages in SQL now, so it asks for
        the page and the total separately (OSD-30)."""
        with patch("src.api.v1.endpoints.circuits.CircuitService.list",
                   new=AsyncMock(return_value=[_circuit()])), \
             patch("src.api.v1.endpoints.circuits.CircuitService.count",
    ...
    def test_the_page_and_the_total_are_filtered_the_same_way(self):
        """A count that ignores a filter the page applies reports a total the page
        cannot reach — the classic pagination defect. Both calls must receive the
        same filters."""
        ...
        with patch("src.api.v1.endpoints.circuits.CircuitService.list", new=list_mock), \
             patch("src.api.v1.endpoints.circuits.CircuitService.count", new=count_mock):
            TestClient(app).get(
                "/api/v1/circuits?edge_type=persistence&promoted=true&min_rung=2&limit=5&offset=10"
            )

        page_filters = {k: v for k, v in list_mock.call_args.kwargs.items()
                        if k not in ("limit", "offset")}
        assert page_filters == count_mock.call_args.kwargs
        assert page_filters["edge_type"] == "persistence"
        assert list_mock.call_args.kwargs["limit"] == 5
        assert list_mock.call_args.kwargs["offset"] == 10
