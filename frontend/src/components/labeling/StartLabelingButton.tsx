...
import { AUTO_GPU, GpuRequest, GpuSelect } from '../common/GpuSelect';

/** Fewest examples a judge can form a label from — the API's own floor. */
const EXAMPLES_FLOOR = 10;

/**
 * The ceiling to offer when the extraction's retention is UNKNOWN.
 *
 * It is the historical hardcoded maximum, kept only for that case. When the
 * caller knows what the extraction stored, that number is the ceiling instead:
 * asking for more than was retained returns fewer rows with no error and no
 * notice, so a control offering 50 against a top-k of 25 quietly misreports
 * what the judge will see.
 */
const EXAMPLES_FALLBACK_CAP = 50;

const clampExamples = (value: number, cap: number): number =>
  Math.max(EXAMPLES_FLOOR, Math.min(cap, value));

interface StartLabelingButtonProps {
  extractionId: string;
  /**
   * Examples this extraction retained per feature (`config.top_k_examples`).
   *
   * Optional because it is not always known at the call site, and an absent
   * value must not narrow the control below what it has always allowed.
   */
  storedExamples?: number;
  disabled?: boolean;
  onSuccess?: () => void;
}
...
export const StartLabelingButton: React.FC<StartLabelingButtonProps> = ({
  ...
  // Only the LOCAL method loads a model onto a card here; the others call out.
  ...
  /*
   * FALSE is this button's historical behaviour — a full relabel — and
   * changing that default would repurpose the control silently. The
   * operator opts in, seeing what it will skip.
   */
  const [skipAdjudicated, setSkipAdjudicated] = useState<boolean>(false);
  const [maxExamples, setMaxExamples] = useState<number>(25);  // Examples per feature, capped by what was stored
  /**
   * What this extraction actually retained is the ceiling. Falls back to the
   * historical 50 only when the caller could not say.
   */
  const exampleCap =
    storedExamples && storedExamples >= EXAMPLES_FLOOR
      ? storedExamples
      : EXAMPLES_FALLBACK_CAP;
  ...
  // Initialize examples-per-feature from DB settings, bounded by what is stored.
  //
  // `labeling_default_max_examples` was a DEAD CONTROL: Settings wrote it and
  // nothing read it, while the batch-size default directly above was wired. So
  // the panel presented two identical-looking defaults, one of which did
  // nothing.
  //
  // The clamp runs whether or not a setting exists, because the component's
  // own default (25) can exceed a narrower extraction — a top-k of 20 would
  // otherwise open showing 25 and send 25, and the judge would silently get 20.
  useEffect(() => {
    if (!isOpen) return;
    const configured = settings.find(s => s.key === 'labeling_default_max_examples');
    const parsed = configured ? parseInt(configured.value) : NaN;
    setMaxExamples(prev => clampExamples(isNaN(parsed) ? prev : parsed, exampleCap));
  }, [isOpen, settings, exampleCap]);
  ...
  const handleStartLabeling = async () => {
    try {
      ...
      await startLabeling({
        ...
        // Sent for LOCAL only. A remote endpoint holds no card of ours, so a GPU
        // on those requests would describe work this app does not do.
        gpu: labelingMethod === LabelingMethod.LOCAL ? gpu : undefined,
        prompt_template_id: selectedTemplateId || undefined,
        batch_size: batchSize,
        skip_adjudicated: skipAdjudicated,
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg p-6 w-full max-w-5xl border border-slate-200 dark:border-slate-700 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-6">
                  ...
                  {labelingMethod === LabelingMethod.LOCAL && (
                    <div>
                      ...
                      <GpuSelect
                        ...
                        disabled={isLoading}
                        allowSplit
                        selectClassName="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed"
                      />
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-col">
                <div className="space-y-4">
                  {/*
                    * WHAT this job labels, before HOW it labels it.
                    *
                    * Unchecked relabels the whole extraction, which is what this
                    * button has always done. Checked fills only the gaps —
                    * leaving verdicts alone, including the refusals the judge
                    * reached deliberately, which cost ~8 s each to re-derive.
                    */}
                  <div>
                    <label className="flex items-start gap-2 text-sm text-slate-700 dark:text-slate-300 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={skipAdjudicated}
                        onChange={(e) => setSkipAdjudicated(e.target.checked)}
                        className="mt-0.5 w-4 h-4 rounded border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-emerald-600 focus:ring-emerald-500 focus:ring-offset-white dark:focus:ring-offset-slate-900"
                      />
                      <span>Skip features that already have a label</span>
                    </label>
                    <p className="mt-1 ml-6 text-xs text-slate-600 dark:text-slate-400">
                      {skipAdjudicated
                        ? 'Labels only features that have never been attempted or that failed. Existing verdicts are left untouched.'
                        : 'Relabels every feature, overwriting existing labels. This is the default.'}
                    </p>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {storedExamples ? (
                        <button
                          type="button"
                          onClick={() => setMaxExamples(exampleCap)}
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setMaxExamples(
                          clampExamples(parseInt(e.target.value) || 25, exampleCap),
                        )
                      }
                      className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
                    />
                    <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">
                      {storedExamples
                        ? `Number of high-activation examples to show the LLM (${EXAMPLES_FLOOR}-${exampleCap}). This extraction stored ${storedExamples} per feature.`
                        : `Number of high-activation examples to show the LLM (${EXAMPLES_FLOOR}-${EXAMPLES_FALLBACK_CAP}, default: 25)`}
                    </p>
                  </div>
