 ...
 *   F6 drop allowSplit from the readout GPU picker     -> "offers "All GPUs" and sends gpu "all""
 *      (multi-GPU Phase 2, 2026-09-14; restored byte-identically, sha256)
 */
...
vi.mock('../../api/jlens', () => ({
  jlensApi: {
    ...
    intervene: vi.fn(),
    checkTokens: vi.fn().mockResolvedValue([]),
    previewRepo: vi.fn(),
    acquire: vi.fn(),
    publish: vi.fn(),
  },
...
// RECORD WHAT THE CARD IS HANDED. `artifactId` decides whether a run is filed
// against the Jacobian lens as rung-2 evidence, and that is a prop, not a
// rendered string — there is no other way to see it from here.
const cardProps: Array<Record<string, unknown>> = [];
vi.mock('../jlens/InterventionCard', () => ({
  InterventionCard: (props: Record<string, unknown>) => {
    cardProps.push(props);
    return <div data-testid="intervention-card-stub" />;
  },
}));
...
describe('the panel sends the request it appears to send', () => {
  ...
  it('submits the model and prompt exactly once, requesting the logit lens', async () => {
    ...
    // BY NAME: the request block also holds the GPU picker.
    await user.selectOptions(screen.getByRole('combobox', { name: 'Model' }), 'm_gemma');
    ...
    expect(jlensApi.readout).toHaveBeenCalledWith({
      ...
      types: ['LOGIT_LENS'],
      // Nothing chosen means Auto — never an index.
      gpu: 'auto',
      // The checkbox is off until ticked: keep the model loaded between readouts.
      unload_after: false,
    });
  });

  it('offers "All GPUs" and sends gpu "all" — a readout can run split', async () => {
    const cards = [
    ...
    useSystemMonitorStore.setState({ gpuList: { gpu_count: 2, gpus: cards } } as never);
    const user = userEvent.setup();
    mockReadout(makeReadout([0, 1, 2]));
    render(<JLensPanel />);
    ...
    await user.selectOptions(screen.getByRole('combobox', { name: 'Model' }), 'm_gemma');
    await user.type(screen.getByPlaceholderText(/capital of France/), 'hello');
    const picker = screen.getByRole('combobox', { name: 'GPU' }) as HTMLSelectElement;
    expect(Array.from(picker.options).map((o) => o.value)).toEqual([
      'auto', cards[0].uuid, cards[1].uuid, 'all',
    ]);
    await user.selectOptions(picker, 'all');
    await user.click(screen.getByRole('button', { name: /Read out/ }));

    expect(jlensApi.readout).toHaveBeenCalledTimes(1);
    expect((jlensApi.readout as ReturnType<typeof vi.fn>).mock.calls[0][0].gpu).toBe('all');
    // The store outlives the test; leave the next one on Auto.
    act(() => useJLensStore.getState().setGpu('auto'));
  });
...
describe('JLensPanel — ranked readouts and interventions', () => {
  ...
  it('intervenes on the READOUT PROMPT and the token\u2019s OWN layers', async () => {
    ...
    // The quick intervention is placed by the request block's GPU picker.
    expect(sent.gpu).toBe('auto');
  });

  it('credits the artifact ONLY on a Jacobian read', async () => {
    /**
     * `artifact_id` is provenance: it appends an `evidence_rung: 2` record to
     * that lens's `interventions.json`, the file that travels to HuggingFace
     * and into a serving runtime. `runIntervention` has gated it on the clicked
     * column since the ranked list existed; this card never did, so a
     * logit-lens finding was filed under `lens_type: JACOBIAN_LENS` — at layers
     * a partial fit was never fitted at.
     *
     * MUTATION CONTROL: pass the slug unconditionally and this fails.
     */
    cardProps.length = 0;
    render(<JLensPanel />);
    seed(makeReadout([0, 1, 2], ['LOGIT_LENS'], 4));
    ...
    // AN ARTIFACT MUST ACTUALLY EXIST, or there is nothing for the gate to
    // withhold and the assertion passes against no gate at all. The first
    // version of this test seeded none: `artifacts.find(...)` was undefined and
    // `?? null` gave null either way — a fixture agreeing by construction, and
    // a mutation deleting the gate sailed through it.
    act(() => {
      useJLensStore.setState({
        modelRepoId: 'org/m',
        artifacts: [{ slug: 'm', layers: [0, 1, 2] } as never],
      });
    });
    await waitFor(() => expect(cardProps.length).toBeGreaterThan(0));

    // LOGIT read -> no credit, even though the artifact is right there.
    expect(cardProps[cardProps.length - 1].artifactId).toBeNull();

    // ...and the same artifact IS credited on a Jacobian read, or the gate
    // would pass by never crediting anything.
    act(() => useJLensStore.setState({ lensMode: 'JACOBIAN_LENS' }));
    await waitFor(() =>
      expect(cardProps[cardProps.length - 1].artifactId).toBe('m'),
    );
  });

  it('CLOSES an open swap composer when the readout changes underneath it', async () => {
    /**
     * The composer holds a token and layer numbers from ONE readout. A new read
     * replaces the prompt the grid describes, and an open composer would submit
     * the old coordinates against the new `readoutPrompt` — scoring a forward
     * pass nobody is looking at.
     *
     * MUTATION CONTROL: empty the clearing effect and this fails.
     */
    const user = userEvent.setup();
    render(<JLensPanel />);
    seed(makeReadout([0, 1, 2], ['LOGIT_LENS'], 4));
    ...
    await user.click(within(column).getAllByRole('button', { name: /Swap/ })[0]);
    expect(await screen.findByTestId('jlens-swap-composer')).toBeTruthy();

    // A different readout arrives.
    act(() => {
      useJLensStore.setState({ readoutPrompt: 'a completely different prompt' });
    });
    await waitFor(() =>
      expect(screen.queryByTestId('jlens-swap-composer')).toBeNull(),
    );
  });

  it('REFUSES a swap until a partner is typed, rather than queueing a doomed one', async () => {
    /**
     * A swap with one coordinate is an additive steer wearing a swap's name,
     * and the backend 422s it. The refusal used to live on the ranked row —
     * Swap was disabled until something was pinned — and it now lives in the
     * composer, because the partner is typed there.
     *
     * WHAT MUST NOT CHANGE is that no request leaves until there are two
     * different coordinates. This asserts the whole gap: opening the composer
     * is not running it, an empty partner cannot run, and neither can a
     * partner identical to the row that was clicked.
     *
     * MUTATION CONTROL: drop `!sameToken` or `target.trim()` from `ready` in
     * SwapComposer and this fails.
     */
    const user = userEvent.setup();
    render(<JLensPanel />);
    seed(makeReadout([0, 1, 2], ['LOGIT_LENS'], 4));
    ...
    const column = screen.getByTestId('ranked-LOGIT_LENS');
    const swap = within(column).getAllByRole('button', { name: /Swap/ })[0];
    await user.click(swap);

    const composer = await screen.findByTestId('jlens-swap-composer');
    const from = within(composer).getByTestId('swap-from').textContent ?? '';
    // Opening is not running.
    expect(jlensApi.intervene as ReturnType<typeof vi.fn>).not.toHaveBeenCalled();
    // Nothing typed yet.
    expect(within(composer).getByTestId('swap-run')).toBeDisabled();

    // The SAME token is not a swap either — one coordinate, twice.
    await user.type(within(composer).getByTestId('swap-target'), from);
    expect(within(composer).getByTestId('swap-run')).toBeDisabled();
    expect(jlensApi.intervene as ReturnType<typeof vi.fn>).not.toHaveBeenCalled();
  });
  ...
  it('sends the TYPED partner as target_token, and the clicked row as the direction', async () => {
    /**
     * The partner is half the experiment: it supplies the second coordinate AND
     * it is the token whose rank gets scored. It used to be taken from PIN
     * ORDER, so re-pinning in a different order ran a different experiment
     * under an identical-looking click. Now it is typed against the row that
     * was picked — which is the whole point of the composer.
     *
     * ARBITRARY, NOT FROM THE READOUT. ' Juniper' never appears in this
     * readout. The server never restricted the partner to tokens on screen
     * (`/jlens/token-check`: "ANY single token has one … including tokens the
     * readout never surfaced, which are precisely the interesting swap
     * targets") and neither may the UI.
     *
     * MUTATION CONTROLS:
     *   * send `swapPartnerFor(token)` instead of the typed value -> fails
     *   * send `target_token: undefined` on the swap path         -> fails
     *   * trim the typed token                                    -> fails
     */
    const user = userEvent.setup();
    (jlensApi.intervene as ReturnType<typeof vi.fn>).mockResolvedValue({
      task_id: 'iv-2',
    ...
    (jlensApi.checkTokens as ReturnType<typeof vi.fn>).mockResolvedValue([
      { token: ' Juniper', ids: [4242], n_tokens: 1, usable: true, detail: 'one token' },
    ]);
    render(<JLensPanel />);
    seed(makeReadout([0, 1, 2], ['LOGIT_LENS'], 4));
    await waitFor(() => expect(screen.getByTestId('jlens-ranked')).toBeTruthy());
    ...
    // LEADING SPACE PRESERVED. ' Juniper' and 'Juniper' are different rows of
    // W_U; a trim here would run a different experiment than the one typed.
    await user.type(within(composer).getByTestId('swap-target'), ' Juniper');
    await user.click(within(composer).getByTestId('swap-run'));
    ...
    expect(sent.target_token).toBe(' Juniper');
    expect(sent.direction_token).not.toBe(' Juniper');
    expect(sent.direction_token).toBeTruthy();
...
describe('unload model after each readout', () => {
  /**
   * User decision 2026-09-13: "give the user the option of toggling an unload
   * between readouts". Off keeps the model loaded for a fast next readout.
   *
   * MUTATION CONTROLS:
   *   * the checkbox's onChange no longer calls setUnloadAfterReadout -> this fails
   *   * `unloadAfterReadout` dropped from the store's partialize       -> this fails
   */
  it('is off by default, and ticking it sets the choice and keeps it across a reload', async () => {
    const user = userEvent.setup();
    render(<JLensPanel />);
    const box = screen.getByLabelText(/unload model after each readout/i) as HTMLInputElement;
    expect(box.checked).toBe(false);

    await user.click(box);

    expect(useJLensStore.getState().unloadAfterReadout).toBe(true);
    // A preference, not a card, so unlike `gpu` it is persisted.
    expect(localStorage.getItem('miStudio-jlens') ?? '').toContain('"unloadAfterReadout":true');
  });
});
