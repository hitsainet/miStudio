...
#: The `gpu` parameter every GPU-bound J-space tool takes. Sent on every call,
#: "auto" included, so the request says which card was asked for.
#:
#: TWO WORDINGS, because the tools differ (multi-GPU Phase 2). Readouts, band
#: reports, interventions and acquisitions run a model split across cards when
#: no single card holds it; a fit never does. An agent cannot infer that from a
#: string-typed field, and one told nothing would ask a fit for "all" and learn
#: only from the failed task.
GPU_PARAMETER = (
    '"auto" (the default) runs on the GPU with the most free memory when the job '
    "starts, and splits a model no single GPU can hold across GPUs. \"all\" splits "
    "the model across every GPU. A GPU UUID (or index) from /api/v1/system/gpu-list "
    "runs on that card; a named card, or a split, that cannot take the job is "
    "refused, never swapped for another. The card the job ran on is recorded as "
    "gpu_uuid on its task row, and every card of a split as gpu_uuids"
)

FIT_GPU_PARAMETER = (
    '"auto" (the default) runs on the GPU with the most free memory when the job '
    "starts. A GPU UUID (or index) from /api/v1/system/gpu-list runs on that card; "
    "a named card that cannot take the job is refused, never swapped for another. "
    'A fit always runs on ONE card: a model no single GPU can hold, and "all", are '
    "refused. The card the job ran on is recorded as gpu_uuid on its task row"
)
...
def register(mcp: FastMCP, client: MiStudioClient, settings: MCPSettings) -> None:
    ...
    async def jlens_readout(
        ...
        gpu: Annotated[str, Field(description=GPU_PARAMETER)] = "auto",
        unload_after: Annotated[bool, Field(description="False (the default) keeps the model loaded on its card, so the next readout of that model skips the load; a later Auto readout reuses that copy. True frees the card when the readout finishes, e.g. for miLLM or another job. A failed readout frees the card either way. Readouts only: probe, band-report and intervention jobs always free the card")] = False,
    ) -> Any:
        ...
        a minute. Later readouts of the same model are fast while it stays
        loaded, which it does unless `unload_after` is set.
        ...
        body: dict[str, Any] = {
            ...
            "top_n": top_n,
            "gpu": gpu,
            # Sent on every call, False included, so the request says what it
            # asked for rather than leaning on the server's default.
    ...
    async def jlens_cost_estimate(
        ...
        weights_bytes: Annotated[int, Field(description="Resident model size in bytes, for a FIT. It dominates a fit's peak — 12.79 of 16.6 GiB on the one measured run — and cannot be inferred from d_model. Omitted, the reported peak excludes the weights and the basis says so")] = 0,
    ) -> Any:
    ...
    async def run_jlens_intervention(
        ...
        target_token: Annotated[Optional[str], Field(description="The token whose RANK is scored in the model's output. Defaults to direction_token. REQUIRED and must DIFFER for coordinate_swap: a swap exchanges two coordinates, and one token would be an additive steer wearing a swap's name A string the tokenizer SPLITS is scored on its FIRST piece only, because a rank is defined over one position — weaker evidence than a single token, and the result says so in `caveat`.")] = None,
        direction: Annotated[Optional[List[float]], Field(description="Explicit d_model vector to act along")] = None,
        direction_token: Annotated[Optional[str], Field(description="Resolve the direction from a token's unembedding row instead of passing d_model floats. A string the tokenizer SPLITS is usable: the direction becomes the MEAN of its pieces' rows, which is not any single token's direction. A single token is the stronger form, and a leading space usually decides which you get")] = None,
    ...
    async def fit_jlens_artifact(
        ...
        gpu: Annotated[str, Field(description=FIT_GPU_PARAMETER)] = "auto",
    ) -> Any:
