...
**Input:** a weighted mixture of tokenized corpora — one or more, each contributing a share of the
samples read. Stage 1 reads a *prefix* of each tokenization's blocks, so block order decides which
text is actually seen; tokenizations are shuffled when written for that reason.
**Output:** the model's raw internal activations at your chosen layer(s) and hook point(s), cached to disk.

This is the expensive stage — it's a full forward pass of the base model over potentially millions of tokens. You configure it from the **Models** panel (layer, hook type, batch size, and the GPU it runs on — Auto, or a specific card).
