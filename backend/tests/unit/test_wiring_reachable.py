...
class TestBothTrainingPathsExcludePadding:
    ...
    def test_the_on_the_fly_branch_applies_the_mask(self):
        ...
        CPU tensor indexed by a GPU mask raises. The filter is now
        `real_token_activations`, tested for behaviour in
        test_training_gpu_placement.py; this pins that the step calls it.
        """
        import ast
        ...
        calls = [
            ...
            if isinstance(node, ast.Call) and getattr(node.func, "id", None) == "real_token_activations"
        ]
        assert calls == [["acts_flat", "attention_masks", "step"]], (
