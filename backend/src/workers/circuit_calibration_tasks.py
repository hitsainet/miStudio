...
def _split(placement) -> Dict[str, Any]:
    ...
    return {"placement": placement} if placement.is_shard else {}
...
def run_circuit_calibration(self, circuit_id: str,
    ...
    "calibration" channel (run_id = circuit_id). The GPU request travels with
    the task; the card is recorded in the calibration manifest."""
    ...
    with self.get_db() as db:
        ...
        try:
            placement, gpu = place_circuit_job(
                gpu_request, kind="calibration", run_id=circuit_id, allow_shard=True,
                required_mb=circuit_required_mb(db, circuit_steering_manifest(db, circuit_id)))
            result = CircuitCalibrationService.run(
                db, circuit_id, config,
                progress_cb=_calibration_progress(db, circuit_id),
                device=placement.device, gpu=gpu, **_split(placement))
            ...
            # A lost GPU lease (multi-GPU Phase 3) is the job's failure, not the operator's stop.
            outcome = "failed" if cancelled.reason == GPU_LEASE_LOST else "cancelled"
            _set_status(db, circuit_id, outcome)
            emit_circuit_run_failed(
                "calibration", circuit_id,
                f"stopped: {cancelled.detail}" if outcome == "failed" else "cancelled",
            )
            return {"status": outcome, "circuit_id": circuit_id,
            ...
            release_circuit_job(placement, kind="calibration", run_id=circuit_id)


@celery_app.task(bind=True, base=DatabaseTask,
                 name="src.workers.circuit_calibration_tasks.reproduce_circuit_calibration",
                 max_retries=0)
@gpu_job("circuit_calibration_reproduce")
def reproduce_circuit_calibration(self, manifest_id: str,
    ...
    manifest with the band-delta verdict. Holds the GPU like a fresh run, and
    places with the request the ORIGINAL calibration was submitted with.
    ...
    with self.get_db() as db:
        ...
        try:
            placement, gpu = place_circuit_job(
                gpu_request, kind="calibration-reproduce", run_id=manifest_id, allow_shard=True,
                required_mb=circuit_required_mb(db, circuit_steering_manifest(db, circuit_id)))
            ...
            # Every card of the job, once the model is unreachable; see
            # `circuit_gpu.release_circuit_job`.
            release_circuit_job(placement, kind="calibration-reproduce", run_id=manifest_id)
