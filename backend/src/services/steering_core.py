...
def layer_device(structure, layer: int, device):
    """The device of decoder layer ``layer`` — where its SAE and steering vector go.

    On a model split across cards (Multi-GPU Phase 2) each layer lives on its
    own card, and a W_dec put on the placement's first card sits on the wrong
    card for every layer accelerate mapped elsewhere. Falls back to ``device``
    with no structure (a single-card caller) or a layer the structure lacks.
    """
    if structure is None:
        return device
    from ..ml.model_devices import module_device

    layers = structure.layers_module
    if not 0 <= int(layer) < len(layers):
        return device
    return module_device(layers[int(layer)]) or device


def _load_wdec_by_layer(sae_ids_by_layer: Dict[int, str], db, device,
                        structure=None) -> Dict[int, Any]:
    """Resolve each layer's decoder weight [d_model, d_sae] onto that layer's device.

    ``structure`` is the loaded model's; without it every decoder goes to
    ``device``, as for a model on one card.

    THE SAE IS BUILT ON THE CPU AND ONLY ITS DECODER GOES TO THE CARD (review
    round 3). The core steers with W_dec alone, and it used to move the whole
    SAE (encoder, decoder, biases: twice the decoder) onto the layer's card
    first: a split then had to keep room there for an encoder it discards, and
    charging the whole SAE per layer refused OLMo-2-13B at FP16 with decoders
    beside layers 11-13 on the node's cards, which fits with the decoders alone.
    """
    ...
    for L, sid in sae_ids_by_layer.items():
        ...
        # The core adds W_dec at the layer's output: an MLP or attention SAE's direction
        # belongs elsewhere (review R2-B). Refused before the SAE loads.
        refuse_non_residual(sae_rec.hook_type, "Steering", error=SteeringCoreError)
        decoder = resolve_decoder_weight(_load_sae_sync(sae_rec, "cpu"))
        if decoder is None:
            raise SteeringCoreError(f"SAE {sid} for layer {L} has no decoder weight")
        wdec[L] = decoder.detach().to(layer_device(structure, L, device))
    return wdec


def _wdec_for(sae_ids_by_layer, db, device, structure):
    # Only a split hands a structure in. Single-card callers keep the exact
    # three-argument call they always made.
    if structure is None:
        return _load_wdec_by_layer(sae_ids_by_layer, db, device)
    return _load_wdec_by_layer(sae_ids_by_layer, db, device, structure=structure)


def circuit_sae_ids_by_layer(circuit) -> Dict[int, str]:
    """{layer: SAE id} for a circuit, from ``circuit.saes``: the SAE its resolver
    loads onto each layer's card. One definition, for the resolution and for the
    split's per-layer allowance (calibration hands it to the loader)."""
    return {int(s["layer"]): s.get("mistudio_sae_id")
            for s in (circuit.saes or []) if s.get("layer") is not None}


def resolve_circuit_members(circuit, db, device,
                            structure=None) -> Tuple[str, List[ResolvedMember]]:
    ...
    ``structure`` (optional): the loaded model's, so each layer's W_dec is put
    on that layer's device when the model is split across cards.
    """
    sae_ids_by_layer = circuit_sae_ids_by_layer(circuit)
    wdec = _wdec_for(sae_ids_by_layer, db, device, structure)
...
def resolve_feature_members(feature_specs: List[Dict[str, Any]], model_id: str,
                            db, device, structure=None) -> Tuple[str, List[ResolvedMember]]:
    ...
    SAE — mixing SAEs at one layer is rejected as ambiguous). ``structure``: as
    for :func:`resolve_circuit_members`.
    """
...
def resolve_cluster_members(cluster_profile_id: str, db, device, structure=None
    ...
    ``structure``: as for :func:`resolve_circuit_members`."""
...
def build_steer_generator(model, tokenizer, structure, resolved_members,
    ...
    def _generate(prompt: str, dial: float, gseed: int) -> str:
        ...
        # The EMBEDDING's device, not `model.device` (the first parameter's).
        # On a split model those can be different cards, and the mismatch then
        # surfaces inside the embedding lookup, far from this line.
...
def load_model_and_structure(model_id: str, db, device, placement=None,
                             sae_ids_by_layer=None):
    """Load a model by miStudio Model id onto ``device`` and discover its
    structure. Returns (model, tokenizer, structure, disable_cache, device).
    Shared by calibration and the recorder.

    ``device`` is the placed device from the task's ``place_job`` — never chosen
    here. A bare ``"cuda"`` put every calibration and recording on CUDA index 0,
    which became the 12 GB card when a second GPU was added.

    ``placement`` (optional): a SPLIT placement loads the model across its cards
    with its per-card budget and no CPU entry (the loader refuses anything
    mapped off the GPUs). A single-card placement, or none, loads onto
    ``device`` exactly as before.

    ``sae_ids_by_layer`` (optional, a split only): the SAE whose decoder the
    resolvers put on each layer's card. The split is mapped to hold each decoder,
    at the fp32 ``_load_sae_sync`` builds, beside its layer (``ml/split_load.py``):
    the decoder alone, because that is all :func:`_load_wdec_by_layer` moves there.
    """
    ...
    if placement is not None and placement.is_shard:
        from .circuit_devices import sae_mb_by_layer

        # The placement's own split strategy and per-card budget, passed
        # through, never spelled here (the core chose the strategy; see
        # `Placement.device_map`). Without `max_memory` accelerate would spread
        # the model over every visible card and may offload to the CPU. The
        # SAEs the resolvers put on each layer's card are mapped for too: the
        # budget keeps only SHARD_RESERVE_MB free per card.
        model, tokenizer, _c, _m = load_model_from_hf(
            repo_id=model_rec.repo_id,
            quant_format=QuantizationFormat(model_rec.quantization),
            cache_dir=resolved, device_map=placement.device_map,
            max_memory=placement.max_memory,
            extra_mb_by_layer=sae_mb_by_layer(db, sae_ids_by_layer or {}, decoder_only=True) or None,
