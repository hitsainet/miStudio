...
class TestTheCeleryIdExistsBeforeTheTaskDoes:
    ...
    MUTATION CONTROL G1 (2026-09-14, multi-GPU Phase 3; restored byte-identically,
    sha256 and `git diff` checked): drop `task_id=task_id` from the helper dispatch
    -> test_the_id_is_generated_not_read_back red. Its text-scrape predecessor was
    satisfied by a comment and could not have gone red.
    """
    ...
    def test_the_endpoint_does_not_dispatch_before_persisting(self):
        ...
        for node in ast.walk(fn):
            ...
            # Multi-GPU Phase 3: the training is queued through the GPU routing helper.
            elif name in ("delay", "apply_async", "dispatch_gpu_task"):
    ...
    def test_the_id_is_generated_not_read_back(self):
        """A CALL keyword, not text: the old scrape (`"apply_async" in source`) was
        satisfied by the comment explaining the fix, so it could not fail."""
        tree = ast.parse((SRC / "api" / "v1" / "endpoints" / "trainings.py").read_text())
        fn = next(n for n in ast.walk(tree)
                  if isinstance(n, ast.AsyncFunctionDef) and n.name == "create_training")
        dispatches = [
            n for n in ast.walk(fn) if isinstance(n, ast.Call) and (
                getattr(n.func, "attr", "") in ("delay", "apply_async")
                or getattr(n.func, "id", "") == "dispatch_gpu_task")
        ]
        assert len(dispatches) == 1, "expected exactly one dispatch of the training"
        passed = {kw.arg: kw.value for kw in dispatches[0].keywords}
        assert isinstance(passed.get("task_id"), ast.Name) and passed["task_id"].id == "task_id", (
