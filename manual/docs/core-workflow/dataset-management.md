...
| **Shuffle Blocks** | ✅ On | Permute the packed blocks before writing them to disk. |
| **Seed** | derived | Seed for that permutation. Leave it blank and one is derived from `(dataset, model, max_length)` and recorded, so the order is reproducible either way. `0` is a valid seed, not a sentinel. |
| **Token filtering** | off | Optional junk filtering (see below) |

Each tokenization is tracked as its own job with live progress, and can be cancelled or deleted independently. The unique key is `(dataset, model, max_length)` — requesting the same combination again reuses the existing tokenization instead of duplicating work, **including its block order**.

:::info Why block order matters
Training and extraction both read a **prefix** of a tokenization's blocks, not a random sample —
`max_samples` rows off the front. So block order decides which text the run actually sees.

Corpora are frequently written one source at a time. OpenHermes-2.5 is: its first 10% of rows holds
2 of its 15 sources, so an unshuffled prefix would never reach the rest. Shuffling makes the prefix
a uniform sample of the whole corpus, which is why it is on by default.

Turn it off only to reproduce an ordered tokenization deliberately. Note that shuffling drops the
final ragged block, so a shuffled tokenization holds one block fewer and every block it keeps is
completely full.
:::
...
- **Tokenization list** — every tokenization with its model, max length, token count, vocab size, and average sequence length. The API additionally returns each tokenization's provenance: `shuffled` and `shuffle_seed`, the `text_column` it consumed, `chat_format`, and whether sequences were packed, padded, truncated, or given special tokens. Query it when you need to know how a corpus was prepared rather than just how large it is.
