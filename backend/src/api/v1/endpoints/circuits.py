...
async def list_circuits(
    ...
    # FILTERED AND PAGINATED IN THE QUERY. This used to load every circuit, apply
    # `edge_type` as a Python list comprehension and slice the page out of the
    # result, so a 50-row page cost a full-table read that grew with the table.
    # `total` comes from the SAME filter builder, because a count that does not
    # match the page is the classic pagination defect.
    filters = dict(promoted=promoted, min_rung=min_rung,
                   granularity=granularity, edge_type=edge_type)
    page = await CircuitService.list(db, limit=limit, offset=offset, **filters)
    total = await CircuitService.count(db, **filters)
...
async def start_faithfulness(circuit_id: str, body: FaithfulnessBody,
    ...
    try:
        ...
        config = CircuitFaithfulnessService.create_config(
            body.model_dump(exclude={"gpu"}))
    ...
    # and nothing dispatched. The request travels with the task.
    ...
    from ....api.v1.endpoints.circuit_discovery import _dispatch_or_restore, _run_sync
    ...
    def _guard_and_mark(sync_db):
        ...
        previous = row.faithfulness_status
    ...
    task = await _dispatch_or_restore(
        db,
        lambda: gpu_delay(run_circuit_faithfulness)(circuit_id, config,
                                               gpu_request=gpu_request),
        Circuit.__table__.update()
        .where(Circuit.id == circuit_id, Circuit.faithfulness_status == "pending")
        .values(faithfulness_status=previous_status))
    await db.execute(
...
async def start_calibration(circuit_id: str, body: CalibrationBody,
    ...
    try:
        ...
        config = CircuitCalibrationService.create_config(
            body.model_dump(exclude={"gpu"}))
    ...
    # Before the pass is marked: an unknown card is a 400 with nothing marked
    # and nothing dispatched. The request travels with the task and is recorded
    # in the calibration manifest, where a reproduction reads it back.
    gpu_request = resolve_gpu_request(body.gpu, can_split=True)
    from ....api.v1.endpoints.circuit_discovery import _dispatch_or_restore, _run_sync
    ...
    def _guard_and_mark(sync_db):
        CircuitCaptureService.assert_no_active_gpu_run(sync_db, gpu_request=gpu_request)
        row = sync_db.query(Circuit).filter(Circuit.id == circuit_id).first()
        previous = row.calibration_status
    ...
    task = await _dispatch_or_restore(
        db,
        lambda: gpu_delay(run_circuit_calibration)(circuit_id, config,
                                              gpu_request=gpu_request),
        Circuit.__table__.update()
        .where(Circuit.id == circuit_id, Circuit.calibration_status == "pending")
        .values(calibration_status=previous_status))
    await db.execute(
        Circuit.__table__.update().where(Circuit.id == circuit_id)
...
class ReproduceCalibrationBody(BaseModel):
    """Optional body for reproduce: the card the reproduction runs on."""

    gpu: Optional[GpuRequestStr] = Field(
        None,
        description=(
            "The GPU for the reproduction. Omitted, the reproduction reuses the "
            "request the original calibration was submitted with; given, it "
            "replaces it. " + GPU_REQUEST_DESCRIPTION
        ),
    )
...
async def reproduce_calibration(manifest_id: str,
                                body: Optional[ReproduceCalibrationBody] = None,
                                db: AsyncSession = Depends(get_db)):
    ...
    # The card the caller names; otherwise the SAME GPU request the original
    # calibration was submitted with (a manifest from before GPUs could be
    # chosen has none: auto). Checked before the mark, so a card that is not on
    # the node is a 400. The body is new in review round 3: MCP had no way to
    # choose the card for a reproduction, unlike the validation reproduce.
    stored_gpu = (m.payload or {}).get("gpu")
    chosen = (
        body.gpu if body is not None and body.gpu is not None
        else stored_gpu.get("request") if isinstance(stored_gpu, dict) else None
    )
    ...
    task = await _dispatch_or_restore(
        db,
        lambda: gpu_delay(reproduce_circuit_calibration)(manifest_id, prior_status,
                                                    gpu_request=gpu_request),
        Circuit.__table__.update()
        .where(Circuit.id == m.circuit_id, Circuit.calibration_status == "pending")
        .values(calibration_status=prior_status))
    await db.execute(
...
async def start_steering_samples(body: SteeringSamplesBody,
    ...
    # Before the row exists: an unknown card is a 400 with no row and nothing
    # dispatched. Stored on the row; the worker places with it.
    gpu_request = resolve_gpu_request(body.gpu, can_split=True)
    from ....api.v1.endpoints.circuit_discovery import _dispatch_or_restore, _run_sync
    ...
    def _guard_and_mark(sync_db):
        ...
        row = SteeringRecordRun(status="pending", artifact_kind=art["kind"],
                                artifact_ref=ref, gpu_request=gpu_request)
        sync_db.add(row)
        sync_db.commit()
        sync_db.refresh(row)
        return row.id
    ...
    try:
        ...
        raise HTTPException(status_code=409, detail=str(e))
    # The marker row was created for THIS request, so a dispatch that raises
    # fails it rather than restoring a previous state.
    task = await _dispatch_or_restore(
        db,
        lambda: gpu_delay(run_circuit_record, gpu_request)(record_run_id, config),
        SteeringRecordRun.__table__.update()
        .where(SteeringRecordRun.id == record_run_id,
               SteeringRecordRun.status == "pending")
        .values(status="failed",
                error="The recording task could not be queued (task broker "
