...
from ....services.dataset_view_identity import display_name_for
...
async def download_dataset(
    ...
    # FR-2: THE IDENTITY IS (repo, config, split), NOT THE REPO.
    #
    # The old guard refused any second download of a repo, which made a
    # multi-config dataset undownloadable — `Arrrlex/models-under-pressure` ships
    # one `training` config to fit a probe on and six `*_balanced` configs to
    # evaluate it, and only the first could ever be fetched. The duplicate rule is
    # still strict, just about the right thing: the SAME view is still a 409, and
    # the message names the config so a reader can see which one.
    existing = await DatasetService.get_dataset_by_view(
        db, request.repo_id, request.config, request.split
    )
    if existing:
        which = f"Dataset {request.repo_id}"
        if request.config:
            which += f" (config {request.config})"
        if request.split:
            which += f" (split {request.split})"
        raise HTTPException(status_code=409, detail=f"{which} already exists")

    # Create dataset record
    dataset_create = DatasetCreate(
        name=display_name_for(request.repo_id, request.config),
        source="HuggingFace",
...
async def tokenize_dataset(
    ...
    try:
        ...
        task = tokenize_dataset_task.delay(
            ...
            # These three were accepted by the schema and never forwarded, which
            # is the same shape of defect as `remove_all_punctuation` below.
            text_column=request.text_column,
            chat_format=request.chat_format,
            pack_sequences=request.pack_sequences,
            # Same rule: accepted by the schema is not the same as reaching the
            # worker. Without these two the row records no shuffle and the blocks
            # are written in corpus order, which is the whole defect.
            shuffle=request.shuffle,
            shuffle_seed=request.shuffle_seed,
            # PRE-EXISTING, and the template the three above were built to:
            # both are accepted by the schema, sent by the UI, and were never
            # forwarded — so the columns always read back as their defaults.
            # Fixing the new fields and leaving these would be the "fixed one
            # representative" pattern this repo keeps recording.
            remove_all_punctuation=request.remove_all_punctuation,
            custom_filter_chars=request.custom_filter_chars,
        )
