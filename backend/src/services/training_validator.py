...
class TrainingValidator:
    ...
    def check_training_quality(
        ...
        # Check inactive latents (too sparse)
        #
        # WHAT THIS NUMBER IS (review R2C-7). The caller passes the count of latents
        # whose ACTIVITY EMA has fallen below 0.01 — a decaying estimate over recent
        # batches, not the "no firing for `dead_neuron_threshold` consecutive steps"
        # that `DeadLatentTracker` measures and that resampling acts on. The two
        # disagree, and calling this one "dead neurons" sent operators to the
        # resampling settings over a number resampling does not read.
        dead_neuron_fraction = dead_neurons / latent_dim
        if dead_neuron_fraction > 0.7:
            warnings.append(
                f"⚠️  Step {step}: {dead_neuron_fraction*100:.1f}% of latents are inactive by the "
                f"activity-EMA estimate ({dead_neurons}/{latent_dim}). "
