...
def full_length_export_step(training_id: str, total_steps: Optional[int]) -> Optional[int]:
    """The step of the training loop's own export when that is what ``community_format/`` holds, else None.

    The loop writes the export at ``total_steps`` (each layer's ``cfg.json`` records it as
    ``mistudio_checkpoint_step``). A finalize writes it at a checkpoint's step, and a
    checkpoint is always written after a step below ``total_steps``. So a recorded step at
    or past ``total_steps`` is the full-length export: the final weights (review R3-A, R2D-1).

    None when there is no export, no recorded step (an export from before the field
    existed), or no ``total_steps`` to compare with; the callers then apply the guards
    they had before.
    """
    if not total_steps:
        return None
    root = community_format_dir(training_id)
    if not root.is_dir():
        return None
    full = []
    for child in sorted(root.iterdir()):
        cfg = child / "cfg.json"
        if not (child.is_dir() and _LAYER_DIR_RE.match(child.name) and cfg.is_file()):
            continue
        try:
            step = json.loads(cfg.read_text()).get("mistudio_checkpoint_step")
        except (OSError, ValueError, AttributeError):
            continue
        if isinstance(step, int) and not isinstance(step, bool) and step >= int(total_steps):
            full.append(step)
    return max(full) if full else None


def full_length_export_refusal(training_id: str, total_steps: Optional[int]) -> Optional[str]:
    """Why a finalize must not run, when the export on disk holds the training's final weights; else None.

    Decided behaviour (review R3-A, R2D-1): Finalize refuses to overwrite a full-length
    export, with or without ``force``. Rebuilding from a checkpoint would replace the
    final weights with older ones and stamp ``finalized_from_step`` on a run that went
    the distance.
    """
    step = full_length_export_step(training_id, total_steps)
    if step is None:
        return None
    return (
        f"This training's export holds its final weights (written at step {step}, the end of the run). "
        "Finalizing from a checkpoint would replace them with older weights, so it is refused."
    )
...
def finalize_from_checkpoint(
    ...
    # THE ONE PLACE EVERY FINALIZE PASSES (review R3-A, R2D-1): the /finalize route, Stop &
    # Finalize, and a task queued before the loop's export was written.
    refusal = full_length_export_refusal(training_id, (training.hyperparameters or {}).get("total_steps"))
    if refusal:
        raise FinalizeError(refusal)
