...
async def resolve_referenced_saes(request, sae, db) -> dict:
    ...
    # Every referenced SAE, the request's own included, is steered by adding its decoder
    # direction at the layer's OUTPUT. An attention-output direction added there skips the
    # layer's MLP, and an MLP-output one skips a post-MLP norm (gemma): a plausible, wrong
    # steer (review R2-B).
    ...
    for sid in referenced_ids:
        hook_reason = non_residual_hook_reason(sae_records[sid].hook_type, "Steering")
        if hook_reason:
            raise HTTPException(422, hook_reason)
        # And an SAE recording no layer is steered at layer 0 by the loader's own
        # ``layer or 0`` (review R3-B, R3B-11). The per-feature layer check below is no
        # substitute: it SKIPS a NULL layer, so it agreed with whatever was requested.
        layer_reason = unrecorded_layer_reason(sae_records[sid].layer, "Steering", sid)
        if layer_reason:
            raise HTTPException(422, layer_reason)
...
async def _kill_orphan_steering_workers() -> int:
    ...
    """
    from .. import steering_workers

    killed = 0
    for pid in sorted(_SPAWNED_WORKER_PIDS):
        # ONLY WHILE THE NUMBER STILL NAMES THE PROCESS SPAWNED UNDER IT (Phase 3 review
        # round 2): the set is never pruned, and a worker that exited long ago may have
        # handed its PID to an unrelated process. See steering_workers.kill_spawned.
        try:
            if steering_workers.kill_spawned(pid):
                killed += 1
...
def _get_gpu_memory_mb() -> Optional[int]:
    """GPU memory used across EVERY card, in MB.

    This process does not know which card the steering worker placed its model
    on, so it reports the total. It read only the first line of nvidia-smi —
    GPU 0 — so once steering could run on another card, exit-mode reported
    freeing 0 MB after killing a worker that held gigabytes.
    """
    import subprocess
    try:
        ...
        if gpu_output.returncode == 0:
            lines = [line.strip() for line in gpu_output.stdout.splitlines() if line.strip()]
            if lines:
...
def _spawn_steering_worker() -> None:
    """Start a steering Celery worker that can see every GPU.

    The environment is the API process's own, unmodified. It used to add
    ``CUDA_VISIBLE_DEVICES="0"``, which pinned all steering to NVML index 0 — the
    12 GB RTX 3080 Ti once a second card took that index. Each steering task now
    places itself through ``services/gpu_placement.place_job``, so the worker
    must be able to see the card it is asked for.

    Both ``_ensure_steering_worker_running`` and ``enter_steering_mode`` spawn
    through here, so the two can no longer disagree about the environment.
    """
    ...
    try:
        ...
        steering_workers.record_spawned(_spawned.pid)
    finally:
        log_file.close()
...
def _is_steering_worker_running() -> tuple[bool, Optional[int]]:
    """Whether the single steering worker is running, by its PID file. Returns (is_running, pid).

    NEVER BY A COMMAND-LINE PATTERN (Phase 3 review round 1). This fell back to
    `pgrep -f steering@`, and every caller SIGKILLs the PID it returns — exit-mode
    and the ensure before each submit — so the pattern kill MIS-E2E-003 removed
    from the orphan sweep survived here: any process whose command line contains
    "steering@" (someone's `grep steering@`, another tool) was killable over HTTP.
    Celery writes the PID file when the worker starts and removes it when it
    stops; a stale file naming a dead process, or a zombie, is not a worker.
    """
    from ..steering_workers import worker_pid_from_file

    # A live PID is not enough (review round 2): a SIGKILLed worker leaves its PID file,
    # and the number may since name an unrelated process, which every caller SIGKILLs.
    pid = worker_pid_from_file(PID_FILE)
...
async def get_steering_mode_status():
    ...
    Multi-GPU Phase 3: in per-card mode steering is active while ANY card has a
    steering worker, and `workers` lists every card's — its PID (None when it has
    none) and whether it is mid-generation. This only knew the single worker, so
    it reported steering off while card workers ran.
    """
    from ....services.gpu_dispatch import per_card_workers
    ...
    if per_card_workers():
        ...
        workers = await asyncio.to_thread(
            lambda: steering_workers.card_worker_status(steering_workers.list_cards()))
        running = [worker["worker_pid"] for worker in workers if worker["worker_pid"]]
        result.update(
            active=is_active or bool(running),
            worker_pid=pid or (running[0] if running else None),
            workers=workers,
        )
    return result


async def _enter_card_steering_mode(gpu: Optional[str]) -> dict:
    """enter-mode in per-card mode: start the steering worker of the card(s) `gpu` names."""
    from .. import steering_workers

    requested = resolve_gpu_request(gpu, can_split=True) if gpu is not None else None
    cards = await asyncio.to_thread(steering_workers.list_cards)
    leases = await asyncio.to_thread(steering_workers._live_leases)
    targets = steering_workers.mode_cards(requested, cards, leases, default_all=False)
    if not targets:
        return {
            "success": False,
            "message": "No GPU is visible, so no steering worker was started",
            "worker_pid": None,
            "already_active": False,
            "workers": [],
        }
    results = await steering_workers.enter_card_workers(targets)
    pids = [str(r["worker_pid"]) for r in results if r["worker_pid"]]
    failed = [r["card"] for r in results if not r["ok"]]
    already = all(r["already_active"] for r in results)
    if failed:
        message = f"Failed to start the steering worker for {', '.join(failed)} within 10s - check logs"
    elif already:
        message = f"Already in steering mode (worker PID: {', '.join(pids)})"
    else:
        message = f"Entered steering mode (worker PID: {', '.join(pids)})"
    return {
        "success": not failed,
        "message": message,
...
async def enter_steering_mode(gpu: Optional[str] = None):
    """
    ...
    Multi-GPU Phase 3, per-card mode: `gpu` (a query parameter) is "auto" — the
    default, the card a steering request would go to now — "all", or one card by
    UUID or index. A card whose worker already runs is left as it is. This used
    to start the SINGLE worker in per-card mode: it consumed only the legacy
    queue, with no card of its own, while every request went to a card's queue.
    ...
    if per_card_workers():
        return await _enter_card_steering_mode(gpu)
    ...
    # Start new steering worker (Popen in the background, without waiting)
    try:
        _spawn_steering_worker()
...
async def _exit_card_steering_mode(card: str) -> dict:
    """exit-mode for ONE card: its steering worker only — not another card's, not the single worker."""
    from .. import steering_workers

    before = await asyncio.to_thread(_get_gpu_memory_mb)
    killed = await asyncio.to_thread(steering_workers.kill_card_worker, card)
    if not killed:
        return {
            "success": True,
            "message": f"Already out of steering mode on {card}",
            "killed_pid": None,
            "gpu_memory_freed_mb": 0,
            "already_inactive": True,
        }
    await asyncio.sleep(3)
    after = await asyncio.to_thread(_get_gpu_memory_mb)
    freed = before - after if before and after else 0
    still_running = await asyncio.to_thread(steering_workers.card_worker_pid, card)
    return {
        "success": still_running is None,
        "message": (f"Exited steering mode on {card}" + (f" - Freed {freed}MB" if freed > 0 else ""))
        if still_running is None else f"The steering worker for {card} may still be running - try again",
        "killed_pid": None,
        "gpu_memory_before": before,
        "gpu_memory_after": after,
        "gpu_memory_freed_mb": freed,
        "already_inactive": False,
    }


@router.post("/exit-mode")
async def exit_steering_mode(gpu: Optional[str] = None):
    """
    ...
    Multi-GPU Phase 3, per-card mode: `gpu` (a query parameter) names ONE card
    whose worker is stopped, leaving the others; without it — or with "all" or
    "auto" — every card's worker is stopped, as before.
    ...
    # Multi-GPU Phase 3: every card's steering worker too — by the PIDs recorded
    # at spawn and each card's PID file, never by a command-line pattern.
    from ....services.gpu_dispatch import per_card_workers
    from ....services.gpu_placement import is_all, is_auto
    from .. import steering_workers

    if per_card_workers() and gpu is not None:
        requested = resolve_gpu_request(gpu, can_split=True)
        if not (is_all(requested) or is_auto(requested)):
            return await _exit_card_steering_mode(requested)
    ...
    card_workers_killed = (
        await asyncio.to_thread(
            lambda: steering_workers.kill_all_card_workers(steering_workers.list_cards()))
        if per_card_workers() else 0
    )
    if not is_active and not card_workers_killed:
        return {
            "success": True,
...
async def submit_async_steering_comparison(
    ...
    # Multi-GPU Phase 1: "auto" or the UUID of the named card. An unknown card
    # is a 400 HERE — before an idle worker is killed and respawned, and before
    # anything is queued. An index becomes the UUID it names now.
    gpu_request = resolve_gpu_request(request.gpu, can_split=True)

    # Ensure steering worker is running (it exits after each task)
    # Multi-GPU Phase 3: in per-card mode, the steering worker of the card this
    # request goes to (single mode: the one worker, exactly as before).
    from ..steering_workers import ensure_steering_worker, steering_card_for, steering_queue_option

    steering_card = await steering_card_for(gpu_request)
    worker_ok, worker_pid = await ensure_steering_worker(steering_card, _ensure_steering_worker_running)
    if not worker_ok:
        raise HTTPException(
            503,
    ...
    task = steering_compare_task.apply_async(
        # That card's worker's queue in per-card mode; none (the static
        # `steering` route) in single mode.
        **steering_queue_option(steering_card),
        kwargs={
            # The RESOLVED card, not what the client typed.
            "request_dict": {**request.model_dump(mode="json"), "gpu": gpu_request},
...
async def compute_cluster_strength_allocation(
    ...
    if not sae.local_path:
        raise HTTPException(400, "SAE has no local path")
    # The hazard prior dots decoder into encoder in the residual basis (review R2-B).
    from ....services.sae_hook_support import non_residual_hook_reason

    hook_reason = non_residual_hook_reason(sae.hook_type, "The cluster strength allocation")
    if hook_reason:
        raise HTTPException(422, hook_reason)
...
async def _compute_multi_layer_allocation_response(
    ...
    for layer, sid in layer_sae_id.items():
        ...
        if not rec.local_path:
            raise HTTPException(400, f"SAE has no local path: {sid}")
        # Every layer's SAE is read in the residual basis (review R2-B).
        from ....services.sae_hook_support import non_residual_hook_reason

        hook_reason = non_residual_hook_reason(rec.hook_type, "The cluster strength allocation")
        if hook_reason:
            raise HTTPException(422, hook_reason)
