...
def _fit_task_harness(tmp_path, cuda: bool, blow_up: bool = False):
    ...
    # A GPU FIT IS ONE PLACED ON A CARD. The task used to decide from
    # `torch.cuda.is_available()`; the placement decides now, so the harness
    # hands it one rather than letting NVML on this machine refuse the job.
    placement = (
        Placement(
            card=GpuCard(
        ...
        if cuda
        else Placement(card=None, device=torch.device("cpu"))
    )
    ...
    with patch("src.ml.jlens_fitter.JacobianFitter", _Fitter), patch(
        ...
        "src.services.gpu_placement.place_job", return_value=placement
    ), patch(
...
def test_the_task_has_DROPPED_the_model_by_the_time_it_releases(tmp_path):
    ...
    # PLACED ON A CARD, which is what makes this a GPU fit now.
    on_a_card = Placement(
        card=GpuCard(
            1, "GPU-247aa582-0d1b-e161-8156-983ed1fefc57",
            "NVIDIA GeForce RTX 3090", 24_576, 23_000,
