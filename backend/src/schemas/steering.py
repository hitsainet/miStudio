...
#: Where a steering result says it ran: the UUID of the card the model was on.
GPU_UUID_DESCRIPTION = "UUID of the GPU the model ran on; null when it ran on the CPU"
#: A model split across GPUs ran on every card in this list, most free first (the first is `gpu_uuid`); layers fill them in CUDA index order.
GPU_UUIDS_DESCRIPTION = (
    "Every GPU (UUID) the model was split across, most free first (the first is gpu_uuid; "
    "layers fill them in CUDA index order); null when it ran on one GPU"
)
...
class SteeringComparisonRequest(BaseModel):
    ...
    # Multi-GPU Phase 1: the endpoint replaces this with "auto" or a UUID
    # before dispatch, so the worker never sees an index.
...
class SteeringComparisonResponse(BaseModel):
    ...
    gpu_uuid: Optional[str] = Field(None, description=GPU_UUID_DESCRIPTION)
    gpu_uuids: Optional[List[str]] = Field(None, description=GPU_UUIDS_DESCRIPTION)
...
class ClusterAllocationResponse(BaseModel):
    ...
    flags: List[str] = Field(default_factory=list, description=("cancellation (boosted members partly cancel; see cancellation_pair) | low_cohesion (below the cohesion gate) | ""default_budget (no member frequency known) | solo (a single member with no known frequency, at the default strength) | ""cap_bound (capped at the sum of solo strengths) | approximate (decoder unreadable, G taken as 1) | ""nonunit_decoder (a decoder column is not unit length) | uniform_weights (no usable similarity) | ""inactive_member (a member with zero similarity gets zero strength) | grain_limited (the 0.1 rounding remainder could not be placed) | ""over_budget (pinned strengths exceed the budget on a rebalance)"))
