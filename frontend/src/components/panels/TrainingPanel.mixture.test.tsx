 ...
 *
 * ON THE FLY (SAE training remediation item 3, 2026-09-15). A run with no
 * extractions now weights each tokenized dataset, positionally over
 * `dataset_ids`. Mutation controls, each applied alone to TrainingPanel.tsx, this
 * file and utils/trainingMixture.test.ts run, the file restored and its sha256
 * re-checked:
 *   F1  the request's positional ids back to `config.extraction_ids` only
 *         -> KILLED: "on the fly, sends dataset_weights in dataset_ids order"
 *   F2  `weightableSources` loses its dataset branch (`if (false)`)
 *         -> KILLED: "on the fly, renders one weight input per selected dataset"
 */
...
describe('the mixture controls reach the request', () => {
  ...
  it('on the fly, sends dataset_weights in dataset_ids order', async () => {
    // No extractions: the worker extracts on the fly and weights each tokenized
    // dataset, positionally over dataset_ids ([web, chat]).
    setup({
      ...baseConfig,
      extraction_ids: undefined,
      dataset_weights_by_extraction: { [DS_CHAT]: 0.25, [DS_WEB]: 0.75 },
    });
    render(<TrainingPanel />);
  ...
  it('on the fly, renders one weight input per selected dataset', async () => {
    setup({ ...baseConfig, extraction_ids: undefined });
    render(<TrainingPanel />);
    ...
    const web = await screen.findByLabelText('OpenWebText-2M');
    const chat = await screen.findByLabelText('OpenHermes-2.5');
    expect(web.id).toBe(`mixture-weight-${DS_WEB}`);
    expect(chat.id).toBe(`mixture-weight-${DS_CHAT}`);
  });
