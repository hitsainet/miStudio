            ...
            "templates": [
                {
                    ...
                    # COMPLETE (review R2F-7). This dict used to omit `latent_dim` and
                    # `total_steps` and was stored exactly as written, producing a
                    # template that cannot build an SAE — the defect R2F-7 names.
                    # Imports are now validated through the same schema `create` uses,
                    # so the fixture carries the fields a real export always carries.
                    "hyperparameters": {
                        "hidden_dim": 512,
                        "latent_dim": 16384,
                        "learning_rate": 0.001,
