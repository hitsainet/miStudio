...
def realised_mixture_record(
    labels: Sequence[str],
    alloc: Sequence[int],
    requested: Optional[Sequence[float]] = None,
) -> List[Dict[str, Any]]:
    """What a training ACTUALLY drew from each source, for the training row.

    A training stored only the REQUESTED `dataset_weights` — a bare positional
    array with nothing naming its positions — and reported the realised split
    only in a log line. So after the fact nobody could tell what a run trained
    on, and a request that could not be honoured (every source fits, so the
    mixture silently follows availability) left no trace outside stdout.
    Extraction already records its realised split per run; this is the
    training counterpart.

    Each entry names its source, so the record cannot be mis-paired the way a
    positional weight array can.
    """
    total = sum(int(a) for a in alloc) or 1
    wanted = normalise_weights(requested, len(alloc)) if requested is not None else None
    return [
        {
            "source": str(label),
            "tokens": int(a),
            "realised_fraction": int(a) / total,
            "requested_weight": (wanted[i] if wanted is not None else None),
        }
        for i, (label, a) in enumerate(zip(labels, alloc, strict=True))
    ]
