...
class TestTheLensIsValidatedOnTheChosenCard:
    """The acquisition loads the model on the card the caller chose.

    It used to take `"cuda"`, the CURRENT device — index 0, which on this node
    is the 12 GB card whatever else is free (0xcc/plans/Multi-GPU-Plan.md,
    Phase 1). The card is chosen AFTER the download, so Auto judges the memory
    free when the load happens rather than minutes before it.

    MUTATION CONTROL: load on `capture_device=None` -> "recorded before the load"
    fails.
    """

    @staticmethod
    def _spies(events, refuse=False):
        import torch

        from src.services.gpu_placement import GpuCard, GpuPlacementError, Placement

        card = GpuCard(1, RTX_UUID, "NVIDIA GeForce RTX 3090", 24_576, 23_000)
        loaded = types.SimpleNamespace(
            name="org/model", d_model=D_MODEL, n_layers=N_LAYERS, n_vocab=64, model=object()
        )

        def place_job(requested, required_mb=None, allow_shard=False):
            # `allow_shard` is part of the event: the acquisition only runs
            # forward passes through split-safe code, so it may split.
            events.append(("place", requested, allow_shard))
            if refuse:
                raise GpuPlacementError(
                    "GPU 1 (NVIDIA GeForce RTX 3090, 900 of 24,576 MB free) cannot "
                    "take this job: it needs ~15,000 MB. Choose another GPU or Auto.",
                    requested=requested,
                )
            return Placement(card=card, device=torch.device("cuda", 1))

        def record_gpu(task_id, gpu_uuid, gpu_uuids=None, attempts=10):
            events.append(("record", task_id, gpu_uuid))
            return True

        def load_for_readout(record, capture_device="cpu", placement=None):
            # The PLACEMENT reaches the load, not its first device.
            events.append(("load", capture_device if placement is None else placement))
            return loaded

        return (
            patch("src.services.gpu_placement.place_job", place_job),
            patch("src.workers.jlens_progress.record_gpu", record_gpu),
            patch("src.services.jlens_model_registry.load_for_readout", load_for_readout),
            patch(
                "src.services.jlens_model_registry.clear_cache",
    ...
    def test_the_card_is_recorded_before_the_load_and_the_load_uses_its_device(
        ...
        events = []
        place, record, load, release = self._spies(events)
        with _harness(tmp_path) as h, place, record, load, release:
            out = _run(h, gpu_request=RTX_UUID)

        from src.services.gpu_placement import GpuCard, Placement

        assert out["published"] is True, out.get("unpublished_reason")
        on_the_3090 = Placement(
            card=GpuCard(1, RTX_UUID, "NVIDIA GeForce RTX 3090", 24_576, 23_000),
            device=torch.device("cuda", 1),
        )
        assert events == [
            ("place", RTX_UUID, True),
            ("record", "acq-1", RTX_UUID),
            ("load", on_the_3090),
            ("release",),
        ]

    def test_a_card_that_cannot_take_it_FAILS_the_task_with_the_reason(self, tmp_path):
        from src.services.gpu_placement import GpuPlacementError

        events = []
        place, record, load, release = self._spies(events, refuse=True)
        with _harness(tmp_path) as h, place, record, load, release, patch(
            "src.workers.jlens_progress.update_row"
        ) as rows:
            with pytest.raises(GpuPlacementError, match="Choose another GPU or Auto"):
                _run(h, gpu_request=RTX_UUID)

        assert events == [("place", RTX_UUID, True)], "something loaded after the refusal"
        failed = [c for c in rows.call_args_list if c.kwargs.get("status") == "failed"]
        assert len(failed) == 1
        assert "Choose another GPU or Auto" in failed[0].kwargs["error_message"]
