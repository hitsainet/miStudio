...
#: Documents the probe and the capture run through the model, and encode, at once.
#: A split keeps room beside each captured layer for one batch's codes
#: (``circuit_devices.model_load_kwargs(encode_tokens=...)``).
CAPTURE_BATCH_DOCS = 8
#: The most tokens one capture encode covers.
CAPTURE_ENCODE_TOKENS = CAPTURE_BATCH_DOCS * MAX_SEQ_LENGTH
DEFAULT_EPSILON = 0.1
...
class CircuitCaptureService:
    ...
    @staticmethod
    def assert_no_active_gpu_run(db, gpu_request=None) -> None:
        ...
        Multi-GPU Phase 3: with a worker per card, one circuit task PER GPU,
        under a per-card lock key — see services/circuit_gpu_lock.py."""
        from .gpu_dispatch import per_card_workers

        if per_card_workers():
            from .circuit_gpu_lock import assert_card_free_for_circuit

            assert_card_free_for_circuit(db, gpu_request)
            return
    ...
    @staticmethod
    def create_run(db, config: Dict[str, Any],
                   gpu_request: Optional[str] = None) -> CircuitCaptureRun:
        """Validate config against the DB and create the run row (pending).

        ``gpu_request`` is the resolved request ("auto" or a GPU UUID) the
        worker places the capture with; stored on the row so a confirm or a
        retry of a failed run reuses it.
        """
        ...
        for entry in layers:
            ...
            # Capture ENCODES each layer's output (resid_post) through its SAE, so an
            # MLP or attention SAE fills the store with codes of the wrong activations,
            # and attribution, validation and faithfulness inherit them (review R2-B).
            from .sae_hook_support import refuse_non_residual

            refuse_non_residual(sae.hook_type, "Circuit capture", error=CaptureConfigError)
        ...
        # THE FOURTH SELECTION SITE — same unordered `.first()` on a pair the
        # uniqueness constraint does not make unique.
        from ..workers.model_tasks import select_tokenization_for_model

        _candidates = db.query(DatasetTokenization).filter(
            DatasetTokenization.dataset_id == dataset_id,
            DatasetTokenization.model_id == model_id,
        ).all()
        tokenization = (
            select_tokenization_for_model(_candidates, model_id, dataset_id)
    ...
    @staticmethod
    def run_capture(db, run_id: str, *, confirmed: bool, device, placement=None,
        ...
        ``device`` is the task's placed device (a split's first card) and
        ``placement`` the whole placement, which may split the model across
        cards. Each SAE goes on its layer's card; inputs on the embedding's.
        """
        ...
        from .circuit_devices import layer_device, model_load_kwargs, release_model
        ...
        manifest = dict(run.manifest)
        # A confirm or a retry runs an existing row without create_run: refuse an MLP or
        # attention SAE here too, before anything loads (review R2-B).
        from .sae_hook_support import refuse_non_residual

        for entry in manifest.get("layers") or []:
            sae_row = db.query(ExternalSAE).filter(ExternalSAE.id == entry["sae_id"]).first()
            if sae_row is not None:
                refuse_non_residual(sae_row.hook_type, "Circuit capture", error=CaptureConfigError)
        ...
        try:
            ...
            model, tokenizer, _config, _meta = load_model_from_hf(
                ...
                # A split is mapped to hold each layer's SAE beside that layer.
                **model_load_kwargs(
                    device, placement, db=db,
                    sae_ids_by_layer={e["layer"]: e["sae_id"] for e in manifest["layers"]},
                    # And each batch's codes beside the layer it encodes.
            ...
            # into the capture. transformers can decline it, and the failure mode
            # is silent: the hooks simply capture nothing.
            if wants_attention:
            ...
            # Discovered BEFORE the SAEs are loaded, to put each on its layer's
            # card. The capture reads activations back to host RAM, so an SAE
            # on another card is still correct (the encode follows the SAE);
            # the placement spreads SAE memory over a split the way the layers
            # are, instead of stacking it on the first card's 1 GB reserve.
            ...
            except Exception as exc:  # noqa: BLE001 - provenance and placement only
                logger.warning("Could not discover the layer structure: %s", exc)
                _struct = None
            ...
            for entry in manifest["layers"]:
                ...
                sae = _load_sae_sync(
                    sae_record,
                    device if _struct is None
                    else layer_device(_struct, entry["layer"], device))
            ...
            for batch_start in range(0, n_docs, batch_size):
                ...
                # The embedding's card, not `model.device` (the first parameter's).
                _capture_batch(
                    model, saes, layer_indices, writers,
                    input_ids.to(input_device(model)),
                    attention_mask.to(input_device(model)),
                    batch_start, lengths,
                    epsilon_by_layer={e["layer"]: e["epsilon"]
                                      for e in manifest["layers"]},
                    floor_by_layer={e["layer"]: e["theta_floor"]
            ...
            release_model(model)
            # No `device=`: every card this process holds memory on, which
            # covers each card of a split.
            cleanup_gpu_memory(
...
def _holds_tensors(module) -> bool:
    """True when an SAE has a parameter or buffer to say which device it is on."""
    from ..ml.model_devices import module_device

    return hasattr(module, "parameters") and module_device(module) is not None
...
def _probe(model, tokenizer, dataset, saes, layer_indices, device,
    ...
    from ..ml.forward_hooks import HookManager, HookType
    from ..ml.model_devices import input_device
    from .circuit_devices import to_module
    ...
    def _sweep(count_with_threshold: bool) -> int:
        ...
        with HookManager(model) as hm:
            ...
            for start in range(0, n, CAPTURE_BATCH_DOCS):
                batch = dataset[start:min(start + CAPTURE_BATCH_DOCS, n)]
                input_ids, mask, lengths = _pad_batch(batch, tokenizer)
                # The embedding's card, not `model.device` (the first parameter's).
                on = input_device(model)
                with torch.no_grad():
                    _ = model(input_ids=input_ids.to(on),
                              attention_mask=mask.to(on))
                for L in layer_indices:
                    acts = hm.activations[f"layer_{L}_residual"][-1]  # [b, s, h]
                    # To THIS layer's SAE, not the job's first card: on a split
                    # the SAEs sit on different cards. `device` is only the
                    # fallback for an SAE holding no tensors.
                    flat = (to_module(acts, saes[L], dtype=torch.float32)
                            if _holds_tensors(saes[L])
                            else acts.to(device=device, dtype=torch.float32)
...
def _capture_batch(model, saes, layer_indices, writers, input_ids, mask,
    ...
    with HookManager(model) as hm:
        ...
        for L in layer_indices:
            ...
            b, s, h = acts.shape
            # To THIS layer's SAE, not the inputs' card: on a split the SAEs sit
            # on their layers' cards. The inputs' card is only the fallback for
            # an SAE holding no tensors.
            flat = (to_module(acts, saes[L], dtype=torch.float32)
                    if _holds_tensors(saes[L])
                    else acts.to(device=input_ids.device, dtype=torch.float32)
                    ).reshape(-1, h)
            with torch.no_grad():
