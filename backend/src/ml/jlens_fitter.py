...
def narrowed_chunk(chunk: int, seq_len: int, d_model: int, n_inputs: int) -> int:
    """Output dimensions in one batched backward, narrowed so its gradients stay under MAX_BACKWARD_BYTES."""
    per_dim = int(seq_len) * int(d_model) * 4 * max(int(n_inputs), 1)
    return max(1, min(int(chunk), MAX_BACKWARD_BYTES // max(per_dim, 1)))


def fit_working_bytes(
    *,
    d_model: int,
    intermediate: int,
    layers_to_target: int,
    n_captured: int,
    max_seq_len: int,
    vocab: int,
    heads: int,
    ...
    """An upper bound on what fitting ONE prompt allocates beyond the model's weights.

    The placement of a fit asks for this on top of the weights (review round 3).
    It used to ask for the loader preflight's fixed 2 GiB, which is a forward
    pass's headroom; a fit keeps the whole forward graph for a batched backward,
    and at the default 550-character prompts (about 128 tokens) the bound for
    gemma-4-12b-it is 3.1 GiB, for OLMo-2-13B 2.9 GiB.

    The terms, ``S`` the longest prompt's tokens and ``p`` the activation bytes:

    * the forward graph kept for the backward, every layer up to the target:
      ``layers_to_target x S x p x (12 d + 5 i)``;
    * the head's logits and one transformed copy at the end of the forward: ``2 S V p``;
    * the batched gradients kept for every captured layer: ``chunk x S x d x p x n_captured``;
    * the fp32 cotangent: ``chunk x S x d x 4``;
    * one layer's batched backward: ``chunk x S x p x (4 i + 4 d) + chunk x heads x S^2 x p``;
    * the per-layer fp32 copy of a gradient with its mean and spread: ``chunk x S x d x 4 x 3``;

    with ``chunk`` narrowed exactly as :meth:`JacobianFitter._fit_one` narrows it.
    VALIDATED against the real ``_fit_one`` under torch's MemTracker at bf16 over
    thirteen shapes (sequence 64-1,024, width 128-320, 4-8 layers, every
    captured-layer count): 1.09-2.25x the measured peak, never below it
    (``tests/unit/test_jlens_fit_is_placed_for_its_backward.py``).
    """
    seq = max(int(max_seq_len), 1)
    p = int(activation_bytes)
    width = narrowed_chunk(chunk, seq, d_model, n_captured)
    graph = int(layers_to_target) * seq * p * (12 * d_model + 5 * intermediate)
    logits = 2 * seq * int(vocab) * p
    captured = width * seq * d_model * p * max(int(n_captured), 1)
    cotangent = width * seq * d_model * 4
    backward = width * seq * p * (4 * intermediate + 4 * d_model) + width * int(heads) * seq * seq * p
    temporaries = width * seq * d_model * 4 * 3
    return int(graph + logits + captured + cotangent + backward + temporaries)
...
class JacobianFitter:
    ...
    def device(self) -> torch.device:
        """Where the fitter's INPUTS go: the input embedding's device, taken from the model.
        ...
        The EMBEDDING's device, not the first parameter's: on a model split
        across cards they differ whenever the checkpoint registers another
        module first. Fits themselves still run on one card (see
        `jlens_fit_tasks.fit_jlens_artifact`); this keeps the input right if
        that changes. A model with no embedding falls back to its first
        parameter, then to the CPU.
        """
        from .model_devices import input_device
    ...
    def _fit_one(self, prompt: str, layers: Sequence[int]) -> Dict[int, torch.Tensor]:
        ...
        * EVERY LAYER FROM ONE SWEEP. Gradients w.r.t. all captured layers come
        ...
        chunk = narrowed_chunk(self.chunk, int(target.shape[1]), d_model, len(inputs))
        if chunk < self.chunk:
            logger.debug(
