...
class LabelingJob(Base):
    ...
    # Local-model labeling only: what the job asked for ("auto" or a GPU UUID)
    # and the card it ran on.
    ...
    #
    # FALSE IS LOAD-BEARING, NOT A PLACEHOLDER. A full-extraction job today
    # relabels everything, and an operator pressing Label expects exactly that.
    # Defaulting this to true would silently repurpose an existing control —
    # they would quietly stop getting the relabel they asked for, with nothing
    # on screen to say so. The resume path does not need it: resume works by
    # naming explicit ids, so this exists purely to make "just fill the gaps"
    # available on the whole-extraction button, as a visible choice.
    skip_adjudicated = Column(
        Boolean,
        nullable=False,
        default=False,
        server_default='false',
        comment="Skip features already adjudicated (succeeded/skipped) by this judge",
    )
