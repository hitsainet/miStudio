...
def task_looks_alive(celery_task_id: Optional[str], row: Any, *, started: bool) -> bool:
    ...
    # WAITING FOR A GPU IS NOT DEAD (multi-GPU Phase 3). A job handed off at
    # placement re-runs its pre-placement writes each time it is republished, so
    # its row can look started while its clock is frozen. See
    # services/gpu_worker_queues.is_waiting_for_gpu; inert in single mode.
    from src.workers.gpu_job import waiting_for_gpu

    if waiting_for_gpu(celery_task_id):
        return True
