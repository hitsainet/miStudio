...
class ExtractionJob(Base):
    ...
    # passive_deletes: features.extraction_job_id and labeling_jobs.extraction_job_id
    # are ON DELETE CASCADE, so the database removes the children instead of the
    # ORM loading every feature (and, through Feature, every activation) first.
    features = relationship(
        "Feature", back_populates="extraction_job", cascade="all, delete-orphan", passive_deletes=True
    )
    labeling_jobs = relationship(
        "LabelingJob", back_populates="extraction_job", cascade="all, delete-orphan", passive_deletes=True
    )
