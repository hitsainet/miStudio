...
import { estimateCompletion, etaTooltip, formatEtaClock } from '../../utils/trainingEta';
...
import { fvuDisplay, fvuSeries } from '../../utils/fvu';
import { inactiveLatentsExplanation } from '../../utils/inactiveLatents';
import { aggregateMetricRows, METRICS_WINDOW_STEPS } from '../../utils/trainingMetricRows';
...
export const TrainingCard: React.FC<TrainingCardProps> = ({
  ...
  const [metricsHistory, setMetricsHistory] = useState<{
    ...
    l1_loss: number[];
    l0_sparsity: number[];
    fvu: number[];           // LEGACY global-mean FVU
    fvu_centred: number[];   // The headline FVU (per-dimension centred)
  ...
  // The centred FVU when reported; otherwise the legacy value, labelled as such.
  const fvuShown = fvuDisplay(training.current_fvu_centred, training.current_fvu);
  ...
  // WHEN WILL IT FINISH, in the viewer's own timezone.
  //
  // Derived during render rather than held in state: the elapsed-time effect
  // above already re-renders this card every second while the run is RUNNING,
  // so the clock stays fresh with no second timer. `estimateCompletion` returns
  // null for anything it cannot honestly estimate — a finished run, step 0, a
  // stale heartbeat — and the badge simply does not render.
  const eta = estimateCompletion({
    status: training.status,
    currentStep: training.current_step,
    totalSteps: training.total_steps,
    startedAt: training.started_at,
    updatedAt: training.updated_at,
    recentSteps: metricsHistory.steps,
    recentTimestamps: metricsHistory.timestamps,
  });
  ...
  useEffect(() => {
    const loadHistoricalMetrics = async () => {
      ...
      try {
        // The last 20 AGGREGATE rows (review R1-C). A logged step writes an aggregate
        // row (layer_idx null), one row per SAE and held-out rows (negative
        // layer_idx); old runs also carry spliced-CE rows whose `loss` is a
        // cross-entropy. Deduplicating 20 raw rows by step kept whichever came
        // first, so the chart could show one layer's or the held-out FVU as the run's.
        // The aggregate rows ONLY, asked of the endpoint (review R1-A, A5): a raw window
        // is shared by every SAE and held-out row of every hook type, so a multi-hook,
        // many-layer run left the card a handful of steps.
        const rows = await fetchTrainingMetrics(training.id, {
          limit: METRICS_WINDOW_STEPS,
          aggregate_only: true,
        });
        const metrics = aggregateMetricRows(rows).slice(-METRICS_WINDOW_STEPS);
        ...
        if (metrics.length > 0) {
          ...
          setMetricsHistory({
            ...
            // NaN, not 0 — same reasoning as FVU below. `l0_sparsity` is a
            // fraction of d_sae, so a missing point plotted as 0 reads as a
            // collapsed dictionary and drags the chart's minimum to zero,
            // flattening the whole trend.
            l0_sparsity: dedupedMetrics.map((m) => m.l0_sparsity ?? NaN),
            ...
            fvu_centred: dedupedMetrics.map((m) => m.fvu_centred ?? NaN),
  ...
  useEffect(() => {
    ...
    if (currentLossValue !== undefined && currentLossValue !== null && currentStep !== undefined) {
      setMetricsHistory((prev) => {
        ...
        // NaN, not 0: a missing L0 means "not reported at this step", and a zero
        // draws a collapsed dictionary that never happened.
        const newL0 = [...prev.l0_sparsity, training.current_l0_sparsity ?? NaN];
        ...
        const newFvuCentred = [...prev.fvu_centred, training.current_fvu_centred ?? NaN];
        ...
        return {
          // Keep the same window the historical fetch uses, or a live card
          // silently truncates to a keyhole while a reloaded one shows the run.
          loss: newLoss.slice(-METRICS_WINDOW_STEPS),
          reconstruction_loss: newReconLoss.slice(-METRICS_WINDOW_STEPS),
          l1_loss: newL1Loss.slice(-METRICS_WINDOW_STEPS),
          l0_sparsity: newL0.slice(-METRICS_WINDOW_STEPS),
          fvu: newFvu.slice(-METRICS_WINDOW_STEPS),
          fvu_centred: newFvuCentred.slice(-METRICS_WINDOW_STEPS),
          dead_neurons: newDeadNeurons.slice(-METRICS_WINDOW_STEPS),
          timestamps: newTimestamps.slice(-METRICS_WINDOW_STEPS),
          steps: newSteps.slice(-METRICS_WINDOW_STEPS),
        };
      });
    }
  }, [training.current_loss, training.current_reconstruction_loss, training.current_l1_loss, training.current_l0_sparsity, training.current_fvu, training.current_fvu_centred, training.current_dead_neurons, training.current_step]);
  ...
  const handleCopyLogs = async () => {
    ...
    const logLines = metricsHistory.steps.map((step, idx) => {
      ...
      // L0 AS A COUNT, not the raw fraction.
      //
      // `l0_sparsity` is a fraction of d_sae: 0.0069 of 16,384 latents is 113
      // active features. Logged as "0.0069" a healthy SAE reads as collapsed —
      // the train_9355afa6 failure (FVU 0.092, 0 dead, ~7 of 30,720 active
      // shown as "L0: 0.000"), already fixed on two other surfaces and missed
      // here. Targets are stated in counts, so the log reports counts.
      const l0Text = !Number.isFinite(sparsity)
        ? '—'
        : typeof latentDim === 'number'
          ? `${formatL0Absolute(sparsity, latentDim)} (${formatL0Percent(sparsity)})`
          : formatL0Percent(sparsity);
      return `[${time}] step=${step}, loss=${loss.toFixed(6)}${lossDetail}, L0=${l0Text}, inactive=${Math.floor(historicalDeadNeurons)}/${latentDim}, lr=${learningRate.toExponential(2)}`;
    });
    ...
    <div className={`${COMPONENTS.card.base} p-3 space-y-2`}>
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            {eta && (
              <span
                ...
                title={etaTooltip(eta)}
                data-testid="training-eta"
              >
                ETA {formatEtaClock(eta.at)}
              </span>
            )}
          ...
          <div className="flex items-center gap-4 text-xs px-6">
            ...
            <span className="text-slate-600 dark:text-slate-400">L0 <span className={`font-semibold ${
              ...
              : '—'}</span></span>
            {/* NOT the resampler's "dead": a fast activity estimate (utils/inactiveLatents.ts). */}
            <span
              className="text-slate-600 dark:text-slate-400"
              title={inactiveLatentsExplanation(training.hyperparameters?.batch_size, training.hyperparameters?.dead_neuron_threshold)}
            >Inactive <span className="text-red-400 font-semibold">{hasMetrics ? Math.floor(deadNeurons) : '—'}</span></span>
          ...
          {/* What the SAEs cost the model, on text the training never read
              (remediation item 6). Completed runs only: the evaluation reads the
              exported SAEs. */}
          {training.status === TrainingStatus.COMPLETED && (
            <div className="px-6">
              <TrainingEvaluationPanel
                ...
                onEvaluate={
                  evaluateTraining
                    ? (id, opts) => (opts ? evaluateTraining(id, opts) : evaluateTraining(id))
                    : undefined
                }
                onRefresh={fetchTraining}
              />
            </div>
          )}
          {training.status === TrainingStatus.RUNNING && (
                  ...
                  <div className="h-52 overflow-y-auto space-y-1">
                    {metricsHistory.loss.length > 0 ? (
                      metricsHistory.steps.slice(-15).map((step, i) => {
                        // Offset from the slice's own start (review R1-C). `length - 15 + i`
                        // went negative below 15 points, so a card with fewer than 15 rendered
                        // no log lines at all.
                        const idx = Math.max(0, metricsHistory.steps.length - 15) + i;

                        const loss = metricsHistory.loss[idx];
                        const reconLoss = metricsHistory.reconstruction_loss[idx];
                        const l1Loss = metricsHistory.l1_loss[idx];
                        const sparsity = metricsHistory.l0_sparsity[idx];
                        const histFvuShown = fvuDisplay(metricsHistory.fvu_centred[idx], metricsHistory.fvu[idx]);
                        const historicalDeadNeurons = metricsHistory.dead_neurons[idx] ?? deadNeurons;
                        const time = new Date(metricsHistory.timestamps[idx]).toLocaleTimeString();
                        ...
                        return (
                          <div key={step} className="text-slate-700 dark:text-slate-300">
                            ...
                            L0={!Number.isFinite(sparsity)
                              ? '—'
                              : training.hyperparameters?.latent_dim
                                ? `${formatL0Absolute(sparsity, training.hyperparameters.latent_dim)} (${formatL0Percent(sparsity)})`
                                : formatL0Percent(sparsity)},
                            {histFvuShown.value !== null && (
                              <> {histFvuShown.isLegacyOnly ? 'FVU(legacy)' : 'FVU'}={histFvuShown.value.toFixed(4)} ({((1 - histFvuShown.value) * 100).toFixed(1)}% var),</>
                            )}
                            inactive={Math.floor(historicalDeadNeurons)}/{training.hyperparameters?.latent_dim || 'N/A'},
                ...
                <div className="w-1/2 flex flex-col gap-2">
                  ...
                  <div className="bg-slate-100 dark:bg-slate-800/30 rounded-lg p-2">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-blue-400 font-mono" data-testid="l0-latest">
                        {(() => {
                          const finite = metricsHistory.l0_sparsity.filter(Number.isFinite);
                          if (finite.length === 0) return '—';
                          const latest = finite[finite.length - 1];
                          return training.hyperparameters?.latent_dim
                            ? formatL0Absolute(latest, training.hyperparameters.latent_dim)
                            : formatL0Percent(latest);
                        })()}
                      </span>
                    </div>
                    {/* THE TREND, in the unit the run is steered by. A sparkline
                        alone cannot distinguish 825 -> 49 from 115 -> 113, and
                        the second is all a 20-point window ever showed. */}
                    {(() => {
                      const finite = metricsHistory.l0_sparsity.filter(Number.isFinite);
                      const dim = training.hyperparameters?.latent_dim;
                      if (finite.length < 2 || !dim) return null;
                      return (
                        <div
                          className="text-[10px] text-slate-500 dark:text-slate-400 font-mono mb-1"
                          data-testid="l0-trend"
                          title={`Active latents per token, first to latest of ${finite.length} logged points, out of ${dim.toLocaleString()}.`}
                        >
                          {formatL0Absolute(finite[0], dim)} → {formatL0Absolute(finite[finite.length - 1], dim)}
                          <span className="ml-1 text-slate-400 dark:text-slate-500">of {dim.toLocaleString()}</span>
                        </div>
                      );
                    })()}
                    <div className="h-16">
                      {metricsHistory.l0_sparsity.filter(Number.isFinite).length > 1 ? (
                        <svg viewBox="0 0 100 40" className="w-full h-full" preserveAspectRatio="none">
                          {/* Subtle grid lines */}
                          <line x1="0" y1="10" x2="100" y2="10" stroke="#334155" strokeWidth="0.5" />
                          <line x1="0" y1="20" x2="100" y2="20" stroke="#334155" strokeWidth="0.5" />
                  ...
                  <div className="bg-slate-100 dark:bg-slate-800/30 rounded-lg p-2">
                    <div className="flex items-center justify-between mb-1">
                      <span
                        ...
                        title={fvuShown.title}
                        data-testid="fvu-label"
                      >
                        {fvuShown.label}
                      </span>
                      <span className="text-xs text-amber-400 font-mono" data-testid="fvu-value">
                        {fvuShown.value !== null
                          ? `${fvuShown.value.toFixed(4)} · ${((1 - fvuShown.value) * 100).toFixed(1)}% var`
                          : 'n/a'}
                        {!fvuShown.isLegacyOnly && fvuShown.legacy !== null && (
                          <span className="text-slate-500" title="Legacy global-mean FVU, for comparison with older runs">
                            {' '}(legacy {fvuShown.legacy.toFixed(4)})
                          </span>
                        )}
                      </span>
                    </div>
                    <div className="h-16">
                      {fvuSeries(metricsHistory.fvu_centred, metricsHistory.fvu).series.filter(Number.isFinite).length > 1 ? (
                        <svg viewBox="0 0 100 40" className="w-full h-full" preserveAspectRatio="none">
                          {/* Subtle grid lines */}
                          <line x1="0" y1="10" x2="100" y2="10" stroke="#334155" strokeWidth="0.5" />
                          <line x1="0" y1="20" x2="100" y2="20" stroke="#334155" strokeWidth="0.5" />
                          ...
                          {(() => {
                            ...
                            const data = fvuSeries(metricsHistory.fvu_centred, metricsHistory.fvu).series.filter(Number.isFinite);
                            const maxVal = Math.max(...data);
                            const minVal = Math.min(...data);
                            const range = maxVal - minVal || 1;
                            const points = data.map((val, i) => {
                        ...
                        <div className="h-full flex items-center justify-center text-slate-500 text-xs">
                          {fvuSeries(metricsHistory.fvu_centred, metricsHistory.fvu).series.some(Number.isFinite) ? 'Waiting...' : 'Not reported'}
                        </div>
                      )}
                    </div>
                  </div>
      ...
      {training.status !== TrainingStatus.COMPLETED && (
        <div className="border-t border-slate-300 dark:border-slate-700 pt-3 flex gap-2">
          ...
          {/* Crashed with checkpoints: offer Resume. A worker that dies leaves the
              row FAILED with its checkpoints intact, and the resume machinery reads
              the newest COMPLETE one and continues at the step after it — the same
              path a pause takes. Without this the only options were Finalize (keep
              the SAE as it was at that checkpoint, train no further) and Retry
              (start again from step 0), so a crash at step 120,000 of 150,000 threw
              away every step. CANCELLED is deliberately excluded: that run was
              stopped on purpose. */}
          {training.status === TrainingStatus.FAILED && checkpoints.length > 0 && (
            <button
              type="button"
              onClick={handleResume}
              disabled={isControlling}
              aria-label="Resume training from checkpoint"
              title="Continue this crashed run from its newest complete checkpoint"
              className={`flex-1 flex items-center justify-center gap-2 rounded-lg ${COMPONENTS.button.primary}`}
            >
              {isControlling ? (
                <Loader className="w-4 h-4 animate-spin" />
