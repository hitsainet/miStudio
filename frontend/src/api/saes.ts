...
export interface SAEExtractionConfig {
  ...
  /**
   * Corpora to extract over. The `dataset_id` QUERY parameter remains the
   * legacy single-corpus form and stays in sync (server stores both), so a
   * caller that sends neither of these behaves exactly as before.
   */
  dataset_ids?: string[];
  /**
   * Relative share of the EVALUATION SAMPLES per dataset, positional over
   * `dataset_ids`. Rows, not tokens — a top-k example slot is a row. Build it
   * only via `utils/extractionMixture`, never by hand.
   */
