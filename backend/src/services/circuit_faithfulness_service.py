...
def expand_circuit_members(db, circuit) -> Dict[int, List[int]]:
    """A circuit's members as {layer: [feature_idx]}, cluster_ref members read from their profiles.

    ONE DEFINITION, for the pass and for its size. A faithfulness pass loads an
    SAE for exactly these layers; the capture it reads prompts from may record
    more. `workers.circuit_gpu.circuit_faithfulness_manifest` sizes the job from
    this, so the size counts the SAEs the pass loads and no others. A cluster
    whose profile has no features adds no layer, here and in the pass alike.
    """
    ...
    return expand_members(list(circuit.members or []), _resolve_cluster)
...
class CircuitFaithfulnessService:
    ...
    def run(db, circuit_id: str, config: Dict[str, Any], *,
            ...
            gpu: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """GPU orchestrator (mirrors CircuitInterventionService.run):

        ``device`` is the task's placed device (a split's first card);
        ``placement`` the whole placement, which may split the model across
        cards; ``gpu`` is the card record (request, uuid, name, and a split's
        uuids) written into the manifest and the result.
        ...
        from .circuit_capture_service import MAX_SEQ_LENGTH, _load_sae_sync, _pad_batch
        from .circuit_capture_store import EventReader
        from .circuit_devices import (layer_device, model_load_kwargs,
                                      release_model)
        ...
        # cluster_ref members resolve via ClusterProfile.members (feature_idx).
        # The job was sized from the same expansion (circuit_faithfulness_manifest).
        by_layer = expand_circuit_members(db, circuit)
        if not by_layer:
        ...
        try:
            ...
            model, tokenizer, _c, _m = load_model_from_hf(
                ...
                # A split is mapped to hold, beside each layer, the SAE this
                # pass loads there: its circuit's layers only.
                **model_load_kwargs(
                    device, placement, db=db,
                    sae_ids_by_layer={L: sae_id_by_layer[L] for L in circuit_layers},
                    # And one prompt's codes beside the layer it encodes.
                    encode_tokens=MAX_SEQ_LENGTH))
            model.eval()
            ...
            for L in circuit_layers:
                ...
                # On its LAYER's card: its sum-suppression hook and the
                # downstream encode run where that layer's hidden state is (D5).
                saes[L] = _load_sae_sync(sae_rec, layer_device(structure, L, device))
    ...
    def _behavior(*, suppress: Dict[int, List[int]], model, structure,
        ...
        # Read the downstream activation at the SAME point the SAE was trained
        # on (R2 B-2). Both resolve through get_hookable_module("residual"),
        # which is the decoder layer output (resid_post) since 2026-09-12 — it
        # was a norm submodule before, for training and for this read alike.
    ...
    def _downstream_sum(model, down_module, sae_d, feature_idxs, input_ids, mask):
        ...
        # On the downstream SAE's card, which is this layer's card on a split.
        z = encode_with_training_normalization(
            sae_d, to_module(captured["h"], sae_d, dtype=torch.float32))
    ...
    def _multi_encoder(sae, feature_idxs):
        ...
        def enc(hidden):
            ...
            # On the SAE's card; suppress_feature_list moves each `a_u` back to
            # the hidden state's.
            z = encode_with_training_normalization(
                sae, to_module(hidden, sae, dtype=torch.float32))
            if isinstance(z, tuple):
                z = z[0]
