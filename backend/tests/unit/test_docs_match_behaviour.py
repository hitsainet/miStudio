...
class TestTheManualDescribesTheGpuFieldThatExists:
    """Review round 4 of multi-GPU Phase 1, 2026-09-14.

    The published manual still said extraction takes a `gpu_id` index, that
    steering is pinned to GPU 0 and that training cannot choose a card. The
    request schemas carry `gpu` ("auto" or a UUID) and no `gpu_id`, so a reader
    following the manual sent a field that is silently dropped.

    MUTATION CONTROL (2026-09-14): restoring the old `optional \\`gpu_id\\``
    row in reference/api/models.md -> `test_no_page_offers_gpu_id_as_a_job_field`
    fails; restoring "steering is pinned to GPU 0" -> the second test fails.
    """

    #: `gpu_id` is still right where it names a MONITORING card.
    _MONITORING = ("gpu-metrics", "system/gpu/", "/system/all")

    def test_the_request_schema_really_has_gpu_and_not_gpu_id(self):
        from src.schemas.model import ActivationExtractionRequest

        assert "gpu" in ActivationExtractionRequest.model_fields
        assert "gpu_id" not in ActivationExtractionRequest.model_fields

    def test_no_page_offers_gpu_id_as_a_job_field(self):
        offending = [
            f"{page.relative_to(REPO)}: {line.strip()}"
            for page in (REPO / "manual" / "docs").rglob("*.md")
            for line in page.read_text().splitlines()
            if "gpu_id" in line and not any(m in line for m in self._MONITORING)
        ]
        assert not offending, "the manual still names a gpu_id job field:\n" + "\n".join(offending)

    def test_no_page_says_steering_or_training_cannot_choose_a_card(self):
        text = "\n".join(p.read_text() for p in (REPO / "manual" / "docs").rglob("*.md")).lower()
        assert "pinned to gpu 0" not in text
        assert "training currently runs on the default cuda device" not in text
...
# ── the MCP manual lists every category and every tool ─────────────────────

_MCP_PAGE = REPO / "manual" / "docs" / "advanced" / "mcp-server.md"

#: Categories deliberately absent from the page, with the reason. An exemption
#: list makes an omission a decision that leaves a record; silence made it an
#: accident — see below.
_MCP_CATEGORIES_UNDOCUMENTED_ON_PURPOSE: dict[str, str] = {}


def _documented_mcp_page() -> str:
    return _MCP_PAGE.read_text()


def test_the_mcp_page_documents_every_category():
    """The page's category table is HAND-MAINTAINED and it had already drifted.

    `jlens` — TWENTY tools, on by default since the J-space arc — had no row at
    all, and the `MCP_TOOL_CATEGORIES` default it printed was missing it too. A
    reader configuring from this page would have switched the category off
    without knowing it existed.

    That is the same shape as the data-model omission this file already guards,
    and the same shape as the reachability defects the MCP server is famous for
    here: the difference between a capability existing and anyone being able to
    find it.
    """
    from src.mcp_server.tools import CATEGORY_MODULES, MILLM_CATEGORY_MODULES

    page = _documented_mcp_page()
    registered = set(CATEGORY_MODULES) | set(MILLM_CATEGORY_MODULES)
    missing = {
        c for c in registered
        if f"**{c}**" not in page
        and c not in _MCP_CATEGORIES_UNDOCUMENTED_ON_PURPOSE
    }
    assert not missing, (
        f"categories with no row in {_MCP_PAGE.name}: {sorted(missing)} — "
        "add one, or add the name to _MCP_CATEGORIES_UNDOCUMENTED_ON_PURPOSE "
        "with the reason"
    )


def test_the_mcp_page_names_every_tool():
    """A category row that omits half its tools documents the category and
    hides the capability, which is the failure this repo keeps having.

    Driven off the LIVE registry rather than a hand-written list — a
    hand-maintained checker is only as good as the list, and the thing being
    checked here is a hand-maintained list.
    """
    import asyncio
    ...
    def _names(modules, is_millm):
        mcp = FastMCP("t")
        client = MagicMock()
        for method in ("get", "post", "put", "delete", "patch", "raw_get"):
            setattr(client, method, AsyncMock(return_value={}))
        gate = MagicMock()
        gate.check = AsyncMock(return_value=(True, None))
        for module in modules:
            module.register(
                mcp, client, gate if is_millm else MCPSettings(allow_anonymous=True)
            )
    ...
    for category, modules in CATEGORY_MODULES.items():
        every |= _names(modules, False)
    for category, modules in MILLM_CATEGORY_MODULES.items():
        every |= _names(modules, True)

    assert len(every) > 100, (
        f"only {len(every)} tools discovered — the registry walk broke, and a "
        "discovery step that discovers nothing makes this a tautology"
    )
    missing = {name for name in every if f"`{name}`" not in page}
    assert not missing, (
        f"{len(missing)} tools are registered but never named in "
        f"{_MCP_PAGE.name}: {sorted(missing)}"
    )


#: Columns whose absence from the data-model page is deliberate.
#:
#: Keep this SMALL and reasoned. Everything here is a column a reader of the
#: reference cannot discover, so each entry is a decision that it does not
#: matter to them.
_UNDOCUMENTED_COLUMNS_ON_PURPOSE = {
    # Surrogate keys and bookkeeping, present on nearly every table and
    # uninteresting to a reader of a data-model reference.
    "id", "created_at", "updated_at",
}

#: Tables whose columns the page documents individually. Adding a table here
#: opts its columns into the check; the page covers many tables only by name.
_COLUMN_DOCUMENTED_TABLES = {
    "features",
    "feature_activations",
}


def test_the_documented_tables_have_their_columns_documented():
    """The table check could not see a column being added, and four were.

    `test_the_data_model_reference_documents_every_table` compares
    `set(Base.metadata.tables)` — TABLE NAMES ONLY. Both `features` and
    `labeling_prompt_templates` were already named on the page, so this arc
    added `activation_frequency_true`, `activation_count_true`,
    `example_sampling` and `activation_display` and the guard stayed green.

    A reference that names a table and then misdescribes its shape is worse
    than one that omits it, because a reader has no signal to go and check.

    Scoped to the tables the page actually documents column-by-column: it
    covers many more tables by name alone, and demanding a column list for all
    of them would be a different (larger) decision.
    """
    import re
    ...
    # SELF-CHECK: the page must already document columns we know are there, or
    # the extraction pattern has drifted and every assertion below is vacuous.
    for known in ("activation_frequency", "max_activation", "prime_token"):
        assert known in mentioned, (
            f"the page no longer mentions {known!r}; this guard is inert"
        )

    missing = {}
    for table_name in sorted(_COLUMN_DOCUMENTED_TABLES):
        table = Base.metadata.tables.get(table_name)
        assert table is not None, f"{table_name} is not a registered table"
        gaps = sorted(
            c.name for c in table.columns
            if c.name not in mentioned
            and c.name not in _UNDOCUMENTED_COLUMNS_ON_PURPOSE
        )
        if gaps:
            missing[table_name] = gaps

    assert not missing, (
        f"these columns exist and the data-model reference does not mention "
        f"them: {missing}. Document them, or add them to "
        f"_UNDOCUMENTED_COLUMNS_ON_PURPOSE with the reason."
    )
