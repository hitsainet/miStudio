...
@gpu_job("dashboard_logit_lens", handoff=False)
def compute_dashboard_data_task(
    ...
    Args:
        ...
        gpu_request: "auto" or a GPU UUID for the logit lens, resolved at
            submit. None (a message queued before this existed) means auto.
    ...
    try:
        ...
        # Only the logit lens uses a GPU. Placed before any work, against the
        # memory free NOW; a card that cannot take it raises GpuPlacementError,
        # which fails this task with its message (max_retries=0, no fallback).
        lens_device = None
        gpu_uuid = None
        if include_logit_lens:
            placement = place_job(gpu_request)
            lens_device, gpu_uuid = placement.device, placement.uuid
            logger.info(
                "Dashboard data for SAE %s: logit lens on %s (requested %s)",
                sae_id, placement.describe(), gpu_request or AUTO,
            )
        ...
        return {
            ...
            # The card the logit lens ran on; this task has no job row to hold it.
            "gpu_uuid": gpu_uuid,
        }
