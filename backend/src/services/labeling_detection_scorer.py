...
_HARD_NEGATIVES_SQL = text("""
    ...
    donors AS (
        ...
        --
        -- Ordering by md5 rather than by feature_id keeps that determinism and
        -- drops the bias: plain `ORDER BY fti.feature_id` always drew the 20
        -- lexicographically-first donors, so every score's negatives came from
        -- one narrow slice of the dictionary (OSD-14). It reuses the `:salt`
        -- this query already binds (the target feature id unless a caller
        -- overrides it), so the draw stays reproducible for a given feature and
        -- panel — which is what keeps two trials over one panel paired.
        ORDER BY md5(fti.feature_id || '\x1f' || :salt)
        LIMIT :donor_limit
    ),
    ranked AS (
