...
class FeatureResponse(BaseModel):
    ...
    #: The same quantity counted BEFORE the junk-token filter. NULL on every
    #: extraction that predates the measurement — those runs genuinely did not
    #: take it, and a default would claim otherwise. Exposed because the ratio
    #: of the two IS the measurement: how much the filter distorts what we
    #: believe about each feature.
    activation_frequency_true: Optional[float] = None
    activation_count_true: Optional[int] = None
    #: DEPRECATED name, kept so existing API consumers do not break. The value
    #: is ACTIVATION CONSISTENCY — see `activation_consistency` below — and it
    #: is not a measure of interpretability: it scores similarity of activation
    #: SHAPE across examples and never inspects which tokens fire, so it pays
    #: MOST for degenerate evidence (r = -0.50 against distinct prime tokens;
    #: 98.8% of single-token features cleared the old "interpretable" bar).
    interpretability_score: float = Field(
        ..., description="Deprecated alias of activation_consistency. Not an interpretability measure."
    )
    max_activation: float
    ...
    @property
    def activation_consistency(self) -> float:
        """How similar the feature's activation SHAPE is across its top examples.

        0.7 x pairwise cosine of binarised activation patterns + 0.3 x a
        sparsity band. It says nothing about whether the feature is
        interpretable, and it is highest for evidence that repeats one token.
        """
        return self.interpretability_score
