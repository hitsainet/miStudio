...
class TestTheMaskIsAppliedNotJustComputed:
    ...
    def test_the_training_loop_indexes_by_the_mask(self):
        """The step ASSIGNS the filtered rows back to `acts_flat`.

        This scraped for the text `acts_flat = kept`. Phase 2 (2026-09-14) moved
        the filter into `real_token_activations`, whose selection is tested for
        behaviour in test_training_gpu_placement.py; what can still be lost is the
        APPLY — calling it and discarding the result. So the syntax tree must show
        `acts_flat = real_token_activations(...)` inside the task.
        """
        ...
        applied = [
            ...
            and [getattr(t, "id", None) for t in node.targets] == ["acts_flat"]
            and isinstance(node.value, ast.Call)
            and getattr(node.value.func, "id", None) == "real_token_activations"
        ]
