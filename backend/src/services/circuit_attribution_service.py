...
def make_passthrough_hook(layer: int, sae, state: PassThroughState):
    ...
    def hook(module, inputs, output):
        ...
        # Computed on the hidden state's card, so `sae` MUST sit on this
        # layer's card: on a model split across GPUs `run` puts it there
        # (plan decision D5), and a misplaced SAE fails loudly here instead of
        # being quietly shuttled across cards inside the graph.
        xf = x.float()
...
class CircuitAttributionService:
    ...
    def run(db, run_id: str, *, prompt_limit: Optional[int] = None,
        ...
        uuids) written into ``report["attribution"]``."""
        import torch
        ...
        from .circuit_capture_service import (MAX_SEQ_LENGTH, _load_sae_sync,
                                              _pad_batch)
        from .circuit_devices import (job_devices, layer_device,
                                      model_load_kwargs, release_model)
        ...
        # EVERY card the job uses. A split's first card holds only the layers
        # that landed there, so its peak alone under-reports the job by the
        # rest of the model — and the PLACED cards, not the current device.
        gpu_devices = [d for d in job_devices(device, placement)
                       if torch.device(d).type == "cuda"]
        on_gpu = bool(gpu_devices)
        t0 = time.monotonic()
        ...
        try:
            ...
            model, tokenizer, _c, _m = load_model_from_hf(
                ...
                **model_load_kwargs(
                    ...
                    sae_ids_by_layer={e["layer"]: e["sae_id"]
                                      for e in manifest.get("layers", [])},
                    # And each layer's codes, kept for the backward (one prompt).
            ...
            for entry in manifest.get("layers", []):
                ...
                # On its LAYER's card: the hook runs there, in the graph the
                # backward pass walks (plan decision D5).
                sae = _load_sae_sync(sae_record, layer_device(structure, L, device))
            ...
            for di in range(len(dataset)):
                ...
                try:
                    _ = model(input_ids=input_ids.to(inputs_on),
                              attention_mask=mask.to(inputs_on))
            ...
            peak_by_device = {str(d): torch.cuda.max_memory_allocated(d) / 2**20
                              for d in gpu_devices}
            peak_vram = sum(peak_by_device.values()) if on_gpu else 0
            ...
            report["attribution"] = {
                ...
                # Summed over the job's cards; per card beside it for a split.
                "peak_vram_mb": round(peak_vram, 1),
                "peak_vram_mb_by_device": ({k: round(v, 1) for k, v in peak_by_device.items()}
                                           if len(peak_by_device) > 1 else None),
                "gpu": gpu,
