...
class LabelingPromptTemplateCreate(BaseModel):
    ...
    max_tokens: int = Field(
        ...
        ge=10,
        le=9999,
        description="Maximum tokens in response (10-9999)"
    )
    top_p: float = Field(
        default=0.9,
        ge=0.0,
    ...
    # Example sampling and strength display
    example_sampling: Literal["top_k", "stratified"] = Field(
        default="top_k",
        description="Which stored examples the judge sees. 'top_k' takes the strongest max_examples (the historical behaviour); 'stratified' spreads the same budget across the feature's stored activation range, so a label is not formed from the extreme upper tail alone."
    )
    activation_display: Literal["absolute", "percent_of_max"] = Field(
        default="absolute",
        description="How each example's strength is stated. 'absolute' prints the raw activation to 3dp; 'percent_of_max' prints it relative to the feature's own peak, which stays legible when the range is wide (at 3dp, distinct weak examples both print as 0.007)."
    )
...
class LabelingPromptTemplateUpdate(BaseModel):
    ...
    max_tokens: Optional[int] = Field(
        ...
        ge=10,
        le=9999,
        description="Updated max tokens (10-9999)"
    )
    ...
    # Example sampling and strength display
    example_sampling: Optional[Literal["top_k", "stratified"]] = Field(
        default=None,
        description="Updated example sampling strategy"
    )
    activation_display: Optional[Literal["absolute", "percent_of_max"]] = Field(
        default=None,
        description="Updated activation strength display"
    )
...
class LabelingPromptTemplateResponse(BaseModel):
    ...
    #: sha256 over every prompt-affecting field of this template.
    #:
    #: COMPUTED, never stored. A stored copy would be a second source of truth
    #: that can disagree with the hash the label writer actually stamps — and the
    #: disagreement would be silent, showing verdicts as fresh when the template
    #: that produced them no longer exists in that form.
    #:
    #: This is what makes the staleness filter reachable. Without it the UI has
    #: no value to send to `?prompt_fingerprint=`, and the whole
    #: adjudicated-by-WHICH-judge capability has no caller.
    ...
    def _fill_prompt_fingerprint(self):
        """Compute the fingerprint from the SAME function the label writer uses.

        Not stored, and not reimplemented here: `features.label_prompt_fingerprint`
        is stamped by `labeling_fingerprint.prompt_fingerprint`, and a second
        implementation would drift silently — showing verdicts as fresh when the
        template that produced them no longer exists in that form.

        `prompt_fingerprint` reads its fields by name off whatever it is given,
        so this response must expose every prompt-affecting column. If one is
        ever dropped from the schema, `test_the_response_fingerprint_matches_the_writer`
        fails rather than the fingerprint quietly changing meaning.
        """
        if not self.prompt_fingerprint:
            from ..services.labeling_fingerprint import prompt_fingerprint

            object.__setattr__(self, "prompt_fingerprint", prompt_fingerprint(self))
        return self
...
class LabelingPromptTemplateExportItem(BaseModel):
    ...
    include_negative_examples: bool
    num_negative_examples: Optional[int] = None
    example_sampling: str = "top_k"
    activation_display: str = "absolute"
    is_detection_template: bool
    include_nlp_analysis: bool
    is_default: bool
