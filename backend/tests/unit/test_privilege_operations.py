...
@pytest.mark.asyncio
async def test_the_orphan_sweep_kills_only_pids_this_process_spawned(monkeypatch):
    """BEHAVIOUR, not source text (this was a scrape for `os.kill`, which the sweep
    now reaches through `steering_workers.kill_spawned`): a live worker recorded at
    spawn is SIGKILLed; a live process nothing recorded is not."""
    import os
    import signal
    import subprocess
    ...
    spawned = subprocess.Popen(["/bin/sleep", "120"])
    stranger = subprocess.Popen(["/bin/sleep", "120"])
    kills = []
    real_kill = os.kill
    monkeypatch.setattr(os, "kill", lambda pid, sig: real_kill(pid, sig) if sig == 0 else kills.append((pid, sig)))
    try:
        steering._SPAWNED_WORKER_PIDS.clear()
        steering._SPAWNED_WORKER_PIDS.add(spawned.pid)
        steering_workers.record_spawned(spawned.pid)
        assert await steering._kill_orphan_steering_workers() == 1
        assert kills == [(spawned.pid, signal.SIGKILL)], "the sweep killed something it never spawned"
    finally:
        monkeypatch.undo()
        steering_workers.SPAWN_START_TICKS.clear()
        for process in (spawned, stranger):
            process.kill()
            process.wait()
