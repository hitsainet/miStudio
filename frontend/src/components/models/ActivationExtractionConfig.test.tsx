 ...
 *   F2 drop allowSplit from the GPU Device picker -> "offers "All GPUs" and sends gpu "all""
 */
...
describe('ActivationExtractionConfig', () => {
  ...
  describe('Start Extraction', () => {
    ...
    it('offers "All GPUs" and sends gpu "all" — activation extraction can run split', async () => {
      const cards = [
      ...
      const picker = screen.getByLabelText('GPU Device') as HTMLSelectElement;
      ...
      await waitFor(() => expect(mockOnExtract).toHaveBeenCalledTimes(1));
      expect(mockOnExtract.mock.calls[0][1].gpu).toBe('all');
    });
