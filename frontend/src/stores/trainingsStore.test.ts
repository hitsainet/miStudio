...
describe('trainingsStore', () => {
  ...
  describe('createTraining', () => {
    ...
    // Review R2, B6: FastAPI sends `detail`, never `message`. A training refused
    // because its extraction lacks a hook type stored "Failed to create training".
    // MUTATION CONTROL F4: createTraining reading `error.response?.data?.message`
    // again -> both tests below fail (verified, bytes restored; R2-E record).
    it("stores the backend's detail when create is refused", async () => {
      const detail =
        "Activation extraction ext_1 holds hook types ['residual'] and this training asks for " +
        "['residual', 'mlp']: it has no activations for ['mlp'].";
      mockedAxios.post.mockRejectedValueOnce({ response: { status: 422, data: { detail } } });

      await expect(useTrainingsStore.getState().createTraining(mockRequest)).rejects.toBeDefined();

      expect(useTrainingsStore.getState().error).toBe(detail);
    });

    it("stores FastAPI's list of validation errors as readable text", async () => {
      mockedAxios.post.mockRejectedValueOnce({
        response: {
          status: 422,
          data: { detail: [{ loc: ['body', 'model_id'], msg: 'Field required', type: 'missing' }] },
        },
      });

      await expect(useTrainingsStore.getState().createTraining(mockRequest)).rejects.toBeDefined();

      expect(useTrainingsStore.getState().error).toBe('model_id: Field required');
    });
  ...
  describe('retryTraining', () => {
    ...
    it('retries on the GPU the failed run asked for', async () => {
      const uuid = 'GPU-247aa582-0d1b-e161-8156-983ed1fefc57';
      const failedTraining: Training = {
        id: 'train_failed_on_3090',
        model_id: 'm_lfm',
        dataset_ids: ['ds_owt'],
        dataset_id: 'ds_owt',
        status: TrainingStatus.FAILED,
      ...
      useTrainingsStore.setState({ trainings: [failedTraining] });
      mockedAxios.post.mockResolvedValueOnce({ data: { ...failedTraining, id: 'train_retry' } });

      await useTrainingsStore.getState().retryTraining('train_failed_on_3090');

      expect(mockedAxios.post).toHaveBeenCalledTimes(1);
      expect(mockedAxios.post.mock.calls[0][1]).toMatchObject({ gpu: uuid });
    });
  });
});
