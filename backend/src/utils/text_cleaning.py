...
class TextCleaner:
    ...
    OPT-IN. Cleaning REWRITES the corpus, and an SAE should learn features of the
    text the model actually reads. It must never run on chat-template-rendered
    text (see TokenizationService.resolve_enable_cleaning), and it is wrong for
    code: generic type parameters like `List<Integer>` are indistinguishable
    from HTML tags and are removed.
    ...
    # HTML/XML patterns
    # Real markup only: `<` or `</` followed by a LETTER. The old `<[^>]+>`
    # also matched `<|im_start|>` (a chat special token) and `a < b and c > d`
    # (a comparison) and deleted both. Generic type parameters such as
    # `List<Integer>` cannot be told apart from tags by a regex and are still
    # removed, which is one reason cleaning is off by default and wrong for code.
    ...
    # Runs of the SAME punctuation character, 4+ long, capped at 3. The old
    # pattern matched any MIX of 4+ marks, so the code `HashMap<>();` became
    # `HashMap<<<`.
    EXCESSIVE_PUNCT_PATTERN = re.compile(r'([!?.,;:\-_=+*/\\|<>(){}\[\]])\1{3,}')

    # Repeated character patterns (spam detection)
    REPEATED_CHAR_PATTERN = re.compile(r'(.)\1{4,}')  # 5+ same character

    # Whitespace normalization
    # Whitespace normalisation that PRESERVES line structure. The old
    # `\s+ -> ' '` erased every newline and every indent: stored OpenWebText and
    # OpenHermes tokenizations contained none of the 306 newline-bearing token
    # types, and Python files became single lines.
    INTERIOR_SPACE_PATTERN = re.compile(r'(?<=\S)[^\S\n]{2,}')  # leading indent untouched
    ...
    def clean(self, text: str) -> Optional[str]:
        ...
        # Normalize whitespace WITHOUT flattening lines: unify line endings,
        # drop trailing spaces, collapse interior runs, cap blank-line runs.
        # Newlines and leading indentation survive, because models read both.
        if self.normalize_whitespace:
            text = text.replace('\r\n', '\n').replace('\r', '\n')
            text = self.TRAILING_SPACE_PATTERN.sub('', text)
            text = self.INTERIOR_SPACE_PATTERN.sub(' ', text)
