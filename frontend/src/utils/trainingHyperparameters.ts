...
/**
 * Share of DOCUMENTS held out of training by default, so every run reports
 * held-out metrics while it trains. At 0 every number a run reports is
 * in-sample, which is how a per-layer FVU gradient went unverified until a
 * separate post-run evaluation. Must match the backend schema default.
 */
export const DEFAULT_HOLDOUT_FRACTION = 0.02;
...
export function configExtrasFromTemplate(template: TrainingTemplate): TemplateExtras {
  ...
  return {
    update: {
      ...
      // A template that predates the field takes the DEFAULT, not 0 — an
      // explicit 0 here would silently override the backend's default.
      holdout_fraction: hp.holdout_fraction ?? DEFAULT_HOLDOUT_FRACTION,
