...
# Probes (032, IDL-52): a DETECTOR may make no causal claim at all.
#
# A probe reports an AUROC on data it was not fitted on. That is evidence of
# detection, and nothing about mechanism — there is no intervention anywhere in
# feature 032, so no probe surface can reach rung 2 by any route. That makes this
# the corpus most entitled to zero occurrences and, being new, the one nothing was
# watching: a review found `probe_monitor_rung.py` shipping `reasons` strings
# straight to the API while neither this audit's globs nor the probe ladder's own
# forbidden-word list looked at them.
#
# DISCOVERED, for the reason the circuit list taught: the prefix is `probe_monitor`
# by decision P8 (plain `probe` already names J-Lens code), so one glob per layer
# opts in every module written later.
PROBE_SURFACES = sorted(
    set(
        list((BACKEND / "services").glob("probe_monitor*.py"))
        + list((BACKEND / "ml").glob("probe_monitor*.py"))
        + list((BACKEND / "api" / "v1" / "endpoints").glob("probe_monitor*.py"))
        + list((BACKEND / "schemas").glob("probe_monitor*.py"))
        + list((BACKEND / "workers").glob("probe_monitor*.py"))
        + list((BACKEND / "mcp_server" / "tools").glob("probe_monitor*.py"))
    )
)
...
# ── Probes: a detector may make no causal claim ─────────────────────────────


class TestProbeSurfacesAreDiscovered:
    def test_the_probe_corpus_is_not_empty(self):
        assert PROBE_SURFACES, (
            "probe discovery matched nothing at all — the prefix is `probe_monitor` "
            "(decision P8), so check the globs before concluding there is no probe code"
        )

    def test_it_contains_the_modules_that_exist_today(self):
        names = {p.name for p in PROBE_SURFACES}
        for expected in (
            "probe_monitor_model.py",
            "probe_monitor_metrics.py",
            "probe_monitor_rung.py",
        ):
            assert expected in names, f"{expected} is not audited"

    def test_a_LATER_probe_module_is_audited_without_editing_this_file(self, tmp_path):
        """The glob, not the list, is the guarantee — proven by exercising it.

        `SURFACES` above was hand-maintained at five files while sixteen circuit
        modules went unaudited. This asserts the probe corpus cannot repeat that:
        a module named by the convention is matched by the pattern itself.
        """
        import fnmatch
        for later in (
            "probe_monitor_service.py",
            "probe_monitor_tasks.py",
            "probe_monitor_export.py",
        ):
            assert fnmatch.fnmatch(later, "probe_monitor*.py"), later


@pytest.mark.parametrize("path", PROBE_SURFACES, ids=lambda p: p.name)
def test_probe_surface_makes_no_causal_claim(path):
    """An AUROC is not a mechanism, on any rung of the probe ladder.

    The probe ladder (IDL-52) tops out at "detects on unseen tasks, compared with
    a judge". No rung of it licenses the word, so unlike the circuit corpus there
    is no rung-2 exemption to grant here.
    """
    violations = _offending_lines(path)
    assert not violations, (
        f"{path.name} states a causal claim in user-facing text. A probe is a "
        "detector: its evidence is an AUROC interval on data it was not fitted "
        "on, and feature 032 runs no intervention at all, so no probe surface "
        "can earn the word.\n"
        + "\n".join(f"  line {ln}: {text}" for ln, text in violations)
    )
