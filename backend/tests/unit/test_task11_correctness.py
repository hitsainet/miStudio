...
_JANITORS = [
    ...
    # 032. A probe run holds a GPU lease AND a multi-gigabyte token memmap, so a
    # phantom run keeps a card and the disk until this sweeps it — and it must not
    # treat Celery-PENDING as alive, which is what this parametrisation checks.
