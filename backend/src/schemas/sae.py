...
class SAEUploadRequest(BaseModel):
    ...
    access_token: Optional[str] = Field(
        None,
        description=(
            "HuggingFace token with write permissions. Optional: when omitted, the stored "
            "(Settings → API Keys) or environment token is used, the same way the preview "
            "and download routes already resolve theirs."
        ),
    )
