...
def extract_messages_from_conversation(
    ...
    """Parse a conversation into ``[{"role": ..., "content": ...}]``.

    WHY THIS EXISTS SEPARATELY FROM `extract_text_from_conversation`. That
    function flattens straight to a string using a hand-written
    ``"<|{role}|>\n{content}\n"`` template. Those markers are **not tokens in
    any of these models' vocabularies** — they tokenize as literal characters —
    so an SAE trained on chat data never saw the turn structure the model is
    actually served with. Returning the turns instead lets the caller hand them
    to `tokenizer.apply_chat_template`, which emits the model's real delimiters.

    Roles are normalised (ShareGPT's ``human``/``gpt`` become
    ``user``/``assistant``) because chat templates dispatch on the canonical
    names and silently mis-render anything else.
    """
    if not conversation:
        return []
    ...
    for i, turn in enumerate(conversation):
        if format == ConversationFormat.SIMPLE_LIST:
            if isinstance(turn, str):
                # A bare list alternates speakers by position; there is no other
                # signal available.
                messages.append({
        ...
        if format == ConversationFormat.OPENAI:
            role, content = turn.get("role", "unknown"), turn.get("content", "")
        elif format == ConversationFormat.SHAREGPT:
            role, content = turn.get("from", "unknown"), turn.get("value", "")
        else:
            role = turn.get("role") or turn.get("from") or "user"
            content = next(
                (turn[k] for k in ("content", "value", "text", "message") if k in turn),
                "",
            )

        # Lowercase the FALLBACK too. `mapping.get(k.lower(), original)` kept
        # the original casing, so "User"/"Assistant"/"System" passed through
        # uncanonicalised — and a chat template that dispatches on the canonical
        # names renders an unrecognised role to nothing at all.
        role = mapping.get(str(role).lower(), str(role).lower())

        # `turn.get("content", "")` returns None when the key is PRESENT and
        # null — routine in OpenAI rows carrying tool_calls, and in ShareGPT
        # dumps. `str(None)` is the literal "None", which then becomes training
        # text and an SAE feature for the token `None`. The `if m["content"]`
        # filter downstream does not help: "None" is truthy.
        messages.append({
            "role": role,
