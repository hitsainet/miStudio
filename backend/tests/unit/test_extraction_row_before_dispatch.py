...
def _is_dispatch(node: ast.Call) -> bool:
    """A queueing call: `task.delay(...)`, or (multi-GPU Phase 3) the GPU routing
    helper's `.delay`-shaped form, `gpu_delay(task)(...)`, whose keywords are the task's."""
    if _callee(node) == "delay":
        return True
    return isinstance(node.func, ast.Call) and _callee(node.func) == "gpu_delay"
...
class TestEveryEndpointDispatchGoesThroughIt:
    ...
    def test_every_delay_is_the_helpers_dispatch(self, endpoint):
        ...
        delays = [c for c in calls if _is_dispatch(c)]
    ...
    def test_the_row_and_the_task_carry_the_same_id(self, endpoint):
        ...
        delay = next(c for c in calls if _is_dispatch(c))
