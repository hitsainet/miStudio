 ...
 *
 * MUTATION CONTROL (2026-09-14, restored byte-identically, sha256):
 *   F22 usesCachedActivations counts an empty list as cached
 *       -> "usesCachedActivations is true only when at least one extraction is selected"
 *          (and TrainingPanel.gpu.test's "ticked with nothing chosen" test)
 */
...
import {
  ...
  usesCachedActivations,
} from './trainingMixture';

describe('usesCachedActivations', () => {
  it('is true only when at least one extraction is selected', () => {
    // An empty list is the "Use Cached Activations" box ticked with nothing
    // chosen: no extraction_ids are sent, and the worker trains on the fly.
    expect(usesCachedActivations(['ext_web'])).toBe(true);
    expect(usesCachedActivations([])).toBe(false);
    expect(usesCachedActivations(undefined)).toBe(false);
  });
});
...
describe('buildMixtureBlock', () => {
  ...
  it('sends an explicit 0, so a historical run can still be reproduced', () => {
    // This test used to assert that 0 produced an EMPTY block, because 0 was
    // the backend default and omitting it kept the request byte-identical. The
    // default is now 0.02, so an omitted 0 would be REPLACED by 0.02 — the user
    // who chose 0 to reproduce an old run would silently get a held-out split.
    expect(
      buildMixtureBlock({
        extractionIds: ['ext_web', 'ext_chat'],
    ...
    ).toEqual({ holdout_fraction: 0 });
  });

  it('omits holdout_fraction when the form never set one, so the backend default applies', () => {
    const block = buildMixtureBlock({
      extractionIds: ['a'],
      weightsByExtraction: undefined,
      holdoutFraction: undefined,
    });
    expect('holdout_fraction' in block).toBe(false);
  });

  it.each([1, 1.5, -0.1, NaN])(
    'omits holdout_fraction at %p, since the backend requires 0 <= f < 1',
    (value) => {
      const block = buildMixtureBlock({
        extractionIds: ['a'],
        weightsByExtraction: undefined,
