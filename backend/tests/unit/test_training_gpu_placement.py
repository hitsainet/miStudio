...
def _unseen_extraction(tmp_path, rows=6, read=2, width=4):
    """An extraction that read `read` rows of an on-disk tokenization of `rows` rows."""
    import json
    ...
    tok = tmp_path / "tok"
    HFDataset.from_dict({
        "input_ids": [[(r + c) % 5 + 1 for c in range(width)] for r in range(rows)],
        "attention_mask": [[1] * width for _ in range(rows)],
    }).save_to_disk(str(tok))
    ext = tmp_path / "ext"
    ext.mkdir()
    (ext / "metadata.json").write_text(json.dumps(
        {"dataset_path": str(tok), "max_samples": read, "num_samples_processed": read}
    ))
    return SimpleNamespace(id="ext_m_x_1", output_path=str(ext))


def test_the_evaluation_loader_uses_the_loader_signature_and_the_placed_card(monkeypatch, tmp_path):
    """The loader is autospecced, so a keyword it does not accept is a TypeError.

    Ported from the spliced-CE loader test (M8/M9) when remediation item 6 replaced
    that step: the post-run evaluation on a cached-activation run loads the base
    model itself, and must load it on the training's placement.
    """
    from src.ml import model_loader
    from src.ml.model_loader import QuantizationFormat
    from src.workers import training_tasks

    loader = create_autospec(model_loader.load_model_from_hf, side_effect=_Stop("stop at the load"))
    monkeypatch.setattr(training_tasks, "load_model_from_hf", loader)
    monkeypatch.setattr(training_tasks.settings, "data_dir", tmp_path)

    training = SimpleNamespace(id="t1", model_id="m_x", gpu_uuid=None, evaluation=None)
    rows = {
        Training: [training],
    ...
    document = training_tasks.run_post_run_evaluation(
        SimpleNamespace(get_db=get_db), training_id="t1", hp={},
        models={(0, "residual"): torch.nn.Linear(2, 2)},
        placement=Placement(card=CARDS[1], device=torch.device("cuda", 1)),
        base_model=None, extractions=[_unseen_extraction(tmp_path)], sae_mb=512.0,
    )
    ...
    # A failed load is RECORDED on the training, and does not raise.
    assert document["status"] == "failed" and "stop at the load" in document["reason"]
    assert training.evaluation["status"] == "failed"
...
class TestTheOnTheFlyPaddingFilter:
    """The on-the-fly buffer's padding filter, and the device its mask is on.

    Was `real_token_activations` in the step loop, which trained on the whole
    batch (pads included) when the mask did not match or every position was
    padding. The buffer uses `activation_mask.select_real_tokens`, which refuses
    the first and yields nothing for the second; its wiring is pinned in
    test_model_activation_source.py and driven in test_training_data_path_e2e.py.
    """

    def test_it_keeps_only_real_positions(self):
        from src.services.activation_mask import select_real_tokens

        acts = torch.arange(8.0).reshape(4, 2)
        kept = select_real_tokens(acts, [[1, 0], [1, 1]])
    ...
    def test_a_mask_that_does_not_match_is_refused(self):
        from src.services.activation_mask import select_real_tokens

        with pytest.raises(ValueError):
            select_real_tokens(torch.arange(8.0).reshape(4, 2), [[1, 0, 1]])

    def test_an_all_padding_batch_yields_no_training_rows(self):
        from src.services.activation_mask import select_real_tokens

        assert select_real_tokens(torch.arange(8.0).reshape(4, 2), [[0, 0], [0, 0]]).shape == (0, 2)
...
def test_every_on_the_fly_batch_goes_to_the_embedding_card():
    """The CALLS, by the syntax tree, in `LayerCapture` — the only place the on-the-fly
    path builds model inputs since remediation item 3. A CPU run cannot tell a
    split model's cards apart, so the tree is the evidence here; the capture itself
    is driven on a real model in test_training_data_path_e2e.py."""
    import ast
    from pathlib import Path
    ...
    capture = next(
        node for node in ast.walk(tree)
        if isinstance(node, ast.ClassDef) and node.name == "LayerCapture"
    )
    assigned = [
        node.value for node in ast.walk(capture)
        if isinstance(node, ast.Assign)
        and any(ast.unparse(target) == "self.model_input_device" for target in node.targets)
    ]
    assert [ast.unparse(value) for value in assigned] == ["input_device(base_model)"]

    devices = {
        call.args[0].id: ast.unparse(keyword.value)
        for call in _calls_to(capture, "tensor")
        if call.args and isinstance(call.args[0], ast.Name)
        for keyword in call.keywords if keyword.arg == "device"
    }
    assert devices["padded_input_ids"] == "self.model_input_device"
    assert devices["attention_masks"] == "self.model_input_device"
...
def test_the_evaluation_inputs_go_to_the_embedding_card(monkeypatch, tmp_path):
    """Ported from the spliced-CE input test (T13) when remediation item 6 replaced that step."""
    ...
    # `progress` since review R1-C: the evaluation's per-batch heartbeat and stop check.
    def measure(model, saes, layer_modules, batches, progress=None):
        seen.extend((ids.device, mask.device) for ids, mask in batches())
        return {"tokens": 1, "predicted_tokens": 1, "ce_base": 1.0, "layers": [], "all_layers_spliced": {}}

    monkeypatch.setattr(
        layer_discovery, "discover_transformer_structure",
        lambda model: SimpleNamespace(layers_module=model.layers),
    )
    monkeypatch.setattr(training_evaluation, "evaluate_spliced_layers", measure)
    monkeypatch.setattr(training_tasks.settings, "data_dir", tmp_path)

    training = SimpleNamespace(id="t1", model_id="m_x", gpu_uuid=None, evaluation=None)
    session = _Session({Training: [training], Model: []}, training)
    ...
    training_tasks.run_post_run_evaluation(
        ...
        models={(0, "residual"): torch.nn.Linear(2, 2)},
        placement=Placement(card=CARDS[1], device=torch.device("cuda", 1)),
        base_model=_SplitModel(), extractions=[_unseen_extraction(tmp_path)], sae_mb=1.0,
    )

    assert seen and set(seen) == {(torch.device("cpu"), torch.device("cpu"))}, (
        "the evaluation inputs went to the first parameter's device, not the embedding's"
    )
    assert training.evaluation["status"] == "completed"
