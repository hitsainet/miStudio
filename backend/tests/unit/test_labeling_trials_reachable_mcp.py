...
class TestResumeLabelingIssuesBothCalls:
    ...
    def test_a_local_batch_carries_the_named_gpu(self):
        """A dropped `gpu` would run a local judge on Auto's card, not the one asked for."""
        mcp, client = TestCaller._register()
        client.get = AsyncMock(return_value={"resume_feature_ids": ["f1"]})
        fn = mcp._tool_manager._tools["resume_labeling"].fn
        uuid = "GPU-247aa582-0d1b-e161-8156-983ed1fefc57"

        asyncio.run(fn(extraction_job_id="extr_x", labeling_method="local", gpu=uuid))

        assert client.post.await_count == 1
        body = client.post.await_args.kwargs["json_body"]
        assert (body["labeling_method"], body["gpu"]) == ("local", uuid)
