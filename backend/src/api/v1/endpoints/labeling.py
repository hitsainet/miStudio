...
from src.models.labeling_job import LabelingMethod
...
def judge_config(config: LabelingConfigRequest, **dump_kwargs) -> dict:
    """The config a labeling job row is built from, with its GPU request resolved.

    Called BEFORE any row exists, so an unknown card is a 400 naming the cards
    that do exist rather than a job that fails later in a worker.

    Only a LOCAL judge runs on a GPU. For every other method the `gpu` key is
    removed and nothing reads NVML: a remote judge has no card to name, and
    recording one would misdescribe the job.
    """
    data = config.model_dump(**dump_kwargs)
    if config.labeling_method == LabelingMethod.LOCAL.value:
        data["gpu"] = resolve_gpu_request(config.gpu, can_split=True)
    else:
        data.pop("gpu", None)
    return data
...
async def start_resume_sweep(
    ...
    # Resolved once, before the sweep row exists, and frozen with the judge
    # config: every batch is built from it by `build_labeling_job_row`.
    frozen_judge = judge_config(body.config, exclude_none=True)
...
async def start_labeling(
    ...
    # Outside the try: the 400 for an unknown card must not be re-mapped below.
    row_config = judge_config(config)
