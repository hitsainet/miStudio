...
def sae_endpoints(monkeypatch, inventory):
    ...
    sae = SimpleNamespace(
        hook_type=None,  # ExternalSAE.hook_type is nullable; the A5 refusal reads it
        # A REAL layer. This stand-in carried none, because nothing read one -- and
        # extraction then hooked ``layer or 0`` for every such SAE without a word. It is
        # refused now (review R3-B, R3B-11), so a fixture with no layer would test GPU
        # routing against a request that never gets that far.
        layer=3,
...
class TestSaeExtractionIsSubmittedWithItsCard:
    def test_the_job_row_stores_the_card_and_the_config_does_not(self, sae_endpoints):
        ...
        # DATASET FIRST: the endpoint resolves and validates the corpora before
        # it creates the job row, so that an unknown dataset is a 404 and not a
        # job that fails in the worker. `_AsyncDB` answers `execute` from a
        # positional queue, so this order tracks the endpoint's query order —
        # the same convention the batch tests below already use.
        db = _AsyncDB(dataset, h.sae, dataset)

        response = asyncio.run(h.endpoint.start_sae_extraction(
            "sae_a", ExtractionConfigRequest(gpu="1"), dataset_id="00000000-0000-4000-8000-0000000000d1", db=db,
        ))
    ...
    def test_an_unknown_card_is_a_400_and_no_job_is_created(self, sae_endpoints):
        ...
        with pytest.raises(HTTPException) as exc:
            asyncio.run(h.endpoint.start_sae_extraction(
                "sae_a", ExtractionConfigRequest(gpu=UNKNOWN_UUID), dataset_id="00000000-0000-4000-8000-0000000000d1", db=db,
            ))
    ...
    def test_every_job_in_a_batch_stores_the_card(self, sae_endpoints):
        ...
        response = asyncio.run(h.endpoint.start_batch_sae_extraction(
            BatchExtractionRequest(sae_ids=["sae_a", "sae_b"], dataset_id="00000000-0000-4000-8000-0000000000d1", gpu=TI_UUID), db=db,
        ))
    ...
    def test_an_unknown_card_is_a_400_and_the_batch_creates_nothing(self, sae_endpoints):
        ...
        with pytest.raises(HTTPException) as exc:
            asyncio.run(h.endpoint.start_batch_sae_extraction(
                BatchExtractionRequest(sae_ids=["sae_a"], dataset_id="00000000-0000-4000-8000-0000000000d1", gpu=UNKNOWN_UUID), db=db,
            ))
