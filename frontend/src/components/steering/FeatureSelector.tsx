...
import { steeringRefusalReason } from '../../utils/saeSteerability';
    ...
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800">
      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-800">
        ...
        >
          ...
          {readySAEs.map((sae) => {
            // Steering refuses an MLP, attention, embedding, resid_pre or resid_mid SAE,
            // and one that records no layer, with a 422. Offering them meant the user
            // picked an SAE, browsed it, chose features, and found out on Generate. Still
            // listed -- so the SAE does not simply vanish -- but disabled, with the reason.
            const refusal = steeringRefusalReason(sae);
            ...
            return (
              ...
              >
                {refusal === null ? label : `${label} — unavailable: ${refusal}`}
              </option>
            );
          })}
        </select>
        ...
        {readySAEs.length > 0 && readySAEs.every((sae) => steeringRefusalReason(sae) !== null) && (
          <p className="mt-2 text-xs text-amber-400">
            None of these can be steered: steering adds a decoder direction at the residual
            stream of a recorded layer.
          </p>
        )}
      </div>
