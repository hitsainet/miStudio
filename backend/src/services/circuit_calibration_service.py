...
def _split(placement) -> Dict[str, Any]:
    """The placement, for a split only: a single card keeps the exact call it always made."""
    return {"placement": placement} if placement is not None and placement.is_shard else {}
...
class CircuitCalibrationService:
    ...
    def run(cls, db, circuit_id: str, config: Dict[str, Any], *,
            ...
            device, gpu: Optional[Dict[str, Any]] = None,
        ...
        (request, uuid, name) written into the calibration manifest.
        ``placement`` is given when the model is SPLIT across cards; see
        :meth:`_build_generation_fns`.
    ...
    @classmethod
    def _build_generation_fns(cls, circuit, db, cfg: Dict[str, Any], *, device,
                              placement=None):
        ...
        ``placement`` is given only for a model SPLIT across cards: the loader
        spreads the model with the placement's budget, and each member's W_dec
        goes to the card its layer was put on.
        ...
        try:
            ...
            if placement is not None and placement.is_shard:
                ...
                # only SHARD_RESERVE_MB free per card).
                model, tokenizer, structure, disable_cache, device = \
                    load_model_and_structure(
                        circuit.model_id, db, device, placement=placement,
                        sae_ids_by_layer=circuit_sae_ids_by_layer(circuit))
                # Which card a layer is on is accelerate's decision, made
                # during the load, so the loaded structure carries it.
                _model_id, resolved = resolve_circuit_members(
                    circuit, db, device, structure=structure)
            else:
                model, tokenizer, structure, disable_cache, device = \
                    load_model_and_structure(circuit.model_id, db, device)
