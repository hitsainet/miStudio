...
def resolve_hf_token(explicit: Optional[str] = None) -> Optional[str]:
    ...
    ⚠ THE PRECEDENCE ABOVE USED TO BE A LIE FOR ONE CLASS OF INPUT. This was
    `explicit or settings.hf_token`, and `"   "` and `"none"` are both TRUTHY —
    so a present-but-empty explicit token suppressed the environment tier
    entirely and fell through only to the database. A UI that sends the string
    "none" for a cleared optional field therefore disabled a perfectly good
    `HF_TOKEN` env var, and the resulting 401 read like a bad credential rather
    than a skipped one. Every tier is now normalised before it is chosen
    between, so "absent" means the same thing at each.
    """
    ...
    token = _clean(explicit) or _clean(settings.hf_token)
    if token is None:
        try:
...
class HuggingFaceSAEService:
    ...
    async def upload_sae(
        ...
        Args:
            ...
            access_token: HuggingFace token with write permissions, or None to use the
                stored/environment one
        ...
        ⚠ THIS WAS THE ONE PATH IN THIS FILE THAT DID NOT RESOLVE ITS TOKEN. The preview and
        download paths both call `resolve_hf_token`; the upload took a REQUIRED `access_token`
        straight to `HfApi`, so a write token stored in Settings → API Keys could not be used and
        the operator had to paste one into every request. Worse, the probe-definition publish
        endpoint's docstring asserted "the same resolver the SAE upload uses" — a claim about a
        neighbouring path that was simply untrue, and the kind of statement nothing tests.
        `test_hf_sae_upload_resolves_its_token.py` now pins the resolution and the refusal.
        """
        token = resolve_hf_token(access_token)
        if not token:
            raise ValueError(
                "no HuggingFace token with write permissions: pass one in the request, set "
                "HF_TOKEN, or store one in Settings → API Keys. Refused before the upload starts, "
                "because an anonymous HfApi fails at the commit with a 401 that reads like a bad "
        ...
        def _upload() -> Dict[str, Any]:
            ...
            # ⚠ `commit_hash` was hardcoded None beside a comment saying it "would need to get
            # from API response" — and the response was already in hand and discarded. A response
            # field that is always None is worse than an absent one: `SAEUploadResponse` declares
            # it, so a caller reads "this upload has no commit" rather than "nobody looked".
            return {
                "repo_id": repo_id,
                "filepath": filepath,
                "url": url,
                "commit_hash": getattr(commit, "oid", None),
            }
