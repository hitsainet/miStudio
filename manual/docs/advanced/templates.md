...
- Can include **weaker examples** of the same feature for contrastive grounding
  (5 by default). These are the feature's own lowest retained activations — they
  *do* activate it, just less strongly. They are **not** non-activating text: no
  service encodes arbitrary text on demand, so nothing can certify a passage as
  one the feature ignores, and the prompt says so explicitly rather than
  claiming otherwise to the judge.
