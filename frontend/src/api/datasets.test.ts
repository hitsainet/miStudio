...
describe('datasets API client', () => {
  ...
  describe('getDatasetSamples', () => {
    it('should fetch samples without parameters', async () => {
      const mockResponse = {
        // The REAL shape, verified against the live endpoint: {index, data: {columns}}.
        // The previous fixture was `{id, text}`, which the API has never returned — it
        // agreed with a wrong type declaration rather than with the server.
        data: [{ index: 0, data: { conversations: [], category: 'orca' } }],
        total: 1,
      };
