...
class TestStopAndFinalizeIsActuallyHandled:
    ...
    async def test_dispatches_finalize_with_the_right_arguments(self):
        ...
        with patch.object(ep.TrainingService, "stop_training", return_value=training), \
             ...
             patch.object(ep.TaskQueueService, "create_task_entry",
                          new=AsyncMock(return_value=SimpleNamespace(id="tq_1"))), \
             patch(
        ...
        assert delay.call_args.kwargs["args"] == ["train_x", None]
        assert delay.call_args.kwargs["task_id"]
...
class TestFinalizeEndpointGuards:
    ...
    async def _call(self, status, **kwargs):
        """Dispatch is `apply_async`, not `delay`, since OSD-36c.

        The endpoint mints the Celery id so the `task_queue` row can carry it,
        and `.delay` cannot take a chosen id. `delay` here is a stand-in object
        exposing the same assertions, so each test still asserts the status code
        AND whether the dispatch fired.
        """
        from src.api.v1.endpoints import trainings as ep
        ...
        with patch.object(ep.TrainingService, "get_training", return_value=training), \
            ...
            try:
                ...
                return None, dispatch, result
            except Exception as e:
                return e, dispatch, None
    ...
    async def test_active_statuses_are_refused(self, status):
        ...
        exc, dispatch, _ = await self._call(
    ...
    async def test_completed_proceeds_with_force(self):
        exc, dispatch, result = await self._call(
            "completed", checkpoint_step=None, allow_failed=False, force=True
        )
        assert exc is None
        assert dispatch.call_args.kwargs["args"] == ["t1", None]
        assert dispatch.call_args.kwargs["task_id"]
    ...
    async def test_explicit_checkpoint_step_is_passed_through(self):
        ...
        assert exc is None
        assert dispatch.call_args.kwargs["args"] == ["t1", 8000]
