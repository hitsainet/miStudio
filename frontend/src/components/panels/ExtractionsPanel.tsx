      ...
      <div className={`mb-6 ${COMPONENTS.card.base} p-4`}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-2">
            {/* 'cancelled' was missing, so cancelled jobs appeared in the list
                but could not be filtered for or away. */}
            {['queued', 'extracting', 'completed', 'failed', 'cancelled'].map((status) => (
              <button
                key={status}
