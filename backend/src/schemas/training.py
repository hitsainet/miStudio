...
class TrainingHyperparameters(BaseModel):
    ...
    evaluate_ce_delta: bool = Field(
        ...
        description=(
            ...
            "with the reconstruction spliced in, against a bias-ablation floor — "
            "after training completes. Every other metric lives in the SAE's own "
            "space; an SAE can reach FVU 0.02 and still wreck the next-token "
            "distribution. Runs after the checkpoint is saved, so a failure "
            "cannot cost a finished run."
        ),
    )
