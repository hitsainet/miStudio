...
def cleanup_orphaned_tasks_task(self):
    ...
    with get_sync_db() as db:
        ...
        for row in rows:
            try:
                # WAITING FOR A GPU IS NOT ORPHANED (multi-GPU Phase 3). A readout
                # or probe marks its row running before it places, and an acquire
                # waits in place with its heartbeat going stale. Inert in single mode.
                if waiting_for_gpu(row.task_id):
                    continue
            ...
            closed += 1
            # Multi-GPU Phase 3: a J-space task that died frees its card(s) —
            # only a lease nothing renews any more.
            from src.workers.gpu_job import release_reaped_leases

            release_reaped_leases(row.task_id)
        if closed:
            db.commit()
