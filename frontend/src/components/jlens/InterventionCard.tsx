...
export function InterventionCard({
  ...
  // ONE IMPLEMENTATION, shared with the swap composer. Two copies of "check
  // this verbatim, never trim it" is how the second one grows a `.trim()`.
  const {
    checks: tokenChecks,
    checking,
    check: checkToken,
  } = useTokenCheck(modelId);

  /** A verdict that says NO DIRECTION EXISTS, as opposed to a weaker one. */
  const emptyToken = (t: string) => {
    const v = tokenChecks[t];
    return v && v.n_tokens === 0 ? v : undefined;
  };
        ...
        : !chosen
          ...
          : layers.length === 0
            ...
            : isSwap && !chosenPartner
              ...
              : // A MULTI-PIECE TOKEN IS NO LONGER A BLOCK. The server takes
                // the mean of the pieces' rows and scores the first piece; the
                // verdict beside each input says so. Blocking here excluded
                // every ordinary word the tokenizer happens to split.
                //
                // ZERO TOKENS STILL IS ONE. `usable` is false for BOTH cases,
                // so removing the whole check also let "encodes to nothing"
                // through — a string with no unembedding row at all, which
                // takes a 202 and a single-GPU slot before the worker refuses
                // it. That is the waste this pre-check exists to prevent.
                emptyToken(chosen)
                ? `Direction: ${emptyToken(chosen)!.detail}`
                : isSwap && emptyToken(chosenPartner)
                  ? `Exchange with: ${emptyToken(chosenPartner)!.detail}`
                  : undefined;
      ...
      {open && (
        <div className="mt-3 space-y-3">
          ...
          <GpuSelect
            ...
            selectClassName="rounded border border-slate-300 bg-white px-2 py-1.5 text-xs text-slate-900 disabled:opacity-50 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
          />
