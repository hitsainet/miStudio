...
class ExtractionConfigRequest(BaseModel):
    ...
    filter_stop_words: bool = Field(
        True,
        description=(
            "Filter common stop words (the, and, is, etc.) from the PRIME token. "
            "Defaults ON since 2026-09-20: measured on the 16k L12 pair, turning it on "
            "cut single-token evidence degeneracy 23.02% -> 17.04% and raised the mean "
            "distinct prime tokens per feature 8.19 -> 10.47, with ZERO features losing "
            "evidence - every one still filled all 25 slots from non-stop-word peaks."
        ),
    )
    ...
    # Which corpora this extraction reads.
    #
    # `dataset_ids` is canonical; the `dataset_id` QUERY PARAMETER is the
    # legacy single-corpus form and stays accepted, so every existing caller
    # works unchanged. The endpoint always stores BOTH keys.
    #
    # Weights are RELATIVE SHARES OVER ROWS, not tokens: a top-k example slot
    # is a row, so "30% of the example slots should come from code" is a
    # statement about rows. They are normalised server-side and capped by what
    # each corpus actually holds. Omit them for an even split across sources —
    # deliberately even, not proportional to size, so a corpus ten times larger
    # does not own ten times the evidence.
    dataset_ids: Optional[list[str]] = Field(
        default=None,
        description="Dataset IDs to extract over. Omit to fall back to the dataset_id query parameter.",
    )
    dataset_weights: Optional[list[float]] = Field(
        default=None,
        description=(
            "Relative share of the evaluation samples per dataset, in dataset_ids order. "
            "Must correspond one-to-one with dataset_ids. Omit for an even split."
        ),
    )

    # Which card runs the extraction. Stored on the job row's `gpu_request`,
    # not in `config`.
...
class ExtractionStatusResponse(BaseModel):
    ...
    #: Every corpus this extraction reads, in mixture order. `dataset_name`
    #: above stays the FIRST of them, so a caller that only knows the old field
    #: keeps rendering exactly what it rendered before. None on rows written
    #: before multi-corpus extraction existed — "not recorded", not "one".
    ...
    # What the job asked for ("auto", "all" or a GPU UUID) and the card it ran
    # on; `gpu_uuids` lists every card when the model was split across GPUs.
    ...
    filter_stop_words: Optional[bool] = True
...
class BatchExtractionRequest(BaseModel):
    ...
    dataset_id: str = Field(
        ...,
        min_length=1,
        description="Dataset ID to use for all extractions (legacy single-corpus form; kept required so existing callers are unaffected)"
    )
    # The same mixture applies to every SAE in the batch. Without these the UI
    # could offer a multi-corpus selection that the batch path silently
    # narrowed to one corpus — a wrong result with no error, which is the
    # failure mode this codebase keeps paying for.
    dataset_ids: Optional[list[str]] = Field(
        default=None,
        description="Dataset IDs to extract over, for every SAE in the batch. Omit to use dataset_id alone.",
    )
    dataset_weights: Optional[list[float]] = Field(
        default=None,
        description="Relative share of the evaluation samples per dataset, in dataset_ids order.",
    )
    ...
    filter_stop_words: bool = Field(True, description="Filter common stop words")
    ...
    # Which card runs every extraction in the batch. Stored on each job row's
    # `gpu_request`; each job is placed when IT starts, against the memory free
    # then.
    gpu: GpuRequestStr = Field(AUTO, description=GPU_REQUEST_DESCRIPTION)
...
class BatchExtractionResponse(BaseModel):
    ...
    dataset_name: Optional[str] = Field(None, description="Dataset name for display (the FIRST corpus under a mixture)")
    dataset_names: Optional[List[str]] = Field(
        None,
        description="Every corpus the batch reads, in mixture order. Without this field the value is silently dropped: these models do not set extra='forbid', so an unknown kwarg is ignored rather than raising.",
    )
