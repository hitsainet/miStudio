...
1. Pick a model and enter a prompt, then **Read out**. **GPU** picks the card (Auto takes
   the one with the most free memory), and **Unload model after each readout** frees that
   card when the readout ends — leave it off to keep the model loaded, so the next readout
   skips the load.
