...
def _routes(monkeypatch, world):
    ...
    def delay(*args, **kwargs):
        queued.append(args)
        return SimpleNamespace(id="the-finalize-task")

    monkeypatch.setattr(training_finalize_tasks.finalize_training_from_checkpoint_task, "delay", delay)
...
def _control_route(monkeypatch, row, *, service_returns=None):
    ...
    monkeypatch.setattr(training_finalize_tasks.finalize_training_from_checkpoint_task, "delay",
