...
def validate_model_for_extraction(model: nn.Module, architecture_hint: Optional[str] = None) -> TransformerStructure:
    ...
    if structure.residual_norm_module is None:
        # Informational only: residual hooks capture the decoder layer output
        # and do not use this module (see get_hookable_module).
        logger.debug(
            f"Could not identify a layer norm module. Layer attrs: {structure.layer_attributes[:15]}"
        )
...
def get_hookable_module(
    ...
    if hook_type == "residual":
        # THE RESIDUAL STREAM IS THE DECODER LAYER'S OUTPUT (resid_post).
        #
        # This used to return the discovered norm module — LFM2's `ffn_norm`,
        # Llama's `post_attention_layernorm` — and only fell back to the layer.
        # A norm's output is RMS-normalised and sits BEFORE the MLP. Verified on
        # a live 390 GB LFM2.5-1.2B extraction (2026-09-12): dividing every
        # stored token by `ffn_norm.weight` gave RMS 0.96-0.996 with ~1% spread,
        # and re-running a row through the extraction's own loader reproduced
        # the file exactly at `ffn_norm` and at cosine 0.74 at the layer output.
        #
        # Every SAE trained here was therefore trained on a signal miLLM never
        # reads: miLLM (`sae_hooker`), miStudio's steering core and J-Lens all
        # hook `layers_module[L]`, and the Neuronpedia export labels the SAE
        # `hook_resid_post`. Training, circuit capture, attribution,
        # faithfulness and intervention all resolve through THIS function, so
        # they move together and stay consistent with each other.
        return layer
