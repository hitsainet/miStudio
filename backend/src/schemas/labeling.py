...
class LabelingConfigRequest(BaseModel):
    ...
    gpu: GpuRequestStr = Field(
        AUTO,
        description=(
            GPU_REQUEST_DESCRIPTION
            + " Used only when labeling_method='local'; the other methods run on "
            "a remote judge and ignore it."
        ),
    )
    ...
    skip_adjudicated: bool = Field(
        False,
        description=(
            "Skip features that already carry a verdict. Defaults to FALSE, "
            "which is the behaviour this endpoint has always had — a full "
            "relabel. True fills only the gaps."
        ),
    )
    ...
    max_examples: Optional[int] = Field(
        ...
        le=1000,
        description=(
            "Number of activation examples per feature (10-1000). If not specified, "
            "uses the template's default. The effective number is bounded by what the "
            "extraction stored (`top_k_examples`); asking for more returns what exists."
        ),
    )
...
class LabelingStatusResponse(BaseModel):
    ...
    # The endpoint and timeout are part of WHICH JUDGE ran, and a resume that
    # cannot reproduce them is not a resume — it is a second, different
    # experiment on the remainder. Both are already persisted on the row and
    # were simply never returned; `openai_api_key` deliberately stays withheld,
    # since the worker resolves it from the job, app settings or the environment
    # without the client ever holding it.
    openai_compatible_endpoint: Optional[str] = None
    api_timeout: Optional[float] = None
    #: Returned so a resume can reproduce the job it continues, and so the UI
    #: can show what a finished job actually did.
    skip_adjudicated: bool = False
    local_model: Optional[str] = None
    #: Local labeling only: what the job asked for ("auto" or a GPU UUID) and
    #: the card it ran on. Both NULL for a remote judge.
    gpu_request: Optional[str] = None
    gpu_uuid: Optional[str] = None
    #: Every card, most free first (the first is `gpu_uuid`); layers fill them in CUDA index order, when the local model was split across GPUs.
