...
export const FRAMEWORK_CONFIGS: Record<string, FrameworkConfig> = {
  ...
  [SAEArchitectureType.JUMPRELU]: {
    ...
    defaults: {
      ...
      // ON, matching the backend default. This said `false` while the payload
      // never sent the flag, so every JumpReLU run from the UI resampled anyway
      // — with a routine that was wrong for JumpReLU (tracker item 1, repaired
      // 2026-09-15: normalised space, threshold reset, Adam moments reset).
      resample_dead_neurons: true,
      target_l0: 0.05,
      ste_bandwidth: 0.5,
    },
    // `target_l0` sets EVERY threshold at step 0 via quantile calibration
    // (training_tasks: `calibrate_thresholds`, default 0.05), so it is the
    // highest-leverage sparsity knob JumpReLU has.
    //
    // The resampling controls are SHOWN. They were hidden on the claim that
    // JumpReLU does not resample; it did, on every run, and
    // `dead_neuron_threshold` was never read. The threshold now decides which
    // latents are dead, and all three controls are sent by `buildLoopBlock`.
    visibleFields: [
      'sparsity_coeff', 'target_l0', 'initial_threshold', 'bandwidth',
      'ste_bandwidth', 'normalize_decoder', 'sparsity_warmup_steps',
      'resample_dead_neurons', 'resample_interval', 'dead_neuron_threshold',
    ],
    hiddenFields: [
      'l1_alpha', 'top_k', 'aux_k', 'aux_loss_alpha',
      'adam_epsilon',
    ],
  },
