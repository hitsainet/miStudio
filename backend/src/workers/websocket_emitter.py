...
def emit_extraction_progress(
    ...
    Args:
        ...
        phase: While status is "extracting", which step is running
            (extracting, merging, statistics). Sent only when given; the same
            value is stored on the row, so a reload shows it too.
...
# ── probe monitors (032 FR-14) ────────────────────────────────────────────────
#
# Channel `probe_monitors/{run_id}`, events `probe_monitor:{progress,completed,
# failed}`. UNDERSCORES, not a hyphen: `_CHANNEL_RE` admits `[a-z][a-z0-9_]*` for
# the topic segment, so a hyphenated topic is rejected by channel validation.

def emit_probe_monitor_progress(run_id: str, progress: float,
                                stage: Optional[str] = None,
                                status: str = "running",
                                message: Optional[str] = None) -> bool:
    """Progress for one probe run. `stage` is the pipeline step, not a percentage:
    a reader needs to know that 40% means "token capture" rather than guessing."""
    return emit_progress(
        f"probe_monitors/{run_id}",
        "probe_monitor:progress",
        {"run_id": run_id, "progress": progress, "stage": stage,
         "status": status, "message": message},
    )


def emit_probe_monitor_completed(run_id: str,
                                 summary: Optional[Dict[str, Any]] = None) -> bool:
    return emit_progress(
        f"probe_monitors/{run_id}",
        "probe_monitor:completed",
        {"run_id": run_id, "status": "completed", "summary": summary or {}},
        retries=3,
    )


def emit_probe_monitor_failed(run_id: str, error: str) -> bool:
    return emit_progress(
        f"probe_monitors/{run_id}",
        "probe_monitor:failed",
        {"run_id": run_id, "status": "failed", "error": error},
        retries=3,
    )
