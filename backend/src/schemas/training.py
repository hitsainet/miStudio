...
from .gpu import AUTO, GPU_REQUEST_DESCRIPTION, GpuRequestStr
...
class TrainingHyperparameters(BaseModel):
    ...
    ste_bandwidth: Optional[float] = Field(
        None,
        gt=0,
        le=5.0,
        description=(
            "Sigmoid STE width for the z gradient in JumpReLU (default 0.5). It is "
            "the ONLY dead-latent revival mechanism a default JumpReLU run has — "
            "resampling is off and AuxK was TopK-only — and it was hardcoded."
        ),
    )
    evaluate_ce_delta: bool = Field(
        True,
        description=(
            "Measure what the SAE reconstruction costs the MODEL — cross-entropy "
            "with the reconstruction spliced in, against mean ablation (zero "
            "ablation for reference) — after training completes, on blocks the "
            "training never read. Every other metric lives in the SAE's own "
            "space; an SAE can reach FVU 0.02 and still wreck the next-token "
            "distribution. Runs after the Community Standard export is saved, so "
            "a failure cannot cost a finished run."
        ),
    )
    evaluation_token_budget: int = Field(
        131_072,
        ge=0,
        le=16_777_216,
        description=(
            "Real tokens the post-run evaluation reads from blocks the training never "
            "read, split across the sources in proportion to the mixture. Each batch "
            "runs 2 + 3 x layers forward passes of the base model. 0 skips the "
            "evaluation. Ignored when evaluate_ce_delta is false."
        ),
    )
    holdout_fraction: float = Field(
        ...
        lt=1.0,
        description=(
            "Fraction of DOCUMENTS reserved from training for out-of-sample "
            "evaluation. Defaults to 0.02 so every run reports held-out metrics "
            "while it trains; at 0 every reported number is in-sample. Pass 0 "
            "explicitly to reproduce a historical run exactly. Split by "
            "document, not token: adjacent positions are near-duplicates."
        ),
    )
    holdout_eval_tokens: Optional[int] = Field(
        None,
        ge=1,
        le=10_000_000,
        description=(
            "Held-out tokens evaluated per layer at every log step, drawn across "
            "every source in proportion to dataset_weights (equal shares when "
            "unset). Omit for 100,000."
        ),
    )
    holdout_eval_chunk_tokens: Optional[int] = Field(
        None,
        ge=1,
        le=1_000_000,
        description=(
            "Held-out tokens per evaluation forward pass. Results do not depend on "
            "it; memory does (about 16 bytes x latents + 64 bytes x width per "
            "token). Omit for 2,048."
        ),
    )
    seed: Optional[int] = Field(
        ...
        ge=0,
        description=(
            "Random seed for weight init, threshold calibration and token "
            "sampling. Omit to derive one from the training id. Persisted, so a "
            "multi-seed comparison is attributable."
        ),
    )
    dataset_weights: Optional[List[float]] = Field(
        None,
        description=(
            "Mixture over the selected sources, one weight per extraction/dataset, "
            "normalised internally. Omit for proportional-to-real-tokens. Without "
            "this the mixture follows padded row count: Bloomberg supplied 19.1% "
            "of sampled activations for 1.2% of the real text."
        ),
    )
    ...
    lr_decay_steps: int = Field(
        ...
        description=(
            ...
            "default) keeps the rate constant after warmup, as every earlier run did. "
            "warmup_steps + lr_decay_steps must not exceed total_steps. Dead-latent "
            "resampling is skipped inside the decay window."
        ),
    )
    ...
    # Dead-latent handling (services/dead_latent_resampling.py)
    dead_neuron_threshold: int = Field(
        1000,
        gt=0,
        description=(
            "A latent is dead once it has not fired on a single token for this many "
            "CONSECUTIVE TRAINING STEPS. Resampling re-initialises the latents past it."
        ),
    )
    resample_dead_neurons: bool = Field(
        True,
        description=(
            "Resample dead latents during training: encoder row and decoder column from "
            "a high-loss input in the normalised space the encoder sees, bias zeroed, a "
            "JumpReLU threshold the new latent clears, and Adam moments reset for those "
            "slices. Every architecture except TopK, which uses its auxiliary loss."
        ),
    )
    resample_interval: int = Field(
        ...
        gt=0,
        description=(
            "Resample every N training steps, once both warmups are over and never "
            "inside the LR decay window."
        ),
    )

    @model_validator(mode="after")
    def validate_lr_schedule(self) -> "TrainingHyperparameters":
        """Warmup and decay must fit in the run, or the curve they describe cannot exist."""
        if self.warmup_steps + self.lr_decay_steps > self.total_steps:
            raise ValueError(
                f"warmup_steps ({self.warmup_steps}) + lr_decay_steps ({self.lr_decay_steps}) "
                f"must not exceed total_steps ({self.total_steps})"
            )
        return self
    ...
    model_config = {
        # KEYS THIS BUILD DOES NOT KNOW ARE KEPT, NOT DROPPED (review R2F-8).
        # Pydantic's default is `extra="ignore"`, so a stored hyperparameter this
        # build has no field for vanished on the first PATCH that round-tripped the
        # template: the client sent back the template's own stored keys, the schema
        # quietly discarded the ones it did not recognise, and the update REPLACES
        # the dict — so a key written by a newer build, or by an older one whose
        # field has since been renamed, was deleted by an unrelated edit with
        # nothing logged. Keeping them makes a template survive a round trip through
        # a build that does not understand all of it.
        "extra": "allow",
        "json_schema_extra": {
            "example": {
                "hidden_dim": 768,
...
class TrainingUpdate(BaseModel):
    ...
    current_fvu: Optional[float] = Field(None, description="LEGACY FVU: var(x-x_hat)/var(x) over every element with one global mean")
    current_fvu_centred: Optional[float] = Field(None, description="Per-dimension-centred FVU (the headline): 0=perfect, 1=mean-only")
...
class TrainingResponse(BaseModel):
    ...
    current_loss: Optional[float] = Field(None, description="Current reconstruction loss")
    current_l0_sparsity: Optional[float] = Field(None, description="Current L0 sparsity")
    current_fvu: Optional[float] = Field(
        None,
        description=(
            "LEGACY Fraction of Variance Unexplained: var(x-x_hat)/var(x) over every element "
            "with ONE global mean. Reads low on activations with large constant offset "
            "dimensions. Kept with this meaning; see current_fvu_centred for the headline."
        ),
    )
    current_fvu_centred: Optional[float] = Field(
        None,
        description=(
            "The headline FVU: sum|x-x_hat|^2 / sum|x-mu|^2 with mu the per-dimension mean, "
            "raw activation space. 0=perfect reconstruction, 1=no better than the mean vector. "
            "NULL for runs recorded before it existed (2026-09-15)."
        ),
    )
    ...
    evaluation: Optional[Dict[str, Any]] = Field(
        None,
        description=(
            "Post-run evaluation on blocks the training never read: status "
            "(pending/running/completed/failed/skipped), reason, config, sources, per-layer "
            "base/spliced/mean-ablated/zero-ablated CE, loss recovered vs mean (headline) and vs "
            "zero, KL(base||spliced), L0, centred FVU, and all-layers-spliced CE. NULL when never "
            "evaluated. Re-run with POST /trainings/{id}/evaluate."
        ),
    )
    ...
    # GPU
    gpu_request: Optional[str] = Field(
        None,
        description=(
            'What the job asked for: "auto" or a GPU UUID. NULL on a row older than '
            "GPU selection, which ran as auto. A retry should send this back as `gpu`."
        ),
    )
    gpu_uuid: Optional[str] = Field(
        None, description="UUID of the GPU the job last started on; NULL until it starts"
    )
    gpu_uuids: Optional[List[str]] = Field(
        None,
        description="Every GPU (UUID) the job's model was split across, most free first "
        "(the first is gpu_uuid; layers fill them in CUDA index order); "
        "NULL for a single-GPU job",
    )
...
class TrainingMetricResponse(BaseModel):
    ...
    # SERVED, SO A READER CAN TELL THE ROWS APART (review R1-C). One logged step
    # writes an aggregate row and one row per SAE, plus held-out rows; without
    # this, a client asking for the last N rows got all of them indistinguishably.
    layer_idx: Optional[int] = Field(
        None,
        description=(
            "NULL: the aggregate over every SAE. A layer index: that SAE alone. "
            "Negative: a held-out row (-1 - layer), or a spliced-CE row written before "
            "2026-09-15 (-1000 - layer)."
        ),
    )
    # WHICH SAE OF THE LAYER (review R1-A, A5). A training over several hook types
    # trains one SAE per (layer, hook); without this, a reader saw two rows with the
    # same layer at one step and could not tell them apart.
    ...
    fvu: Optional[float] = Field(
        None,
        description="LEGACY FVU: var(x-x_hat)/var(x) over every element with one global mean",
    )
    fvu_centred: Optional[float] = Field(
        None,
        description=(
            "Per-dimension-centred FVU (the headline): sum|x-x_hat|^2 / sum|x-mu|^2. "
