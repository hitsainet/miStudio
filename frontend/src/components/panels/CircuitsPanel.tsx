...
// The picker in this panel's own label/input style.
const GPU_LABEL = 'block text-xs text-slate-600 dark:text-slate-400 mb-1';
const GPU_SELECT =
  'w-full rounded bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 px-2 py-1.5 text-sm text-slate-900 dark:text-slate-100 disabled:opacity-50';
...
function CircuitDetail({ id, onChanged }: { id: string; onChanged: () => void }) {
  ...
  // One card choice for this circuit's GPU runs (faithfulness and calibration).
  ...
  const runFaithfulness = () => {
    ...
    setFaithError(null);
    setFaithBusy(true);
    circuitsApi.startFaithfulness(circuit.id, { mode: faithMode, gpu })
      .then(() => load())
  ...
  return (
    <div className="mt-3 border-t border-slate-200 dark:border-slate-700/60 pt-3 space-y-3">
      ...
      <div className="border-t border-slate-200 dark:border-slate-700/60 pt-3 max-w-xs">
        {/* One picker serves faithfulness AND calibration; both run split, so
            it offers "All GPUs". If either stops splitting, drop allowSplit. */}
        <GpuSelect
          id={`circuit-${circuit.id}-gpu`}
          label="GPU — faithfulness & calibration runs"
          value={gpu}
          onChange={setGpu}
          allowSplit
          disabled={
            faithBusy || calibBusy
            || ['pending', 'running'].includes(circuit.faithfulness_status ?? '')
            || ['pending', 'running'].includes(circuit.calibration_status ?? '')
          }
          labelClassName={GPU_LABEL}
          selectClassName={GPU_SELECT}
        />
      </div>
...
function CaptureTab() {
  ...
  const confirmRun = (id: string) => {
    setError(null);
    // The estimate's probe and the full pass go to the same card choice.
    circuitsApi.confirmCapture(id, { gpu })
...
function DiscoveryRunDetail({ runId, onChanged }: { runId: string; onChanged: () => void }) {
  ...
  const startAttribution = () => {
    if (!run) return;
    setAttrBusy(true);
    circuitsApi.startAttribution(run.id, { gpu })
      .then(() => { load(); onChanged(); })
      .catch((e) => setError(String(e.message ?? e)))
  ...
  return (
    <div className="mt-3 border-t border-slate-200 dark:border-slate-700/60 pt-3 space-y-3">
      ...
      {run.status === 'completed' && (
        <div className="max-w-xs">
          <GpuSelect
            id={`attribution-${run.id}-gpu`}
            label="GPU — attribution pass"
            value={gpu}
            onChange={setGpu}
