...
export function ExtractionTemplateForm({
  ...
  // CARRIED, NOT SHOWN. The form has no micro-batch input and never sent the key,
  // so every save of every extraction template silently dropped it and the backend
  // fell back to `batch_size` — a stored template that had been tuned down to fit a
  // card came back untuned after any edit. Held in state so the value survives a
  // round trip; `undefined` stays `undefined`, which the backend reads as "use
  // batch_size", so a template that never set one is unaffected.
  const [microBatchSize] = useState<number | undefined>(template?.micro_batch_size);
