...
class ModelService:
    ...
    async def delete_model(
        db: AsyncSession, model_id: str, cascade_tokenizations: bool = False
    ) -> Optional[dict]:
        """
        ...
        ⚠ IT ALSO TAKES THE MODEL'S TOKENIZATIONS, and that is why
        `cascade_tokenizations` exists. `dataset_tokenizations.model_id` is
        `ON DELETE CASCADE`, so deleting a model removes every tokenization row
        that names it — **while the Arrow data stays on disk**, orphaned and
        unreferenced. It happened: deleting `m_88d55564` to change a model's
        precision silently removed all five tokenization rows for 81 GB of data,
        and they had to be rebuilt by hand, because there is no database backup
        newer than 2025-12 and WAL archiving is off.

        So a model with tokenizations is REFUSED unless the caller says
        explicitly that it wants them gone. To change a model's precision, use
        **Re-download on the same model id** — tokenizations are selected by
        `model_id`, so keeping the id keeps them.

        Args:
            db: Database session
            model_id: Model ID
            cascade_tokenizations: delete the model's tokenization rows too.
                Default False, so the destructive reading is never the one a
        ...
        Returns:
            ...
            Format: {
                ...
                "deleted_tokenizations": int
            }

        Raises:
            ValueError: if a training references the model, or if tokenizations
                exist and `cascade_tokenizations` is False.
        """
        db_model = await ModelService.get_model(db, model_id)
        if not db_model:
            return None
        ...
        # Guard: tokenizations cascade SILENTLY (models.id → CASCADE), taking the
        # rows while their Arrow directories stay on disk. Name them, count them,
        # and refuse unless the caller asked for them to go.
        from ..models.dataset_tokenization import DatasetTokenization
        tokenizations = (
            await db.execute(
                select(DatasetTokenization.id).where(
                    DatasetTokenization.model_id == model_id
                )
            )
        ).scalars().all()
        if tokenizations and not cascade_tokenizations:
            shown = ", ".join(tokenizations[:5])
            more = "" if len(tokenizations) <= 5 else f" and {len(tokenizations) - 5} more"
            raise ValueError(
                f"Model {model_id} has {len(tokenizations)} tokenization(s) "
                f"({shown}{more}) that deleting it would REMOVE, leaving their "
                f"tokenized data on disk with nothing referencing it. To change "
                f"quantization or refresh the files, use Re-download on this same "
                f"model id, which keeps the tokenizations. To delete anyway, pass "
                f"cascade_tokenizations."
            )
        ...
        return {
            ...
            "deleted_tokenizations": len(tokenizations),
        }
