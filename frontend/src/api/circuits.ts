...
export const circuitsApi = {
  ...
  /** Launch the full capture for an 'estimated' run, on the card in `body.gpu`. */
  confirmCapture: (id: string, body: { gpu: string }) =>
    fetchAPI<{ id: string; task_id: string; status: string }>(
      `/circuit-capture/${id}/confirm`, { method: 'POST', body: JSON.stringify(body) }),
  ...
  startAttribution: (id: string, body: { prompt_limit?: number; gpu: string }) =>
    fetchAPI<{ id: string; task_id: string; status: string }>(
  ...
  reproduceManifest: (id: string, body: { gpu: string }) =>
    fetchAPI<{ reproduce_of: string; task_id: string; status: string }>(
      `/validation-manifests/${id}/reproduce`, { method: 'POST', body: JSON.stringify(body) }),
