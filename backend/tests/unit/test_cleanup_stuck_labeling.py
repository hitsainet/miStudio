...
def _run(jobs, monkeypatch, *, alive=False, stalled=None, trial=None):
    ...
    # A real session's `.update()` returns an INT rowcount, and the janitor
    # logs it with %d. Leaving it as a Mock made the whole task throw once the
    # feature-release query was added — the fake was lying about the contract,
    # not the code getting it wrong.
