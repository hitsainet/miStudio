...
export function useModelExtractionProgress(modelId?: string) {
  ...
  useEffect(() => {
    ...
    const handleExtractionUpdate = (event: any) => {
      ...
      // Backend sends { type: 'extraction_progress', model_id, extraction_id, progress, status, message, phase? }
      const data = event.data || event;
      ...
      if (data.model_id && data.extraction_id && data.progress !== undefined && data.status && data.message) {
        updateExtractionProgress(
          ...
          data.status,
          data.message,
          // Only messages from a running step carry a phase; the rest leave
