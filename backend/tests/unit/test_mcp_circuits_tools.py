...
class TestValidationToolSmoke:
    ...
    def test_reproduce_validation(self):
        ...
        # Omitted: no body, so the endpoint reuses the original run's request
        # rather than being told "auto".
        assert kwargs.get("json_body") is None

    @pytest.mark.parametrize("tool_name, path", [
        ("reproduce_validation", "/validation-manifests/vman_1/reproduce"),
        ("reproduce_calibration", "/circuits/calibration-manifests/vman_1/reproduce"),
    ])
    def test_reproduce_sends_the_card_it_is_given(self, tool_name, path):
        """MCP parity with the Manifest drawer's picker (review round 3).

        MUTATION CONTROL, verified red then restored: either tool posting no
        body regardless of `gpu` fails this test.
        """
        tool, client = _register_and_get(tool_name)
        anyio.run(lambda: tool.run({"manifest_id": "vman_1", "gpu": "GPU-abc"}))
        assert client.post.await_count == 1
        client.post.assert_awaited_once_with(path, json_body={"gpu": "GPU-abc"})
