...
describe('ClusterBudgetBar', () => {
  ...
  // R3-D (R3D-12). CONTROL: the old copy ('A member rarely activates …') turns this red.
  it('says what inactive_member means: zero similarity, zero strength', () => {
    useSteeringStore.setState({
      clusterBudget: { ...budget, flags: ['inactive_member'] },
      selectedFeatures: [feature(1, 1.0)],
    });
    render(<ClusterBudgetBar />);
    expect(screen.getByText('A member has no similarity to the cluster — it gets no strength')).toBeInTheDocument();
    expect(screen.queryByText(/rarely activates/)).not.toBeInTheDocument();
  });
