...
class TaskQueueData(BaseModel):
    ...
    gpu_uuid: Optional[str] = Field(
        None, description="The GPU (UUID) the task ran on, recorded when it started; null before then"
    )
    gpu_uuids: Optional[List[str]] = Field(
        None,
        description="Every GPU (UUID) of a task whose model was split across GPUs, most free first "
        "(the first is gpu_uuid; layers fill them in CUDA index order); "
        "null for a single-GPU task",
    )
