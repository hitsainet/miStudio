...
export interface DatasetTokenization {
  ...
  /**
   * How this tokenization was built. All optional: undefined means the backend
   * did not record it, which is the truth for every tokenization written before
   * these columns existed — they must not claim a provenance they never had.
   */
  text_column?: string;
  chat_format?: 'auto' | 'chat_template' | 'plain' | 'none';
  pack_sequences?: boolean;
  padding?: string;
  truncation?: string;
  add_special_tokens?: boolean;
  /**
   * Whether the packed blocks were permuted before being written, and with which
   * seed. Undefined means the backend did not report it — every tokenization
   * built before these columns existed is in fact unshuffled, but it must not
   * claim a shuffle it never had. 0 is a legitimate seed, never a sentinel.
   */
  shuffled?: boolean;
  shuffle_seed?: number;
}
...
/**
 * One row from `GET /datasets/{id}/samples`.
 *
 * ⚠ THIS TYPE PREVIOUSLY DECLARED `{id, text, split, metadata, tokens, token_count}`,
 * WHICH THE ENDPOINT HAS NEVER RETURNED. Verified against the live API: it returns
 * `{"index": 0, "data": {…the row's actual columns…}}` — the column names are whatever
 * the source dataset has (`conversations`, `category`, `topic`, …), because a probe
 * dataset or a preview needs the real columns rather than an assumed `text` field.
 *
 * Nothing consumed the wrong shape: `getDatasetSamples` had no production caller, and
 * the only thing "verifying" it was a test whose mock was written to match the
 * declaration. A fixture that agrees with the declaration by construction cannot
 * disagree with reality — so the error survived until something needed the columns.
 */
export interface DatasetSample {
  /** Row position in the dataset, 0-based. */
  index: number;
  /** The row itself: column name → value. Values are arbitrary JSON. */
