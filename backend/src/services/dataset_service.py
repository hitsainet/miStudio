...
class DatasetService:
    ...
    async def get_dataset_by_repo_id(
        ...
        """
        # ⚠ `.first()`, NOT `.scalar_one_or_none()` (032 FR-2). Several rows per
        # repo became legal when the duplicate guard moved to (repo, config, split),
        # and `scalar_one_or_none` RAISES `MultipleResultsFound` on two — a 500 on
        # any caller, for data that is now valid. Ordered by `created_at` so "the
        # dataset for this repo" is at least deterministic; a caller that needs a
        # specific view must ask for one with `get_dataset_by_view`.
        result = await db.execute(
            select(Dataset)
            .where(Dataset.hf_repo_id == repo_id)
    ...
    @staticmethod
    async def get_dataset_by_view(
        db: AsyncSession,
        repo_id: str,
        config: Optional[str] = None,
        split: Optional[str] = None,
    ) -> Optional[Dataset]:
        """The row for ONE view of a repo: (repo_id, config, split) — 032 FR-2.

        This is what the download endpoint's 409 compares. Config and split live in
        `extra_metadata` rather than columns, so the match is done in Python over the
        repo's rows: JSONB `->>` comparison would have to special-case SQL NULL
        against a JSON `null` against a missing key, and a pre-FR-2 row has all
        three shapes across the estate. `view_key_of_metadata` normalises them in one
        place that is unit-tested.
        """
        from .dataset_view_identity import view_key, view_key_of_metadata

        wanted = view_key(repo_id, config, split)
        result = await db.execute(
            select(Dataset)
            .where(Dataset.hf_repo_id == repo_id)
            .order_by(Dataset.created_at.asc())
        ...
        for row in result.scalars().all():
            if view_key_of_metadata(repo_id, row.extra_metadata) == wanted:
                return row
        return None
