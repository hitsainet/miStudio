...
def register(mcp: FastMCP, client: MiStudioClient, settings: MCPSettings) -> None:
    ...
    async def download_model(
        ...
        gpu: Annotated[str, Field(description="'auto' (the GPU with the most free memory when the download starts; a model no single GPU can hold is split across GPUs), 'all' (split the model across every GPU) or a GPU UUID from /system/gpu-list. The download loads the model once to read its architecture. A named card, or a split, that cannot take it is refused, never swapped.")] = "auto",
    ) -> Any:
