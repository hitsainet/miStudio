...
describe('useTrainingWebSocket', () => {
  ...
  describe('Event Handler Registration', () => {
    it('should register all event handlers on mount', () => {
      ...
      // Should register 6 event handlers (training:evaluation added by remediation item 6)
      ...
      expect(mockOn).toHaveBeenCalledWith('training:evaluation', expect.any(Function));
    });
    ...
    it('should cleanup event handlers on unmount', () => {
      ...
      // Should unregister 6 event handlers
      ...
      expect(mockOff).toHaveBeenCalledWith('training:evaluation', expect.any(Function));
    });
  });

  describe('Event Handler: training:evaluation (SAE training remediation, item 6)', () => {
    it('writes the evaluation onto the training', () => {
      renderHook(() => useTrainingWebSocket([]));
      const handler = mockOn.mock.calls.find((call) => call[0] === 'training:evaluation')?.[1];
      const evaluation = { status: 'completed', ce_base: 2.1, layers: [] };

      handler({ training_id: 'training-1', evaluation });

      expect(mockUpdateTrainingStatus).toHaveBeenCalledTimes(1);
      expect(mockUpdateTrainingStatus).toHaveBeenCalledWith('training-1', { evaluation });
    });

    it('ignores an event with no evaluation rather than erasing a recorded one', () => {
      renderHook(() => useTrainingWebSocket([]));
      const handler = mockOn.mock.calls.find((call) => call[0] === 'training:evaluation')?.[1];

      handler({ training_id: 'training-1' });
  ...
  describe('Event Handler: training:progress', () => {
    ...
    it('should carry the centred FVU beside the legacy one, never in its place', () => {
      // Remediation item 5: `fvu` keeps the legacy global-mean meaning and
      // `fvu_centred` is the headline. Swapping them would label an old-scale
      ...
      expect(mockUpdateTrainingStatus).toHaveBeenCalledWith(
        'training-1',
        expect.objectContaining({ current_fvu: 0.2611, current_fvu_centred: 0.3204 })
      );
    });
