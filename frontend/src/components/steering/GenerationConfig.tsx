  ...
  return (
    <div className={`${COMPONENTS.card.base} ${compact ? 'p-3' : 'p-4'} space-y-4`}>
      ...
      <div className="flex items-center justify-between">
        <h3 className={`font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-2 ${compact ? 'text-sm' : 'text-base'}`}>
      ...
      {/* The card every compare, sweep and combined generation runs on. */}
      <GpuSelect
        id="steering-gpu"
        label="GPU"
        value={gpu}
        onChange={setGpu}
        disabled={isGenerating || isCombinedGenerating || isSweeping}
        allowSplit
        labelClassName="block text-xs text-slate-600 dark:text-slate-400 mb-1"
        selectClassName="w-full px-2 py-1.5 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed"
      />
      ...
      <div className={`grid ${compact ? 'grid-cols-2' : 'grid-cols-3'} gap-3`}>
        {/* Max Tokens */}
        <div>
          <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Max Tokens</label>
          <input
            type="number"
            min="1"
            max="2048"
            ...
            className="w-full px-2 py-1.5 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:border-emerald-500"
          />
        </div>

        {/* Temperature */}
        <div>
          <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Temperature</label>
          <input
            type="number"
            min="0"
            max="2"
        ...
        {/* Top P */}
        <div>
          <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Top P</label>
          <input
            type="number"
            min="0"
            max="1"
        ...
        {/* Num Samples */}
        <div>
          <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Samples</label>
          <input
            type="number"
            min="1"
            max="5"
        ...
        {/* Seed */}
        <div>
          <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Seed (optional)</label>
          <input
            type="number"
            placeholder="Random"
            ...
            className="w-full px-2 py-1.5 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-emerald-500"
          />
        </div>
      </div>
      ...
      <button
        ...
        className="w-full py-1.5 text-xs text-slate-600 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300 flex items-center justify-center gap-1 border-t border-slate-200 dark:border-slate-800 pt-3"
      >
        {showAdvanced ? (
          <>
      ...
      {showAdvanced && (
        <div className={`grid ${compact ? 'grid-cols-2' : 'grid-cols-3'} gap-3 pt-2`}>
          {/* Repetition Penalty */}
          <div>
            <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Repetition Penalty</label>
            <input
              type="number"
              min="1"
              max="2"
          ...
          {/* Presence Penalty */}
          <div>
            <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Presence Penalty</label>
            <input
              type="number"
              min="-2"
              max="2"
          ...
          {/* Frequency Penalty */}
          <div>
            <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Frequency Penalty</label>
            <input
              type="number"
              min="-2"
              max="2"
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-4 h-4 rounded border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-emerald-500 focus:ring-emerald-500 focus:ring-offset-slate-950"
            />
            <label htmlFor="do-sample" className="text-xs text-slate-600 dark:text-slate-400">
              Enable Sampling
            </label>
          </div>
        </div>
