        ...
        for line in result.stdout.strip().split('\n'):
            ...
            if len(parts) >= 4:
                try:
                    ...
                    # An unknown UUID is not GPU 0. Attributing it there
                    # blamed the wrong card once a second GPU existed.
                    gpu_id = uuid_to_idx.get(gpu_uuid)
                    if gpu_id is None:
                        logger.warning(
                            f"[GPU Watchdog] Process {pid} is on GPU {gpu_uuid}, "
                            "which nvidia-smi did not list; skipping it"
                        )
                        continue
