...
class FeatureTokenIndex(Base):
    ...
    __table_args__ = (
        ...
        #: One row per (extraction, feature, rank). Without this a grouping run
        #: that failed partway and was re-run could leave TWO rows for a
        #: feature's rank-1 token, and the hard-negative donor query joins on
        #: exactly that — `WHERE normalized_token = … AND token_rank = 1` — so a
        #: duplicated row would be drawn twice and weight one donor's passages
        #: double in every detection score, silently (OSD-11).
        UniqueConstraint(
            "extraction_id", "feature_id", "token_rank",
            name="uq_fti_extraction_feature_rank",
        ),
    )
