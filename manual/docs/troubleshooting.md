...
| OOM during extraction | Reduce batch size; use a quantized model; pick a bigger GPU in the form's GPU picker (the `gpu` field) |
...
| Held-out scores look unexpectedly good or bad versus another run | The two tokenizations differ in block order, so "unseen text" means different things | Check `shuffled` on each tokenization. A prefix of an unshuffled corpus is its opening slice, and the held-out tail can be different material — see [Which text](/core-workflow/sae-training) |
| Card reads Completed but metrics and progress keep advancing | Running a build older than the terminal-status stop check | Upgrade. The loop now polls its row every 25 steps and halts on any terminal status. Pressing Stop again will not help — the row is already terminal |
