...
class TestDuplicateContextsAreFolded:
    """Top-k slots must hold distinct evidence, not copies of one passage.

    A corpus repeats itself. Measured on the 32k L12 extraction (2026-09-19),
    7,744 of 32,726 features -- 24% -- had all 25 of their examples firing on a
    single token, and one inspected feature's entire evidence was 25 copies of
    the Apache licence header at different sample indices. A labeller reading
    those 25 learns exactly what it would learn from 1.

    MUTATION CONTROLS (one at a time, restore by bytes, verify sha256):
      D1 key `_content_key` on example["sample_index"] instead of the context
           -> test_identical_contexts_collapse_to_one_slot FAILS (the licence
              headers sit at DIFFERENT sample indices; that is the whole point)
      D2 drop the `activation > held_activation` branch in `_absorb_duplicate`
           (first-seen wins)
           -> test_the_strongest_instance_of_a_duplicate_survives FAILS
      D3 delete the `seen.pop(evicted_key, None)` line after heapreplace
           -> test_evicting_content_also_forgets_its_key FAILS
      D4 make `_content_key` return a constant when there are no tokens
           -> test_examples_without_context_are_never_folded FAILS
      D5 key on the window alone, dropping prime_activation_index
           -> test_the_prime_position_is_part_of_the_identity FAILS
    """

    @staticmethod
    def _example(sample_index, tokens, prime_index=1, activation=1.0):
        """An example shaped like the ones extraction actually builds."""
        return {
            "sample_index": sample_index,
            "max_activation": float(activation),
            "tokens": list(tokens),
            "prime_activation_index": prime_index,
            "prime_token": tokens[prime_index],
        }

    def _add(self, heap, examples_with_acts, feature=0):
        feature_indices = np.array([feature] * len(examples_with_acts))
        activations = np.array([a for a, _ in examples_with_acts], dtype=float)
        examples = [ex for _, ex in examples_with_acts]
        heap.add_batch(feature_indices, activations, examples)

    def test_identical_contexts_collapse_to_one_slot(self):
        """THE defect. Same text, different rows -- one slot, not five."""
        heap = IncrementalTopKHeap(num_features=1, top_k=5)
        licence = ["is", "distributed", "on", "an", "AS", "IS", "BASIS"]

        # Five different source files, byte-identical licence header.
        self._add(heap, [
            (4.40 - i * 0.01, self._example(sample_index=100 + i, tokens=licence))
            for i in range(5)
        ])

        final = heap.get_heaps()
        assert len(final[0]) == 1, (
            f"expected the 5 copies to fold into 1 slot, got {len(final[0])}"
        )
        assert heap.duplicates_suppressed == 4

    def test_the_strongest_instance_of_a_duplicate_survives(self):
        """Batches do not arrive in activation order, so first-seen is wrong."""
        heap = IncrementalTopKHeap(num_features=1, top_k=5)
        ctx = ["def", "main", "(", ")", ":"]

        # Weakest first, strongest third -- first-seen would keep 1.0.
        self._add(heap, [
            (1.0, self._example(1, ctx)),
            (9.5, self._example(2, ctx)),
            (3.2, self._example(3, ctx)),
        ])

        final = heap.get_heaps()
        assert len(final[0]) == 1
        activation, example = final[0][0]
        assert activation == 9.5, f"kept the wrong instance: {activation}"
        assert example["sample_index"] == 2

    def test_distinct_contexts_all_survive(self):
        """The Bloomberg byline case: same SHAPE, different content, all kept.

        Its 25 examples all read " story: <NAME> at <email>" and differ only by
        the name. Those are real, varied evidence for what the feature detects
        and must not be folded together.
        """
        heap = IncrementalTopKHeap(num_features=1, top_k=10)
        names = ["Hertling", "Papuc", "Hytha", "Reynolds", "Putka"]

        self._add(heap, [
            (31.0 - i * 0.1, self._example(200 + i, ["this", "story", ":", name]))
            for i, name in enumerate(names)
        ])

        final = heap.get_heaps()
        assert len(final[0]) == 5, (
            "bylines differing only by name are distinct evidence, not duplicates"
        )

    def test_examples_without_context_are_never_folded(self):
        """Fails OPEN. No content to key on -> every example is unique.

        Callers that build minimal example dicts must keep their old behaviour.
        Failing closed here would silently discard evidence, which is the worse
        error for a quality filter.
        """
        heap = IncrementalTopKHeap(num_features=1, top_k=5)
        self._add(heap, [
            (float(i + 1), {"sample_index": i, "max_activation": float(i + 1)})
            for i in range(5)
        ])

        final = heap.get_heaps()
        assert len(final[0]) == 5
        assert heap.duplicates_suppressed == 0

    def test_the_prime_position_is_part_of_the_identity(self):
        """Same window, peak on a different token = a different observation."""
        heap = IncrementalTopKHeap(num_features=1, top_k=5)
        window = ["the", "cat", "sat"]

        self._add(heap, [
            (2.0, self._example(1, window, prime_index=0)),
            (3.0, self._example(2, window, prime_index=2)),
        ])

        assert len(heap.get_heaps()[0]) == 2

    def test_dedup_frees_slots_for_distinct_evidence(self):
        """The payoff: a small top_k spends its slots on variety, not copies."""
        heap = IncrementalTopKHeap(num_features=1, top_k=3)
        dup = ["copyright", "notice", "here"]

        # Four copies of one passage arrive FIRST and outrank the rest.
        batch = [(9.0 - i * 0.01, self._example(i, dup)) for i in range(4)]
        # Then three genuinely different, weaker passages.
        batch += [
            (5.0, self._example(10, ["alpha", "beta", "gamma"])),
            (4.0, self._example(11, ["delta", "epsilon", "zeta"])),
            (3.0, self._example(12, ["eta", "theta", "iota"])),
        ]
        self._add(heap, batch)

        final = heap.get_heaps()
        assert len(final[0]) == 3
        windows = {tuple(ex["tokens"]) for _act, ex in final[0]}
        assert len(windows) == 3, f"slots hold copies, not variety: {windows}"
        assert tuple(dup) in windows, "the strongest passage should still be held"

    def test_evicting_content_also_forgets_its_key(self):
        """Eviction bookkeeping: a stale key would suppress future evidence.

        If the evicted example's key stayed in `seen`, that content could never
        earn a slot again even though nothing in the heap holds it any more.
        """
        heap = IncrementalTopKHeap(num_features=1, top_k=2)
        weak = ["weak", "one", "here"]

        self._add(heap, [
            (1.0, self._example(1, weak)),
            (2.0, self._example(2, ["mid", "two", "here"])),
            (3.0, self._example(3, ["strong", "three", "here"])),
        ])

        from src.services.extraction_vectorized import _content_key

        evicted_key = _content_key(self._example(1, weak))
        assert evicted_key not in heap.seen[0], (
            "the evicted content's key outlived its heap entry and would "
            "suppress that content from ever being stored again"
        )
        # And the key set never exceeds what the heap actually holds.
        assert len(heap.seen[0]) == len(heap.heaps[0]) == 2

    def test_suppression_count_is_reported_in_stats(self):
        heap = IncrementalTopKHeap(num_features=1, top_k=5)
        same = ["a", "b", "c"]
        self._add(heap, [(1.0, self._example(i, same)) for i in range(3)])

        assert heap.get_stats()["duplicates_suppressed"] == 2
