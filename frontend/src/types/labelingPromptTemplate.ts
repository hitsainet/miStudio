...
export interface LabelingPromptTemplate {
   ...
   *
   * Sent to the coverage endpoint as `prompt_fingerprint` to ask which verdicts
   * came from a DIFFERENT judge. Computed server-side by the same function that
   * stamps `features.label_prompt_fingerprint`, so the two cannot disagree.
   */
  ...
  /** User prompt template; must contain {examples_block} or {tokens_table} */
...
export interface LabelingPromptTemplateCreate {
  ...
  /** System message for the LLM */
  system_message: string;
  /** User prompt template; must contain {examples_block} or {tokens_table} (required) */
  user_prompt_template: string;
...
export const LabelingPromptTemplateConstraints = {
  ...
  // A user_prompt_template must carry AT LEAST ONE of these. It named only
  // `{tokens_table}` while the backend has always accepted either, and nothing
  // referenced it — dead and wrong at once (OSD-15/16). Now it is what the
  // panel's validation reads, so the two cannot drift apart again.
  requiredPlaceholders: ['{examples_block}', '{tokens_table}'] as const,
} as const;
