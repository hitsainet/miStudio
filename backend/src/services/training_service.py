...
class UnprocessableTrainingRequest(ValueError):
    """A well-formed training request the data named cannot serve; the endpoint answers 422.

    A ValueError, so every caller that already handles the service's refusals still does.
    """


class InsufficientCheckpointDisk(Exception):
    """This training's checkpoints will not fit on the volume; the endpoint answers 409.

    DELIBERATELY NOT A ValueError. ``create_training``'s caller maps ValueError to
    400 — "you sent something invalid" — and this request is not invalid: it is
    well-formed, names real data, and would be accepted tomorrow on an emptier
    disk. What is in conflict is the state of the machine, which is what 409
    says. Inheriting from ValueError would have made this a 400 silently, by
    inheritance, with nobody having decided it.
    """
...
async def checkpoint_disk_verdict(
    db: AsyncSession,
    ...
    *,
    ...
    """Is there room on the checkpoint volume for what this training would write?

    The async adapter onto ``checkpoint_disk.verdict_from_rows`` — the worker has
    its own sync one, and the RULE lives in neither, so the answer at create time
    and the answer at run time cannot drift.

    The checkpoints of other trainings are loaded ONLY when pruning is actually
    live (enabled and out of dry-run). With retention at its defaults nothing can
    be reclaimed, so the expensive load would compute a number that is discarded.
    """
    from sqlalchemy.orm import selectinload

    from ..models.app_setting import AppSetting
    from .checkpoint_disk import verdict_from_rows
    from .checkpoint_retention import SETTING_KEYS, policy_from_values

    setting_rows = (
        await db.execute(select(AppSetting).where(AppSetting.key.in_(SETTING_KEYS)))
    ).scalars().all()
    policy = policy_from_values({row.key: row.value for row in setting_rows})

    statement = select(Training)
    if policy.enabled and not policy.dry_run:
        statement = statement.options(selectinload(Training.checkpoints))
...
class TrainingService:
    ...
    async def create_training(
        ...
        Args:
            ...
            gpu_request: ``"auto"`` or a GPU UUID, as the endpoint resolved
                ``training_data.gpu``. When omitted it is resolved here, so a
                direct caller cannot silently drop a named card.

        Returns:
            Created training object

        Raises:
            ValueError: the request names a GPU this node does not have.
        """
        if gpu_request is None:
            from .gpu_placement import AUTO, GpuPlacementError, resolve_request

            try:
                gpu_request = resolve_request(getattr(training_data, "gpu", AUTO))
        ...
        if extraction_ids:
            requested_hooks = list(hyperparameters_dict.get("hook_types") or ["residual"])
            for ext_id in extraction_ids:
                found = (
                    await db.execute(
                        select(ActivationExtraction.id, ActivationExtraction.hook_types)
                        .where(ActivationExtraction.id == ext_id)
                    )
                ).first()
                if not found:
                    raise ValueError(f"Activation extraction not found: {ext_id}")
                # EVERY REQUESTED HOOK, IN EVERY EXTRACTION (review R1-A, A5). The worker
                # reads layer_{L}_{hook}.npy per SAE and refused a missing hook only at
                # load, after the job was queued and had waited for its card.
                missing = [hook for hook in requested_hooks if hook not in (found.hook_types or [])]
                if missing:
                    raise UnprocessableTrainingRequest(
                        f"Activation extraction {ext_id} holds hook types {list(found.hook_types or [])} "
                        f"and this training asks for {requested_hooks}: it has no activations for "
                        f"{missing}. Extract those hook types, or train on {list(found.hook_types or [])}."
                    )

        if "total_steps" not in hyperparameters_dict:
            raise ValueError("hyperparameters.total_steps is required")

        # WILL ITS CHECKPOINTS FIT? (debt R2D-7 / R3D-15) Refused here, before a row
        # exists and before anything is queued, because the alternative is finding out
        # mid-run: a full disk raises inside the loop, the task's handler marks the run
        # FAILED, and a FAILED run's checkpoints are precisely what its resume needs.
        # The message carries the arithmetic (forecast, free, reserve) so the refusal
        # names its own remedy.
        verdict = await checkpoint_disk_verdict(db, hyperparameters_dict)
        if not verdict.fits:
            raise InsufficientCheckpointDisk(verdict.message(), verdict)
        ...
        db_training = Training(
            ...
            # The worker reads this at start; a resume re-reads the same row, so
            # a resumed run asks for the card the user chose.
    ...
    async def resume_training(
        ...
        """
        Resume a paused or crashed training job from its newest complete checkpoint.
        ...
        # PAUSED **OR FAILED**. A crashed run — a killed worker, an OOM, a node that
        # rebooted — is reaped to FAILED with its checkpoints intact, and
        # `resume_training_task` already continues from the newest COMPLETE one,
        # refusing when there is none. Before this, the only recoveries a crash left
        # were Finalize (keep the SAE as it stood at that checkpoint and train no
        # further) and Retry (start again at step 0), so a crash at step 120,000 of
        # 150,000 threw away every step — while the machinery to continue it existed
        # and worked. CANCELLED is deliberately excluded: that run was stopped on
        # purpose, and resuming it would undo the operator's decision.
        if db_training.status not in (TrainingStatus.PAUSED.value, TrainingStatus.FAILED.value):
            return None

        resuming_after_a_crash = db_training.status == TrainingStatus.FAILED.value
        db_training.status = TrainingStatus.RUNNING.value
        if resuming_after_a_crash:
            # THE ROW MUST STOP CLAIMING IT CRASHED. Left as they were, a RUNNING row
            # still carries the previous failure's message and traceback and a
            # `completed_at` saying it finished — a record contradicting itself, and
            # the UI would show a crash banner over a live run.
            db_training.error_message = None
            db_training.error_traceback = None
            db_training.completed_at = None
        if celery_task_id:
            db_training.celery_task_id = celery_task_id
    ...
    async def get_metrics(
        ...
        Args:
            ...
            aggregate_only: Only the aggregate rows (layer_idx NULL): one per logged
                step, however many SAEs the run trains. A logged step writes one row
                per SAE and one held-out row per SAE beside its aggregate, so a
                window of N raw rows holds N / (1 + 2 x layers x hook types) steps;
                on a 5-layer, 3-hook run a card asking for 400 rows got 12 steps
                (review R1-A, A5).
        ...
        if aggregate_only:
            query = query.where(TrainingMetric.layer_idx.is_(None))
