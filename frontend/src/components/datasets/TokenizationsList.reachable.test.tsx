...
describe('TokenizationsList wiring', () => {
  ...
  it('NEGATIVE CONTROL — an untouched form sends no text_column and does not pack', async () => {
    ...
    // 2048 is the block length every corpus here is packed to; a 512 default
    // quietly produced a second, shorter tokenization of the same dataset.
    expect(payload.max_length).toBe(2048);
    ...
    // Shuffling is ON unless the operator turns it off: extraction reads a
    // PREFIX of the blocks, so corpus order means every extraction sees the
    // same opening slice. An untouched form must not produce an ordered corpus.
    expect(payload.shuffle).toBe(true);
    // Omitted, not null: blank means "derive one and record it".
    expect('shuffle_seed' in payload).toBe(false);
  });

  it('sends shuffle=false only when Shuffle Blocks is unticked', async () => {
    render(<TokenizationsList datasetId="ds_bloomberg" />);
    await openCreateForm();
    ...
    fireEvent.click(screen.getByLabelText(/shuffle blocks/i));
    fireEvent.click(screen.getByRole('button', { name: /^create$/i }));

    await waitFor(() => expect(createTokenization).toHaveBeenCalledTimes(1));
    // `false` must be SENT, not omitted — omitting it falls back to the backend
    // default of true, so a deliberate opt-out would silently shuffle anyway.
    expect(createTokenization.mock.calls[0][2]).toMatchObject({ shuffle: false });
  });

  it('sends a pinned shuffle_seed, and 0 is a real seed not an empty field', async () => {
    render(<TokenizationsList datasetId="ds_bloomberg" />);
    await openCreateForm();
    ...
    fireEvent.change(screen.getByLabelText(/seed/i), { target: { value: '0' } });
    fireEvent.click(screen.getByRole('button', { name: /^create$/i }));
