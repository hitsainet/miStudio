...
class TestGetHookableModule:
    ...
    def test_residual_is_the_whole_decoder_layer_not_a_norm(self, llama_model):
        """The residual stream is the decoder layer's OUTPUT (resid_post).

        This asserted `isinstance(module, nn.LayerNorm)` — pinning the defect.
        A norm module's output is RMS/Layer-normalised and sits before the MLP,
        so every SAE trained on it learned a different signal from the one
        miLLM encodes and steers at the layer output.
        """
        structure = discover_transformer_structure(llama_model)
        layer = structure.layers_module[0]
