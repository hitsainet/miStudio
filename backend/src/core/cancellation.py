...
#: `OperatorCancelled.reason` for a job stopped because it can no longer prove it
#: holds its GPU lease (multi-GPU Phase 3; `services/gpu_job_claim.LeaseRenewer`).
#: NOT the operator's stop: the row is not terminal, so the stop is recorded as
#: the job's failure, with why (`cooperative_cancel`).
GPU_LEASE_LOST = "gpu_lease_lost"


def _gpu_lease_lost() -> Optional[str]:
    """Why the job in this process lost its GPU lease, or None. Never raises.

    Imported lazily: `src.core` must not import `src.services` at module load.
    """
    try:
        from ..services.gpu_job_claim import lease_lost_reason

        return lease_lost_reason()
    except Exception as exc:  # noqa: BLE001 - a broken check must not stop a job by itself
        logger.warning("Could not check the GPU lease of this job: %s", exc)
        return None
...
def _probe_monitor_run():
    from ..models.probe_monitor import ProbeMonitorRun
    return ProbeMonitorRun


def _probe_monitor_judge_run():
    from ..models.probe_monitor import ProbeMonitorJudgeRun
    return ProbeMonitorJudgeRun


def _labeling_resume_sweep():
...
register(CancelScope(
    ...
    #: A DELETED EXTRACTION STOPS. This was "continue", which made sense while
    #: the task created its own row after starting — a missing row could mean
    #: "not written yet". Since 2a982a5f the API commits the row BEFORE
    #: dispatch and the task refuses to start without one, so a missing row
    #: can only mean someone deleted it. On 2026-09-12 a deleted extraction's
    #: worker kept computing statistics for 20 more minutes over 390 GB of
    #: unlinked files — the space was not freed and every queued job, including
    #: a model download, sat behind it.
    missing_row="cancelled",
))

register(CancelScope(
    kind="probe_monitor_run",
    model=_probe_monitor_run,
    #: `stage` moves through rendering → … → rung and is NOT the status; only
    #: these three end the run.
    terminal_values=frozenset({"completed", "failed", "cancelled"}),
    #: A DELETED RUN STOPS, for the reason activation extraction now does: the
    #: API commits the row before dispatch and the task refuses to start without
    #: one, so a missing row can only mean somebody deleted it. A probe run holds
    #: a GPU lease and writes a multi-gigabyte token memmap — continuing over an
    #: unlinked artifact directory would keep the card and the disk while nothing
    #: can ever read the result.
    missing_row="cancelled",
))

register(CancelScope(
    kind="probe_monitor_judge",
    model=_probe_monitor_judge_run,
    terminal_values=frozenset({"completed", "failed", "cancelled"}),
    #: Also "cancelled": a judge run is HTTP calls to an external endpoint, and a
    #: deleted row means nobody is waiting for the answers — continuing would
    #: keep spending tokens against the served model for a result with no home.
    missing_row="cancelled",
))
...
#: A SWEEP OUTLIVES ANY ONE OF ITS TASKS, which is exactly why it needs its own
#: scope. Cancelling the batch currently in flight stops that batch; it says
#: nothing about whether the operator wants the remaining twenty-six. Those are
#: different intentions and the two scopes keep them apart: stopping a sweep
#: lets the in-flight batch finish and enqueues nothing more, while cancelling
#: the batch job stops that batch alone.
register(CancelScope(
    kind="labeling_sweep",
    model=_labeling_resume_sweep,
    #: `status` is a plain String here and CAN carry "cancelled", but the sweep
    #: still needs the timestamp: it separates "the operator asked" from "the
    #: sweep stopped", and the gap between those two is a whole batch — up to
    #: 4.4 hours. Without it an operator cannot tell a sweep that is finishing
    #: its current batch from one that has ignored them.
    request_field="cancel_requested_at",
    #: Consistent with `labeling`: the row being gone is a stop signal, since
    #: there is nothing left to record progress against and nobody waiting.
    missing_row="cancelled",
))
...
register(CancelScope(
    ...
    completed_at_field=None,
    # DELETING THE DATASET IS A STOP. The UI offers cancel-then-delete; with
    # "continue" the delete removed the only row the check reads, so a deleted
    # 313.7 GB download kept running (2026-09-13). The row exists before the task
    # is dispatched, so "missing" can only mean deleted.
    missing_row="cancelled",
))
...
register(CancelScope(
    ...
    terminal_values=frozenset({"ready", "error", "cancelled"}),
    started_at_field=None,
    # Deleting the dataset (or the tokenization) removes this row — a stop, not a
    # reason to keep packing. Safe because tokenize_dataset_task creates the row
...
register(CancelScope(
    ...
    started_at_field="started_at",
    #: A DELETED TRAINING STOPS, as a deleted extraction does. The row is
    #: created before dispatch, so a missing row means someone deleted it —
    #: typically Stop then Delete before the loop's next status check. On
    #: 2026-09-14 that crashed the worker with AttributeError and left 18 GB
    #: allocated on the card until its next task.
    missing_row="cancelled",
))
...
class CancelCheck:
    ...
    def __init__(
        ...
        #: Why, when the stop is not the operator's (a lost GPU lease).
    ...
    def _poll(self) -> bool:
        if self._cancelled:
            return True
        self._last_poll = time.monotonic()
        # A JOB THAT LOST ITS GPU LEASE STOPS HERE (multi-GPU Phase 3). Asked
        # before the row: the row says nothing about it, and every GPU job's loop
        # already polls this checker at the finest boundary it can abandon work.
        lost = _gpu_lease_lost()
        if lost:
            self._cancelled = True
            self._reason = GPU_LEASE_LOST
            self._detail = lost
            return True
        scope = self._scope
        try:
    ...
    def _fetch(self, db: Any) -> Any:
        ...
        #
        # ⚠ IT ALSO DISCARDS THE CALLER'S UNCOMMITTED CHANGES, and that has cost real
        # data. Refreshing every attribute from the database reverts any assignment the
        # task has made to this row and not yet committed. A probe run assigned
        # `artifact_dir` and its GPU placement, then reached its first stage boundary —
        # where the cancel check runs BEFORE the stage's own commit — and both were
        # silently NULL on a run that otherwise completed, leaving a 7.7 GB token memmap
        # that `DELETE /runs/{id}` could no longer find. See
        # `services/probe_monitor_run.execute_probe_run`.
        #
        # This is not a bug here: refreshing is the point. Callers that assign to a row
        # and then call a checker on the SAME session must commit first.
        return (
            db.query(model)
    ...
    def raise_if_cancelled(self, detail: str = "") -> None:
        if self():
            raise OperatorCancelled(
                self._scope.kind, self._target_id, self._reason or "cancelled",
                "; ".join(part for part in (self._detail, detail) if part),
            )

    @property
    def detail(self) -> Optional[str]:
        """Why the stop happened, when it was not the operator's (a lost GPU lease)."""
        return self._detail
...
class CancelWatchdog:
    """Stop the enclosed block when its scope says stop — inside code with no callback.

    WHY THIS EXISTS. `load_dataset` is one blocking call, and its only in-process
    hook was tqdm. `datasets` binds its progress-bar class when the library is first
    imported, and the worker imports it at startup (tokenization, extraction and
    training services), so the tqdm swap in `download_dataset_task` never reached the
    bars that actually tick. On 2026-09-13 a download was cancelled and then deleted
    in the UI; it kept pulling a 313.7 GB dataset for another 30 minutes with its
    progress stuck at 0%, and the solo worker had to be SIGKILLed to free the queue.

    HOW. A daemon thread polls the scope (a fresh DB session per poll) and, on a stop,
    raises `OperatorCancelled` in the calling thread via `PyThreadState_SetAsyncExc`.
    That keeps cancellation cooperative: the exception surfaces at the next Python
    bytecode, `finally` blocks and the task's own `except OperatorCancelled` cleanup
    run, and the process survives — unlike killing a solo-pool worker, which strands
    `acks_late` messages. Delivery waits for any C call in progress to return (one
    file's read, one Arrow write), so latency is `interval_s` plus that unit.

    The exception is not injected once the block has exited, so a cancel that arrives
    after the work finished cannot surface in unrelated code.
    """
    ...
    def __init__(
        ...
        target_id: Any,
        *,
        interval_s: float = DEFAULT_MIN_INTERVAL_S,
        checker: Any = None,
    ) -> None:
        self._kind = kind
        self._target_id = target_id
        self._interval_s = interval_s
        self._check = checker if checker is not None else cancel_checker(
            kind, target_id, min_interval_s=0.0
        )
        self._stop = None
        self._lock = None
        self._armed = False
    ...
    def __enter__(self) -> "CancelWatchdog":
        ...
        self._thread = threading.Thread(
            target=self._watch,
            args=(target_thread,),
            name=f"cancel-watchdog:{self._kind}:{self._target_id}",
            daemon=True,
        )
        self._thread.start()
        return self
    ...
    def _watch(self, target_thread: int) -> None:
        ...
        while not self._stop.wait(self._interval_s):
            if not self._check.poll_now():  # a failed poll returns False, never raises
                continue
            with self._lock:
                if not self._armed:
                    return
                self._armed = False
                kind, target_id = self._kind, self._target_id
                reason = getattr(self._check, "reason", None) or "cancelled"

                def _init(exc: OperatorCancelled) -> None:
                    OperatorCancelled.__init__(
                        exc, kind, target_id, reason, "stopped by the cancel watchdog"
                    )

                # CPython instantiates the class with no arguments at delivery, so the
                # scope, target and reason are bound into a subclass here.
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if cancelled.reason == GPU_LEASE_LOST:
                    # NOT THE OPERATOR'S STOP. Nothing made the row terminal:
                    # the job lost its GPU lease and stops as a FAILURE, with
                    # why. A scope whose status column has no "failed" still
                    # records the message.
                    message = f"Stopped: {cancelled.detail or 'its GPU lease was lost'}. Run it again."
                    if not record_progress(cancelled.scope, tid, status="failed", error_message=message):
                        record_progress(cancelled.scope, tid, error_message=message)
