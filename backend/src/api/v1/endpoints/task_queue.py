...
def _serialize_task(task, entity_info, can_retry: bool = True) -> dict:
    ...
    return {
        ...
        "can_retry": can_retry,
        # The card the task ran on (J-lens runs, model downloads). The request
        # it asked for is retry_params["gpu"] / ["gpu_request"].
        "gpu_uuid": task.gpu_uuid,
        # Every card, most free first (layers fill them in CUDA index order), when
        # the task's model was split across GPUs.
        "gpu_uuids": getattr(task, "gpu_uuids", None),
...
async def retry_task(
    ...
    if task.task_type == "download" and task.entity_type == "model":
        ...
        celery_task = download_and_load_model.delay(
            ...
            # The card the failed attempt asked for. Absent on a row written
            # before GPUs were chosen, which the task reads as auto.
            gpu_request=retry_params.get("gpu_request"),
        )
        ...
        # ROWS WRITTEN BEFORE model_id WAS RECORDED CANNOT BE RETRIED BLIND.
        # That is the entire pre-existing backlog — precisely what anyone would
        # click retry on. `.delay()` serialises kwargs unchecked, so without this
        # the API returns 202 and the worker dies on a TypeError.
        model_id_for_retry = retry_params.get("model_id")
        if not model_id_for_retry:
            raise HTTPException(
                status_code=422,
                detail=(
                    "This tokenization failure predates model_id being recorded, "
                    "so it cannot be retried automatically. Start the tokenization "
                    "again from the dataset, naming the model."
                ),
            )

        celery_task = tokenize_dataset_task.delay(
            dataset_id=task.entity_id,
            # `tokenizer_name` is NOT a parameter of tokenize_dataset_task, and
            # `model_id` — which is required — was never passed. `.delay()`
            # serialises kwargs without checking them, so this retry was
            # accepted with a 202 and then died in the worker with a TypeError.
            model_id=model_id_for_retry,
            ...
            # Carry the arc's parameters, or a retry silently changes what the
            # tokenization IS — and the provenance columns then record the
            # wrong-but-plausible values.
            chat_format=retry_params.get("chat_format", "auto"),
            pack_sequences=retry_params.get("pack_sequences", False),
            # Same reason, and it bites harder here: without these a retry
            # rebuilds the corpus UNSHUFFLED while the row goes on recording the
            # seed of the attempt that failed — an ordered dataset describing
            # itself as permuted, which nothing downstream can detect.
            shuffle=retry_params.get("shuffle", True),
            shuffle_seed=retry_params.get("shuffle_seed"),
            remove_all_punctuation=retry_params.get("remove_all_punctuation"),
            custom_filter_chars=retry_params.get("custom_filter_chars"),
            return_attention_mask=retry_params.get("return_attention_mask", True),
        )
