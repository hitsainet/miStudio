...
export function AcquireLensCard({
  ...
  // The acquisition VALIDATES the lens by reading out through it, which loads
  // the model. Publishing is an upload and takes no card, so it sends none.
        ...
        <div className="mt-3 space-y-3">
          ...
          {mode === 'acquire' ? (
            <>
              ...
              <GpuSelect
                id="jlens-acquire-gpu"
                label="GPU — for validating the downloaded lens"
                value={gpu}
                onChange={setGpu}
                disabled={busy || Boolean(queued)}
                allowSplit
