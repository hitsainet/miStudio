...
**When the worker actually stops.** The training loop re-reads its own row every 25 steps and halts
on **any terminal status** — `cancelled`, `failed` or `completed` — not only on `cancelled`. That
matters for **Stop & Finalize**, whose finalize step replaces `cancelled` with `completed`: the
loop treats that as terminal too and stops. A card reading Completed is therefore a card whose
worker has released the GPU, not one that may still be training over the export you just asked
for.
