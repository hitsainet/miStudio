...
def decide_compatibility(
    ...
    def check(candidate: Dict[str, Any], warnings: List[str]) -> CompatibilityDecision:
        ...
        conflict = hook_conflict(ref.hook_type, candidate)
        if conflict:
            return CompatibilityDecision("block", warnings=[conflict])
        if (
    ...
    # A multi-hook training imports a residual, an MLP and an attention SAE of one layer and
    # one width: n_features and layer cannot tell them apart, the hook can (review R3-B, B8).
    hooked = [s for s in candidates if not hook_conflict(ref.hook_type, s)]
    if not hooked:
        return CompatibilityDecision(
            "block",
            warnings=[
                f"No local SAE with n_features={ref.n_features} is recorded at a hook compatible "
                f"with the definition's ({ref.hook_type!r}); import as unbound profile instead"
            ],
        )
    layered = [s for s in hooked if ref.layer is None or s.get("layer") == ref.layer]
    pick = (layered or hooked)[0]
    return check(pick, [f"Bound to {pick['id']} (id differs from definition)"])


def hook_conflict(definition_hook: Optional[str], candidate: Dict[str, Any]) -> Optional[str]:
    """Why a definition recorded at ``definition_hook`` cannot bind to ``candidate``, or None.

    Member indices name features of ONE dictionary. The residual, MLP and attention SAEs of a
    multi-hook training share a layer and a width, so an MLP definition bound to the residual
    SAE steers unrelated features (review R3-B, B8). Hooks are compared by the kind the
    refusals' rule decides; an unrecorded hook reads as residual there, and so here.
    """
    from .sae_hook_support import KIND_RESIDUAL, classify_hook

    local_hook = candidate.get("hook_type")
    wanted = classify_hook(definition_hook) or KIND_RESIDUAL
    found = classify_hook(local_hook) or KIND_RESIDUAL
    if wanted == found:
        return None
    return (
        f"SAE hook mismatch: definition={definition_hook!r} ({wanted}), local {candidate['id']}="
        f"{local_hook!r} ({found}) — member indices name features of a different dictionary"
    )
