...
#: The MCP server does not import the backend's schemas, so the wording of
#: src/schemas/gpu.py GPU_REQUEST_DESCRIPTION is repeated here.
GPU_PARAM_DESCRIPTION = (
    '"auto" (default) runs on the GPU with the most free memory when the task '
    ...
    "runs on that card. A named card, or a split, that cannot take the job fails "
    "the task, never swapped for another."
)
...
def register(mcp: FastMCP, client: MiStudioClient, settings: MCPSettings) -> None:
    ...
    async def steer_compare(
        ...
        gpu: Annotated[str, Field(description=GPU_PARAM_DESCRIPTION)] = "auto",
    ) -> Any:
