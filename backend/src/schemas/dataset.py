...
class DatasetTokenizeRequest(BaseModel):
    ...
    max_length: int = Field(2048, ge=1, le=8192, description="Maximum sequence length in tokens")
    ...
    enable_cleaning: bool = Field(
        False,
        description=(
            "Rewrite text before tokenizing: strip HTML tags and URLs, collapse "
            "interior runs of spaces, cap repeated punctuation. OFF by default: "
            "SAE training data should be the text the model actually reads. "
            "Measured 2026-09-12 with it on, stored OpenWebText and OpenHermes "
            "tokenizations held no newline tokens at all (0 of 614,400 sampled), "
            "chat turn markers were stripped, and code lost its line breaks and "
            "indentation. Leave it off for code and chat."
        ),
    )
    ...
    text_column: Optional[str] = Field(
        ...
        max_length=255,
        description=(
            "Column to read text from. Omit to auto-detect. Auto-detection falls "
            "back to the first text-like column, which is how Bloomberg was "
            "tokenized on 'Headline' (mean 17 tokens) while 'Article' (mean 464) "
            "went unused."
        ),
    )
    chat_format: Literal["auto", "chat_template", "plain", "none"] = Field(
        "auto",
        description=(
            "How to render conversation columns. 'chat_template' uses the "
            "tokenizer's own template so the SAE sees the model's real turn "
            "delimiters; 'plain' concatenates content with no role markers; "
            "'auto' picks the template when the tokenizer has one. Do NOT wrap "
            "raw web text in a chat template."
        ),
    )
    pack_sequences: bool = Field(
        False,
        description=(
            "Concatenate documents into full max_length blocks instead of padding "
            "each one. For short-document corpora this is the difference between "
            "~3% and ~99% real-token occupancy."
        ),
    )
    shuffle: bool = Field(
        True,
        description=(
            "Permute the packed blocks before writing them. ON by default, because "
            "activation extraction reads a PREFIX of the tokenized dataset "
            "(select(range(max_samples))): without a shuffle every extraction sees "
            "the same opening slice. Measured 2026-09-17, OpenHermes-2.5 is written "
            "one source at a time, so its first 10% of rows holds 2 of its 15 "
            "sources. Turn it off only to reproduce an ordered tokenization "
            "deliberately."
        ),
    )
    shuffle_seed: Optional[int] = Field(
        ...
        ge=0,
        description=(
            "Seed for the block permutation. Omit it and one is derived from "
            "(dataset, model, max_length) and recorded on the row, so the order is "
            "reproducible either way. 0 is a legitimate seed, not a sentinel."
        ),
    )
...
class DatasetTokenizationResponse(BaseModel):
    ...
    # ── How this tokenization was built ─────────────────────────────────────
    #
    # All eight were RECORDED AND UNREADABLE: the columns exist on the row and
    # no caller could ever see them, so "is this corpus shuffled?" or "which
    # column was it tokenized on?" could only be answered by opening the
    # database. Surfacing only the two new ones would be the "fixed one
    # representative, never generalized" pattern this repo keeps recording.
    #
    # Every one is Optional with a None default, because NULL means NOT RECORDED
    # for every row written before its column existed. A response that defaulted
    # them to a concrete value would make historical rows assert a provenance
    # they never had — the same misattribution the columns were added to stop.
    text_column: Optional[str] = Field(None, description="Column the text was read from. NULL means auto-detected.")
    chat_format: Optional[str] = Field(None, description="How conversation columns were rendered. NULL means not recorded.")
    pack_sequences: Optional[bool] = Field(None, description="Whether documents were concatenated into full blocks. NULL means not recorded.")
    padding: Optional[str] = Field(None, description="Padding strategy actually used. NULL means not recorded.")
    truncation: Optional[str] = Field(None, description="Truncation strategy actually used. NULL means not recorded.")
    add_special_tokens: Optional[bool] = Field(None, description="Whether BOS/EOS were added. NULL means not recorded.")
    shuffled: Optional[bool] = Field(
        None,
        description=(
            "Whether the packed blocks were permuted before being written. NULL "
            "means not recorded — every tokenization built before this column is "
            "in fact unshuffled, but must not claim a shuffle it never had."
        ),
    )
    shuffle_seed: Optional[int] = Field(
        None,
        description=(
            ...
            "reproducible. 0 is a legitimate seed, never a sentinel."
        ),
    )
