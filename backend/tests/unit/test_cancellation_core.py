...
class TestMissingRowPolicy:
    ...
    def test_a_deleted_row_is_ignored_where_deletion_is_unrelated(self):
        # This used `activation_extraction` as its example, which pinned a
        # defect: deleting a running extraction did not stop it. That scope now
        # treats deletion as the signal (below); `sae_extraction` still does not.
        check = C.cancel_checker("sae_extraction", "gone", db=_session(None))
        assert check() is False

    def test_a_deleted_activation_extraction_stops_its_job(self):
        """2026-09-12: a deleted extraction's worker computed statistics for 20
        more minutes over unlinked files, holding the queue behind it."""
