...
async def _local_sae_summaries(db: AsyncSession) -> list:
    # The hook is part of the binding (review R3-B, B8): a multi-hook training's residual and
    # MLP SAEs share a layer and a width.
    rows = (
        await db.execute(
            select(
                ExternalSAE.id, ExternalSAE.n_features, ExternalSAE.layer, ExternalSAE.model_name,
                ExternalSAE.hook_type,
            )
        )
    ).all()
    return [
