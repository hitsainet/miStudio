...
def layer_metrics_payload(row) -> Dict[str, Any]:
    """Final training metrics for ONE layer, from that layer's last metrics row.

    `final_metrics_scope: "layer"` says these numbers belong to this SAE alone.
    """
    return {
        "final_loss": row.loss,
        "final_l0_sparsity": row.l0_sparsity,
        "final_l0_mean": row.l0_mean,
        "final_fvu": row.fvu,
        "final_fvu_centred": row.fvu_centred,
        "final_dead_neurons": row.dead_neurons,
        "final_metrics_step": row.step,
        "final_metrics_scope": "layer",
    }


def run_metrics_payload(training) -> Dict[str, Any]:
    """The run-level fallback, LABELLED as run-level.

    `training.current_loss` and `current_l0_sparsity` are averages across every
    SAE the run trained. Copying them onto each imported SAE unlabelled made
    L11, L12 and L13 of one run carry bit-identical `final_l0_sparsity` — a
    physically impossible result that hid exactly the per-layer difference a
    reader would consult it for. Used only when a training predates per-layer
    metric rows; `final_metrics_scope: "run"` says so.
    """
    return {
        "final_loss": training.current_loss,
        "final_l0_sparsity": training.current_l0_sparsity,
        "final_metrics_scope": "run",
    }


async def final_layer_metrics_row(db, training_id: str, layer: int, hook_type: str):
    """The last IN-SAMPLE metrics row for one (layer, hook) of a training.

    Held-out rows are stored at `layer_idx = -1 - layer`, so matching
    `layer_idx == layer` selects the in-sample series only.
    """
    result = await db.execute(
        select(TrainingMetric)
        .where(
            TrainingMetric.training_id == training_id,
            TrainingMetric.layer_idx == layer,
            TrainingMetric.hook_type == hook_type,
        )
...
class SAEManagerService:
    ...
    async def _import_single_sae(
        ...
        # Final metrics for THIS layer, not the run's average across layers.
        layer_row = await final_layer_metrics_row(db, training.id, layer, hook_type)
        final_metrics = (
            layer_metrics_payload(layer_row) if layer_row is not None
            else run_metrics_payload(training)
        )
