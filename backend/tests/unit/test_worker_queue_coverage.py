...
class TestPerCardGpuQueuesHaveConsumers:
    """Multi-GPU Phase 3: `gpu.<uuid>` and `gpu.auto` are chosen per JOB by
    `services/gpu_dispatch`, so they appear in no `task_routes` entry and in no
    container's CELERY_QUEUES — the GPU supervisor derives them from the live
    inventory. That exemption is verified here, not assumed: a container runs
    the supervisor, and the supervisor's own function consumes exactly the
    queues the dispatcher produces.

    MUTATION CONTROLS: see test_gpu_workers_entrypoint.py (same wiring, executed).
    """

    def test_a_container_runs_the_per_card_gpu_workers(self):
        assert MANIFEST.exists()
        types = []
        for doc in yaml.safe_load_all(MANIFEST.read_text()):
            if doc and doc.get("kind") == "Deployment":
                for c in doc["spec"]["template"]["spec"]["containers"]:
                    types.extend(e.get("value") for e in c.get("env", []) if e["name"] == "SERVICE_TYPE")
        assert types.count("celery-gpu-workers") == 1

    def test_every_card_and_the_shared_queue_are_consumed_and_nothing_else(self):
        from src.services.gpu_claim import AUTO_QUEUE
        ...
        cards = [GpuCard(i, f"GPU-0000000{i}-0000-0000-0000-000000000000", "card", 24_000, 20_000) for i in range(3)]
        consumed = [spec.argv[list(spec.argv).index("-Q") + 1].split(",") for spec in worker_specs(cards, python="py")]
        assert consumed == [[gpu_queue_for(card.uuid), AUTO_QUEUE] for card in cards]

    def test_low_priority_is_not_on_a_per_card_worker(self):
        """The 2026-07-26 lesson, per card: cleanup must not wait behind GPU work."""
        from src.services.gpu_placement import GpuCard
        from src.workers.gpu_supervisor import worker_specs

        for spec in worker_specs([GpuCard(0, "GPU-00000000-0000-0000-0000-000000000000", "c", 1, 1)], python="py"):
            assert "low_priority" not in spec.argv[list(spec.argv).index("-Q") + 1].split(",")
