...
class TestFitCountIsBinding:
    ...
    def test_the_boundary_is_the_templates_sixty_percent(self):
        """Every template says refuse BELOW 60%: 6/10 stands, 5/10 refuses.

        This test used to claim 5/10 was a fit "the template does not ask to
        refuse". Both live templates ask exactly that ("below 6/10 ... classify
        as noise"; "below 60% fit ... uninterpretable"), so the enforcement let
        through the one band the prompt told the judge to refuse.
        """
        assert MIN_FIT_RATIO == 0.6
        assert _enforce_refusal(_label(fit_count="6/10"))["specific"] == (
            "proper_noun_entities"
        )
        assert _enforce_refusal(_label(fit_count="5/10"))["specific"] == REFUSAL_LABEL
        # Proportional, not /10: the v2 template reports over 25 examples.
        assert _enforce_refusal(_label(fit_count="15/25"))["specific"] != REFUSAL_LABEL
        assert _enforce_refusal(_label(fit_count="14/25"))["specific"] == REFUSAL_LABEL
