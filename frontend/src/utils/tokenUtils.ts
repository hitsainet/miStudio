...
/**
 * Byte-level BPE maps every raw byte to a printable character so the vocabulary
 * is text. GPT-2, Llama 3 and LFM2 all use it. The table is fixed by the
 * algorithm: bytes that are already printable map to themselves, and the rest
 * are lifted into U+0100 and up, in order.
 *
 * Built once, lazily, and inverted — we only ever decode.
 */
let byteDecoder: Map<string, number> | null = null;

const getByteDecoder = (): Map<string, number> => {
 ...
 *
 * WHY. Stripping the 'Ġ' marker is not decoding. Every NON-ASCII token stayed
 * mangled: 'âĢĻs' is the byte-level spelling of "’s" (U+2019 + 's'), and it was
 * the SECOND most common prime token on the 16k L12 extraction at 3,015 rows.
 * So the feature browser showed readers mojibake wherever the corpus used a
 * typographic apostrophe, an em dash, or any accented letter.
 *
 * FAILS OPEN. A token containing any character outside the byte-level table is
 * not a byte-level token — SentencePiece's '▁the', a special token like
 * '<|endoftext|>' — and is returned UNCHANGED for the marker-stripping below to
 * handle. Returning something mangled would be worse than returning the input.
 */
export const decodeByteLevelToken = (token: string): string => {
  const decoder = getByteDecoder();
  const bytes: number[] = [];
  for (const ch of token) {
    const b = decoder.get(ch);
    if (b === undefined) return token; // not byte-level: leave it alone
    bytes.push(b);
  }
  try {
...
export const cleanToken = (token: string): string => {
  // Decode byte-level BPE FIRST. 'Ġthe' becomes ' the' and is trimmed below;
  // 'âĢĻs' becomes "’s", which no amount of marker-stripping could produce.
  const decoded = decodeByteLevelToken(token);
  ...
  // Decoding turns the space marker into a REAL leading space, so it has to go
  // here rather than via the 'Ġ' regex above. Leading only: trailing space in a
  // token is content, and trimming it would silently join words downstream.
