...
import { buildLoopBlock, scheduleError, usesResampling } from '../../utils/trainingLoopFields';
...
export function TrainingTemplateForm({
  ...
  // `??`, not `||`: a template saved with no warmup loaded as 1,000 steps, and
  // saving it again wrote the 1,000 back — which also moved the schedule check,
  // refusing a decay that filled the run exactly.
  const [warmupSteps, setWarmupSteps] = useState(template?.hyperparameters.warmup_steps ?? 1000);
  const [lrDecaySteps, setLrDecaySteps] = useState(template?.hyperparameters.lr_decay_steps ?? 0);
  ...
  useEffect(() => {
    if (template) {
      ...
      setLrDecaySteps(template.hyperparameters.lr_decay_steps ?? 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const scheduleProblem = scheduleError(totalSteps, warmupSteps, lrDecaySteps);
    if (scheduleProblem) {
    ...
    try {
      // The template's stored values, with the form's edits applied over them
      // (R2D-4). The form shows twenty hyperparameters. A stored template carries
      // more, and the backend update replaces the whole dict. A cleared field is
      // sent as null (top level) or left out (hyperparameters). An `undefined` left
      // here is dropped by JSON.stringify, and a PATCH that omits a field keeps its
      // stored value, so a cleared description or metadata would come back.
      const data = {
        name: name.trim(),
        description: description.trim() || null,
        model_id: modelId.trim() || null,
        dataset_ids: overlayDatasetIds(template?.dataset_ids, datasetId),
        encoder_type: encoderType,
        hyperparameters: overlayHyperparameters(template?.hyperparameters, {
          ...
          // LR decay, and the three resampling fields for every framework that
          // resamples (all but TopK) — the same block the training panel sends.
          ...buildLoopBlock({
            architecture_type: encoderType,
          ...
          {showAdvanced && (
            <div className="mt-4 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label htmlFor="lr-decay-steps" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                    LR Decay Steps
                  </label>
                  <input
                    id="lr-decay-steps"
                    type="number"
                    value={lrDecaySteps}
                    onChange={(e) => setLrDecaySteps(parseInt(e.target.value, 10) || 0)}
                    min="0"
                    className="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                    disabled={isSubmitting}
                  />
                  <p className="text-xs text-slate-500 mt-1">Linear decay to 0 over the final N steps (0 = off)</p>
                </div>
              ...
              {/* Dead-latent resampling — every framework except TopK, which
                  revives dead latents with its auxiliary loss instead. */}
              {usesResampling(encoderType) && (
              <div className="grid grid-cols-3 gap-4">
                <div>
