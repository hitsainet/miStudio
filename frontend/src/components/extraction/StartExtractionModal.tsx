...
import { buildExtractionMixture, primaryDatasetId } from '../../utils/extractionMixture';
...
export const StartExtractionModal: React.FC<StartExtractionModalProps> = ({
  ...
  // Weights are KEYED BY DATASET ID, never a positional array held in state.
  // `buildExtractionMixture` is the only place they are flattened, and it
  // flattens in `dataset_ids` order. A positional array here would silently
  // extract the wrong mixture the moment the selection order changed, and the
  // server's log line would report the realised split truthfully — which reads
  // like confirmation.
  ...
  const setDatasetWeight = (id: string, raw: string) => {
    const parsed = Number(raw);
    setDatasetWeights((prev) => ({
      ...prev,
  ...
  // DEFAULTS ON since 2026-09-20. Measured on the 16k L12 pair: turning it on
  // cut single-token evidence degeneracy 23.02% -> 17.04% and raised mean
  // distinct prime tokens 8.19 -> 10.47, with ZERO features losing evidence.
  ...
  // Which card the extraction runs on — Auto, or a card's UUID.
  ...
  const applyTemplate = (template: ExtractionTemplate) => {
    ...
    if (meta.filter_special !== undefined) {
      ...
      setFilterStopWords(meta.filter_stop_words ?? true);
    }
  ...
  const handleStartExtraction = async () => {
    ...
    try {
      ...
      // The ONE place weights become a positional array, in dataset_ids order.
      const mixture = buildExtractionMixture({
        datasetIds: selectedDatasetIds,
        weightsByDataset: datasetWeights,
      });
      // The legacy query parameter / batch field: always the first corpus,
      // matching what the server stores as config["dataset_id"].
      const primaryId = primaryDatasetId(selectedDatasetIds) as string;
      ...
      if (selectedSAEIds.length === 1) {
        ...
        await startSAEExtraction(selectedSAEIds[0], primaryId, config);
        ...
        const batchRequest: BatchExtractionRequest = {
          ...
          // Every SAE in the batch reads the same mixture. Without this the
          // batch path would narrow a multi-corpus selection to one corpus
          ...
          min_activation_frequency: minActivationFrequency,
          auto_nlp: autoNlp,
          // Every job in the batch is placed by the same choice.
          gpu,
        };
          ...
          <div className="p-6 space-y-4">
            ...
            {!showSuccessMessage && (
              <>
                ...
                <div className="space-y-3">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <label className="block text-xs text-slate-600 dark:text-slate-400">
                          Select Datasets{selectedDatasetIds.length > 0 ? ` (${selectedDatasetIds.length} selected)` : ''}
                        </label>
                        {readyDatasets.length > 0 && (
                          <div className="flex items-center gap-2 text-xs">
                            <button
                              type="button"
                              onClick={() => setSelectedDatasetIds(readyDatasets.map((d) => d.id))}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {readyDatasets.map((dataset) => {
                          ...
                          return (
                            <label
                              key={dataset.id}
                              className={`flex items-center gap-2 px-3 py-2 border rounded cursor-pointer text-sm ${
                                checked
                            ...
                            >
                              ...
                              />
                              <span className="truncate text-slate-900 dark:text-white" title={dataset.name}>
                                {dataset.name}
                              </span>
                            </label>
                          );
                      ...
                      {/* Weights only exist when there is a mixture to express. */}
                        ...
                        <div className="mt-3">
                          ...
                          <div className="space-y-1">
                            {selectedDatasetIds.map((id) => {
                              ...
                              const raw = datasetWeights[id];
                              const value = Number.isFinite(raw) && raw >= 0 ? raw : 1;
                              const total = selectedDatasetIds.reduce((sum, key) => {
                                const w = datasetWeights[key];
                              ...
                              return (
                                <div key={id} className="flex items-center gap-2 text-sm">
                                  <span
                                    className="flex-1 truncate text-slate-700 dark:text-slate-300"
                                    title={ds?.name ?? id}
                                  >
                                    {ds?.name ?? id}
                                  </span>
                                  <input
                                    id={`mixture-weight-${id}`}
                                    type="number"
                                    min={0}
                                    step={0.05}
                                    value={value}
                                    onChange={(e) => setDatasetWeight(id, e.target.value)}
                                    className="w-24 px-2 py-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded text-right text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500"
                                  />
                          ...
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            Relative share of the evaluation samples per source, normalised
                            server-side. Leave every weight equal for an even split across
                            sources. The realised split is capped by what each corpus holds.
                          </p>
                        </div>
                      )}
                    </div>
                ...
                <GpuSelect
                  id="sae-extraction-gpu"
                  label="GPU"
                  value={gpu}
                  onChange={setGpu}
                  disabled={isSubmitting || isLoadingExtraction}
                  allowSplit
                  labelClassName="block text-xs text-slate-600 dark:text-slate-400 mb-1"
                  selectClassName="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed"
                />
            ...
            {showSuccessMessage ? (
              ...
              <>
                ...
                <button
                  ...
                  disabled={isSubmitting || isLoadingExtraction || selectedSAEIds.length === 0 || selectedDatasetIds.length === 0}
