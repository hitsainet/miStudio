...
from .gpu_placement import GpuPlacementError
...
class ExportConfig:
    ...
    # The logit lens's GPU: "auto" or a GPU UUID, resolved at submit. None on a
    # job stored before this existed, which means auto.
...
class NeuronpediaExportService:
    ...
    async def start_export(
        ...
        # The export labels every SAE resid_post and names it by layer alone, so an MLP
        # or attention SAE would be published as a residual one (review R1-A, A5). The
        # endpoint answers this with 422.
        from .sae_hook_support import refuse_non_residual, refuse_unrecorded_layer

        refuse_non_residual(sae.hook_type, "Neuronpedia export")
        # And the export NAMES the SAE by layer (``layer_20_res_16k``, ``{model}/20-...``)
        # and wrote ``layer or 0`` for an SAE recording none: published, permanently, as a
        # layer-0 SAE (review R3-B, R3B-11). The endpoint answers this with 422.
        refuse_unrecorded_layer(sae.layer, "The Neuronpedia export", sae.id)
        ...
        try:
            ...
            if config.include_logit_lens:
                ...
                try:
                    ...
                    # NOT SKIPPED. The card the export asked for cannot take the
                    # work; exporting without the logit lens would answer a
                    # different request than the one made. The handler below
                    # fails the job with the placement's own message.
                    raise
    ...
    async def _compute_logit_lens(
        ...
        # The live export runs as a BackgroundTask in the API process, so the
        # card is resolved and passed, never made current. MULTI-GPU PHASE 3: in
        # per-card mode the card is also LEASED for the computation, so no GPU
        # worker places a job on it meanwhile (review round 1, R1-5).
        from .logit_lens_service import logit_lens_lease

        async with logit_lens_lease(config.gpu, kind="export_logit_lens") as device:
    ...
    @staticmethod
    def exported_hook(sae: ExternalSAE) -> tuple:
        """``(hook name to publish, whether the SAE RECORDED that hook)``.

        The export refuses every non-residual SAE (review R1-A, A5) and every SAE recording
        no layer (R3-B, R3B-11), so an accepted one is a residual SAE at a known layer --
        or one whose hook was never recorded, which every consumer here reads as residual.
        SAELens and Neuronpedia both require a hook name, so one is written either way.

        What must not happen is the NULL becoming a CLAIM: re-importing this directory makes
        ``resolve_sae_hook`` read the exported cfg.json and RECORD what it finds, so an
        unrecorded hook came back as a claimed ``blocks.L.hook_resid_post`` (R3B-13). The
        flag therefore travels with the name, is written into both files, and
        ``hook_name_from_sae_config`` honours it.
        """
        from .sae_hook_support import refuse_unrecorded_layer

        refuse_unrecorded_layer(sae.layer, "The Neuronpedia export", sae.id)
        return get_transformerlens_hook_name(sae.layer, "resid_post"), sae.hook_type is not None
    ...
    def _generate_metadata_json(self, sae: ExternalSAE, model_name: str) -> dict:
        ...
        hook_name, hook_recorded = self.exported_hook(sae)
        ...
        return {
            ...
            "hook_name": hook_name,
            # False = miStudio assumed resid_post because the SAE recorded no hook (R3B-13).
            "hook_point_recorded": hook_recorded,
    ...
    def _generate_feature_json(
        ...
        result = {
            ...
            "statistics": {
                ...
                # Shape similarity across examples — NOT a quality score.
                "activation_consistency": feature.interpretability_score,
            },
        }
        ...
        for feature in features:
            ...
            explanations.append({
                ...
                "explanations": [{
                    ...
                    # No explanation score is computed. This slot is where an
                    # explanation's quality (how well it predicts activations)
                    # belongs; filling it with activation consistency presented
                    # a shape metric as a verdict on the label.
                    "score": None,
    ...
    async def _generate_saelens_format(
        ...
        # Generate cfg.json
        model_id = get_transformerlens_model_id(model_name)
        _, hook_recorded = self.exported_hook(sae)

        cfg = build_neuronpedia_config(
            model_name=model_name,
            # NOT ``or 0``: an SAE recording no layer is refused before it reaches here
            # (review R3-B, R3B-11), rather than published as layer 0.
            layer=sae.layer,
        ...
        # This file is what a re-import reads the hook from, so it carries whether the hook
        # was recorded or assumed (review R3-B, R3B-13).
        cfg["hook_point_recorded"] = hook_recorded
