...
class TestSuccessfulVerdict:
    ...
    def test_records_the_judges_self_assessment(self, writer, job):
        """`fit_count` and `confidence` reach the row. Until 2026-09-22 the
        parser carried them to this writer and the writer dropped them, so a
        confident label and a hedged one were indistinguishable afterwards."""
        feature = _feature()
        _persist(writer, job, feature,
                 {"category": "semantic", "specific": "legal_terms", "description": "",
                  "fit_count": "18/25", "confidence": "medium"})
        assert feature.label_fit_count == "18/25"
        assert feature.label_confidence == "medium"

    def test_the_real_parser_output_reaches_the_row(self, writer, job):
        """End to end through `_parse_dual_label`, not a hand-built dict whose
        keys agree with the writer by construction."""
        from src.services.openai_labeling_service import OpenAILabelingService
        parser = OpenAILabelingService.__new__(OpenAILabelingService)
        label = parser._parse_dual_label(
            '{"misses": [3, 9], "fit_count": "8/10", "category": "semantic", '
            '"specific": "court_rulings", "description": "d", "confidence": "high"}',
            "fallback",
        )
        feature = _feature()
        _persist(writer, job, feature, label)
        assert feature.name == "court_rulings"
        assert feature.label_fit_count == "8/10"
        assert feature.label_confidence == "high"

    def test_a_relabel_without_self_assessment_clears_the_old_one(self, writer, job):
        """A stale "9/10 high" beside a new verdict from a template that never
        asks would describe a judgement nobody made."""
        feature = _feature(label_fit_count="9/10", label_confidence="high")
        _persist(writer, job, feature,
                 {"category": "semantic", "specific": "legal_terms", "description": ""})
        assert feature.label_fit_count is None
        assert feature.label_confidence is None

    def test_an_overlong_self_assessment_is_clipped_not_fatal(self, writer, job):
        feature = _feature()
        _persist(writer, job, feature,
                 {"category": "semantic", "specific": "legal_terms", "description": "",
                  "fit_count": "7/10 " + "x" * 100,
                  "confidence": "high, because every example is a ruling"})
        assert len(feature.label_fit_count) == 32
        assert feature.label_confidence == "high, because ev"
    ...
    def test_a_refusal_is_a_verdict_not_a_failure(self, writer, job):
        ...
        fit ratio is below 0.6. It is an adjudication and must never be redone —
