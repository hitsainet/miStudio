...
def split_documents(
    ...
    held = choose_held_documents(documents, holdout_fraction, seed)
    # np.isin, not a Python generator over every index: a real extraction is
    # ~3x10^8 indices, which the per-index `in set` loop took minutes to walk.
    is_held = np.isin(flat_indices // seq_len, held)
    return flat_indices[~is_held], flat_indices[is_held]


def choose_held_documents(documents: np.ndarray, holdout_fraction: float, seed: int) -> np.ndarray:
    """Which documents are held out: ``round(len * fraction)`` of them, by ``RandomState(seed)``.

    ONE RULE FOR BOTH TRAINING PATHS. The cached path splits the documents of an
    extraction; the on-the-fly path splits the rows of a tokenization
    (:func:`split_rows`). Both call this, so the same seed and the same row set
    hold out the same rows whichever path reads them.
    """
    ...
    return np.sort(rng.choice(documents, size=n_hold, replace=False))


def split_rows(
    n_rows: int, holdout_fraction: float, seed: int
) -> Tuple[np.ndarray, np.ndarray]:
    """Split a tokenization's rows into ``(train_rows, held_rows)``, both sorted.

    The on-the-fly path's version of :func:`split_documents`: a tokenized row is
    a document (or a packed block of them), and the rows held out are chosen by
    :func:`choose_held_documents` over ``arange(n_rows)`` — what the cached path
    would choose over an extraction of those rows whose every row has a real
    token. A row that turns out to be all padding is held (or trained on) and
    contributes nothing.

    A fraction of 0 gives every row to training.
    """
    rows = np.arange(int(n_rows), dtype=np.int64)
    if not (0.0 <= holdout_fraction < 1.0):
        raise ValueError(
            f"holdout_fraction must be in [0, 1), got {holdout_fraction}"
        )
    if holdout_fraction == 0.0 or rows.size == 0:
        return rows, np.empty(0, dtype=np.int64)
    held = choose_held_documents(rows, holdout_fraction, seed)
    return rows[~np.isin(rows, held)], held.astype(np.int64)


def select_real_tokens(acts, attention_masks):
    """The real-token rows of captured activations, in row-major order. Strict.

    ``acts`` is ``(rows, seq, d)`` or already flattened ``(rows*seq, d)``;
    ``attention_masks`` are the batch's own lists (as :func:`build_padded_batch`
    returns them). The mask is built on the CPU, so it indexes activations on any
    device (HookManager keeps its captures on the CPU while the input ids sit on
    the model's card).

    STRICTER THAN ``real_token_activations`` IN THE TRAINING TASK, deliberately:
    that one trained on the whole batch, pads included, when the mask did not
    match or when every position was padding — tolerable for one step, wrong for
    a buffer that is served for thousands. Here a mismatch raises and an
    all-padding batch yields no rows.
    """
    import torch

    real = torch.as_tensor(attention_masks, dtype=torch.bool).reshape(-1)
    flat = acts.reshape(-1, acts.shape[-1])
    if real.numel() != flat.shape[0]:
        raise ValueError(
            f"attention mask covers {real.numel()} positions but {flat.shape[0]} "
            f"activations were captured; refusing to guess which are padding"
        )
    return flat[real]
