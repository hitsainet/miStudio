...
#: Statuses a worker writes only once its task is running. QUEUED is left out
#: on purpose: a queued row waits behind whatever holds the GPU, so its age says
#: nothing about whether its task is alive.
STARTED_STATUSES = frozenset({"loading", "extracting", "saving"})


def extraction_has_started(status) -> bool:
    """Whether a row's status means its task has begun running.

    THIS WAS ALWAYS FALSE. The janitor asked
    ``str(extraction.status).lower() in ("extracting", "running", "processing")``.
    `status` is an ``SQLEnum(ExtractionStatus)`` column, so it loads as an enum
    member, and ``str()`` of a ``(str, Enum)`` member is ``"ExtractionStatus.
    EXTRACTING"``, not ``"extracting"``. With ``started=False``,
    ``task_looks_alive`` never consults the row's age, and Celery reports PENDING
    for a task whose worker died, so every row that had a task id was treated as
    alive and a dead extraction was never reaped. The test fixture for this
    janitor used the plain string ``"EXTRACTING"``, for which the expression
    happened to work.

    Compares the enum's VALUE, and accepts a plain string too.
    """
    value = getattr(status, "value", status)
    return str(value).lower() in STARTED_STATUSES
        ...
        try:
            ...
            for extraction in stuck_extractions:
                ...
                if extraction.celery_task_id:
                    ...
                    # never be false for a row carrying a task id. A started
                    # row is judged by its own clock, which the merge and
                    # statistics phases keep fresh with a 60 s heartbeat.
                    from src.workers.task_heartbeat import task_looks_alive

                    task_is_running = task_looks_alive(
                        extraction.celery_task_id,
                        extraction,
                        started=extraction_has_started(getattr(extraction, "status", None)),
                    )
                    if task_is_running:
                        logger.info(
                ...
                if not task_is_running:
                    ...
                    db.commit()
                    cleaned_count += 1
                    # Multi-GPU Phase 3: free the card(s) the dead execution
                    # leased — only a lease nothing renews any more.
                    from src.workers.gpu_job import release_reaped_leases

                    release_reaped_leases(extraction.celery_task_id)
