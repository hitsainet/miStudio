...
def estimate_fit_working_mb(
    ...
    if not (hidden and n_layers and vocab and heads):
        logger.warning(
            "%s records no hidden size, layer count, vocabulary or head count, so its fit's working "
            "memory cannot be sized; it is placed with the fixed activation headroom",
