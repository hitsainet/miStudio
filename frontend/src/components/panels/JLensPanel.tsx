...
import { SwapComposer } from '../jlens/SwapComposer';
...
export function JLensPanel() {
  ...
  /**
   * The row whose swap is being composed, or null.
   *
   * A swap used to fire on the click and take its partner from PIN ORDER. The
   * partner is half the experiment and the token whose rank is scored, so it is
   * now typed against the row that was picked rather than configured elsewhere
   * and hoped for.
   */
  const [swapFor, setSwapFor] = useState<
    { token: string; layers: number[]; type: string } | null
  >(null);
  // CLEARED WITH THE MODEL. `setModelId` deliberately drops `pinned`, `meta`
  // and `tokens` because a token belongs to ONE model's vocabulary — and
  // `swapFor` holds exactly such a token plus that model's layer numbers. It
  // survived the switch, so after changing model and re-reading, the composer
  // reappeared pre-filled with the previous model's token and layers.
  // ...AND ON A RE-READ. A new readout replaces the prompt the grid describes,
  // and the composer holds a token and layers from the OLD one. Left open, it
  // would submit the previous readout's coordinates against the new
  // `readoutPrompt` — scoring a forward pass nobody is looking at, which is the
  // same class of mistake as intervening on the draft instead of the read.
  useEffect(() => {
    setSwapFor(null);
  }, [modelId, readoutPrompt]);
  ...
  const layerBudget = () => {
    ...
    return Math.max(1, Math.floor((stack || 1) / 4));
  };
  ...
  const runIntervention = async (
    ...
    lensType?: string,
    /**
     * The partner the reader typed, and layers they chose.
     *
     * When present the layers are USED AS GIVEN: the deepest-quarter trim exists
     * to stop a one-click swap hooking the whole stack, and silently overriding
     * an explicit choice would be a different experiment than the one on screen.
     * The server still records `over_layer_budget`, and the composer says so
     * before the click.
     */
    ...
    try {
      const targeted = explicitTarget ? layers : interventionLayers(layers);
      const accepted = await jlensApi.intervene({
        ...
        // The same card the readout it acts on was placed by.
        gpu,
        ...
        target_token:
          primitive === 'coordinate_swap'
            ? (explicitTarget ?? swapPartnerFor(token))
            : undefined,
        strength: 1,
        k: 4,
    ...
    <div className="flex h-[calc(100dvh-3.5rem)] flex-col px-6 pt-8">
      ...
      <section className="mb-4 shrink-0 rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-800">
        <div className="flex flex-wrap items-end gap-2">
          ...
          <GpuSelect
            ...
            selectClassName="rounded border border-slate-300 bg-white px-2 py-1.5 text-sm text-slate-900 disabled:opacity-50 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100"
          />
          <label
            htmlFor="jlens-unload-after-readout"
            className="flex items-center gap-2 self-end pb-2 text-xs font-medium text-slate-600 dark:text-slate-400"
            title="Loaded is faster for the next readout of this model; unloading frees the GPU for miLLM or another job. A failed readout always frees it."
          >
            <input
              id="jlens-unload-after-readout"
              type="checkbox"
              checked={unloadAfterReadout}
              onChange={(e) => setUnloadAfterReadout(e.target.checked)}
              disabled={isLoading}
              className="h-4 w-4 rounded border-slate-300 dark:border-slate-600"
            />
            Unload model after each readout
          </label>
      ...
      <div className="mb-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <InterventionCard
          ...
          pinned={pinned}
          layers={axis}
          // ONLY A JACOBIAN READ CREDITS THE JACOBIAN ARTIFACT — the same rule
          // `runIntervention` has applied since the ranked-list path existed,
          // and this card never did. In LOGIT_LENS mode `axis` is the whole
          // stack and the artifact may be a partial fit (gemma-4-12b-it is
          // L42-45 of 48), so this was filing `evidence_rung: 2` records under
          // `lens_type: JACOBIAN_LENS` for findings the Jacobian played no part
          // in, at layers it was never fitted at.
          //
          // The worker's coverage refusal turned that silent mis-filing into a
          // visible failure on every run. Gating here fixes the cause rather
          // than the symptom.
          artifactId={
      ...
      {!meta ? (
        ...
        <RankedReadouts
          ...
          // OPENS THE COMPOSER, does not fire. The partner is typed against
          // the row that was picked; it used to be taken from pin order, so
          // re-pinning in a different order ran a different experiment under
          // an identical-looking click.
          onSwap={(t, l, ty) => {
            setInterventionNote(null);
            setSwapFor({ token: t, layers: l, type: ty });
          }}
          // NO PARTNER IS NAMED UP FRONT ANY MORE. The button no longer promises
          // a specific exchange, because the reader chooses it next.
          swapDisabledFor={() => undefined}
        />
        {swapFor && (
          <SwapComposer
            modelId={modelId}
            from={swapFor.token}
            fromLayers={swapFor.layers}
            // THE CLICKED COLUMN'S OWN AXIS, not the union across lens types.
            // `fullSpan` spans every lens, so on a partial Jacobian fit — 
            // gemma-4-12b-it is fitted at L42-45 of 48 — it would offer the
            // whole stack as though the lens covered it.
            lensLayers={meta.layers_by_type[swapFor.type] ?? []}
            budget={layerBudget()}
            busy={interventionBusy}
            onCancel={() => setSwapFor(null)}
            onSubmit={(target, layers) => {
              setSwapFor(null);
