...
that concatenation. This column records which corpus a row came from, which is
what makes per-corpus statistics possible.

CORRECTION (2026-09-22): this docstring originally claimed that two corpora
"contribute overlapping index ranges" and that `sample_index` therefore stops
being an identity key. That was wrong. Because the offset is GLOBAL, indices
stay unique across corpora within one extraction, and every consumer is scoped
by extraction. The column is PROVENANCE, not a collision fix — no consumer
needed changing, and none was changed.
