...
| `POST` | `/{id}/extract-activations` | Start activation extraction (202); config includes layers, hook types, and optional `gpu` (`"auto"` or a GPU UUID) |
...
While an extraction's `status` is `extracting`, `phase` says which step is running: `extracting` (the GPU pass), `merging` (the per-batch files are merged into one file per layer) or `statistics`. Both GET endpoints and the `extraction:progress` message carry it, and it is stored on the extraction, so a reloaded page shows the same step. It is cleared when the extraction completes; a failed or cancelled extraction keeps the step it stopped in.
