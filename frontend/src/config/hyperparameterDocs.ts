...
export const HYPERPARAMETER_DOCS: Record<string, HyperparameterDoc> = {
  ...
  lr_decay_steps: {
    ...
    purpose: 'Linearly decays the learning rate to 0 over the final N training steps. Off (0) by default.',
    description:
      'After warmup the learning rate is constant. With lr_decay_steps = N, the last N steps ramp it linearly from the full rate down to 0, so the final weights settle instead of stopping mid-update. warmup_steps + lr_decay_steps must not exceed total_steps. Dead-latent resampling is skipped inside the decay window, where a new latent would have no learning rate left to train with.',
    examples: [
      {
        value: 0,
        effect: 'Off: constant learning rate to the end (default)',
        useCase: 'Matches every run trained before this setting existed',
      },
      {
        value: 10000,
        effect: 'Decay over the last 10k steps',
        useCase: 'A 50k-step run: the final 20% anneals',
      },
    ],
    recommendations: [
      'Default: 0 (off)',
      'Common choice: 10-20% of total_steps',
      'A resumed run continues the curve where it stopped',
    ],
    relatedParams: ['learning_rate', 'warmup_steps', 'total_steps', 'resample_interval'],
  },
  ...
  dead_neuron_threshold: {
    ...
    purpose: 'Consecutive training steps a latent must go without firing before it counts as "dead" for resampling.',
    description:
      'A latent is dead once it has not fired on a single token of the batch for this many consecutive training steps (each step is batch_size tokens). This is the value resampling reads: at every resample_interval, latents past it are re-initialised. The dead-neuron count shown in training metrics is a separate, faster-moving activity estimate.',
    examples: [
      {
        value: 1000,
    ...
    recommendations: [
      'Form default: 10,000 steps (the API default is 1,000)',
      'Set to 5-10% of total_steps',
      'If many dead neurons persist: decrease l1_alpha (or sparsity_coeff for JumpReLU)',
      'Resampling helps but a well-tuned sparsity penalty is more important',
    ],
    relatedParams: ['resample_dead_neurons', 'resample_interval', 'l1_alpha'],
  },
  ...
  resample_dead_neurons: {
    ...
    description:
      'When enabled, every resample_interval steps the latents that have not fired for dead_neuron_threshold steps are re-initialised from high-loss inputs (Anthropic recipe): the encoder row and decoder column point along the normalised input, the encoder bias is zeroed, a JumpReLU threshold is set so the latent fires on that input, and the optimizer moments for those weights are reset. Every framework except TopK (which uses its auxiliary loss) supports it. It is skipped inside the LR decay window. Not a substitute for tuning the sparsity penalty.',
    examples: [
      {
        value: 'true',
