...
class ModelDownloadRequest(BaseModel):
    ...
    gpu: GpuRequestStr = Field(
        AUTO,
        description=GPU_REQUEST_DESCRIPTION
        + " The download loads the model once, to read its architecture, on this card.",
    )
...
class ActivationExtractionRequest(BaseModel):
    ...
    max_seq_length: Optional[int] = Field(
        None, ge=1, le=8192,
        description=(
            "Truncate sequences to this length. Omit to use whatever the "
            "tokenization produced. This replaces a hardcoded 512 cap that "
            "silently discarded everything beyond it."
        ),
    )
    micro_batch_token_budget: Optional[int] = Field(
        None, ge=1,
        description=(
            "Tokens (rows x seq_len) per forward pass. The micro-batch shrinks "
            "as the window grows so memory stays bounded. Defaults to "
            "micro_batch_size x 512, which preserves the old envelope."
        ),
    )
    top_k_examples: Optional[int] = Field(10, ge=1, le=100, description="Number of top activating examples to save")
    # Was `gpu_id: int = 0`, an NVML index. The frontend already sent `gpu`, so
    # the value it chose was dropped as an unknown field and every extraction
    # ran on index 0 — the 3080 Ti since the second card was added.
...
class ActiveExtractionResponse(BaseModel):
    ...
    phase: Optional[str] = Field(None, description="Step of a running extraction: extracting, merging or statistics")
