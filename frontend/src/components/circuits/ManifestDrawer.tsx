...
export function ManifestDrawer({
  ...
  // A reproduction re-runs the interventions, so it is GPU work with its own card choice.
  ...
  const reproduce = () => {
    ...
    setReproChildId(null);
    circuitsApi
      .reproduceManifest(manifestId, { gpu })
      .then((r) => {
              ...
              <div className="border-t border-slate-300 dark:border-slate-700/60 pt-3 space-y-2">
                {manifest.kind === 'edge_batch' && (
                  <div className="max-w-xs">
                    <GpuSelect
                      id="reproduce-gpu"
                      label="GPU — reproduction run"
                      value={gpu}
                      onChange={setGpu}
                      disabled={reproBusy}
                      // This drawer reproduces through the validation endpoint,
                      // whose edge-batch reproduction runs split. It cannot
                      // reproduce any other kind (calibration manifests have
                      // their own endpoint, not called from here). Decided by
                      // kind, not by the render guard above, so widening that
                      // guard cannot offer "All GPUs" to another kind unchecked.
                      allowSplit={manifest.kind === 'edge_batch'}
                      labelClassName="block text-xs text-slate-600 dark:text-slate-400 mb-1"
                      selectClassName="w-full rounded bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 px-2 py-1.5 text-xs text-slate-900 dark:text-slate-100 disabled:opacity-50"
                    />
                  </div>
                )}
                <button
