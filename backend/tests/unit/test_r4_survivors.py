...
class TestTheMaskIsAppliedNotJustComputed:
    ...
    def test_the_training_loop_indexes_by_the_mask(self):
        """The on-the-fly buffer WRITES the filtered rows, not the captured tensor.

        This scraped for `acts_flat = kept`, then pinned `acts_flat =
        real_token_activations(...)` in the step loop. Remediation item 3
        (2026-09-15) moved on-the-fly extraction into
        `ModelActivationSource._forward_rows`; what can still be lost is the APPLY
        — filtering and then writing the unfiltered capture. So the tree must show
        `real = activation_mask.select_real_tokens(...)` and `real` written into
        the storage. Driven in test_model_activation_source.py and
        test_training_data_path_e2e.py (no served vector is a padding position).
        """
        from src.services import model_activation_source

        tree = ast.parse(inspect.getsource(model_activation_source))
        forward = next(
        ...
        filtered = [
            ...
            and [getattr(t, "id", None) for t in node.targets] == ["real"]
            and isinstance(node.value, ast.Call)
            and getattr(node.value.func, "attr", None) == "select_real_tokens"
        ]
        written = [
            node for node in ast.walk(forward)
            if isinstance(node, ast.Assign)
            and isinstance(node.targets[0], ast.Subscript)
            and "real" in {n.id for n in ast.walk(node.value) if isinstance(n, ast.Name)}
        ]
        assert len(filtered) == 1 and len(written) == 1, (
...
class TestTheHoldoutIsEvaluatedNotJustDeleted:
    ...
    def test_the_holdout_is_evaluated_under_no_grad(self):
        ...
        This was a text window around `holdout_activations.items()` that had to
        contain `torch.no_grad()` and `.eval()`. Remediation item 2 (2026-09-15)
        moved the evaluation into `holdout_evaluation.evaluate_holdout`, whose
        no-grad, eval-mode and restore-on-failure behaviour is DRIVEN in
        test_holdout_evaluation.py::TestTheEvaluationLeavesTrainingAlone. What is
        left to pin here is that the log step calls it, per held-out set, with the
        ...
        calls = [
            ...
            if isinstance(node, ast.Call) and getattr(node.func, "attr", None) == "evaluate_holdout"
        ]
        assert calls == [["model_ref", "held", "device", "holdout_eval_chunk_tokens"]], calls
...
class TestTheRetryUsesTheIdItAdvertises:
    ...
    MUTATION CONTROL G2 (2026-09-14, multi-GPU Phase 3; restored byte-identically,
    sha256 and `git diff` checked): drop `extraction_id=new_extraction_id` from the
    routed `gpu_delay(extract_activations)(...)` call -> the generated id test red.
    """
    ...
    def _retry_delay_kwargs():
        ...
        for node in ast.walk(tree):
            ...
            # Multi-GPU Phase 3: `gpu_delay(extract_activations)(...)`, the GPU
            # routing helper's .delay-shaped form; the task's keywords are on
            # the outer call.
            routed = (
                isinstance(node.func, ast.Call)
                and isinstance(node.func.func, ast.Name)
                and node.func.func.id == "gpu_delay"
                and bool(node.func.args)
                and isinstance(node.func.args[0], ast.Name)
                and node.func.args[0].id == "extract_activations"
            )
            if direct or routed:
                out.append({
                    kw.arg: (
