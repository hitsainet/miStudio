...
class LabelingContextFormatter:
    ...
    def format_mistudio_context(
        ...
        # HOW STRENGTH IS STATED, decided once for the whole block.
        #
        # `{:.3f}` is fine while every example comes from the top of the
        # distribution, and stops being fine the moment the range widens: this
        # function's own docstring shows two DISTINCT examples both printing
        # "activation: 0.007". A stratified span whose weakest member renders
        # identically to its strongest tells the judge nothing changed.
        #
        # The peak is taken from the POSITIVE examples and reused for the weak
        # block. Normalising the weak block against its own maximum would render
        # its strongest member as 100%, which is the opposite of what it is
        # there to show.
        display = template_config.get('activation_display', 'absolute')
        peak = max(
            (e.get('max_activation') or 0.0) for e in examples
        ) if examples else 0.0

        def _strength(value: float) -> str:
            """One example's strength, as the configured display states it."""
            if display == 'percent_of_max' and peak > 0:
                return f"strength: {100.0 * value / peak:.0f}% of this feature's peak"
            return f"activation: {value:.3f}"
        ...
        for idx, example in enumerate(examples, start=1):
            ...
            formatted_lines.append(
                f"Example {idx} ({_strength(max_activation)}): {context}"
            )

        # A CONTRAST BLOCK THAT IS NOT BELOW THE POSITIVES IS A FALSE CLAIM.
        #
        # The weak examples are the feature's lowest RETAINED activations,
        # chosen without reference to which rows the prompt is showing. Under
        # top_k that is safe: positives are ranks 1-10, weak are the bottom
        # five. Under stratified on a 20-row extraction the positives are ranks
        # 1,3,5…19 and the weak block is 20,18,16,14,12 — INTERLEAVED. Measured
        # on the eb48 shape: four of the five passages captioned "lower
        # activation" are strictly stronger than four of the ten presented as
        # positives, and under percent_of_max the prompt prints positives
        # ending at 82% above a "weaker" block leading with 89%.
        #
        # A silent, empty contrast block is honest. A false one is not — and
        # this block exists precisely because the previous wording made a claim
        ...
        if negative_examples:
            strongest_weak = max(
                (e.get('max_activation') or 0.0) for e in negative_examples
            )
            if strongest_weak >= weakest_shown:
                # DEBUG, not WARNING: on a bulk run with contrast enabled this
                # is per feature, and 53,088 warning lines is how a log stops
                # being read. The condition is a property of the sampling
                # configuration, not an incident — one occurrence tells you
                # everything the 53,088th does.
                logger.debug(
                    "feature %s: dropping the contrast block — its strongest "
                    "member (%.4f) is not below the weakest shown example "
                    "(%.4f), so captioning it 'lower activation' would be "
                    "false. This happens when the sampled positives reach far "
                    "enough down the stored set to interleave with them.",
                    feature_id, strongest_weak, weakest_shown,
                )
                negative_examples = None

        # Format the weaker examples if provided
        if negative_examples:
            formatted_lines.append("")  # Blank line separator
            # NOT "NEGATIVE EXAMPLES", AND NOT "does not activate".
            #
            # These rows are the feature's OWN stored activations, taken from
            # the bottom of what was retained — so they are weak POSITIVES, not
            # non-activating text. On the L46 extraction the smallest stored
            # activation is 0.67, strictly positive; the detection scorer's own
            # docstring records that there is no encode-on-text service and
            # therefore no way to certify any passage as non-activating.
            #
            # Telling a judge that activating text does not activate invites it
            # to contrast against something that is not a contrast. The honest
            # claim is "weaker", never "absent" — the same stance
            # `negative_ceiling` already takes.
            formatted_lines.append(
                "WEAKER EXAMPLES (same feature, lower activation). "
                "These also activate the feature, more weakly. They are NOT "
                "non-activating text:"
            )
    ...
    def format_anthropic_logit(
        ...
        logit_text_parts = []
        if logit_effects:
            # ACCEPT BOTH KEYS, and read the producer's first.
            #
            # Every producer in the codebase writes `top_promoted` /
            # `top_suppressed` — labeling_service.py:1598-1601 and again at
            # :1798-1801, the block being duplicated across the OPENAI and
            # OPENAI_COMPATIBLE branches — while this reader asked only for
            # `promoted`/`suppressed`. The Anthropic template's logit section
            # has therefore always printed "No logit effects available.", which
            # is the quiet kind of wrong: the block renders, so nothing looks
            # broken.
            #
            # A strict rename to `top_*` would only redden the existing fixtures
            # in test_labeling_context_formatter.py, which are built with the
            # legacy key and prove nothing about the producer. Accepting both
            # fixes the live path and keeps those green; the test that BITES is
            # the new one built with the producer's key.
            promoted = (
                logit_effects.get('top_promoted')
                or logit_effects.get('promoted')
                or []
            )
            suppressed = (
                logit_effects.get('top_suppressed')
                or logit_effects.get('suppressed')
                or []
            )

            # `or 10`, NOT `get(..., 10)`.
            #
            # Both count columns are nullable with no default, so the key is
            # PRESENT with value None and the `get` default never fires — and
            # `promoted[:None]` returns the whole list. This block rendered for
            # the first time in this arc (the reader was asking for a key no
            # producer writes), so the latent slice bug became reachable with
            # it: a template with NULL counts would dump the entire logit-lens
            # token list into the prompt instead of ten tokens.
            if promoted:
                top_count = template_config.get('top_promoted_tokens_count') or 10
            ...
            if suppressed:
                top_count = template_config.get('top_suppressed_tokens_count') or 10
    ...
    def _clean_token(token: str) -> str:
        ...
        """
        # DECODE BEFORE STRIPPING. This text goes into the JUDGE'S PROMPT, so a
        # mangled token is not a cosmetic problem — it is what the model reads
        # and labels from. Stripping 'Ġ' handles the space marker and nothing
        # else: 'âĢĻs' is the byte-level spelling of "’s" and was the SECOND
        # most common prime token on the 16k L12 extraction (3,015 rows), so a
        # judge was being shown mojibake for most possessives and contractions.
        from src.utils.token_filters import decode_byte_level_token

        cleaned = decode_byte_level_token(token)
        ...
        # Decoding turns the space marker into a REAL space, which the marker
        # lstrip above cannot see. Leading only — a trailing space is content.
