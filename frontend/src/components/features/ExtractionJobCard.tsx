...
import { LabelingCoverageStrip } from '../labeling/LabelingCoverageStrip';
...
export const ExtractionJobCard: React.FC<ExtractionJobCardProps> = ({
  ...
  // A deliberate stop is its OWN terminal state — not active, not completed,
  // not failed. Without this the card had no branch for it, so a cancelled job
  // rendered as "Unknown" and, because the delete affordance below is gated on
  // `isCompleted || isFailed`, could never be removed from the list at all.
  ...
  return (
    <div className={`${COMPONENTS.card.base} px-4 py-3 ${COMPONENTS.border.hover} transition-colors`}>
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 flex-wrap flex-1 min-w-0">
            ...
            <span className={`text-xs px-2 py-0.5 rounded ${
              ...
              isFailed ? 'bg-red-500/10 text-red-400' :
              isDeleting ? 'bg-orange-500/10 text-orange-400' :
              // Slate, not red and not yellow: an operator stop is neither a
              // failure nor something still pending. The backend makes the same
              // distinction by writing CANCELLED rather than FAILED.
              isCancelled ? 'bg-slate-500/10 text-slate-400' :
              isActive ? 'bg-blue-500/10 text-blue-400' :
              'bg-yellow-500/10 text-yellow-400'
            }`}>
              {extraction.status === 'completed' ? 'Completed' :
               extraction.status === 'failed' ? 'Failed' :
               extraction.status === 'cancelled' ? 'Cancelled' :
        ...
        <div className="flex items-center gap-2 flex-shrink-0 ml-2">
          ...
          {isCompleted && (
            <StartLabelingButton
              ...
              // What the run retained per feature. The labeling control bounds
              // itself by this, because asking for more than was stored returns
              // fewer rows silently.
              storedExamples={extraction.config?.top_k_examples}
              onSuccess={() => {
          ...
          {/* `isCancelled` belongs here for the same reason `isFailed` does: the
              job has reached a terminal state and holds nothing worth keeping.
              The backend agrees — delete_extraction refuses only QUEUED and
              EXTRACTING, so a cancelled row was always deletable by API and
              only the button was missing. */}
          {(isCompleted || isFailed || isCancelled) && !isDeleting && onDelete && (
            <button
              type="button"
              onClick={() => {
      ...
      {/*
        * Labeling coverage. The "what still needs doing?" answer that did not
        * exist: the OUTCOME of a labeling attempt was recorded nowhere, so a
        * failure written as a fake label read as finished work to every count in
        * the product — 16,824 of them across the estate, one extraction carrying
        * 2,161 since April with nothing on any screen to say so.
        */}
      {isCompleted && !isDeleting && (
        <div className="mb-3">
          <LabelingCoverageStrip extractionId={extraction.id} />
        </div>
      )}
      ...
      {/* Statistics boxes shown when expanded.
          There was an "Interpretable" tile here. It reported the share of
          features whose activation-consistency score exceeded 0.5 — a score that
          measures activation SHAPE, never which tokens fire, and is highest for
          evidence repeating a single token (98.8% of such features passed). It
          was not a measure of interpretability, so it is not shown as one. */}
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <button
                            ...
                            /*
                             * Aqua silently PROTECTS a row from bulk relabelling
                             * and had no tooltip or legend anywhere, so the one
                             * colour with a consequence was the one nothing
                             * explained.
                             */
                            title={
                              feature.star_color === 'aqua'
                                ? 'Enhanced labeling completed. Bulk labeling will not overwrite this label.'
                                : feature.star_color === 'purple'
                                ? 'Enhanced labeling in progress.'
                                : feature.is_favorite
                                ? 'Starred by you.'
