...
class Training(Base):
    ...
    #: LEGACY Fraction of Variance Unexplained: var(x - x_hat) / var(x) over every
    #: element with ONE global mean. Kept with that meaning so no stored value
    #: changes what it says; it reads low on activations with large constant
    #: offset dimensions. Before 2026-09-15 only JumpReLU reported it.
    current_fvu = Column(Float, nullable=True)
    #: The headline FVU: sum|x - x_hat|^2 / sum|x - mu|^2, mu per dimension, raw
    #: space. 0.0 = perfect reconstruction, 1.0 = no better than predicting the
    #: mean vector. NULL for runs recorded before it existed.
    current_fvu_centred = Column(
        Float,
        nullable=True,
        comment=(
    ...
    #: The post-run evaluation (remediation item 6): what the SAEs cost the MODEL
    #: on blocks the training never read. One document — status, config, sources,
    #: per-layer CE/KL/L0/FVU — instead of CE numbers overloaded onto
    #: `training_metrics` columns. Shape: `services/training_evaluation.py`.
    evaluation = Column(
        JSONB,
        nullable=True,
        comment=(
