...
from ....services.gpu_dispatch import dispatch_gpu_task
...
from ....services.training_service import (
    TrainingService,
    UnprocessableTrainingRequest,
    InsufficientCheckpointDisk,
)
from ....services.checkpoint_service import CheckpointService
from ....services.task_queue_service import TaskQueueService
from ....workers.training_tasks import extraction_ids_of, train_sae_task, resume_training_task
...
async def create_training(
    ...
    """
    # Before the try: its `except Exception` would turn this 400 into a 500.
    # An unknown card is refused here, before a row exists or a task is queued.
    # So is "all" for a run on cached activations: it loads no base model and
    # the worker never splits it. `extraction_ids_of` is the worker's own test,
    # so the endpoint and the placement cannot disagree about which runs split.
    gpu_request = resolve_gpu_request(
        training.gpu, can_split=extraction_ids_of(training) is None
    )

    try:
        db_training = await TrainingService.create_training(
            db, training, gpu_request=gpu_request
        )
        ...
        dispatch_gpu_task(train_sae_task, gpu_request=gpu_request, args=[db_training.id], task_id=task_id)

        # Refresh to get updated record
        db_training = await TrainingService.get_training(db, db_training.id)

        return db_training
    except InsufficientCheckpointDisk as e:
        # The request is well-formed and the MACHINE cannot serve it: 409 Conflict,
        # not 400. Before this, a training that could not possibly fit was accepted,
        # queued, given a GPU, and failed hours later with its checkpoints — the
        # things a resume needs — written onto a volume with no room left.
        raise HTTPException(status_code=409, detail=str(e))
    except UnprocessableTrainingRequest as e:
        # The request is valid and the named data cannot serve it (review R1-A, A5).
...
@router.post("/{training_id}/evaluate", status_code=202)
async def evaluate_training(
    training_id: str = Path(..., description="Training job ID"),
    token_budget: Optional[int] = Query(
        None, ge=1, le=16_777_216,
        description="Tokens to read from blocks the training never read; defaults to the "
                    "training's evaluation_token_budget",
    ),
    gpu: Optional[str] = Query(
        None, description='GPU to run on: "auto", "all", an index or a UUID; defaults to the '
                          "training's own request",
    ),
    force: bool = Query(
        False, description="Queue even though an evaluation is recorded as pending or running",
    ),
    db: AsyncSession = Depends(get_db),
):
    """Re-run the post-run evaluation of a completed training (remediation item 6).

    Queues a GPU job that loads the training's base model and its exported SAEs,
    reads blocks each extraction never read, and records base / spliced /
    mean-ablated / zero-ablated CE, KL, L0 and centred FVU in
    ``trainings.evaluation``. Returns 202; the result arrives on the training.
    """
    from datetime import datetime, timezone

    from ....core.config import settings
    from ....services.training_evaluation import ACTIVE_STATUSES, EVALUATION_VERSION, STATUS_PENDING
    from ....workers.training_evaluation_tasks import evaluate_training_task

    db_training = await TrainingService.get_training(db, training_id)
    if not db_training:
        raise HTTPException(status_code=404, detail=f"Training not found: {training_id}")
    if db_training.status != TrainingStatus.COMPLETED.value:
        raise HTTPException(
            status_code=409,
            detail=f"Only a completed training can be evaluated; this one is {db_training.status}",
        )
    current = db_training.evaluation or {}
    if current.get("status") in ACTIVE_STATUSES and not force:
        raise HTTPException(
            status_code=409,
            detail=(
                f"An evaluation is already {current.get('status')} "
                f"(task {current.get('task_id')}). Re-send with force=true if it is stuck."
            ),
        )
    ...
    if not community_dir.exists():
        raise HTTPException(
            status_code=409,
            detail="This training has no Community Standard export to evaluate; finalize it first",
        )

    # Resolved before anything is written: an unknown card is a 400 with nothing queued.
    gpu_request = resolve_gpu_request(gpu or db_training.gpu_request or "auto", can_split=True)
    task_id = str(uuid.uuid4())
    db_training.evaluation = {
    ...
    dispatch_gpu_task(
        ...
        gpu_request=gpu_request,
        kwargs={"training_id": training_id, "gpu_request": gpu_request, "token_budget": token_budget},
        task_id=task_id,
    )
    return {
        "data": {
...
@router.get("/{training_id}/evaluation")
async def get_training_evaluation(
    training_id: str = Path(..., description="Training job ID"),
    db: AsyncSession = Depends(get_db),
):
    """The training's recorded evaluation, or null when it has never been evaluated."""
    db_training = await TrainingService.get_training(db, training_id)
    if not db_training:
        raise HTTPException(status_code=404, detail=f"Training not found: {training_id}")
    return {"data": db_training.evaluation}
    ...
    try:
        if action == "pause":
            ...
            if db_training is None:
                await _refuse_pausing_a_completed_training(db, training_id)
        elif action == "resume":
            ...
            if db_training is None:
                db_training, message = await _stop_the_evaluation_of_a_completed_training(
                    db, training_id, action
                )
            ...
            if db_training is None:
                ...
                from ....services.training_finalize_service import (
                    full_length_export_step,
                ...
                total_steps = (getattr(db_training, "hyperparameters", None) or {}).get("total_steps")
                # Only claim a finalize if there is actually a checkpoint to
                # finalize from — otherwise the user is told their SAE was saved
                # when nothing was written. And never over the loop's own export
                # (review R3-A, R2D-1): a Stop that lands as the run finishes finds
                # the final weights already saved, and the run is marked completed.
                if full_length_export_step(training_id, total_steps) is not None:
                    message = (
                        "Training stopped as it finished: its full-length export is saved, so "
                        "nothing is finalized from a checkpoint and the run is marked completed"
                    )
                elif list_checkpoint_steps(training_id):
                    # Through the same helper as /finalize, so Stop & Finalize is
                    # visible in Active Operations too. Leaving this on a bare
                    # `.delay` would make one finalize route visible and the other
                    # not, which is worse than both being invisible.
                    await _queue_visible(
                        db,
                        task_type="training_finalize",
                        training_id=training_id,
                        dispatch=lambda task_id: (
                            finalize_training_from_checkpoint_task.apply_async(
                                args=[training_id, None], task_id=task_id
                            )
                        ),
                    )
...
async def _stop_the_evaluation_of_a_completed_training(db, training_id: str, action: str):
    """A Stop on a COMPLETED training cancels only its pending or running evaluation (review R3-A, R2D-1).

    Decided behaviour: once a training has written its full-length export it is COMPLETED
    and stays so; a Stop (or Stop & Finalize) stops the evaluation that follows and
    finalizes nothing. The request goes into the evaluation's record, which the
    evaluation reads between batches. Returns ``(training, message)``, or ``(None, None)``
    when there is nothing to stop (the caller's 400 stands).
    """
    from ....services.training_evaluation import request_evaluation_stop

    db_training = await TrainingService.get_training(db, training_id)
    if db_training is None or db_training.status != TrainingStatus.COMPLETED.value:
        return None, None
    document = request_evaluation_stop(db_training.evaluation, requested_by=action)
    if document is None:
        return None, None
    db_training.evaluation = document
    await db.commit()
    message = (
        "This training already completed and its full-length export is saved; only its "
        f"{document.get('trigger', 'post_run').replace('_', '-')} evaluation is being stopped"
    )
    if action != "stop":
        message += ", and nothing is finalized from a checkpoint"
    return db_training, message


async def _refuse_pausing_a_completed_training(db, training_id: str) -> None:
    """A Pause on a COMPLETED training is a 409 with the reason (review R3-A, R2D-10); otherwise the caller's 400 stands."""
    from ....services.training_evaluation import ACTIVE_STATUSES

    db_training = await TrainingService.get_training(db, training_id)
    if db_training is None or db_training.status != TrainingStatus.COMPLETED.value:
        return
    detail = "This training is completed and its export is saved, so there is nothing to pause."
    if (db_training.evaluation or {}).get("status") in ACTIVE_STATUSES:
        detail += " Its evaluation is still running: send stop to cancel only the evaluation."
...
async def get_training_metrics(
    ...
    aggregate_only: bool = Query(
        False,
        description=(
            "Only the aggregate rows (layer_idx NULL), one per logged step. Without it a "
            "window of rows is shared by every SAE and held-out row of every hook type."
        ),
    ),
    db: AsyncSession = Depends(get_db)
):
...
async def _queue_visible(
    db: AsyncSession, *, task_type: str, training_id: str, dispatch
) -> str:
    """Record the row, THEN dispatch, and return the Celery task id.

    WHY THIS EXISTS. Finalize and prune created no `task_queue` row, so neither
    appeared in Active Operations: an operator pressed Finalize, got a 202, and
    watched a page that showed nothing running. The work was happening; the only
    evidence was the worker log.

    WHY IT IS ONE HELPER AND NOT A LINE AT EACH CALL SITE. The ordering is the
    whole point — the row must exist before the task can move it — and this
    repo has already paid for that assumption living apart from the code that
    depends on it: a guard once required a row "created by the API before
    dispatch" which nothing created, and every UI extraction was silently
    refused for a week. Owning both halves here means a new dispatch site cannot
    get the order wrong, and there is one place to read it.

    THE CELERY ID IS MINTED HERE, before either half. `update_task_status` takes
    no task id, so the row could otherwise only learn it in a second write after
    dispatch — and a task that starts in between would be moving a row that does
    not yet name it. Minting it first means the row carries the real id from the
    moment it exists.

    The row is created `queued`; the task moves it. A worker that dies writes
    nothing, which is what the endpoint's heartbeat reconciliation expects.
    """
    ...
    try:
        ...
        # THE ROW IS VISIBILITY; THE DISPATCH IS THE WORK. A finalize that an
        # operator asked for must not die because its Active-Operations row could
        # not be written — that would turn a bookkeeping failure into a lost
        # operation, which is the wrong way round. The order is still row-then-
        # dispatch on the path that matters, so a task cannot move a row that does
        # not exist yet.
        logger.exception(
            "Could not record a %s task_queue row for %s; dispatching anyway, so "
            "the operation will not appear in Active Operations",
            task_type, training_id,
        )
    dispatch(celery_id)
    return celery_id
...
async def prune_checkpoints_now(
    ...
    task_id = await _queue_visible(
        db,
        task_type="checkpoint_prune",
        training_id=training_id,
        dispatch=lambda task_id: prune_single_training_task.apply_async(
            args=[training_id], task_id=task_id
        ),
    )
    return {
        "data": {"training_id": training_id, "task_id": task_id, "status": "queued"}
    }
...
async def finalize_training(
    ...
    # NEVER OVER THE LOOP'S OWN EXPORT, force or not (review R3-A, R2D-1). `force` still
    # re-finalizes a run that was finalized early; the final weights are not replaceable.
    from ....services.training_finalize_service import full_length_export_refusal

    refusal = full_length_export_refusal(
        training_id, (getattr(db_training, "hyperparameters", None) or {}).get("total_steps")
    )
    if refusal:
    ...
    task_id = await _queue_visible(
        ...
        task_type="training_finalize",
        training_id=training_id,
        dispatch=lambda task_id: finalize_training_from_checkpoint_task.apply_async(
            args=[training_id, checkpoint_step], task_id=task_id
        ),
    )
    return {
        "data": {
