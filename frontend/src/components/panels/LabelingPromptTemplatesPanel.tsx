...
export function LabelingPromptTemplatesPanel() {
  ...
  // ONE source of truth for the placeholder rule, shared with the backend's
  // validator (`labeling_prompt_template.py`: either `{examples_block}` or
  // `{tokens_table}`). Both call sites below read it, so the rule cannot drift
  // between create and edit — and the constant it reads is no longer the dead,
  // single-placeholder one that named only `{tokens_table}` (OSD-16).
  const missingPlaceholder = (template?: string) =>
    !LabelingPromptTemplateConstraints.requiredPlaceholders.some(
      (placeholder) => template?.includes(placeholder)
    );

  const PLACEHOLDER_ERROR =
    `User prompt template must contain one of: ${LabelingPromptTemplateConstraints.requiredPlaceholders.join(', ')}`;

  // Handle create template
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      if (missingPlaceholder(formData.user_prompt_template)) {
        showNotification('error', PLACEHOLDER_ERROR);
        return;
      }
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      // Only when the field is being changed: an update that omits it keeps the
      // stored template, which already passed this rule.
      if (formData.user_prompt_template && missingPlaceholder(formData.user_prompt_template)) {
        showNotification('error', PLACEHOLDER_ERROR);
        return;
      }
