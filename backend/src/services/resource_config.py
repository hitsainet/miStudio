...
class ResourceConfig:
    ...
    @staticmethod
    def get_system_resources(device: Any = None) -> Dict[str, Any]:
        """Get current system resource availability.

        Args:
            device: The GPU to describe — a worker's placed ``torch.device``, a
                GPU UUID or index, or ``None``/``"auto"`` for the card Auto
                would pick (the most free memory). This read card 0 whatever the
                job used; since 2026-09-13 card 0 is the 12 GB card.

        The ``gpu_*`` fields describe that one card, as before. ``gpu_index``
        and ``gpu_uuid`` name it, and ``gpu_memory_available_gb`` is its live
        free memory as NVML reports it (every process counted, miLLM's too).
        """
        from ..utils.resource_estimation import gpu_memory_view
        ...
        view = gpu_memory_view(device)
        if view is not None:
            resources.update({
                "gpu_available": True,
                "gpu_index": view["index"],
                "gpu_uuid": view["uuid"],
                "gpu_name": view["name"],
                "gpu_total_memory_gb": view["total_bytes"] / (1024**3),
                "gpu_memory_allocated_gb": view["allocated_bytes"] / (1024**3),
                "gpu_memory_reserved_gb": view["reserved_bytes"] / (1024**3),
                "gpu_memory_available_gb": view["free_bytes"] / (1024**3),
            })
        else:
    ...
    def calculate_extraction_config(
        ...
        Args:
            ...
            device: The GPU the job runs on; see :meth:`get_system_resources`.
        ...
        """
        resources = cls.get_system_resources(device)
...
def _usable_gpu_memory(device: Any) -> Optional[Tuple[int, int, str]]:
    """``(free_bytes, total_bytes, where)`` a load onto ``device`` can use.

    ``"auto"`` lets accelerate spread the weights over every visible card, so a
    load can use the free memory of all of them together. A named CUDA device
    can use only its own — checking card 0 for a job placed on card 1 is the
    single-GPU assumption this replaced. ``"cuda"`` with no index is the
    current device, which :func:`gpu_placement.place_job` sets.

    A list or tuple of torch indices is a SPLIT placement's cards
    (``Placement.max_memory``'s keys): the load can use those cards together
    and no others, so a card the split was not given is not counted.

    None when there is nothing to check: no CUDA, or a non-CUDA device.
    """
    import torch
    ...
    if isinstance(device, (list, tuple)):
        indices = [int(index) for index in device]
        pairs = [torch.cuda.mem_get_info(index) for index in indices]
        where = "GPUs " + " + ".join(
            f"cuda:{index} ({torch.cuda.get_device_name(index)})" for index in indices
        ) + " together"
        return sum(free for free, _ in pairs), sum(total for _, total in pairs), where
    if str(device) == "auto":
        count = torch.cuda.device_count()
        pairs = [torch.cuda.mem_get_info(index) for index in range(count)]
        where = "this GPU" if count == 1 else f"the {count} visible GPUs together"
        return sum(free for free, _ in pairs), sum(total for _, total in pairs), where
    try:
    ...
    return free, total, f"cuda:{index} ({torch.cuda.get_device_name(index)})"
...
def preflight_gpu_capacity(
    *,
    ...
    if needed_gb <= free_gb:
        logger.info(
            "GPU preflight OK for %s on %s: ~%.1f GB needed (%.1f GB weights at %s "
            "+ %.1f GB headroom), %.1f GB free",
            model_name, where, needed_gb, weights_gb, quantization,
    ...
    remedy = (
        ...
        if suggestion
        else f"No supported quantization fits {where}; use a smaller model."
    )

    raise VRAMInsufficientError(
        f"{model_name} does not fit on {where}. "
