...
class TestGpuTasksAreRoutedToTheGpuQueue:
    ...
    def test_each_cards_steering_queue_is_consumed_by_that_cards_worker(self, tmp_path, monkeypatch):
        """Multi-GPU Phase 3: per-card steering workers.

        The endpoint routes a request to `steering_queue_option(card)`; the worker
        `spawn_card_worker(card)` starts must consume exactly that queue (and the
        legacy `steering` queue it drains). Executed with a recording Popen — the
        per-card argv is computed, so a literal scrape cannot see it.
        """
        from src.api.v1 import steering_workers
        from src.services import steering_worker_card

        monkeypatch.setattr(steering_worker_card, "_run_dir", lambda: tmp_path)
        card = "GPU-247aa582-0d1b-e161-8156-983ed1fefc57"
        seen = []
        steering_workers.spawn_card_worker(
            card, popen=lambda argv, **kwargs: seen.append(argv) or type("P", (), {"pid": 1})())
        steering_workers.SPAWNED_PIDS_BY_CARD.clear()

        (argv,) = seen
        consumed = argv[argv.index("-Q") + 1].split(",")
        assert steering_workers.steering_queue_option(card)["queue"] in consumed
        assert "steering" in consumed
