...
class LabelingTrialService:
    ...
    async def start_trial(
        ...
        # A TRIAL HAS NO LOCAL JUDGE. `run_trial` always asks an OpenAI-compatible
        # client, so a 'local' trial used to be accepted, queued, and then sent
        # to api.openai.com with api_key "unused" — a failure that says nothing
        # about local models, after the panel was already locked. Refused here,
        # before any read or row.
        if config.get("labeling_method") == "local":
            raise TrialError(
                "labeling_method 'local' is not supported for trials: a trial "
                "judges through an OpenAI-compatible endpoint. Use "
                "'openai_compatible' (miLLM serves local models that way)."
            )
