...
from .logit_lens_service import get_logit_lens_service, logit_lens_device, LogitLensResult
...
class LocalPushConfig:
    ...
    # The torch device the logit lens runs on, placed by the push task. None
    # means auto, resolved when the lens is computed.
...
class NeuronpediaLocalPushService:
    ...
    async def _compute_dashboard_data_if_needed(
        ...
        from .gpu_job_claim import GpuLeaseLost
        ...
        try:
            ...
            # NOT A LENS TO SKIP (multi-GPU Phase 3, review round 2): the push no longer
            # holds its card. Swallowed here, it pushed on as if nothing had happened.
            raise
    ...
    async def push_sae_to_local(
        ...
        try:
            ...
            # ``sae.layer or 0`` published an SAE recording no layer as layer 0, and the
            # neuron upsert is ON CONFLICT (model, layer, index), so it overwrote whatever
            # really was layer 0 (review R3-B, R3B-11). The endpoint answers this with a
            # 422; a task queued before it did reaches here.
            from .sae_hook_support import unrecorded_layer_reason

            layer_reason = unrecorded_layer_reason(
                sae.layer, "The local Neuronpedia push", sae_id
            )
            if layer_reason:
                return LocalPushResult(
                    success=False,
                ...
                if config.include_explanations:
                    ...
                    if explanation_text:
                        await client.create_explanation(
                            ...
                            # scoreV1 is the EXPLANATION'S quality score. None is
                            # computed here, so it keeps create_explanation's default
                            # of 0.0 — Neuronpedia's convention for unscored — rather
                            # than carrying activation consistency as a verdict.
                        )
