...
function setup(config: Record<string, unknown>) {
  (useTrainingsStore as never as { mockReturnValue: (value: unknown) => void }).mockReturnValue({
  ...
  (useModelsStore as never as { mockReturnValue: (value: unknown) => void }).mockReturnValue({
    models: [{ id: 'm_b55c6926', name: 'gemma-4-12B-it', status: 'ready',
               architecture_config: { num_hidden_layers: 48, hidden_size: 3840 } }],
    fetchModels: vi.fn(),
  });
  (useDatasetsStore as never as { mockReturnValue: (value: unknown) => void }).mockReturnValue({
    datasets: [{ id: DS_OWT, name: 'OpenWebText-2M', status: 'ready' }],
    fetchDatasets: vi.fn(),
  });
  (useTrainingWebSocket as never as { mockReturnValue: (value: unknown) => void }).mockReturnValue({});
  (useDeletionProgressWebSocket as never as { mockReturnValue: (value: unknown) => void }).mockReturnValue({});
  (useWebSocketContext as never as { mockReturnValue: (value: unknown) => void }).mockReturnValue({
