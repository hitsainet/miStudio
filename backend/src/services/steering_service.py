...
from ..ml.model_devices import (
    detach_dispatch_hooks,
    empty_cache_on,
    input_device,
    module_device,
    off_gpu_modules,
...
#: The free memory a card must have before a steering model load is attempted.
#: This was a post-hoc check against "the current device"; it is now the
#: requirement handed to gpu_placement, so Auto skips a card that cannot take
#: the model and a named card without it is refused with the figures.
MIN_FREE_MB_FOR_MODEL_LOAD = 2000
...
def _emergency_gpu_cleanup():
    ...
    try:
        ...
        # Only a process that initialised CUDA has anything to release, and
        # only on the cards steering placed work on: a synchronize on any other
        # card would CREATE a context there on the way out.
        if torch.cuda.is_available() and torch.cuda.is_initialized():
            devices = _steering_service.cuda_devices() if _steering_service is not None else []
            for device in devices:
                try:
                    ...
                    logger.warning(f"[Emergency GPU Cleanup] {device} cleanup error: {e}")
            logger.warning(f"[Emergency GPU Cleanup] Cleared cache on {len(devices)} GPU(s)")
...
#: Bytes a steering SAE holds per value on a GPU: `load_sae` halves it there.
STEERING_SAE_BYTES_PER_VALUE = 2


def request_sae_mb(
    request_sae_id: Optional[str],
    d_model: Optional[int],
    n_features: Optional[int],
    feature_sae_ids: "List[Optional[str]]" = (),
    sae_meta_map: "Optional[Dict[str, SaeMeta]]" = None,
) -> float:
    """MB of every distinct SAE a steering request loads beside its model, for placement.

    Known before any SAE is loaded — placement comes first — from the request's
    metadata: the request-level SAE's ``d_model``/``n_features``, and each other
    referenced SAE's :class:`SaeMeta`. Sized as ``circuit_gpu.circuit_required_mb``
    sizes calibration's SAEs (encoder and decoder, plus biases here), at steering's
    fp16. Each distinct SAE once: one object serves every feature that names it.
    An SAE whose dimensions are unknown counts nothing.
    """
    from .base_model_budget import sae_weights_mb

    dims: Dict[str, Tuple[Optional[int], Optional[int]]] = {}
    for sid, meta in (sae_meta_map or {}).items():
        dims[sid] = (meta.d_model, meta.n_features)
    if request_sae_id is not None:
        dims[request_sae_id] = (d_model, n_features) if d_model or n_features else dims.get(
            request_sae_id, (None, None))
    wanted = {request_sae_id, *feature_sae_ids} - {None}
    return sum(
        sae_weights_mb(*dims.get(sid, (None, None)), STEERING_SAE_BYTES_PER_VALUE) for sid in wanted
    )
...
class SteeringService:
    ...
    def __init__(self):
        ...
        # Where work runs. CPU until a request is placed: every generation path
        # calls place_model() before it loads anything, so no load can reach
        # "the current device" (index 0) by default.
        self._device: torch.device = torch.device("cpu")
        #: The most recent placement: the card VRAM reporting reads.
        self._placement: Optional[Placement] = None
        #: The card each cached model was loaded onto.
        self._model_placements: Dict[str, Placement] = {}

    # =========================================================================
    # GPU PLACEMENT (Multi-GPU Phase 1)
    # =========================================================================
    ...
    def place_model(
        ...
        """The card(s) ``model_id`` runs on for this request, honouring the model cache.

        * Loaded, and the request is Auto: it stays wherever it is — no reload.
        * Loaded on the card the request names: it stays. A model SPLIT across
          cards never satisfies a request naming one card, even its first.
        * Loaded somewhere other than asked: it is unloaded here and placed again.
        * Not loaded: placed by ``gpu_placement.place_job`` against live free
          memory, needing the model's estimated size (or
          ``MIN_FREE_MB_FOR_MODEL_LOAD`` when the size is unknown). A model no
          single card holds is SPLIT across cards (Multi-GPU Phase 2).

        Makes the first card current and this service's working device, so the
        SAEs a request loads after this land on a card the model uses; each SAE
        then moves to its own layer's card when its hook is registered.

        Raises:
            GpuPlacementError: no card, or set of cards, can take the model as
                asked. A named card is never swapped for another, nor for the CPU.
        """
        current = self._model_placements.get(model_id) if model_id in self._loaded_models else None
        if current is not None:
            if is_auto(gpu) or self._names_card(gpu, current):
                # MULTI-GPU PHASE 3: LEASED AGAIN BEFORE IT IS USED. Every
                # generation path force-reloads the model on the placement kept
                # here, and `load_model` re-reads each card's free memory to
                # budget the reload. A worker that outlives its task (its
                # self-exit fires only when the PID file is its own) keeps this
                # placement while the next request's claim holds nothing, so the
                # reload would read, and fill, a card another job has leased.
                # A no-op in single mode and inside an execution that already
                # holds these cards; otherwise it leases them or waits, as
                # `jlens_progress.reuse_card` does for a resident J-lens copy.
                #
                # AN AUTO REQUEST DOES NOT WAIT FOR ITS OWN KEPT MODEL'S CARD
                # (review round 1). The kept weights are themselves what the job
                # now holding that card needs, and waiting could only end in a
                # refusal after the steering wait while a free card sat idle. Auto
                # never asked for that card: release the model and place again.
                # A named card or "all" still waits and then refuses with its reason.
                from .gpu_job_claim import cards_held_by_other_jobs, claim_exact_cards

                busy = cards_held_by_other_jobs(current.all_cards) if current.all_cards and is_auto(gpu) else []
                if not busy:
                    if current.all_cards:
                        claim_exact_cards(current.all_cards, gpu)
                    self._use_placement(current)
                    return current
                logger.info(
                    "[Placement] Model %s is kept on %s, which another job now holds (%s); "
                    "releasing it and placing the request again",
                    model_id, current.describe(), ", ".join(card.describe() for card in busy),
                )
            else:
                logger.info(
                    "[Placement] Model %s is on %s but %r was requested; unloading to move it",
                    model_id, current.describe(), gpu,
                )
            self.unload_model(model_id)

        # The model's size, not the 2 GB floor. Against the floor, Auto found a
        # card with 2 GB free for every model, so a model no card holds went to
        # one card and died in the preflight — a split could never be chosen.
        placement = place_job(
            gpu, required_mb=self._placement_need_mb(model_id, model_path, sae_mb), allow_shard=True,
        )
        self._use_placement(placement)
        return placement

    def _placement_need_mb(self, model_id: str, model_path: Optional[str], sae_mb: float = 0.0) -> float:
        """The memory a steering load of ``model_id`` needs, for placement.

        The same arithmetic ``preflight_gpu_capacity`` refuses a load with —
        FP16 weights plus its activation headroom — so the card(s) placement
        chooses are the card(s) the preflight then accepts. Unknown size (no
        readable config) falls back to the free-memory floor, which can place
        on one card and never split: there is nothing to split against.

        Plus ``sae_mb``, the request's SAEs, which sit on the model's card(s)
        beside it — sized as calibration sizes its SAEs, encoder and decoder
        each, at steering's fp16 (:func:`request_sae_mb`). Without them a model
        just under one card's free memory was placed alone on that card, and the
        SAEs then did not fit beside it.
        """
        from transformers import AutoConfig

        from ..ml.model_loader import estimate_parameter_count
        from .resource_config import _ACTIVATION_HEADROOM_GB, _BYTES_PER_PARAM

        try:
            params_count = estimate_parameter_count(
                AutoConfig.from_pretrained(
                    self._resolve_load_path(model_id, model_path), trust_remote_code=True
                )
            )
        except Exception as exc:  # noqa: BLE001 - unknown size: the floor applies
            logger.info("No size estimate for %s (%s); placing against the floor", model_id, exc)
            params_count = None
        if not params_count:
            return MIN_FREE_MB_FOR_MODEL_LOAD + sae_mb
        need_mb = (params_count * _BYTES_PER_PARAM["FP16"] + _ACTIVATION_HEADROOM_GB * 1024**3) / 1024**2
        return max(float(MIN_FREE_MB_FOR_MODEL_LOAD), need_mb + sae_mb)

    @staticmethod
    def _names_card(gpu: GpuRequest, placement: Placement) -> bool:
        """Whether an explicit request names where ``placement`` is.

        ``"all"`` is satisfied by a placement over every visible card: a split,
        or on a node with one card, that card. A named card
        is satisfied only by a single-card placement on that card: a split whose
        FIRST card is the one named still spreads the model over another card
        the user did not choose.
        """
        if is_all(gpu):
            # Every visible card, however many there are. On a one-card node
            # "all" IS that card: treated as unsatisfied, every request asking
            # for it unloaded the model and placed it again on the same card.
            try:
                visible = {normalise_uuid(card.uuid) for card in list_cards()}
            except Exception:  # noqa: BLE001 - place_job reports the real error
                return False
            return visible == {normalise_uuid(uuid) for uuid in placement.uuids}
        if placement.card is None or placement.is_shard:
            return False
        if isinstance(gpu, int) or str(gpu).strip().isdigit():
            # An index means whatever card holds it now.
            try:
                gpu = resolve_request(gpu)
            except Exception:  # noqa: BLE001 - place_job reports the real error
                return False
        return normalise_uuid(gpu) == normalise_uuid(placement.card.uuid)

    def _use_placement(self, placement: Placement) -> None:
        self._placement = placement
        self._device = placement.device
        if placement.device.type == "cuda":
            torch.cuda.set_device(placement.device)

    def cuda_devices(self) -> List[torch.device]:
        """The CUDA devices this service has placed work on, and no others.

        The steering worker sees every GPU now. A synchronize on a card creates
        a CUDA context there — hundreds of MB — so cleanup must never sweep
        cards this process did not use.
        """
        # EVERY card of a split, not its first: a split model's memory, kernels
        # and cache sit on each card it spans, and cleanup built from
        # `placement.device` alone left the second card's share unreleased.
        devices = {d for placement in self._model_placements.values() for d in placement.all_devices}
        if self._placement is not None:
            devices.update(self._placement.all_devices)
        return sorted((d for d in devices if d.type == "cuda"), key=lambda d: d.index or 0)

    def synchronize(self) -> None:
        """Wait for queued kernels on every card steering uses."""
        for device in self.cuda_devices():
            torch.cuda.synchronize(device)

    def empty_caches(self) -> None:
        """Release cached allocator memory on every card steering uses.

        A bare ``torch.cuda.empty_cache()`` acts on the current device only —
        the first card of a split model.
        """
        if torch.cuda.is_available():
            empty_cache_on(self.cuda_devices())

    def _placement_for(self, model: Any) -> Optional[Placement]:
        """The placement a cached model was loaded with; the latest one otherwise."""
        for model_id, (cached, _tokenizer) in self._loaded_models.items():
            if cached is model:
                return self._model_placements.get(model_id, self._placement)
        return self._placement

    @staticmethod
    def _is_split(model: Any, placement: Optional[Placement]) -> bool:
        """Whether ``model`` is dispatched across more than one device."""
        if placement is not None and placement.is_shard:
            return True
        device_map = getattr(model, "hf_device_map", None)
        return isinstance(device_map, dict) and len({str(v) for v in device_map.values()}) > 1

    def _release_model(self, model: Any, placement: Optional[Placement]) -> None:
        """Drop a model's hold on its card(s) before its references are deleted.

        One card: moved to the CPU, as before. A SPLIT model is never moved: its
        accelerate dispatch hooks would keep sending activations to the cards
        while the weights sat in host RAM (accelerate warns "You shouldn't move a
        model that is dispatched", and raises outright when any module is
        offloaded), and copying tens of GB to host memory only to delete it
        frees nothing sooner. Its hooks are detached instead, so they hold no
        references once the model is dropped.
        """
        if self._is_split(model, placement):
            detach_dispatch_hooks(model)
            return
        model.to("cpu")

    def cleanup_gpu(self, model: Optional[PreTrainedModel] = None, devices=()) -> None:
        """
        Clean up GPU memory to prevent memory leaks.

        Releases cached memory on the cards this service placed work on — not on
        every visible card, where the synchronize would create a context.
        ...
        Args:
            ...
            devices: Cards to release as well as the ones this service still
                records — a model just unloaded is no longer in
                ``_model_placements``, so its cards must be named here.
        """
        try:
            ...
            if torch.cuda.is_available():
                devices = sorted(
                    {*self.cuda_devices(), *(d for d in devices if d.type == "cuda")},
                    key=lambda d: d.index or 0,
                )
                for device in devices:
                    with torch.cuda.device(device):
                        torch.cuda.empty_cache()
                        torch.cuda.synchronize(device)
                if devices:
                    logger.info(f"[GPU Cleanup] Cleared cache on {[str(d) for d in devices]}")

            # Second garbage collection pass, then release what it freed
            gc.collect()
            if torch.cuda.is_available():
    ...
    def unload_model(self, model_id: str) -> bool:
        ...
        try:
            ...
            placement = self._model_placements.pop(model_id, None)

            # Clear any hooks
            self._clear_all_model_hooks(model)

            # One card: to the CPU first. A split model: dispatch hooks detached,
            # never moved (see _release_model). The placement is kept on
            # self._placement, so cleanup below still covers every card it used.
            try:
                self._release_model(model, placement)
            except Exception:
                pass
            ...
            # Clean up GPU — on THIS model's cards too. Its placement was popped
            # above, and when the last request used another model it is no
            # longer the current placement either, so `cuda_devices()` alone
            # left its cards (the second card of a split, or its one card)
            # holding reserved blocks that NVML, and so placement, still counts.
            self.cleanup_gpu(devices=placement.all_devices if placement is not None else ())
    ...
    async def load_sae(
        ...
        # READ ON THE CPU (review round 3). Read onto the card, the fp32 state
        # dict and the fp32 module built from it sat there together before the
        # half(): four times the fp16 SAE that placement and the split planner
        # charge for — while a model kept from the last request, which every
        # generation path force-reloads only AFTER its SAEs, still filled the card.
        ...
        if config is not None:
            d_in = config.d_in
            d_sae = config.d_sae
            # B11: the Gemma Scope and SAELens loaders invent ``blocks.0.hook_resid_post``
            # for a file recording no layer, and an invented 0 is indistinguishable from a
            # recorded 0 here -- so steering would add the decoder at layer 0 believing the
            # checkpoint said so. They now mark it; an assumed layer is ignored in favour of
            # the database row's, and when that is NULL too the SAE is refused rather than
            # steered somewhere it was never trained (review R3-B, R3B-11/B11).
            if (config.extra_metadata or {}).get("hook_point_assumed"):
                from .sae_hook_support import refuse_unrecorded_layer

                refuse_unrecorded_layer(layer, "Steering", sae_id)
                sae_layer = layer
            else:
            ...
            # ``layer or 0`` steered layer 0 for an SAE recording no layer, and a steer at
            # the wrong layer reads as a weak or odd steer, never as an error (review R3-B,
            # R3B-11). The steering endpoints refuse such an SAE with a 422; this is the
            # loader's own guard, for every other caller.
            from .sae_hook_support import refuse_unrecorded_layer
        ...
        del state_dict
        # Convert to FP16 if on CUDA to match model dtype, BEFORE it moves: the
        # card only ever holds the fp16 copy. `.type`: a torch.device never
        # equals the string "cuda", even torch.device("cuda").
        on_cuda = self._device.type == "cuda"
        if on_cuda:
            sae_model.half()
        # A SPLIT'S SAEs STAY ON THE CPU until their hooks put each on its
        # layer's card (`_register_steering_hooks`). Loaded onto the placement's
        # first card they only moved off it again in `load_model`, and meanwhile
        # took memory on a card the kept model still filled.
        home = (torch.device("cpu") if on_cuda and self._placement is not None and self._placement.is_shard
                else self._device)
        sae_model.to(home)
        sae_model.eval()
    ...
    async def load_model(
        ...
        saes_by_layer: Optional[Dict[int, List["LoadedSAE"]]] = None,
        ...
        Args:
            ...
            force_reload: Whether to reload even if cached
            gpu: "auto", an index or a UUID; see :meth:`place_model`. Ignored
                when ``placement`` is given.
            placement: Where to load, when the caller already placed the request
                (the generation paths do, so the SAEs they load first share the
                model's card).
            saes_by_layer: The request's SAEs by the layer each is hooked at. A
                SPLIT is mapped so each layer's card holds its SAEs beside the
                layer (``_plan_split``), with the SAEs off the cards while the
                budget is read; one card ignores it.

        Returns:
            Tuple of (model, tokenizer)
        """
        if placement is None:
            placement = self.place_model(model_id, gpu, model_path)
        ...
        if force_reload and cache_key in self._loaded_models:
            ...
            try:
                ...
                old_placement = self._model_placements.pop(cache_key, None)
                # Clear hooks before cleanup
                self._clear_all_model_hooks(old_model)
                # One card: to the CPU. A split: hooks detached, never moved.
                self._release_model(old_model, old_placement)
                del old_model
                gc.collect()
                self.empty_caches()
            except Exception as e:
        ...
        load_path = self._resolve_load_path(model_id, model_path)
        ...
        # Load onto the placed card, named explicitly — or, for a model no single
        # card holds, SPLIT over exactly the placement's cards with its per-card
        # budget, both taken from the placement. An automatic map with no budget
        # spread a model over every visible card and could silently put layers
        # on the CPU.
        device = placement.device
        on_cuda = device.type == "cuda"
        split = placement.is_shard
        if split:
            # Budgeted NOW, from the memory each card has free as the weights
            # load — not when the cards were chosen. Every generation path
            # force-reloads the model on the placement the cache KEPT, so a
            # budget read at the first placement was reused for every request
            # after it, however the cards had filled or emptied since; and on
            # the first load the SAEs have taken memory on the first card
            # between placement and this line. A stale budget lets accelerate
            # fill a card past what it has, and the load dies part-way.
            #
            # The request's SAEs leave the cards first (review round 2). They
            # loaded onto the placement's first card, and each moves to its
            # hooked LAYER's card when its hook is registered. Read with them in
            # place, the budget charged every SAE to the first card, while the
            # layer it goes to kept only SHARD_RESERVE_MB for it; `_plan_split`
            # below charges each SAE to its layer's card instead.
            if saes_by_layer and on_cuda:
                for held in self._distinct_saes(saes_by_layer):
                    self._move_sae_to(held, torch.device("cpu"))
                empty_cache_on(placement.all_devices)
            placement = self._budget_split_from_free_memory(placement)
        # The budget a split loads with: the placement's, or the one mapped
        # before loading below (`_plan_split`).
        split_max_memory = placement.max_memory if split else None

        if on_cuda:
            # Refuse a model this card cannot hold, with the arithmetic, before a
            # single weight is read. place_job was only asked for the free-memory
            # floor — the size is unknown until the path is resolved — so a 12B
            # model named onto the 12 GB card would otherwise run out of memory
            # partway through from_pretrained.
            from transformers import AutoConfig

            from ..ml.model_loader import estimate_parameter_count
            from .resource_config import preflight_gpu_capacity

            try:
                config = AutoConfig.from_pretrained(load_path, trust_remote_code=True)
                params_count = estimate_parameter_count(config)
            except Exception as exc:  # noqa: BLE001 - unknown size: the floor below still applies
                logger.info(
                    "No size estimate for %s (%s); only the free-memory floor applies", model_id, exc
                )
                config = None
                params_count = None
            # Steering always loads at float16 (below). Against the memory the
            # load can use: its one card, or a split's own cards together.
            preflight_gpu_capacity(
                params_count=params_count,
                quantization="FP16",
                device=tuple(placement.max_memory) if split else device,
                model_name=model_id,
            )
            if split and config is not None:
                split_max_memory = self._plan_split(config, placement, model_id, saes_by_layer)

        # CRITICAL: Check the card's memory BEFORE loading. place_job judged it a
        # moment ago; this re-reads it, because another process may have taken
        # the memory since. A split reads every card it was given.
        if on_cuda:
            gpu_free_mb = sum(
                torch.cuda.mem_get_info(d)[0] for d in placement.all_devices
            ) / 1024**2 if split else torch.cuda.mem_get_info(device)[0] / 1024**2
            if gpu_free_mb < MIN_FREE_MB_FOR_MODEL_LOAD:
                raise RuntimeError(
                    f"Insufficient GPU memory for model loading on {placement.describe()}. "
                    f"Available: {gpu_free_mb:.0f}MB, Required: ~{MIN_FREE_MB_FOR_MODEL_LOAD}MB minimum. "
                    f"Another process may be holding GPU memory (check for zombie processes)."
                )
            logger.info(f"[GPU Memory] {gpu_free_mb:.0f}MB free on {placement.describe()} before model load")

        if split:
            # GPUs ONLY (operator decision 3): the budget has no "cpu" key, so
            # accelerate cannot offload a layer to host RAM. It can still map one
            # to "disk", which is refused below.
            model = AutoModelForCausalLM.from_pretrained(
                load_path,
                torch_dtype=torch.float16,
                device_map=placement.device_map,
                max_memory=split_max_memory,
                trust_remote_code=True,
            )
        else:
            model = AutoModelForCausalLM.from_pretrained(
                ...
                device_map={"": device} if on_cuda else None,
                trust_remote_code=True,
            )
        ...
        # CRITICAL: Verify the model is on the card(s) it was placed on
        if on_cuda:
            self._refuse_off_placement(model, model_id, placement)

        self._loaded_models[cache_key] = (model, tokenizer)
        self._model_placements[cache_key] = placement
        logger.info(f"Loaded model {model_id} on {placement.describe()}")
    ...
    def _plan_split(
        ...
        """The budget a split steering load passes, verified against transformers' own map.

        Steering loads through ``from_pretrained`` directly, not the shared loader,
        so it maps its own split (``ml/split_load.py``): transformers holds back the
        largest layer's size on the lowest-index card, so a split the budgets held
        spilled to disk and was refused only after every weight had been read.
        Mapped with the kwargs the load passes: float16, no quantization, remote
        code trusted. A config that cannot be mapped here keeps the placement's
        budget, and the post-load check still refuses anything off the GPUs.

        ``saes_by_layer`` (review round 2): every hook moves its SAE to its
        layer's card, and the budget keeps only ``SHARD_RESERVE_MB`` free on a
        card. Each layer's SAEs are charged to the card that layer maps to, so
        the split is mapped to hold them or refused before loading.

        Raises:
            RuntimeError: transformers would map part of the model off the split's GPUs.
        """
        ...
        try:
            plan = plan_split_load(
                ...
                dtype=torch.float16,
                trust_remote_code=True,
                model_name=f"Steering model {model_id}",
                extra_mb_by_layer={
                    layer: sum(cls._steering_sae_mb(sae) for sae in group)
                    for layer, group in (saes_by_layer or {}).items()
                } or None,
            )
        except SplitDoesNotFit as refusal:
            raise RuntimeError(str(refusal)) from refusal
        return placement.max_memory if plan is None else dict(plan.max_memory)

    @staticmethod
    def _saes_by_layer(pairs) -> Dict[int, List[LoadedSAE]]:
        """{layer: the distinct SAEs hooked there} from (layer, SAE) pairs; a None SAE is skipped.

        The generation paths build this from the same (layer, SAE) choice their
        hooks make, and hand it to :meth:`load_model`.
        """
        by_layer: Dict[int, List[LoadedSAE]] = {}
        for layer, sae in pairs:
            if sae is None:
                continue
            group = by_layer.setdefault(int(layer), [])
            if all(sae is not other for other in group):
                group.append(sae)
        return by_layer

    @staticmethod
    def _distinct_saes(saes_by_layer: Dict[int, List[LoadedSAE]]) -> List[LoadedSAE]:
        distinct: List[LoadedSAE] = []
        for group in saes_by_layer.values():
            for sae in group:
                if all(sae is not other for other in distinct):
                    distinct.append(sae)
        return distinct

    @staticmethod
    def _steering_sae_mb(sae: LoadedSAE) -> float:
        """MB an SAE holds on its card: its tensors as loaded, else its dimensions at fp16."""
        from .base_model_budget import sae_weights_mb

        model = getattr(sae, "model", None)
        tensors = [*model.parameters(), *model.buffers()] if isinstance(model, nn.Module) else []
        if tensors:
            return sum(t.numel() * t.element_size() for t in tensors) / 1024**2
        return sae_weights_mb(sae.d_in, sae.d_sae, STEERING_SAE_BYTES_PER_VALUE)

    @staticmethod
    def _budget_split_from_free_memory(placement: Placement) -> Placement:
        """The same cards in the same order, each budgeted from the memory free on it now.

        A card's budget is its free memory less ``SHARD_RESERVE_MB``, the rule
        ``gpu_placement.place_job`` applies when it chooses the cards; only the
        moment of reading changes. Read from ``torch.cuda.mem_get_info``, which,
        like NVML, counts every process on the card. The cards themselves are
        never changed here: which cards a model runs on is placement's decision.
        """
        from dataclasses import replace

        budgets = {
            device.index: max(int(torch.cuda.mem_get_info(device)[0] // 1024**2) - SHARD_RESERVE_MB, 0)
            for device in placement.devices
        }
        if budgets != placement.max_memory_mb:
            logger.info(
                "[Placement] Split budget for %s re-read at load: %s MB (was %s MB when placed)",
                placement.describe(), budgets, placement.max_memory_mb,
            )
        return replace(placement, max_memory_mb=budgets)

    def _refuse_off_placement(self, model: Any, model_id: str, placement: Placement) -> None:
        """Raise unless every parameter is on one of the placement's cards.

        Checked over ALL parameters, not the first: on a split the first
        registered parameter (the embedding) sits on whichever card accelerate
        filled first — accelerate sorts ``max_memory`` by index, so that is
        cuda:0, not necessarily ``placement.device`` — and a check against one
        device would refuse every correct split while letting a stray layer on
        the CPU through. A refused model is released before raising, so its
        weights do not stay on the cards with nothing referencing them.
        """
        offloaded = off_gpu_modules(model)
        allowed = set(placement.all_devices)
        strays = {}
        named = (
            model.named_parameters() if hasattr(model, "named_parameters")
            else enumerate(model.parameters())
        )
        for name, param in named:
            if param.device not in allowed:
                strays.setdefault(str(param.device), str(name))
        if not offloaded and not strays:
            logger.info(f"[GPU Verification] Model confirmed on {placement.describe()}")
            return
        where = (
            ", ".join(f"{module} on {target}" for module, target in list(offloaded.items())[:3])
            if offloaded
            else ", ".join(f"{name} on {dev}" for dev, name in list(strays.items())[:3])
        )
        try:
            self._release_model(model, placement)
        ...
        if torch.cuda.is_available():
            empty_cache_on(placement.all_devices)
        raise RuntimeError(
            f"Model {model_id} failed to load on {placement.describe()}: {where}. "
            f"miStudio runs steering models on GPUs only. This usually means GPU memory is "
            f"insufficient or held by another process. Check for zombie processes with: "
            f"nvidia-smi --query-compute-apps=pid --format=csv"
        )

    def _resolve_load_path(self, model_id: str, model_path: Optional[str]) -> str:
        """Where ``from_pretrained`` reads ``model_id`` from."""
    ...
    def _ensure_model_on_gpu(self, model: PreTrainedModel) -> None:
        ...
        # A SPLIT model is never moved: `model.to(self._device)` would pull every
        # layer onto the first card — the model is split because that card
        # cannot hold it — and leave accelerate's hooks routing to the old cards.
        # Its placement was verified over every parameter at load; there is
        # nothing safe to "restore" here, so a stray is reported, not moved.
        placement = self._placement_for(model)
        if self._is_split(model, placement):
            allowed = set(placement.all_devices) if placement is not None else set()
            strays = {str(p.device) for p in model.parameters() if allowed and p.device not in allowed}
            if strays:
                logger.error(
                    f"[Model Device] Split model has parameters on {sorted(strays)}, outside "
                    f"{placement.describe()}; not moving a split model. Reload it."
                )
            return
    ...
    def _register_steering_hooks(
        ...
        *,
        ...
        for (sid, layer), layer_features in groups.items():
            ...
            # The SAE lives on its hooked LAYER's device. SAEs load onto the
            # placement's first card, before the model exists; on a split model
            # a layer on the other card then copied its steering vector across
            # the PCIe link on every forward pass. On one card this is a no-op.
            self._move_sae_to(
                group_sae,
                module_device(target_module) if isinstance(target_module, nn.Module) else None,
            )
    ...
    @staticmethod
    def _move_sae_to(sae: LoadedSAE, device: Optional[torch.device]) -> None:
        """Put an SAE on ``device`` (its hooked layer's), recording where it is now.

        Left alone when the layer holds no tensors to take a device from, or the
        SAE is already there. The dtype is not changed: the hook casts the
        steering vector to the hidden state's dtype.
        """
        if device is None or sae.device == str(device):
            return
        sae.model.to(device)
        sae.device = str(device)
    ...
    async def _generate_text(
        ...
        # Inputs go to the EMBEDDING's device. `self._device` is the placement's
        # first card; accelerate fills a split's cards in index order, so on a
        # split the embedding can be on the other card and the lookup fails.
        inputs = tokenizer(prompt, return_tensors="pt").to(input_device(model))
        # Every card the model spans: kernels queued on the second card of a
        # split are not waited for by a synchronize on the first.
        placement = self._placement_for(model)
        sync_devices = [
            d for d in (placement.all_devices if placement is not None else (self._device,))
            if d.type == "cuda"
        ]
        ...
        try:
            ...
            # The model's card, never "the current device".
    ...
    async def _calculate_perplexity(
        ...
        inputs = tokenizer(
            ...
            truncation=True,
            max_length=2048,
        # The embedding's device, as for generation; the loss inside the model
        # moves the labels to the logits' card itself.
    ...
    async def generate_comparison(
        ...
        try:
            # Load SAE and model
            # Multi-GPU Phase 1: choose the card FIRST. The SAEs load onto this
            # service's device before the model does; placing afterwards could
            # put the SAEs and the model on two different cards.
            # Sized with the SAEs, which sit beside the model on its card(s).
            placement = self.place_model(
                model_id, request.gpu, model_path,
                sae_mb=request_sae_mb(
                    request.sae_id, sae_d_model, sae_n_features,
                    [f.sae_id for f in request.selected_features], sae_meta_map),
            )
            ...
            model, tokenizer = await self.load_model(
                model_id, model_path, force_reload=True, placement=placement,
                # Each feature's hook uses its own SAE from the map, or the
                # request's (the multi-strength path hooks that one throughout).
                saes_by_layer=self._saes_by_layer(
                    (f.layer, steering_saes.get(f.sae_id or request.sae_id, sae))
                    for f in request.selected_features),
            )
            ...
            if has_multi_strength:
                ...
                return SteeringComparisonResponse(
                    ...
                    created_at=utc_now(),
                    gpu_uuid=placement.uuid, gpu_uuids=placement.gpu_columns()["gpu_uuids"],
                )
    ...
    async def generate_strength_sweep(
        ...
        try:
            ...
            # service's device before the model does; placing afterwards could
            # put the SAEs and the model on two different cards.
            # Sized with its SAE, which sits beside the model on its card(s).
            placement = self.place_model(
                model_id, request.gpu, model_path,
                sae_mb=request_sae_mb(request.sae_id, sae_d_model, sae_n_features),
            )
            ...
            model, tokenizer = await self.load_model(
                model_id, model_path, force_reload=True, placement=placement,
                # Every strength hooks this one SAE at the swept layer.
                saes_by_layer=self._saes_by_layer([(request.layer, sae)]),
            )
    ...
    async def generate_combined(
        ...
        try:
            ...
            # SAE actually used at hook time, never merely the request. Resolved
            # before the load, which maps a split around each layer's SAE.
            resolved_sae_ids = [
    ...
    def _get_system_vram_usage_gb(self) -> float:
        """
        VRAM used on the card steering's model is on, by ALL processes.

        Read from NVML by the card's UUID (as System Monitor reads it). It read
        NVML index 0 — a different card from the model's whenever steering ran
        anywhere else. Falls back to this process's allocation on that card if
        NVML is unavailable.

        Returns:
            VRAM usage in GB; 0.0 before anything has been placed on a GPU,
            since steering then holds no card to report.
        """
        placement = self._placement
        if placement is None or placement.card is None:
            return 0.0
        try:
            import pynvml
            pynvml.nvmlInit()
            try:
                # Summed over every card of a split: the first card alone
                # under-reported a split model by the share on the other card,
                # so a clear-cache looked like it freed less than it did.
                return sum(
                    pynvml.nvmlDeviceGetMemoryInfo(pynvml.nvmlDeviceGetHandleByUUID(card.uuid)).used
                    for card in placement.all_cards
                ) / (1024 ** 3)
            finally:
                pynvml.nvmlShutdown()
        except Exception as e:
            ...
            if torch.cuda.is_available():
                return sum(
                    torch.cuda.memory_allocated(device) for device in placement.all_devices
                ) / (1024 ** 3)
            return 0.0
    ...
    def clear_cache(self) -> dict:
        ...
        for model_id, (model, tokenizer) in list(self._loaded_models.items()):
            try:
                # One card: to the CPU. A split model: hooks detached, not moved.
                self._release_model(model, self._model_placements.get(model_id))
                del model
                del tokenizer
            except Exception as e:
        ...
        # The cards to release, read before the dictionaries forget them.
        devices = self.cuda_devices()

        # Clear the dictionaries
        self._loaded_saes.clear()
        self._loaded_models.clear()
        self._model_placements.clear()
