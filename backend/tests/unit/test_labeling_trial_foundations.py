...
class TestJunkFilterCannotSilentlySucceed:
    ...
    def test_a_fully_filtered_run_raises_instead_of_completing_at_zero(self):
        ...
        # The bottom retrieval is patched too, not because this test cares
        # about the contrast block, but because it now runs on the same path and
        # a Mock session cannot serve a real query. Patching it keeps this test
        # about the ONE thing it is named for: a fully-filtered run must raise
        # rather than report success at zero.
        with patch.object(
            service, "_retrieve_top_examples_batch_sync",
            return_value={"feat_1": [{"prime_token": ".", "max_activation": 1.0}]},
        ), patch.object(
            service, "_retrieve_bottom_examples_batch_sync", return_value={},
