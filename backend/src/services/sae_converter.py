...
class SAEConverterService:
    ...
    def load_auto(
        ...
        if config:
            ...
            # for a file that records no layer, and mark it. The SAE download task writes
            # this ``layer`` onto the row whenever the resolver found none, so passing an
            # invented 0 on would record layer 0 as a fact -- which is the whole defect.
            assumed = bool((config.extra_metadata or {}).get("hook_point_assumed"))
            metadata = {
                "model_name": config.model_name,
                "hook_point": None if assumed else config.hook_point,
                "layer": None if assumed else config.hook_point_layer,
                "d_in": config.d_in,
                "d_sae": config.d_sae,
