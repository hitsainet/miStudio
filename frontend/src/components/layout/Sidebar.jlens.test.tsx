...
describe('J-Lens nav placement', () => {
  ...
  it('places jlens between circuits and steering in the nav registry', () => {
    ...
    // ⚠ THIS USED TO ASSERT `circuits + 1`, WHICH WAS AN INCIDENTAL TRUTH, NOT THE
    // CONTRACT. J-Lens's requirement (FPRD §3.1, and the comment beside `navItems`) is
    // that it sits IMMEDIATELY BEFORE STEERING; that it also happened to follow Circuits
    // was a property of the layout at the time, and it broke the moment feature 032 added
    // Probes after Circuits — a test failing for a reason its own name disclaims. What is
    // asserted now is the real contract plus the real ordering relation.
    expect(ids.indexOf('jlens')).toBeGreaterThan(ids.indexOf('circuits'));
