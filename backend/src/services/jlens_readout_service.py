...
def _input_device(model: Any) -> Optional[str]:
    """Where input ids go: the input embedding's device, else the first parameter's.

    On a model SPLIT across cards the first registered parameter need not be
    the embedding — a checkpoint that registers `lm_head` first puts it on the
    last card — and ids sent there fail inside the embedding lookup on the
    first card. A model that exposes no embedding (a test double) falls back to
    `_model_device`, which is what this always read.
    """
    from ..ml.model_devices import input_device

    try:
        return str(input_device(model))
...
class ReadoutService:
    ...
    def __init__(
        ...
        # serving card.
        #
        # THE EMBEDDING'S DEVICE, not the first parameter's — see `_input_device`.
        self.capture_device = capture_device or _input_device(model) or "cpu"
    ...
    def _capture_residuals(self, input_ids: torch.Tensor, layers: Sequence[int]) -> CapturedResiduals:
        ...
        handles = []
        # MOVED HERE, where the model runs, not left to each caller. The band
        # report handed this the tokenizer's CPU ids unmoved, so a band report
        # on a model resident on one card died in the embedding lookup with
        # "index is on cpu". A no-op for callers that already moved them.
        input_ids = input_ids.to(self.capture_device)
