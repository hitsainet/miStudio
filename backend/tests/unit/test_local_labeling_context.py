...
def local_service():
    ...
    # The caller always names the device now; these tests never load a model.
    return LocalLabelingService(model_name="phi3", device="cpu")
...
class TestParseDualLabel:
    ...
    def test_parse_invalid_json_reports_a_failure_not_a_semantic_verdict(self, local_service):
        """An unparseable response is a FAILURE and must say so.

        This test previously asserted `category == "semantic"`, pinning the
        defect rather than preventing it: a parse failure was written to
        `features` as a real semantic verdict with a placeholder name, and no
        test anywhere — not even a string test — could separate it from a label
        the judge actually produced. It was the worst of the failure paths,
        because the others at least left `error_feature` behind.
        """
        ...
        assert result["category"] != "semantic", (
            "a parse failure must not masquerade as a semantic verdict"
        )
        assert result["description"] == ""
        assert result.get("error"), (
            "a failure must carry the reason it failed; without it "
            "`label_error` is empty and the operator has nothing to act on"
        )
