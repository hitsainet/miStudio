...
class ReadoutRequest(BaseModel):
    ...
    # Which GPU captures the residuals. The readout maths stays on the CPU.
    gpu: GpuRequestStr = Field(AUTO, description=GPU_REQUEST_DESCRIPTION)

    # Whether the readout frees its card when it ends. READOUTS ONLY: the probe,
    # band-report and intervention tasks share the model cache and always release.
    unload_after: bool = Field(
        False,
        description=(
            "Off (the default) keeps the model loaded on its card, so the next "
            "readout of that model skips the load. On frees the card when the "
            "readout finishes, e.g. for miLLM or another job. A failed readout "
            "frees the card either way. Readouts only: probe, band-report and "
            "intervention jobs always free the card when they end."
        ),
    )
...
class ProbeRequest(BaseModel):
    ...
    artifact_id: Optional[str] = None
    # Which GPU captures the residuals the probe scores.
    gpu: GpuRequestStr = Field(AUTO, description=GPU_REQUEST_DESCRIPTION)
