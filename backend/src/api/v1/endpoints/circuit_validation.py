...
async def start_validation(run_id: str, body: ValidateBody,
    ...
    try:
        scope = CircuitInterventionService.create_scope(
    ...
    # in the manifest, which is where a reproduction reads it back.
    ...
    def _guard_and_mark(sync_db):
        ...
        previous = row.validation_status
    ...
    task = await _dispatch_or_restore(
        db,
        lambda: gpu_delay(validate_circuit_edges)(run.id, scope, gpu_request=gpu_request),
        CircuitDiscoveryRun.__table__.update()
        .where(CircuitDiscoveryRun.id == run.id,
               CircuitDiscoveryRun.validation_status == "pending")
        .values(validation_status=previous_status))
    await db.execute(
        CircuitDiscoveryRun.__table__.update()
        .where(CircuitDiscoveryRun.id == run.id)
...
class ReproduceBody(BaseModel):
    ...
    gpu: Optional[GpuRequestStr] = Field(
        ...
        description=(
            ...
            "request the original validation was submitted with; given, it "
...
async def reproduce_manifest(manifest_id: str,
                             body: Optional[ReproduceBody] = None,
                             db: AsyncSession = Depends(get_db)):
    ...
    The Manifest drawer sends the card its picker names. This endpoint used to
    take no body, so that choice was dropped and every reproduction ran on the
    original's request whatever the picker said."""
    from ....services.manifest_service import ManifestService
    from ....workers.circuit_validation_tasks import validate_circuit_edges
    ...
    # was submitted with (a manifest from before GPUs could be chosen has none:
    # auto). Checked before the mark, so a card that is not on the node is a
    # 400, not a failed pass.
    chosen = body.gpu if body is not None and body.gpu is not None else _manifest_gpu_request(m)
...
def _manifest_gpu_request(manifest: ValidationManifest) -> Optional[str]:
    """The GPU request a manifest's run was submitted with, or None (auto)."""
    gpu = (manifest.payload or {}).get("gpu")
    return gpu.get("request") if isinstance(gpu, dict) else None
