...
interface JLensState {
  ...
  /**
   * The card readouts and the panel's quick interventions run on: `"auto"` or
   * a GPU UUID. Deliberately NOT persisted — a saved UUID can name a card that
  ...
  /**
   * Whether a readout frees its model from the GPU when it finishes. Off (the
   * default) keeps it loaded, so the next readout of the same model skips the
   * load; on frees the card for miLLM or another job. A failed readout frees
   * the card either way.
   */
  unloadAfterReadout: boolean;
  setUnloadAfterReadout: (unload: boolean) => void;
...
export const useJLensStore = create<JLensState>()(
  devtools(
    persist(
      ...
      fetchReadout: async () => {
        ...
        try {
          ...
          const accepted = await jlensApi.readout({
            ...
            gpu: get().gpu,
            unload_after: get().unloadAfterReadout,
          });
      ...
      setUnloadAfterReadout: (unloadAfterReadout) => set({ unloadAfterReadout }),
      ...
      }),
      {
        ...
        partialize: (state) => {
          const setup = {
            ...
            // A preference, not a card, so unlike `gpu` it survives a reload.
            unloadAfterReadout: state.unloadAfterReadout,
          };
          const results = {
            meta: state.meta,
            tokens: state.tokens,
