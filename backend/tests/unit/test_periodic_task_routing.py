...
class TestNoScheduledTaskWaitsBehindGpuWork:
    """Multi-GPU Phase 3: the janitors must never land on a per-card GPU queue.

    A janitor that reaps a dead GPU job — and now frees its GPU leases — queued
    behind GPU work on the very card it is meant to free would never run while it
    was needed. Resolved through Celery's router and the supervisor's own queue
    list, not by reading configuration back.
    """

    def test_no_beat_task_resolves_to_a_gpu_queue(self):
        offenders = {}
        for entry in celery_app.conf.beat_schedule.values():
            name = entry["task"]
            for options in ({}, entry.get("options", {})):
                queue = _queue_name(celery_app.amqp.router.route(options, name))
                if queue.startswith("gpu."):
                    offenders[name] = queue
        assert not offenders, f"scheduled tasks routed to GPU queues: {offenders}"

    def test_no_beat_task_is_a_gpu_queue_task(self):
        from src.services.gpu_dispatch import runs_on_gpu_queues

        gpu = sorted(entry["task"] for entry in celery_app.conf.beat_schedule.values()
                     if runs_on_gpu_queues(celery_app.tasks[entry["task"]]))
        assert not gpu, f"the dispatcher would send these scheduled tasks to a GPU queue: {gpu}"
