 ...
 *   F7 drop allowSplit from the acquire GPU picker -> "acquire offers "All GPUs" and sends gpu "all""
...
describe('GPU placement', () => {
  /**
   * An acquisition VALIDATES the lens by reading out through it, which loads the
   * model — GPU work, so it carries the card choice. Publishing is an upload and
   * holds no card, so it must not claim one.
   *
   * MUTATION CONTROLS:
   *   * delete `gpu,` from runAcquire's request -> "acquire sends gpu" fails
   *   * add `gpu` to runPublish's request       -> "publish sends none" fails
   */
  it('acquire sends gpu "auto" when no card was chosen', async () => {
    mount();
    open();
    fireEvent.click(screen.getByRole('button', { name: /Look inside/ }));
    await waitFor(() => screen.getByText(/2 candidates/));
  ...
  it('acquire offers "All GPUs" and sends gpu "all" — validating the lens can run split', async () => {
    const cards = [
