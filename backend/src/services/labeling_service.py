...
class LabelingService:
    ...
    def build_labeling_job_row(
        *,
        ...
        return LabelingJob(
            ...
            # What the job asked for: "auto" or a GPU UUID, already resolved by
            # the endpoint. Only a LOCAL judge runs on a GPU; every other method
            # records none, because naming a card it will never touch would
            # misdescribe the job. NULL on a local job means auto.
            gpu_request=(
                config.get("gpu")
    ...
    def _persist_label_outcome(
        ...
        *,
        ...
        # And HOW SURE it was. Parsed off the response and previously dropped
        # here, so a confident label and a hedged one were indistinguishable.
        # Written on every success — None when the template does not ask —
        # so a relabel never leaves a stale self-assessment behind.
        # Clipped to the column width: a judge that answers "high, because ..."
        # must not fail the whole label write on a truncation error.
        feature.label_fit_count = (label.get("fit_count") or None) and label["fit_count"][:32]
        feature.label_confidence = (label.get("confidence") or None) and label["confidence"][:16]
        ...
        try:
            ...
            try:
                if labeling_method == LabelingMethod.LOCAL.value:
                    ...
                    # THE ONLY LABELING METHOD THAT USES A GPU, so the only one
                    # that is placed. Judged against live free memory now, not
                    # when the job was queued. A card that cannot take the job
                    # raises GpuPlacementError, which the handler below records
                    # as this job's failure — never a quiet move to another card.
                    # Sized, so a judge no single card holds can be split across
                    # cards (LocalLabelingService loads it split-safe).
                    placement = gpu_placement.place_job(
                        labeling_job.gpu_request,
                        required_mb=local_judge_required_mb(local_model),
                        allow_shard=True,
                    )
                    # Recorded BEFORE the load, so a load that dies on the card
                    # still says which card(s) it died on.
                    for column, value in placement.gpu_columns().items():
                        setattr(labeling_job, column, value)
                    labeling_job.updated_at = datetime.now(timezone.utc)
                    self.db.commit()
                    logger.info(
                        "Labeling job %s: local judge %s on %s (requested %s)",
                        labeling_job_id, local_model, placement.describe(),
                        labeling_job.gpu_request or gpu_placement.AUTO,
                    )
                    labeling_service = LocalLabelingService(
