...
export function ExtractionTemplateForm({
  ...
  // EDITABLE since 2026-09-25 (OSD-15). It was carried-but-not-shown: an earlier
  // fix stopped every save silently dropping the key (a template tuned down to fit
  // a card came back untuned after any edit), but left no way to SET one, so a
  // tuned template could only ever be created through the API. `undefined` stays
  // `undefined`, which the backend reads as "use batch_size", so a template that
  // never set one is unaffected — clearing the input restores exactly that.
  const [microBatchSize, setMicroBatchSize] = useState<number | undefined>(
    template?.micro_batch_size
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = {
        name: name.trim(),
        // NULL, NOT `undefined` — otherwise a cleared description cannot be cleared.
        // `undefined` is dropped by JSON.stringify, so the key never reaches the
        // backend, and the update service uses `model_dump(exclude_unset=True)`: a
        // missing key means "leave unchanged", so the old text came back every time.
        // `null` is the convention TrainingTemplateForm already established for this
        // exact defect (review round 2, lane R2-F, control F5).
      ...
      <div className="space-y-4">
        ...
        <div className="border-t border-slate-300 dark:border-slate-700 pt-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label
                htmlFor="extraction-template-max-samples"
                className="block text-xs text-slate-600 dark:text-slate-400 mb-1"
              >Evaluation Samples</label>
              {/* step 1 for the same reason as Top-K: step=100 silently blocked
                  every value that was not a multiple of 100 (OSD-48). */}
              <input
                id="extraction-template-max-samples"
                type="number"
                value={maxSamples}
            ...
            <div>
              <label
                htmlFor="extraction-template-top-k"
                className="block text-xs text-slate-600 dark:text-slate-400 mb-1"
              >Top-K Examples per Feature</label>
              {/*
                step MUST stay 1. HTML5 validates a number against min+step and the
                browser then refuses to submit a non-conforming value SILENTLY:
                React's onSubmit never fires and nothing is displayed. With
                min=10 step=10 only 10, 20, 30 … were accepted, so a template with
                top_k_examples=25 — the value every extraction on this estate uses —
                could not be saved through this form at all (OSD-48). The backend
                accepts any integer in 10..1000.
              */}
              <input
                id="extraction-template-top-k"
                type="number"
                value={topKExamples}
          ...
          <div className="mt-4 bg-white dark:bg-slate-900 rounded-lg border border-slate-300 dark:border-slate-700 p-4">
            ...
            {showContextWindow && (
              <div className="mt-4 space-y-3">
                ...
                <div>
                  <label
                    htmlFor="extraction-template-micro-batch-size"
                    className="block text-xs text-slate-600 dark:text-slate-400 mb-1"
                  >
                    Micro-Batch Size <span className="text-slate-500">(optional)</span>
                  </label>
                  <input
                    id="extraction-template-micro-batch-size"
                    type="number"
                    value={microBatchSize ?? ''}
                    onChange={(e) => setMicroBatchSize(
                      e.target.value === '' ? undefined : Number(e.target.value)
                    )}
                    min={1}
                    max={256}
                    placeholder="use Batch Size"
                    className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500"
                    disabled={isSubmitting}
                  />
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    Splits each batch on the GPU to cut peak VRAM without changing results. Leave empty to use Batch Size.
                  </p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
