...
class TokenizationService:
    ...
    #: Ceiling for a derived seed. Any non-negative int would do; int32 matches
    #: what `training_tasks.py` derives for a run and keeps the value legible in
    #: the `shuffle_seed` column.
    SHUFFLE_SEED_MODULUS = 2 ** 31 - 1

    @staticmethod
    def resolve_shuffle_seed(
        shuffle: bool,
        ...
        *,
        ...
        """The seed this tokenization will actually use, or None when not shuffling.

        A SEPARATE PURE FUNCTION ON PURPOSE. This repo has been bitten five times
        in one arc by a guard "satisfied by the wrong occurrence" — an AST check
        matched a comment, a scrape matched the text describing the code instead
        of the code. The answer that works is to extract the decision, unit-test
        it, and assert the CALL. Left inline in a 1,200-line Celery task, the only
        way to test this is to drive the whole task or to scrape source, and a
        source scrape fails open.

        Derived when the caller supplies nothing, following the rule
        `training_tasks.py` states for its own seed: persist it, or the schema's
        promise of reproducibility is false. Keyed on the same
        (dataset, model, max_length) triple that identifies the tokenization row
        and its directory, so rebuilding one reproduces its block order while a
        different length or model gets its own.

        Returning None is the ONLY way to say "not shuffled": 0 is a legitimate
        seed, so no sentinel value is available.
        """
        if not shuffle:
            return None
        if shuffle_seed is not None:
            return int(shuffle_seed)
        key = f"{dataset_id}:{model_id}:{max_length}".encode("utf-8")
        return zlib.crc32(key) % TokenizationService.SHUFFLE_SEED_MODULUS

    @staticmethod
    def shuffle_blocks(dataset: HFDataset, seed: Optional[int]) -> HFDataset:
        """Permute every block, and write the new order down physically.

        WHY THIS EXISTS. Activation extraction reads a PREFIX —
        `dataset.select(range(max_samples))` — and nothing upstream shuffles, so
        every extraction in this estate used the opening slice of its corpus.
        The bias is undetectable downstream, because the held-out evaluation
        draws its rows from just after the prefix — adjacent data, same slice.

        MEASURED 2026-09-17: OpenHermes-2.5 is written one source at a time — 14
        of its 15 labeled sources occupy a single contiguous row range — so a
        10% prefix holds only airoboros2.2 and CamelAI, 11.3% of the corpus, and
        never reaches glaive-code-assist (18.2%). Bloomberg and codeparrot, by
        contrast, measured unordered (Spearman -0.06 against date; 30.3% of
        33,090 repos in the first quarter), so this is a guarantee the corpora do
        not otherwise provide, not a fix aimed at one known-bad pair.

        A FULL PERMUTATION, not a shuffle buffer. Block i of the result is a
        uniformly random block from anywhere in the corpus, so any prefix is a
        uniform sample of the whole. A buffer would mix only locally and would
        leave the far end of a large corpus unreachable to a short prefix.

        `flatten_indices()` matters: `shuffle()` alone returns a view backed by an
        `indices.arrow` mapping, leaving the data physically in corpus order for
        every later reader to indirect through. Materialising costs one rewrite —
        the tokenized sets here total 9.7 GB, against the 666 GB of activations
        they produce — and keeps every consumer reading a plain sequential
        dataset, which is what they already assume.

        `seed=None` means no shuffle, and the dataset is returned untouched.
        """
        if seed is None:
            return dataset
        return dataset.shuffle(seed=int(seed)).flatten_indices()
