...
#: The `gpu` parameter every circuit GPU tool carries, sent as the body's `gpu`.
#: Every circuit GPU job runs split (Multi-GPU Phase 2: capture, attribution,
#: validation, faithfulness, calibration, the recorder), so "all" is named here.
GpuParam = Annotated[str, Field(description=(
    '"auto" (default) runs on the GPU with the most free memory when the job '
    "starts, and splits a model no single GPU can hold across GPUs; 'all' splits "
    "the model across every GPU; a GPU UUID (or index) from GET /system/gpu-list "
    "runs on that card. A named card, or a split, that cannot take the job is "
    "refused, never swapped for another"))]

#: A reproduction's card. Unlike `GpuParam` it has no "auto" default: omitted,
#: the reproduction keeps the request the ORIGINAL run was submitted with, and
#: only a value the caller actually passes replaces it.
ReproduceGpuParam = Annotated[Optional[str], Field(description=(
    "Omit to reuse the GPU request the original run was submitted with. "
    '"auto" runs on the GPU with the most free memory when the job starts, and '
    "splits a model no single GPU can hold across GPUs; 'all' splits the model "
    "across every GPU; a GPU UUID (or index) from GET /system/gpu-list runs on "
    "that card. A named card, or a split, that cannot take the job is refused, "
...
def register(mcp: FastMCP, client: MiStudioClient, settings: MCPSettings) -> None:
    ...
    async def start_circuit_capture(
        ...
        confirm=true to launch the full capture (there is no separate confirm
        tool; the REST confirm endpoint is what the UI uses). Managed GPU task.
    ...
    async def run_attribution_pass(run_id: Annotated[str, Field(description="Discovery run id from run_circuit_discovery")],
                                   prompt_limit: Annotated[Optional[int], Field(description="Prompts used for the gradient attribution pass")] = None,
                                   gpu: GpuParam = "auto") -> Any:
        ...
        return await client.post(f"/circuit-discovery/{run_id}/attribution",
                                 json_body={"prompt_limit": prompt_limit, "gpu": gpu})
    ...
    @mcp.tool()
    async def reproduce_calibration(
        manifest_id: Annotated[str, Field(description="A calibration manifest id (from list_validation_manifests, kind='calibration') to re-run")],
        gpu: ReproduceGpuParam = None,
    ) -> Any:
        ...
        return await client.post(
            ...
            json_body={"gpu": gpu} if gpu is not None else None)
    ...
    async def record_steering_samples(
        ...
        return await client.post(
            "/circuits/steering-samples", json_body={
                ...
                "max_tokens": max_tokens, "seed": seed, "gpu": gpu})
    ...
    @mcp.tool()
    async def reproduce_validation(
        manifest_id: Annotated[str, Field(description="Validation manifest id from list_validation_manifests")],
        gpu: ReproduceGpuParam = None,
    ) -> Any:
        ...
        within-tolerance verdict. GPU pass on `gpu`, or on the original run's
