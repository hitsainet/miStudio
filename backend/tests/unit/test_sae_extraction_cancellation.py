...
class TestShapeAItStops:
    ...
    def test_the_sampling_loop_abandons_before_reading_a_batch(self):
        ...
        # The loop iterates a PLAN now (`batch_spans` yields (start, stop) pairs
        # that never straddle a corpus boundary) instead of a bare range, so the
        # anchor moved. The assertion below is unchanged and is the real guard:
        # the cancel poll must still precede the batch read.
        loop = src.index("for batch_start, batch_end in planned_batches:")
