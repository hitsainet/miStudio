...
async def get_gpu_metrics(
    # Required: with more than one card there is no card a caller "means" by
    # default, and index 0 changed identity when a GPU was added (2026-09-13).
    gpu_id: int = Query(..., ge=0, description="GPU index to query (see /system/gpu-list)")
):
    """
...
async def get_all_monitoring_data(
    gpu_id: Optional[int] = Query(
        None,
        ge=0,
        description="GPU index for the GPU-specific block. Omit it to get every card in `gpu.cards`.",
    )
):
    """
    ...
    Args:
        gpu_id: GPU index for GPU-specific metrics. When omitted, `gpu.cards`
            carries every card instead of one being picked for the caller.

    Returns:
        Dictionary containing all monitoring data

    Raises:
        HTTPException 400: If gpu_id names a card that does not exist
    """
    try:
        gpu_service = get_gpu_monitor_service()
        system_service = get_system_monitor_service()
        ...
        if gpu_available:
            ...
            def card(index: int) -> Dict[str, Any]:
                return {
                    "gpu_id": index,
                    "metrics": gpu_service.get_gpu_metrics(index).to_dict(),
                    "info": gpu_service.get_gpu_info(index).to_dict(),
                    "processes": [proc.to_dict() for proc in gpu_service.get_gpu_processes(index)],
                }

            if gpu_id is None:
                # No card named: report every card rather than choosing one.
                response["gpu"] = {
                    "gpu_count": gpu_count,
                    "selected_gpu_id": None,
                    "cards": [card(index) for index in range(gpu_count)],
                }
            else:
                if gpu_id >= gpu_count:
                    # This used to fall back to GPU 0 silently, so a stale
                    # selection showed one card's numbers under another's id.
                    raise HTTPException(
                        status_code=400,
                        detail=f"Invalid GPU ID: {gpu_id}. Available GPUs: 0-{gpu_count - 1}",
                    )
                selected = card(gpu_id)
                response["gpu"] = {
                    "gpu_count": gpu_count,
