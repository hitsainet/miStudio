...
import type { LensTokenMessage, LensType } from '../../types/jlens';
import { RankedReadouts } from './RankedReadouts';

/**
 * `Object.entries` widens the key to `string`, and `LensTypeSlice.type` is the
 * narrower `LensType`. Left implicit, every `render` in this file was a
 * type error the runtime happily ignored — 14 of them, one per call site, and
 * a fifteenth arrived with the next test. Naming the return type here is what
 * makes the mismatch a single fixable thing instead of a growing tax.
 */
function tok(
  position: number,
...
describe('RankedReadouts', () => {
  ...
  it('does NOT tell the reader to pin anything — the composer asks', async () => {
    /**
     * The partner used to come from PIN ORDER, so with none pinned the tooltip
     * sent you off to pin one. It is typed in the composer this click opens,
     * and pinning now enables nothing here — an instruction to satisfy a
     * prerequisite that no longer exists is worse than none.
     *
     * MUTATION CONTROL: restore "with a pinned token" and this fails.
     */
    render(<RankedReadouts {...base({ onSwap: vi.fn(), types: ['JACOBIAN_LENS'] })} />);
    const col = screen.getByTestId('ranked-JACOBIAN_LENS');
    const swap = within(col).getAllByTitle(/Swap dog/)[0];
    expect(swap.getAttribute('title')).toMatch(/choose what to exchange it with/);
    expect(swap.getAttribute('title')).not.toMatch(/pinned token/);
  });
