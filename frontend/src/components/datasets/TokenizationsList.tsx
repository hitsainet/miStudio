...
export function buildTokenizationPayload(values: TokenizationFormValues) {
  ...
  return {
    ...
    // ALWAYS SENT, unlike `text_column` above. Omitting this would fall back to
    // the backend default of true, so an operator who deliberately unticked it
    // would get a shuffled corpus anyway and the row would say so. `false` is a
    // real instruction here, not an empty field.
    shuffle: values.shuffle,
    // Sent only when the operator pinned one. Omitted means "derive and record
    // it", which is what makes the order reproducible either way. Note 0 is a
    // legitimate seed, so this cannot use `|| undefined`.
    ...(values.shuffleSeed.trim() !== '' && Number.isFinite(Number(values.shuffleSeed))
      ? { shuffle_seed: Number(values.shuffleSeed) }
      : {}),
  };
}
...
export function TokenizationsList({ datasetId }: TokenizationsListProps) {
  ...
  // ON by default: extraction reads a PREFIX of the tokenized blocks, so an
  // unshuffled corpus means every extraction sees the same opening slice.
  // The seed is free text so it can be left blank — blank means "derive one and
  // record it", which keeps the order reproducible either way.
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Back to the safe default, not to the last thing chosen: leaving this
      // off after one deliberate unshuffled run would silently order the next
      // corpus too.
      setShuffle(true);
      setShuffleSeed('');
        ...
        <div className={`${COMPONENTS.card.base} p-4 space-y-3`}>
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              type="number"
              value={maxLength}
              onChange={(e) => setMaxLength(Math.max(1, Math.min(8192, parseInt(e.target.value) || 2048)))}
              min={1}
              max={8192}
              className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded text-slate-900 dark:text-slate-100 text-sm"
            />
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-4 h-4 bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded text-emerald-500 focus:ring-emerald-500"
              />
              <label htmlFor="shuffle-blocks" className="text-sm font-medium text-slate-900 dark:text-slate-100">
                Shuffle Blocks
              </label>
            </div>
            <p className="text-xs text-slate-500">
              Permute the blocks so any later extraction samples the whole
              corpus. Extraction reads the first N blocks, so without this every
              extraction sees the same opening slice — 6.5% of Bloomberg, which
              is dated financial news. Leave this on unless you deliberately
              want corpus order.
            </p>
            {shuffle && (
              <div className="space-y-1">
                <label htmlFor="shuffle-seed" className="text-xs font-medium text-slate-700 dark:text-slate-300">
                  Seed (optional)
                </label>
                <input
                  type="number"
                  ...
                  placeholder="derive one and record it"
                  className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded text-slate-900 dark:text-slate-100 text-sm"
                />
                <p className="text-xs text-slate-500">
                  Leave blank and a seed is derived from the dataset, model and
                  length, then recorded — so the order is reproducible either
                  way. 0 is a valid seed.
                </p>
              </div>
            )}
          </div>
