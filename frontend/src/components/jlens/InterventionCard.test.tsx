...
describe('InterventionCard', () => {
  ...
  it('WARNS about a multi-piece token, with the leading-space hint, but does not block it', async () => {
    /**
     * A multi-piece token USED to be blocked here. That excluded most of the
     * interesting vocabulary: the common case is not a phrase but an ordinary
     * word the tokenizer splits into subwords, and the server now takes the
     * MEAN of the pieces' rows and scores the FIRST piece.
     *
     * So the verdict is advice, not a gate — and the advice still matters,
     * because the spaced form is usually one token and therefore strictly
     * better evidence.
     *
     * MUTATION CONTROL: stop rendering the verdict and this fails; re-add a
     * `rejected(...)` clause to `blocked` and the enabled-button assertion
     * fails.
     */
    (jlensApi.checkTokens as ReturnType<typeof vi.fn>).mockResolvedValue([
      {
        ...
        n_tokens: 2,
        usable: false,
        // THE TEXT THE SERVER ACTUALLY RETURNS NOW. A fixture asserting the old
        // "defined for a SINGLE token" sentence agrees with nothing: the
        // endpoint stopped producing it when multi-piece became usable, so the
        // test would keep passing against a verdict no user can ever see.
        detail:
          "2 tokens [' Rom', 'e'] — usable: the direction is their MEAN and the " +
          "score is the rank of the first piece alone, which is weaker than a " +
          "single-token target. Try ' Rome' with a leading space — that is one " +
    ...
    await waitFor(() =>
      expect(screen.getByTestId('token-verdict')).toHaveTextContent(/weaker/)
    );
    ...
    // NOT A GATE. The run is legal; the reader has been told it is weaker.
    expect(
      screen.getByRole('button', { name: /Run with control/i })
    ).toBeEnabled();
  });

  it('BLOCKS a direction that encodes to NOTHING, unlike a multi-piece one', async () => {
    /**
     * `usable` is false for two different things. Allowing multi-piece targets
     * by removing the whole `rejected(...)` clause also allowed n_tokens === 0
     * — a string with no unembedding row, which burns a 202 and a single-GPU
     * slot before the worker refuses it.
     *
     * MUTATION CONTROL: make `emptyToken` return undefined and this fails.
     */
    (jlensApi.checkTokens as ReturnType<typeof vi.fn>).mockResolvedValue([
      {
        token: '\u200b',
    ...
    await waitFor(() =>
      expect(screen.getByTestId('token-verdict')).toHaveTextContent(/Encodes to nothing/)
    );
    expect(
      screen.getByRole('button', { name: /Run with control/i })
    ).toBeDisabled();
