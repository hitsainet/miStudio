...
def register(mcp: FastMCP, client: MiStudioClient, settings: MCPSettings) -> None:
    ...
    async def resume_labeling(
        ...
        gpu: Annotated[str, Field(description="labeling_method=local only: 'auto' (the GPU with the most free memory when the job starts; a judge no single GPU can hold is split across GPUs), 'all' (split the judge across every GPU) or a GPU UUID from /system/gpu-list. A named card, or a split, that cannot take the job is refused, never swapped.")] = "auto",
    ) -> Any:
    ...
    async def start_labeling_resume_sweep(
        ...
        gpu: Annotated[str, Field(description="labeling_method=local only: 'auto' (the GPU with the most free memory when each batch starts; a judge no single GPU can hold is split across GPUs), 'all' (split the judge across every GPU) or a GPU UUID from /system/gpu-list. Frozen onto the sweep with the rest of the judge config.")] = "auto",
    ) -> Any:
    ...
    async def run_labeling_trial(
        ...
        labeling_method: Annotated[str, Field(description="'openai' or 'openai_compatible'. 'local' is refused: a trial always judges through an OpenAI-compatible endpoint")] = "openai_compatible",
