...
async def download_model(
    ...
    # Before the row exists: an unknown card is a 400 naming the real ones.
    ...
    try:
        ...
        # Queue download job with Celery. The GPU request travels with the task;
        # the model row has no column for it.
...
async def redownload_model(
    ...
    # BEFORE the existing files are deleted below: an unknown card must not cost
    # the user the model they already had.
    gpu_request = resolve_gpu_request(request.gpu, can_split=True)
...
async def delete_model(
    model_id: str,
    cascade_tokenizations: bool = Query(
        False,
        description=(
            "Also delete this model's tokenization rows. They cascade whether "
            "you ask or not, so without this the delete is REFUSED when any "
            "exist. To change quantization, use Re-download on the same model "
            "id instead — that keeps the tokenizations."
        ),
    ),
    db: AsyncSession = Depends(get_db)
):
    ...
    ⚠ `dataset_tokenizations.model_id` is `ON DELETE CASCADE`, so deleting a
    model silently removes its tokenization rows while their Arrow directories
    remain on disk. That cost five rows for 81 GB of data once already, with no
    database backup newer than 2025-12. The delete is therefore refused (409)
    when tokenizations exist unless `cascade_tokenizations=true`.

    Args:
        model_id: Model ID (string format: m_{uuid})
        cascade_tokenizations: opt in to deleting the tokenization rows too
        db: Database session

    Raises:
        HTTPException: 404 if model not found; 409 if it is in use by a training,
            or if tokenizations exist and the cascade was not requested
    """
    try:
        result = await ModelService.delete_model(
            db, model_id, cascade_tokenizations=cascade_tokenizations
        )
    except ValueError as e:
...
async def extract_model_activations(
    ...
    # WHICH CARD, RESOLVED BEFORE ANYTHING EXISTS. "auto" stays "auto" and is
    # judged against live free memory when the job starts; a named card becomes
    # its UUID now, while it still means the card the user was shown. An unknown
    # card is a 400 here — outside the try below, whose handler would turn it
    # into a 500 — so no row is written and nothing is queued.
    #
    # The frontend has sent `gpu` since eb8c920f. This endpoint read `gpu_id`,
    # found none, and ran every extraction on index 0 — the 3080 Ti since the
    # second card was added.
    gpu_request = resolve_gpu_request(request.gpu, can_split=True)
    ...
    try:
        ...
        # THE ROW FIRST, THEN THE TASK.
        #
        # Handing the id over (above) met a guard added two weeks earlier: the
        # task refuses any id that has no row, on the stated assumption that
        # "the row is created by the API before dispatch". Nothing created it.
        # From 2026-09-05 every extraction started here returned 202 and was
        # refused within a second — and since the refusal had no row to mark
        # failed either, the UI showed nothing at all.
        with get_sync_db() as sync_db:
            task = ExtractionDatabaseService.create_row_then_dispatch(
                sync_db,
                dispatch=lambda: gpu_delay(extract_activations)(
                    model_id=model_id,
                    dataset_id=request.dataset_id,
                    layer_indices=request.layer_indices,
                    hook_types=request.hook_types,
                    ...
                    # Without these the old 512 cap is removed with NO replacement
                    # control: an 8192 tokenization runs at micro-batch 1 with no
                    # way to intervene.
                    max_seq_length=request.max_seq_length,
                    micro_batch_token_budget=request.micro_batch_token_budget,
                    gpu_request=gpu_request,
        ...
        if isinstance(e, IntegrityError):
            # The id is second-granular, so two starts for one model inside the
            # same second name the same row. The second is refused rather than
            # silently sharing the first one's id.
            raise HTTPException(
                status_code=409,
                detail=(
                    f"An extraction for model '{model_id}' was started in the same "
                    f"second. Start it again."
                ),
            )
        logger.exception("Failed to queue extraction job")
        raise HTTPException(
            status_code=500,
...
async def get_active_extraction(
    ...
    return {
        "data": {
            ...
            # Which step of an EXTRACTING job is running: extracting, merging
            # or statistics. Without it a reload shows "Extracting N/N" for
            # the whole merge and statistics.
            "phase": active_extraction.phase,
            ...
            "gpu_request": active_extraction.gpu_request,
            "gpu_uuid": active_extraction.gpu_uuid,
...
async def list_model_extractions(
    ...
    for db_ext in db_extractions:
        extraction_map[db_ext.id] = {
            ...
            "gpu_request": db_ext.gpu_request,
            "gpu_uuid": db_ext.gpu_uuid,
...
async def retry_extraction(
    ...
    # THE RETRY RUNS WHERE THE ORIGINAL ASKED TO RUN. This read the legacy
    # `gpu_id` index, which names a different card once another is added. The
    # stored request is re-resolved rather than copied so a named card that has
    # since left the node is a 400 now, not a failure in the worker later — and
    # outside the try below, whose handler would make it a 500. A NULL request
    # on a row from before requests were recorded means auto.
    gpu_request = resolve_gpu_request(original_extraction.gpu_request or AUTO, can_split=True)
    ...
    try:
        ...
        # Row first, then the task — the task refuses an id with no row. See
        # ExtractionDatabaseService.create_row_then_dispatch.
        with get_sync_db() as sync_db:
            task = ExtractionDatabaseService.create_row_then_dispatch(
                sync_db,
                dispatch=lambda: gpu_delay(extract_activations)(
                    ...
                    max_seq_length=getattr(request, "max_seq_length", None),
                    micro_batch_token_budget=getattr(request, "micro_batch_token_budget", None),
                    # The id was generated, logged, returned to the caller, and
                    # used for the WebSocket channel — and never passed, so the
                    # worker minted its own. The client then watched a channel
                    # nothing published to, and the response named a row that
                    # does not exist.
    ...
    for extraction_id in request.extraction_ids:
        try:
            ...
            with get_sync_db() as sync_db:
                ...
                # A LIVE EXTRACTION IS TOLD TO STOP BEFORE ITS ROW GOES.
                #
                # Deleting used to remove the row and the files and tell the
                # worker nothing: on 2026-09-12 it computed statistics for 20
                # more minutes over 390 GB of unlinked files (not freed until
                # the process exited), and every queued job waited behind it.
                # The worker's checker treats the vanished row as a cancel
                # (missing_row="cancelled"); this call additionally revokes a
                # task that has not started yet, so it never runs at all.
                if extraction.status in (
                    ...
                    outcome = request_cancel(
                        "activation_extraction",
                        extraction_id,
                        reason="Extraction deleted by user",
                        celery_task_id=extraction.celery_task_id,
                        db=sync_db,
                    )
                    logger.info(
                        "Extraction %s was live (%s) when deleted: %s",
                        extraction_id, extraction.status.value, outcome.detail,
                    )
